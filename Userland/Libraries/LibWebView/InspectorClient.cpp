...
void InspectorClient::set_is_windowed(bool is_windowed)
{
    ...
    auto const* is_visible_string = is_windowed ? "false" : "true";
    auto const script = MUST(String::formatted("inspector.setCloseInspectorButtonVisibility({});"sv, is_visible_string));
    m_inspector_web_view.run_javascript(script);
}
...
void InspectorClient::load_inspector()
{
    ...
    AK::StringView display = "block"sv;
    if (m_is_windowed) {
        display = "none"sv;
    }

    AK::String button_element = MUST(String::formatted(R"~~~(
