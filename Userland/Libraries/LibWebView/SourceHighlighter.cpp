...
String highlight_source(URL::URL const& url, StringView source, Syntax::Language language, HighlightOutputMode mode)
{
...
String SourceHighlighterClient::to_html_string(URL::URL const& url, HighlightOutputMode mode) const
{
...
<body>)~~~"sv);
    ...
    auto resolve_url_for_attribute = [&](Utf32View const& attribute_value) -> Optional<URL::URL> {
        ...
        if (auto resolved = Web::DOMURL::parse(attribute_url_without_quotes, url); resolved.is_valid())
            return resolved;
        return {};
    };
