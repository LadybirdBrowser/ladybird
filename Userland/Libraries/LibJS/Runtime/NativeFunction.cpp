...
GC::Ref<NativeFunction> NativeFunction::create(Realm& allocating_realm, Function<ThrowCompletionOr<Value>(VM&)> behaviour, i32 length, PropertyKey const& name, Optional<Realm*> realm, Optional<Object*> prototype, Optional<StringView> const& prefix)
{
    ...
    auto function = allocating_realm.heap().allocate<NativeFunction>(allocating_realm, GC::create_heap_function(vm.heap(), move(behaviour)), prototype.value(), *realm.value());
...
{
    return realm.heap().allocate<NativeFunction>(realm, name, GC::create_heap_function(realm.heap(), move(function)), realm.intrinsics().function_prototype());
}
