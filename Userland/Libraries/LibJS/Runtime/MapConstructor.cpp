...
JS_DEFINE_NATIVE_FUNCTION(MapConstructor::group_by)
{
    ...
    struct KeyedGroupTraits : public Traits<GC::Handle<Value>> {
        static unsigned hash(GC::Handle<Value> const& value_handle)
        {
            return ValueTraits::hash(value_handle.value());
        }

        static bool equals(GC::Handle<Value> const& a, GC::Handle<Value> const& b)
        {
    ...
    auto groups = TRY((JS::group_by<OrderedHashMap<GC::Handle<Value>, GC::MarkedVector<Value>, KeyedGroupTraits>, void>(vm, items, callback_function)));
