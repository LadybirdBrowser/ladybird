...
JS_DEFINE_NATIVE_FUNCTION(IteratorPrototype::map)
{
    ...
    auto closure = GC::create_heap_function(realm.heap(), [mapper = GC::Ref { mapper.as_function() }](VM& vm, IteratorHelper& iterator) -> ThrowCompletionOr<Value> {
...
JS_DEFINE_NATIVE_FUNCTION(IteratorPrototype::filter)
{
    ...
    auto closure = GC::create_heap_function(realm.heap(), [predicate = GC::Ref { predicate.as_function() }](VM& vm, IteratorHelper& iterator) -> ThrowCompletionOr<Value> {
        auto const& iterated = iterator.underlying_iterator();
...
JS_DEFINE_NATIVE_FUNCTION(IteratorPrototype::take)
{
    ...
    auto closure = GC::create_heap_function(realm.heap(), [integer_limit](VM& vm, IteratorHelper& iterator) -> ThrowCompletionOr<Value> {
        auto const& iterated = iterator.underlying_iterator();
...
JS_DEFINE_NATIVE_FUNCTION(IteratorPrototype::flat_map)
{
    ...
    // 5. Let closure be a new Abstract Closure with no parameters that captures iterated and mapper and performs the following steps when called:
    auto closure = GC::create_heap_function(realm.heap(), [flat_map_iterator, mapper = GC::Ref { mapper.as_function() }](VM& vm, IteratorHelper& iterator) mutable -> ThrowCompletionOr<Value> {
        auto const& iterated = iterator.underlying_iterator();
        return flat_map_iterator->next(vm, iterated, iterator, *mapper);
    });

    auto abrupt_closure = GC::create_heap_function(realm.heap(), [flat_map_iterator](VM& vm, IteratorHelper& iterator, Completion const& completion) -> ThrowCompletionOr<Value> {
