...
EventResult EventHandler::fire_input_event(FlyString const& event_name, FlyString const& input_type, HTML::Navigable& navigable, u32 code_point)
{
    ...
    if (auto* focused_element = document->focused_element()) {
        if (is<HTML::NavigableContainer>(*focused_element)) {
            auto& navigable_container = verify_cast<HTML::NavigableContainer>(*focused_element);
            if (navigable_container.content_navigable())
                return fire_input_event(event_name, input_type, *navigable_container.content_navigable(), code_point);
        }
...
EventResult EventHandler::handle_keydown(UIEvents::KeyCode key, u32 modifiers, u32 code_point)
{
    ...
    if (document->cursor_position()) {
        ...
        if (key == UIEvents::KeyCode::Key_Backspace && node.is_editable()) {
            ...
            fire_input_event(UIEvents::EventNames::input, UIEvents::InputTypes::deleteContentBackward, m_navigable, code_point);
            return EventResult::Handled;
        }
        ...
        if (key == UIEvents::KeyCode::Key_Delete && node.is_editable()) {
            ...
            m_edit_event_handler->handle_delete_character_after(document, *document->cursor_position());
            fire_input_event(UIEvents::EventNames::input, UIEvents::InputTypes::deleteContentForward, m_navigable, code_point);
            return EventResult::Handled;
        }
        ...
        if (key == UIEvents::KeyCode::Key_Return && node.is_editable()) {
            ...
            if (input_element) {
                ...
                fire_input_event(UIEvents::EventNames::input, UIEvents::InputTypes::insertParagraph, m_navigable, code_point);
                return EventResult::Handled;
            }
        }
        ...
        if (!should_ignore_keydown_event(code_point, modifiers) && node.is_editable()) {
            ...
            if (key == UIEvents::KeyCode::Key_Return)
                ...
                fire_input_event(UIEvents::EventNames::input, UIEvents::InputTypes::insertText, m_navigable, code_point);
            return EventResult::Handled;
        }
    }
