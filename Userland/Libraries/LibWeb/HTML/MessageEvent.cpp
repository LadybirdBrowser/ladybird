...
Variant<GC::Handle<WindowProxy>, GC::Handle<MessagePort>, Empty> MessageEvent::source() const
{
    if (!m_source.has_value())
        return Empty {};

    return m_source.value().downcast<GC::Handle<WindowProxy>, GC::Handle<MessagePort>>();
}
...
void MessageEvent::init_message_event(String const& type, bool bubbles, bool cancelable, JS::Value data, String const& origin, String const& last_event_id, Optional<MessageEventSource> source, Vector<GC::Handle<MessagePort>> const& ports)
{
