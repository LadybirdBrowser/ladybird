...
{
    return GC::create_heap_function(heap, move(function));
}
    ...
    OnFetchScriptComplete on_complete)
{
    ...
    if (module_map.is_fetching(url, module_type)) {
        module_map.wait_for_change(realm.heap(), url, module_type, [on_complete, &realm](auto entry) -> void {
            HTML::queue_global_task(HTML::Task::Source::Networking, realm.global_object(), GC::create_heap_function(realm.heap(), [on_complete, entry] {
    ...
    OnFetchScriptComplete on_complete)
{
    ...
    WebIDL::upon_fulfillment(loading_promise, GC::create_heap_function(realm.heap(), [&realm, record, &module_script, on_complete](JS::Value) -> WebIDL::ExceptionOr<JS::Value> {
    ...
    WebIDL::upon_rejection(loading_promise, GC::create_heap_function(realm.heap(), [state, &module_script, on_complete](JS::Value) -> WebIDL::ExceptionOr<JS::Value> {
