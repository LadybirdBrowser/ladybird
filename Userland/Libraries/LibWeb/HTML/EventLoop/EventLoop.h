...
public:
    ...
    void spin_until(GC::SafeFunction<bool()> goal_condition);
    void spin_processing_tasks_with_source_until(Task::Source, GC::SafeFunction<bool()> goal_condition);
    void process();
    ...
    Vector<GC::Handle<DOM::Document>> documents_in_this_event_loop() const;

    Vector<GC::Handle<HTML::Window>> same_loop_windows() const;
...
private:
    ...
    Vector<GC::Ref<EnvironmentSettingsObject>> m_backup_incumbent_settings_object_stack;
