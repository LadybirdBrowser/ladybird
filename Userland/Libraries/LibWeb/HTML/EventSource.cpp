...
WebIDL::ExceptionOr<GC::Ref<EventSource>> EventSource::construct_impl(JS::Realm& realm, StringView url, EventSourceInit event_source_init_dict)
{
    ...
    fetch_algorithms_input.process_response = [event_source](GC::Ref<Fetch::Infrastructure::Response> response) {
        ...
        // 4. Otherwise, announce the connection and interpret res's body line by line.
            ...
            auto process_body_chunk = GC::create_heap_function(realm.heap(), [event_source, pending_data = ByteBuffer()](ByteBuffer body) mutable {
            ...
            auto process_end_of_body = GC::create_heap_function(realm.heap(), []() {
                // This case is handled by `process_event_source_end_of_body` above.
            });
            auto process_body_error = GC::create_heap_function(realm.heap(), [](JS::Value) {
                // This case is handled by `process_event_source_end_of_body` above.
            });
...
void EventSource::announce_the_connection()
{
    ...
    // EventSource object.
    HTML::queue_a_task(HTML::Task::Source::RemoteEvent, nullptr, nullptr, GC::create_heap_function(heap(), [this]() {
        if (m_ready_state != ReadyState::Closed) {
...
void EventSource::reestablish_the_connection()
{
    ...
    HTML::queue_a_task(HTML::Task::Source::RemoteEvent, nullptr, nullptr, GC::create_heap_function(heap(), [&]() {
...
void EventSource::dispatch_the_event()
{
    ...
    HTML::queue_a_task(HTML::Task::Source::RemoteEvent, nullptr, nullptr, GC::create_heap_function(heap(), [this, event]() {
        if (m_ready_state != ReadyState::Closed)
