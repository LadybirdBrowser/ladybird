...
Vector<GC::Handle<DOM::Node>> HTMLSlotElement::assigned_nodes(AssignedNodesOptions options) const
{
    // 1. If options["flatten"] is false, then return this's assigned nodes.
    if (!options.flatten) {
        Vector<GC::Handle<DOM::Node>> assigned_nodes;
...
Vector<GC::Handle<DOM::Element>> HTMLSlotElement::assigned_elements(AssignedNodesOptions options) const
{
    // 1. If options["flatten"] is false, then return this's assigned nodes, filtered to contain only Element nodes.
    if (!options.flatten) {
        Vector<GC::Handle<DOM::Element>> assigned_nodes;
