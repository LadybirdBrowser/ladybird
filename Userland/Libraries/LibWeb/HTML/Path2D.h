...
public:
    static WebIDL::ExceptionOr<GC::Ref<Path2D>> construct_impl(JS::Realm&, Optional<Variant<GC::Handle<Path2D>, String>> const& path);
...
private:
    Path2D(JS::Realm&, Optional<Variant<GC::Handle<Path2D>, String>> const&);
