...
void CanvasRenderingContext2D::stroke_internal(Gfx::Path const& path)
{
    ...
    shadow_to_stroke_internal(path); // Draw shadow of stroke
...
void CanvasRenderingContext2D::fill_internal(Gfx::Path const& path, Gfx::WindingRule winding_rule)
{
    ...
    shadow_to_fill_internal(path, winding_rule); // Draw shadow of fill
...
void CanvasRenderingContext2D::shadow_to_fill_internal(Gfx::Path const& path, Gfx::WindingRule winding_rule)
{
    auto* painter = this->painter();
    if (!painter)
        return;
    ...
    painter->restore(); // Restore the painter's state to what it was before the shadow transformations.

    did_draw(path_to_fill.bounding_box());
}

void CanvasRenderingContext2D::shadow_to_stroke_internal(Gfx::Path const& path)
{
    auto* painter = this->painter();
    if (!painter)
        return;
