...
WebIDL::ExceptionOr<GC::Ref<Path2D>> Path2D::construct_impl(JS::Realm& realm, Optional<Variant<GC::Handle<Path2D>, String>> const& path)
{
    return realm.heap().allocate<Path2D>(realm, realm, path);
}

// https://html.spec.whatwg.org/multipage/canvas.html#dom-path2d
Path2D::Path2D(JS::Realm& realm, Optional<Variant<GC::Handle<Path2D>, String>> const& path)
    ...
    , CanvasPath(static_cast<Bindings::PlatformObject&>(*this))
{
    ...
    if (path->has<GC::Handle<Path2D>>()) {
        this->path() = path->get<GC::Handle<Path2D>>()->path();
        return;
    }
