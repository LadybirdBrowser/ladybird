...
Vector<GC::Handle<Navigable>> Navigable::child_navigables() const
{
...
ErrorOr<void> Navigable::initialize_navigable(GC::Ref<DocumentState> document_state, GC::Ptr<Navigable> parent)
{
    ...
    GC::Ref<SessionHistoryEntry> entry = *heap().allocate_without_realm<SessionHistoryEntry>();
    // URL: document's URL
...
Navigable::ChosenNavigable Navigable::choose_a_navigable(StringView name, TokenizedFeature::NoOpener no_opener, ActivateTab activate_tab, Optional<TokenizedFeature::Map const&> window_features)
{
    ...
    // applicable option from the following list:
        ...
        // --> If the user agent has been configured such that in this instance it will create a new top-level traversable
            ...
            auto create_new_traversable = GC::create_heap_function(heap(), move(create_new_traversable_closure));
...
static CrossOriginOpenerPolicy obtain_a_cross_origin_opener_policy(GC::Ref<Fetch::Infrastructure::Response>, Fetch::Infrastructure::Request::ReservedClientType const& reserved_client)
{
    ...
    GC::Ptr<GC::Function<void()>> completion_steps)
{
    ...
    // NavigationParams' response field is GC::Ref
    ...
    queue_global_task(Task::Source::NavigationAndTraversal, *active_window(), GC::create_heap_function(heap(), [this, entry, navigation_params = move(navigation_params), navigation_id, completion_steps]() mutable {
        ...
        else if (navigation_params.get<GC::Ref<NavigationParams>>()->response->status() == 204 || navigation_params.get<GC::Ref<NavigationParams>>()->response->status() == 205) {
...
WebIDL::ExceptionOr<void> Navigable::navigate(NavigateParams params)
{
    ...
    if (url.scheme() == "javascript"sv) {
        ...
        queue_global_task(Task::Source::NavigationAndTraversal, *active_window(), GC::create_heap_function(heap(), [this, url, history_handling, initiator_origin_snapshot, csp_navigation_type, navigation_id] {
    ...
    Platform::EventLoopPlugin::the().deferred_invoke([this, source_snapshot_params, target_snapshot_params, csp_navigation_type, document_resource, url, navigation_id, referrer_policy, initiator_origin_snapshot, response, history_handling, initiator_base_url_snapshot] {
        ...
        queue_global_task(Task::Source::NavigationAndTraversal, *active_window(), GC::create_heap_function(heap(), [this] {
        ...
        GC::Ref<DocumentState> document_state = *heap().allocate_without_realm<DocumentState>();
        ...
        GC::Ref<SessionHistoryEntry> history_entry = *heap().allocate_without_realm<SessionHistoryEntry>();
        ...
        populate_session_history_entry_document(history_entry, source_snapshot_params, target_snapshot_params, navigation_id, navigation_params, csp_navigation_type, true, GC::create_heap_function(heap(), [this, history_entry, history_handling, navigation_id] {
            ...
            traversable_navigable()->append_session_history_traversal_steps(GC::create_heap_function(heap(), [this, history_entry, history_handling, navigation_id] {
...
WebIDL::ExceptionOr<void> Navigable::navigate_to_a_fragment(URL::URL const& url, HistoryHandlingBehavior history_handling, UserNavigationInvolvement user_involvement, Optional<SerializationRecord> navigation_api_state, String navigation_id)
{
    ...
    traversable->append_session_history_synchronous_navigation_steps(*this, GC::create_heap_function(heap(), [this, traversable, history_entry, entry_to_replace, navigation_id, history_handling] {
...
WebIDL::ExceptionOr<GC::Ptr<DOM::Document>> Navigable::evaluate_javascript_url(URL::URL const& url, Origin const& new_document_origin, String navigation_id)
{
    auto& vm = this->vm();
...
void Navigable::reload()
{
    ...
    traversable->append_session_history_traversal_steps(GC::create_heap_function(heap(), [traversable] {
...
void perform_url_and_history_update_steps(DOM::Document& document, URL::URL new_url, Optional<SerializationRecord> serialized_data, HistoryHandlingBehavior history_handling)
{
    ...
    GC::Ref<SessionHistoryEntry> new_entry = document.heap().allocate_without_realm<SessionHistoryEntry>();
    ...
    traversable->append_session_history_synchronous_navigation_steps(*navigable, GC::create_heap_function(document.realm().heap(), [traversable, navigable, new_entry, entry_to_replace, history_handling] {
