...
JS::ThrowCompletionOr<void> CustomElementRegistry::define(String const& name, WebIDL::CallbackType* constructor, ElementDefinitionOptions options)
{
    ...
    Vector<GC::Handle<DOM::Element>> upgrade_candidates;
...
Variant<GC::Handle<WebIDL::CallbackType>, JS::Value> CustomElementRegistry::get(String const& name) const
{
...
Optional<String> CustomElementRegistry::get_name(GC::Handle<WebIDL::CallbackType> const& constructor) const
{
...
WebIDL::ExceptionOr<GC::Ref<JS::Promise>> CustomElementRegistry::when_defined(String const& name)
{
    auto& realm = this->realm();
    ...
    auto existing_definition_iterator = m_custom_element_definitions.find_if([&name](GC::Handle<CustomElementDefinition> const& definition) {
        return definition->name() == name;
    });
...
void CustomElementRegistry::upgrade(GC::Ref<DOM::Node> root) const
{
    ...
    Vector<GC::Handle<DOM::Element>> candidates;
...
{
    auto definition_iterator = m_custom_element_definitions.find_if([&](GC::Handle<CustomElementDefinition> const& definition) {
