...
static Layout::Node& insertion_parent_for_block_node(Layout::NodeWithStyle& layout_parent, Layout::Node& layout_node)
{
    ...
    Vector<GC::Handle<Layout::Node>> children;
    {
...
void TreeBuilder::create_layout_tree(DOM::Node& dom_node, TreeBuilder::Context& context)
{
    ...
    if (is_button_layout && !display.is_grid_inside() && !display.is_flex_inside()) {
        ...
        Vector<GC::Handle<Node>> sequence;
...
void TreeBuilder::remove_irrelevant_boxes(NodeWithStyle& root)
{
    ...
    Vector<GC::Handle<Node>> to_remove;
...
static void wrap_in_anonymous(Vector<GC::Handle<Node>>& sequence, Node* nearest_sibling, CSS::Display display)
{
...
Vector<GC::Handle<Box>> TreeBuilder::generate_missing_parents(NodeWithStyle& root)
{
    Vector<GC::Handle<Box>> table_roots_to_wrap;
...
void TreeBuilder::missing_cells_fixup(Vector<GC::Handle<Box>> const& table_root_boxes)
{
