...
{
    auto process_request_body_chunk_length = GC::create_heap_function(vm.heap(), move(input.process_request_body_chunk_length));
    auto process_request_end_of_body = GC::create_heap_function(vm.heap(), move(input.process_request_end_of_body));
    auto process_early_hints_response = GC::create_heap_function(vm.heap(), move(input.process_early_hints_response));
    auto process_response = GC::create_heap_function(vm.heap(), move(input.process_response));
    auto process_response_end_of_body = GC::create_heap_function(vm.heap(), move(input.process_response_end_of_body));
    auto process_response_consume_body = GC::create_heap_function(vm.heap(), move(input.process_response_consume_body));
