...
void IncrementalReadLoopReadRequest::on_chunk(JS::Value chunk)
{
    ...
    if (!chunk.is_object() || !is<JS::Uint8Array>(chunk.as_object())) {
        continue_algorithm = GC::create_heap_function(realm.heap(), [&realm, process_body_error = m_process_body_error] {
    ...
    // 3. Otherwise:
        ...
        continue_algorithm = GC::create_heap_function(realm.heap(), [bytes = move(bytes), body = m_body, reader = m_reader, task_destination = m_task_destination, process_body_chunk = m_process_body_chunk, process_end_of_body = m_process_end_of_body, process_body_error = m_process_body_error] {
...
void IncrementalReadLoopReadRequest::on_close()
{
    ...
    Fetch::Infrastructure::queue_fetch_task(m_task_destination, GC::create_heap_function(m_reader->heap(), [this] {
...
void IncrementalReadLoopReadRequest::on_error(JS::Value error)
{
    ...
    Fetch::Infrastructure::queue_fetch_task(m_task_destination, GC::create_heap_function(m_reader->heap(), [this, error = move(error)] {
