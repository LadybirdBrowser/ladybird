...
WebIDL::ExceptionOr<GC::Ref<Request>> Request::construct_impl(JS::Realm& realm, RequestInfo const& input, RequestInit const& init)
{
    ...
    // 6. Otherwise:
        ...
        VERIFY(input.has<GC::Handle<Request>>());

        // 2. Set request to input’s request.
        input_request = input.get<GC::Handle<Request>>()->request();

        // 3. Set signal to input’s signal.
        input_signal = input.get<GC::Handle<Request>>()->signal();
    }
    ...
    Vector<GC::Handle<DOM::AbortSignal>> signals;
    ...
    if (input.has<GC::Handle<Request>>())
        input_body = input.get<GC::Handle<Request>>()->request()->body();
    ...
    if (!init_body && input_body.has_value()) {
        ...
        if (input.has<GC::Handle<Request>>() && input.get<GC::Handle<Request>>()->is_unusable())
