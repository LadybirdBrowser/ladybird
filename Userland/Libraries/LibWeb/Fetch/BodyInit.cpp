...
WebIDL::ExceptionOr<Infrastructure::BodyWithType> safely_extract_body(JS::Realm& realm, BodyInitOrReadableBytes const& object)
{
    ...
    if (auto const* stream = object.get_pointer<GC::Handle<Streams::ReadableStream>>()) {
...
WebIDL::ExceptionOr<Infrastructure::BodyWithType> extract_body(JS::Realm& realm, BodyInitOrReadableBytes const& object, bool keepalive)
{
    ...
    if (auto const* stream_handle = object.get_pointer<GC::Handle<Streams::ReadableStream>>()) {
    ...
    else if (auto const* blob_handle = object.get_pointer<GC::Handle<FileAPI::Blob>>()) {
    ...
    // 10. Switch on object.
    TRY(object.visit(
        [&](GC::Handle<FileAPI::Blob> const& blob) -> WebIDL::ExceptionOr<void> {
        ...
        [&](GC::Handle<WebIDL::BufferSource> const& buffer_source) -> WebIDL::ExceptionOr<void> {
            // Set source to a copy of the bytes held by object.
            source = MUST(WebIDL::get_buffer_source_copy(*buffer_source->raw_object()));
            return {};
        },
        [&](GC::Handle<XHR::FormData> const& form_data) -> WebIDL::ExceptionOr<void> {
        ...
        [&](GC::Handle<DOMURL::URLSearchParams> const& url_search_params) -> WebIDL::ExceptionOr<void> {
        ...
        [&](GC::Handle<Streams::ReadableStream> const& stream) -> WebIDL::ExceptionOr<void> {
