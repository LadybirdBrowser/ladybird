...
void EventTarget::add_an_event_listener(DOMEventListener& listener)
{
    ...
    if (listener.signal) {
        // NOTE: `this` and `listener` are protected by AbortSignal using GC::SafeFunction.
...
WebIDL::CallbackType* EventTarget::get_current_value_of_event_handler(FlyString const& name)
{
    ...
    if (event_handler->value.has<ByteString>()) {
        ...
        event_handler->value = GC::Ptr(realm.heap().allocate_without_realm<WebIDL::CallbackType>(*function, settings_object));
    }
