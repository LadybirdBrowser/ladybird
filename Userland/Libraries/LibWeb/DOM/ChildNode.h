...
public:
    ...
    WebIDL::ExceptionOr<void> before(Vector<Variant<GC::Handle<Node>, String>> const& nodes)
    {
    ...
    WebIDL::ExceptionOr<void> after(Vector<Variant<GC::Handle<Node>, String>> const& nodes)
    {
        auto* node = static_cast<NodeType*>(this);
    ...
    WebIDL::ExceptionOr<void> replace_with(Vector<Variant<GC::Handle<Node>, String>> const& nodes)
    {
        auto* node = static_cast<NodeType*>(this);
...
private:
    GC::Ptr<Node> viable_previous_sibling_for_insertion(Vector<Variant<GC::Handle<Node>, String>> const& nodes)
    {
        auto* node = static_cast<NodeType*>(this);
        ...
        for (auto* sibling = node->previous_sibling(); sibling; sibling = sibling->previous_sibling()) {
            ...
            for (auto const& node_or_string : nodes) {
                if (!node_or_string.template has<GC::Handle<Node>>())
                    continue;

                auto const& node_in_vector = node_or_string.template get<GC::Handle<Node>>();
    ...
    GC::Ptr<Node> viable_next_sibling_for_insertion(Vector<Variant<GC::Handle<Node>, String>> const& nodes)
    {
        auto* node = static_cast<NodeType*>(this);
