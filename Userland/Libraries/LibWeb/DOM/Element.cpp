...
GC::Ptr<Layout::Node> Element::create_layout_node(NonnullRefPtr<CSS::StyleProperties> style)
{
...
GC::Ptr<Layout::NodeWithStyle> Element::create_layout_node_for_display_type(DOM::Document& document, CSS::Display const& display, NonnullRefPtr<CSS::StyleProperties> style, Element* element)
{
...
{
    return queue_a_task(source, HTML::main_thread_event_loop(), document(), GC::create_heap_function(heap(), move(steps)));
}
...
GC::Ref<Geometry::DOMRectList> Element::get_client_rects() const
{
    ...
    Vector<GC::Handle<Geometry::DOMRect>> rects;
...
void Element::enqueue_an_element_on_the_appropriate_element_queue()
{
    ...
    if (reactions_stack.element_queue_stack.is_empty()) {
        ...
        // NOTE: `this` is protected by GC::SafeFunction
        HTML::queue_a_microtask(&document(), GC::create_heap_function(relevant_agent.heap(), [this]() {
            auto& relevant_agent = HTML::relevant_agent(*this);
            auto* custom_data = verify_cast<Bindings::WebEngineCustomData>(relevant_agent.custom_data());
            auto& reactions_stack = custom_data->custom_element_reactions_stack;
