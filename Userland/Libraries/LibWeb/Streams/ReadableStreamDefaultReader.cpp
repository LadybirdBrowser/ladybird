...
GC::Ref<JS::Promise> ReadableStreamDefaultReader::read()
{
...
GC::Ref<WebIDL::Promise> ReadableStreamDefaultReader::read_all_bytes_deprecated()
{
    ...
    auto success_steps = GC::create_heap_function(realm.heap(), [promise, &realm](ByteBuffer bytes) {
        auto buffer = JS::ArrayBuffer::create(realm, move(bytes));
        WebIDL::resolve_promise(realm, promise, buffer);
    });

    auto failure_steps = GC::create_heap_function(realm.heap(), [promise, &realm](JS::Value error) {
