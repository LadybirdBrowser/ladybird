...
IntersectionObserver::IntersectionObserver(JS::Realm& realm, GC::Ptr<WebIDL::CallbackType> callback, Optional<Variant<GC::Handle<DOM::Element>, GC::Handle<DOM::Document>>> const& root, Vector<double>&& thresholds)
...
Vector<GC::Handle<IntersectionObserverEntry>> IntersectionObserver::take_records()
{
    // 1. Let queue be a copy of this’s internal [[QueuedEntries]] slot.
    Vector<GC::Handle<IntersectionObserverEntry>> queue;
...
Variant<GC::Handle<DOM::Element>, GC::Handle<DOM::Document>, Empty> IntersectionObserver::root() const
{
    if (!m_root)
        return Empty {};
...
Variant<GC::Handle<DOM::Element>, GC::Handle<DOM::Document>> IntersectionObserver::intersection_root() const
{
    ...
    return GC::make_handle(global_object().page().top_level_browsing_context().active_document());
}
...
CSSPixelRect IntersectionObserver::root_intersection_rectangle() const
{
    ...
    if (intersection_root.has<GC::Handle<DOM::Document>>()) {
        auto document = intersection_root.get<GC::Handle<DOM::Document>>();
        ...
        VERIFY(intersection_root.has<GC::Handle<DOM::Element>>());
        auto element = intersection_root.get<GC::Handle<DOM::Element>>();
