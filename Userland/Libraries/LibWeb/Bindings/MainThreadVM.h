...
struct CustomElementReactionsStack {
    ...
    Vector<Vector<GC::Handle<DOM::Element>>> element_queue_stack;
    ...
    Vector<GC::Handle<DOM::Element>> backup_element_queue;
...
struct WebEngineCustomData final : public JS::VM::CustomData {
    ...
    virtual void spin_event_loop_until(GC::SafeFunction<bool()> goal_condition) override;

    GC::Handle<HTML::EventLoop> event_loop;
    ...
    GC::Handle<JS::Realm> internal_realm;
    ...
    Vector<GC::Handle<DOM::Element>>& current_element_queue() { return custom_element_reactions_stack.element_queue_stack.last(); }
    Vector<GC::Handle<DOM::Element>> const& current_element_queue() const { return custom_element_reactions_stack.element_queue_stack.last(); }
};
