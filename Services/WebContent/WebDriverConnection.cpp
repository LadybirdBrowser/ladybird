...
void WebDriverConnection::close_session(IPCMessages::CloseSession::Resolver resolve)
{
...
#define RESOLVE(...) resolve(Web::WebDriver::Response { __VA_ARGS__ })
#define ER() resolve(Web::WebDriver::Response { JsonValue{} })

#define VALUE_OR_RESOLVE(unwrapped, expr)                    \
    auto res##unwrapped = expr;                              \
    if (res##unwrapped.is_error())                           \
        return RESOLVE(res##unwrapped.release_error()); \
    auto unwrapped = res##unwrapped.release_value();

#define RESOLVE_IF_ERROR(expr)                          \
    {                                                         \
        auto err_result = expr;                               \
        if (err_result.is_error())                            \
            return RESOLVE(err_result.release_error()); \
    }

// 9.1 Get Timeouts, https://w3c.github.io/webdriver/#dfn-get-timeouts
void WebDriverConnection::get_timeouts(IPCMessages::GetTimeouts::Resolver resolve)
{
...
void WebDriverConnection::set_timeouts(JsonValue payload, IPCMessages::SetTimeouts::Resolver resolve)
{
    ...
    RESOLVE_IF_ERROR(Web::WebDriver::json_deserialize_as_a_timeouts_configuration_into(payload, m_timeouts_configuration));
    ...
    RESOLVE(Web::WebDriver::timeouts_object(m_timeouts_configuration));
}

// 10.1 Navigate To, https://w3c.github.io/webdriver/#navigate-to
void WebDriverConnection::navigate_to(JsonValue payload, IPCMessages::NavigateTo::Resolver resolve)
{
    // 1. If the current top-level browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_top_level_browsing_context_is_open());
    ...
    if (!payload.is_object() || !payload.as_object().has_string("url"sv))
        RESOLVE(Web::WebDriver::Error::from_code(Web::WebDriver::ErrorCode::InvalidArgument, "Payload doesn't have a string `url`"sv));
...
void WebDriverConnection::get_current_url(IPCMessages::GetCurrentUrl::Resolver resolve)
{
...
void WebDriverConnection::back(IPCMessages::Back::Resolver resolve)
{
...
void WebDriverConnection::forward(IPCMessages::Forward::Resolver resolve)
{
    // 1. If session's current top-level browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_top_level_browsing_context_is_open());
...
void WebDriverConnection::refresh(IPCMessages::Refresh::Resolver resolve)
{
    // 1. If the current top-level browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_top_level_browsing_context_is_open());
...
void WebDriverConnection::get_title(IPCMessages::GetTitle::Resolver resolve)
{
        // 1. If the current top-level browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_top_level_browsing_context_is_open());
...
void WebDriverConnection::get_window_handle(IPCMessages::GetWindowHandle::Resolver resolve)
{
    ...
    RESOLVE(JsonValue { current_top_level_browsing_context()->top_level_traversable()->window_handle() });
}

// 11.2 Close Window, https://w3c.github.io/webdriver/#dfn-close-window
void WebDriverConnection::close_window(IPCMessages::CloseWindow::Resolver resolve)
{
...
void WebDriverConnection::switch_to_window(String handle, IPCMessages::SwitchToWindow::Resolver resolve)
{
    ...
    if (!found_matching_context)
        return RESOLVE(Web::WebDriver::Error::from_code(Web::WebDriver::ErrorCode::NoSuchWindow, "Window not found"sv));
...
void WebDriverConnection::new_window(JsonValue payload, IPCMessages::NewWindow::Resolver resolve)
{
...
void WebDriverConnection::switch_to_frame(JsonValue payload, IPCMessages::SwitchToFrame::Resolver resolve)
{
    // 1. Let id be the result of getting the property "id" from parameters.
    if (!payload.is_object() || !payload.as_object().has("id"sv))
        return RESOLVE(Web::WebDriver::Error::from_code(Web::WebDriver::ErrorCode::InvalidArgument, "Payload doesn't have property `id`"sv));
    ...
    if (!id.is_null() && !id.is_number() && !Web::WebDriver::represents_a_web_element(id))
        return RESOLVE(Web::WebDriver::Error::from_code(Web::WebDriver::ErrorCode::InvalidArgument, "Payload property `id` is not null, a number, or a web element"sv));
    ...
    // -> id is a Number object
        ...
        if (!id_value.has_value())
            return RESOLVE(Web::WebDriver::Error::from_code(Web::WebDriver::ErrorCode::InvalidArgument, MUST(String::formatted("Frame ID {} is invalid", id))));

        // 2. If session's current browsing context is no longer open, return error with error code no such window.
        RESOLVE_IF_ERROR(ensure_current_browsing_context_is_open());
...
void WebDriverConnection::switch_to_parent_frame(JsonValue, IPCMessages::SwitchToParentFrame::Resolver resolve)
{
    ...
    RESOLVE_IF_ERROR(Web::WebDriver::ensure_browsing_context_is_open(current_parent_browsing_context()));
...
void WebDriverConnection::get_window_rect(IPCMessages::GetWindowRect::Resolver resolve)
{
...
void WebDriverConnection::set_window_rect(JsonValue payload, IPCMessages::SetWindowRect::Resolver resolve)
{
        if (!payload.is_object())
        return RESOLVE(Web::WebDriver::Error::from_code(Web::WebDriver::ErrorCode::InvalidArgument, "Payload is not a JSON object"sv));
...
void WebDriverConnection::maximize_window(IPCMessages::MaximizeWindow::Resolver resolve)
{
...
void WebDriverConnection::minimize_window(IPCMessages::MinimizeWindow::Resolver resolve)
{
...
void WebDriverConnection::fullscreen_window(IPCMessages::FullscreenWindow::Resolver resolve)
{
...
void WebDriverConnection::consume_user_activation(IPCMessages::ConsumeUserActivation::Resolver resolve)
{
...
void WebDriverConnection::find_element(JsonValue payload, IPCMessages::FindElement::Resolver resolve)
{
    ...
    if (!location_strategy.has_value())
        return RESOLVE(Web::WebDriver::Error::from_code(Web::WebDriver::ErrorCode::InvalidArgument, MUST(String::formatted("Location strategy '{}' is invalid", location_strategy_string))));
...
void WebDriverConnection::find_elements(JsonValue payload, IPCMessages::FindElements::Resolver resolve)
{
        // 1. Let location strategy be the result of getting a property named "using" from parameters.
    VALUE_OR_RESOLVE(location_strategy_string, Web::WebDriver::get_property(payload, "using"sv))
    auto location_strategy = Web::WebDriver::location_strategy_from_string(location_strategy_string);
...
void WebDriverConnection::find_element_from_element(JsonValue payload, String element_id, IPCMessages::FindElementFromElement::Resolver resolve)
{
        // 1. Let location strategy be the result of getting a property named "using" from parameters.
    VALUE_OR_RESOLVE(location_strategy_string, Web::WebDriver::get_property(payload, "using"sv));
    auto location_strategy = Web::WebDriver::location_strategy_from_string(location_strategy_string);
...
void WebDriverConnection::find_elements_from_element(JsonValue payload, String element_id, IPCMessages::FindElementsFromElement::Resolver resolve)
{
        // 1. Let location strategy be the result of getting a property named "using" from parameters.
    VALUE_OR_RESOLVE(location_strategy_string, Web::WebDriver::get_property(payload, "using"sv));
    auto location_strategy = Web::WebDriver::location_strategy_from_string(location_strategy_string);
...
void WebDriverConnection::find_element_from_shadow_root(JsonValue payload, String shadow_id, IPCMessages::FindElementFromShadowRoot::Resolver resolve)
{
...
void WebDriverConnection::find_elements_from_shadow_root(JsonValue payload, String shadow_id, IPCMessages::FindElementFromShadowRoot::Resolver resolve)
{
        // 1. Let location strategy be the result of getting a property called "using".
    VALUE_OR_RESOLVE(location_strategy_string, Web::WebDriver::get_property(payload, "using"sv));
    auto location_strategy = Web::WebDriver::location_strategy_from_string(location_strategy_string);
...
void WebDriverConnection::get_active_element(IPCMessages::GetActiveElement::Resolver resolve)
{
...
void WebDriverConnection::get_element_shadow_root(String element_id, IPCMessages::GetElementShadowRoot::Resolver resolve)
{
        // 1. If session's current browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_browsing_context_is_open());
...
void WebDriverConnection::is_element_selected(String element_id, IPCMessages::IsElementSelected::Resolver resolve)
{
        // 1. If the current browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_browsing_context_is_open());
...
void WebDriverConnection::get_element_attribute(String element_id, String name, IPCMessages::GetElementAttribute::Resolver resolve)
{
        // 1. If session's current browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_browsing_context_is_open());
...
void WebDriverConnection::get_element_property(String element_id, String name, IPCMessages::GetElementProperty::Resolver resolve)
{
        // 1. If session's current browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_browsing_context_is_open());
...
void WebDriverConnection::get_element_css_value(String element_id, String name, IPCMessages::GetElementCssValue::Resolver resolve)
{
        // 1. If session's current browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_browsing_context_is_open());
...
// 12.4.5 Get Element Text, https://w3c.github.io/webdriver/#dfn-get-element-text
 void WebDriverConnection::get_element_text(String element_id, IPCMessages::GetElementText::Resolver resolve)
 {
        // 1. If the current browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_browsing_context_is_open());
...
void WebDriverConnection::get_element_tag_name(String element_id, IPCMessages::GetElementTagName::Resolver resolve)
{
        // 1. If session's current browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_browsing_context_is_open());
...
void WebDriverConnection::get_element_rect(String element_id, IPCMessages::GetElementRect::Resolver resolve)
{
        // 1. If the current browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_browsing_context_is_open());
...
void WebDriverConnection::is_element_enabled(String element_id, IPCMessages::IsElementEnabled::Resolver resolve)
{
        // 1. If the current browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_browsing_context_is_open());
...
void WebDriverConnection::get_computed_role(String element_id, IPCMessages::GetComputedRole::Resolver resolve)
{
        // 1. If the current top-level browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_top_level_browsing_context_is_open());
...
void WebDriverConnection::get_computed_label(String element_id, IPCMessages::GetComputedLabel::Resolver resolve)
{
        // 1. If the current browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_browsing_context_is_open());
...
void WebDriverConnection::element_click(String element_id, IPCMessages::ElementClick::Resolver resolve)
{
        // 1. If the current browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_browsing_context_is_open());
...
void WebDriverConnection::element_clear(String element_id, IPCMessages::ElementClear::Resolver resolve)
{
        // 1. If session's current browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_browsing_context_is_open());
...
void WebDriverConnection::element_send_keys(String element_id, JsonValue payload, IPCMessages::ElementSendKeys::Resolver resolve)
{
...
void WebDriverConnection::get_source(IPCMessages::GetSource::Resolver resolve)
{
        // 1. If session's current browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_browsing_context_is_open());
...
void WebDriverConnection::execute_script(JsonValue payload, IPCMessages::ExecuteScript::Resolver resolve)
{
        auto* window = current_browsing_context().active_window();
    auto& vm = window->vm();
...
void WebDriverConnection::execute_async_script(JsonValue payload, IPCMessages::ExecuteAsyncScript::Resolver resolve)
{
        auto* window = current_browsing_context().active_window();
    auto& vm = window->vm();
...
void WebDriverConnection::get_all_cookies(IPCMessages::GetAllCookies::Resolver resolve)
{
        // 1. If the current browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_browsing_context_is_open());
...
void WebDriverConnection::get_named_cookie(String name, IPCMessages::GetNamedCookie::Resolver resolve)
{
        // 1. If the current browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_browsing_context_is_open());
...
void WebDriverConnection::add_cookie(JsonValue payload, IPCMessages::AddCookie::Resolver resolve)
{
...
void WebDriverConnection::delete_cookie(String name, IPCMessages::DeleteCookie::Resolver resolve)
{
        // 1. If the current browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_browsing_context_is_open());
...
void WebDriverConnection::delete_all_cookies(IPCMessages::DeleteAllCookies::Resolver resolve)
{
        // 1. If the current browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_browsing_context_is_open());
...
void WebDriverConnection::perform_actions(JsonValue payload, IPCMessages::PerformActions::Resolver resolve)
{
        // 1. If session's current browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_browsing_context_is_open());
...
void WebDriverConnection::release_actions(IPCMessages::ReleaseActions::Resolver resolve)
{
        // 1. If session's current browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_browsing_context_is_open());
...
void WebDriverConnection::dismiss_alert(IPCMessages::DismissAlert::Resolver resolve)
{
        // 1. If the current top-level browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_top_level_browsing_context_is_open());
    ...
    if (!current_browsing_context().page().has_pending_dialog())
        return RESOLVE(Web::WebDriver::Error::from_code(Web::WebDriver::ErrorCode::NoSuchAlert, "No user dialog is currently open"sv));
...
void WebDriverConnection::accept_alert(IPCMessages::AcceptAlert::Resolver resolve)
{
        // 1. If the current top-level browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_top_level_browsing_context_is_open());
...
void WebDriverConnection::get_alert_text(IPCMessages::GetAlertText::Resolver resolve)
{
        // 1. If the current top-level browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_top_level_browsing_context_is_open());
...
void WebDriverConnection::send_alert_text(JsonValue payload, IPCMessages::SendAlertText::Resolver resolve)
{
    ...
    case Web::Page::PendingDialog::Confirm:
        ...
        return RESOLVE(Web::WebDriver::Error::from_code(Web::WebDriver::ErrorCode::ElementNotInteractable, "Only prompt dialogs may receive text"sv));
    ...
    default:
        ...
        return RESOLVE(Web::WebDriver::Error::from_code(Web::WebDriver::ErrorCode::UnsupportedOperation, "Unknown dialog type"sv));
    }
...
void WebDriverConnection::take_screenshot(IPCMessages::TakeScreenshot::Resolver resolve)
{
        // 1. If session's current top-level browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_top_level_browsing_context_is_open());
...
void WebDriverConnection::take_element_screenshot(String element_id, IPCMessages::TakeElementScreenshot::Resolver resolve)
{
        // 1. If session's current browsing context is no longer open, return error with error code no such window.
    RESOLVE_IF_ERROR(ensure_current_browsing_context_is_open());
...
void WebDriverConnection::print_page(JsonValue payload, IPCMessages::PrintPage::Resolver resolve)
{
        dbgln("FIXME: WebDriverConnection::print_page({})", payload);
    return RESOLVE(Web::WebDriver::Error::from_code(Web::WebDriver::ErrorCode::UnsupportedOperation, "Print not implemented"sv));
}
...
void WebDriverConnection::ensure_top_level_browsing_context_is_open(IPCMessages::EnsureTopLevelBrowsingContextIsOpen::Resolver resolve)
{
    RESOLVE_IF_ERROR(ensure_current_top_level_browsing_context_is_open());
    ER();
}
...
#undef RESOLVE
#undef ER
#undef VALUE_OR_RESOLVE
#undef RESOLVE_IF_ERROR