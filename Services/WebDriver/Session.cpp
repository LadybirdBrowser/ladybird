...
ErrorOr<NonnullRefPtr<Web::WebDriver::Promise<Session::NewSession>>> Session::create(NonnullRefPtr<Client> client, JsonValue initial_capabilities, Web::WebDriver::SessionFlags flags)
{
    auto session_creation_promise = Web::WebDriver::Promise<NewSession>::construct();
    ...
    session_start_promise->when_resolved([final_capabilities = move(initial_capabilities), session, flags, session_creation_promise](auto&) mutable -> ErrorOr<void> {
        ...
        Vector<NonnullRefPtr<Web::WebDriver::Promise<JsonValue>>> setup_promises;

        auto set_user_prompt_handler_promise = Web::WebDriver::Promise<JsonValue>::construct();
        ...
        if (has_flag(flags, Web::WebDriver::SessionFlags::Http)) {
            ...
            if (auto strategy = capabilities.get_string("pageLoadStrategy"sv); strategy.has_value()) {
                ...
                auto set_page_load_strategy_promise = Web::WebDriver::Promise<JsonValue>::construct();
            ...
            if (auto strict_file_interactability = capabilities.get_bool("strictFileInteractability"sv); strict_file_interactability.has_value()) {
                ...
                auto set_strict_file_interactiblity_promise = Web::WebDriver::Promise<JsonValue>::construct();
            ...
            if (auto timeouts = capabilities.get_object("timeouts"sv); timeouts.has_value()) {
                auto set_timeouts_promise = Web::WebDriver::Promise<JsonValue>::construct();
        ...
        auto set_is_webdriver_active_promise = Web::WebDriver::Promise<JsonValue>::construct();
        ...
        auto after_all_setup_promises = Web::WebDriver::Promise<JsonValue>::after(move(setup_promises));
...
NonnullRefPtr<Web::WebDriver::Promise<Empty>> Session::close()
{
    if (m_close_promise)
        return *m_close_promise;

    m_close_promise = Web::WebDriver::Promise<Empty>::construct();
    ...
    Vector<NonnullRefPtr<Web::WebDriver::Promise<JsonValue>>> all_session_close_promises;
    ...
    if (s_sessions.is_empty()) {
        ...
        for (auto& it : m_windows) {
            auto close_session_promise = Web::WebDriver::Promise<JsonValue>::construct();
    ...
    auto after_all_sessions_closed_promise = Web::WebDriver::Promise<JsonValue>::after(move(all_session_close_promises));
...
ErrorOr<NonnullRefPtr<Core::LocalServer>> Session::create_server(NonnullRefPtr<ServerPromise> promise)
{
    ...
    server->on_accept = [this_ref = NonnullRefPtr { *this }, promise](auto client_socket) {
        ...
        inner_promise->when_resolved([web_content_connection, this_ref, promise](JsonValue& window_handle_value) {
            ...
            auto set_strict_file_interactability_promise = Web::WebDriver::Promise<JsonValue>::construct();
...
void Session::set_timeouts(JsonValue payload, NonnullRefPtr<Web::WebDriver::Promise<JsonValue>> top_level_promise)
{
    auto inner_promise = Web::WebDriver::Promise<JsonValue>::construct();
...
void Session::close_window(NonnullRefPtr<Web::WebDriver::Promise<JsonValue>> top_level_promise)
{
    auto inner_promise = Web::WebDriver::Promise<JsonValue>::construct();
    top_level_promise->add_child(inner_promise);
...
void Session::switch_to_window(StringView handle, NonnullRefPtr<Web::WebDriver::Promise<JsonValue>> top_level_promise)
{
