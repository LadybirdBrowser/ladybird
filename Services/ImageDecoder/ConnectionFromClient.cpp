...
NonnullRefPtr<ConnectionFromClient::Job> ConnectionFromClient::make_decode_image_job(i64 request_token, Core::AnonymousBuffer encoded_buffer, Optional<Gfx::IntSize> ideal_size, Optional<ByteString> mime_type)
{
    return Job::construct(
        ...
        [strong_this = NonnullRefPtr(*this), request_token](DecodeResult result) -> ErrorOr<void> {
            i64 session_id = 0;
            ...
            strong_this->async_did_decode_image(request_token, result.is_animated, result.loop_count, move(result.bitmaps), move(result.durations), result.scale, move(result.color_profile), session_id);
            strong_this->m_pending_jobs.remove(request_token);
            return {};
        },
        [strong_this = NonnullRefPtr(*this), request_token](Error error) -> void {
            if (strong_this->is_open())
                strong_this->async_did_fail_to_decode_image(request_token, MUST(String::formatted("Decoding failed: {}", error)));
            strong_this->m_pending_jobs.remove(request_token);
        });
}

void ConnectionFromClient::decode_image(Core::AnonymousBuffer encoded_buffer, Optional<Gfx::IntSize> ideal_size, Optional<ByteString> mime_type, i64 request_token)
{
    if (!encoded_buffer.is_valid()) {
        dbgln_if(IMAGE_DECODER_DEBUG, "Encoded data is invalid");
        async_did_fail_to_decode_image(request_token, "Encoded data is invalid"_string);
        return;
    }

    m_pending_jobs.set(request_token, make_decode_image_job(request_token, move(encoded_buffer), ideal_size, move(mime_type)));
}

void ConnectionFromClient::cancel_decoding(i64 request_token)
{
    if (auto job = m_pending_jobs.take(request_token); job.has_value()) {
        job.value()->cancel();
    }
}
