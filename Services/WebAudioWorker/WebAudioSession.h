...
struct WorkletState {
    ...
    struct ProcessorRegistration {
        String name;
        Vector<Web::WebAudio::Render::AudioParamDescriptor> descriptors;
        u64 generation { 0 };
    };
    ...
    Function<void(String const&, Vector<Web::WebAudio::Render::AudioParamDescriptor> const&, u64)>
        processor_registration_callback;
    Function<void(u64 module_id, u64 required_generation, bool success, String const& error_name,
        ...
        module_evaluation_callback;

    Threading::Mutex ports_mutex;
    HashMap<u64, IPC::TransportHandle> processor_port_handles;
...
struct StreamState {
    ...
    Threading::Mutex media_stream_destination_streams_mutex;
    ...
    HashMap<u64, MediaElement> media_stream_destination_streams;
    HashMap<u64, MediaStream> media_stream_streams;
    Atomic<ScriptProcessorStreamMap*> script_processor_streams { nullptr };
};

class WebAudioSession : public RefCounted<WebAudioSession>
    , public Weakable<WebAudioSession> {
public:
    WebAudioSession(u64 session_id, u32 device_sample_rate_hz, u32 device_channel_count,
        Core::AnonymousBuffer timing_buffer, int timing_notify_write_fd, int client_id);
    ~WebAudioSession();
    ...
    void set_media_element_audio_source_streams(
        Vector<Web::WebAudio::Render::MediaElementAudioSourceStreamDescriptor>&&);
    void set_media_stream_audio_source_streams(
        Vector<Web::WebAudio::Render::MediaStreamAudioSourceStreamDescriptor> const&);
    ...
    void set_worklet_node_ports(Vector<Web::WebAudio::Render::WorkletNodePortDescriptor>);
    ...
    void set_worklet_processor_registration_callback(
        Function<void(String const&, Vector<Web::WebAudio::Render::AudioParamDescriptor> const&, u64)>);
    void set_worklet_module_evaluation_callback(
        Function<void(u64 module_id, u64 required_generation, bool success, String const& error_name,
            String const& error_message, Vector<String> failed_processor_registrations)>);

    using InputStreamReadyCallback = Function<void(Optional<Audio::InputStreamDescriptor>)>;

    ErrorOr<void> create_audio_input_stream_async(Audio::DeviceHandle device_handle, u64 capacity_frames,
        InputStreamReadyCallback);
    void destroy_audio_input_stream(u64 stream_id);

    void set_suspended(bool suspended, u64 generation);

    bool render_one_quantum(u64 output_playback_head_frames, u64 output_tail_frames);
...
private:
    ...
    struct ThreadLoopState {
        ...
        Optional<u64> pending_suspend_output_tail_frame;
        u64 pending_suspend_generation { 0 };
        u64 acknowledged_suspend_generation { 0 };
    ...
    void notify_worklet_processor_registered_from_render_thread(
        String const& name, Vector<Web::WebAudio::Render::AudioParamDescriptor> const& descriptors,
        u64 generation);
    void notify_worklet_module_evaluated_from_render_thread(u64 module_id, u64 required_generation,
    ...
    void synchronize_media_stream_destination_streams(Render::GraphDescription const&, Render::GraphResources&);
    void request_media_stream_binding_async(u64 provider_id,
        Web::WebAudio::Render::AudioInputStreamMetadata metadata,
    ...
    Render::AudioContextPlaybackPolicy m_playback_policy;
