...
static void apply_output_wait_decision(Render::AudioContextPlaybackPolicy::OutputWaitDecision const& decision)
{
    switch (decision.action) {
    case Render::AudioContextPlaybackPolicy::OutputWaitDecision::Action::RenderNow:
        break;
    case Render::AudioContextPlaybackPolicy::OutputWaitDecision::Action::Sleep:
        (void)Core::System::sleep_ms(decision.sleep_ms);
        break;
    case Render::AudioContextPlaybackPolicy::OutputWaitDecision::Action::Yield:
        sched_yield();
        break;
    }
}
...
ErrorOr<void> WebAudioRenderThread::open_output_stream(u32 target_latency_ms,
    OutputStreamIsOpenCallback callback)
{
    ...
    if (!m_output_session)
        m_output_session = TRY(Audio::SingleSinkSessionClient::try_create());
    ...
    auto request_or_error = m_output_session->request_output_sink(
        [this, request_target_latency_ms, fail_pending_requests](Audio::OutputSink const& output_sink) {
...
void WebAudioRenderThread::ensure_thread_started()
{
    ...
    m_render_thread = Threading::Thread::construct("RenderThread"sv, [this] { return render_thread_main(); });
...
intptr_t WebAudioRenderThread::render_thread_main()
{
    ...
    while (!m_should_stop.load(AK::MemoryOrder::memory_order_acquire)) {
        ...
        for (;;) {
            ...
            auto decision = Render::AudioContextPlaybackPolicy::decide_when_output_ring_has_room(
                available_bytes, quantum_bytes, bytes_per_frame, sample_rate_hz, render_cost_ema_ns);
            if (decision.action == Render::AudioContextPlaybackPolicy::OutputWaitDecision::Action::RenderNow)
                break;
            apply_output_wait_decision(decision);
        }
        ...
        u64 const output_tail_frames = m_output_state.rendered_frames;
        u64 const output_playback_head_frames = output_tail_frames
            - static_cast<u64>(ring->available_to_read() / bytes_per_frame);

        for (auto& session : sessions) {
            bool did_render = session->render_one_quantum(output_playback_head_frames,
                output_tail_frames);
            if (did_render) {
        ...
        auto bytes = ReadonlyBytes { reinterpret_cast<u8 const*>(m_mix_interleaved.data()),
            m_mix_interleaved.size() * sizeof(float) };
        ...
        while (ring->available_to_write() < quantum_bytes) {
            ...
            auto decision = Render::AudioContextPlaybackPolicy::decide_when_output_ring_has_room(
                ring->available_to_write(), quantum_bytes, bytes_per_frame, sample_rate_hz,
                render_cost_ema_ns);
            if (decision.action == Render::AudioContextPlaybackPolicy::OutputWaitDecision::Action::RenderNow)
                continue;
            apply_output_wait_decision(decision);
        ...
        if (perf_log_enabled) {
            ...
            if (last_perf_log_ms == 0 || (now_ms - last_perf_log_ms) >= 2000) {
                ...
                dbgln("[WebAudio][Perf] render_ema_ms={:.3f} quantum_ms={:.3f} ratio={:.3f}", render_cost_ms,
                    quantum_duration_ms, render_ratio);
            }
        }
    }
