    ...
    , m_timing_notify_write_fd(timing_notify_write_fd)
{
    ...
    if (m_timing_buffer.is_valid() && m_timing_buffer.size() >= sizeof(TimingFeedbackPage))
...
bool WebAudioSession::render_one_quantum()
{
    ...
    bool const quantum_is_suspended = m_requested_suspended.load(AK::MemoryOrder::memory_order_acquire) != 0;
    ...
    if (!quantum_is_suspended || !m_thread_state.last_timing_page_was_suspended)
        update_timing_page_and_notify(quantum_is_suspended, suspend_generation);
...
void WebAudioSession::update_timing_page_and_notify(bool quantum_is_suspended, u64 suspend_generation)
{
    ...
    write_timing_page(*m_timing_page, context_sample_rate_hz, m_device_channel_count, m_scratch.rendered_frames, underruns, graph_generation, quantum_is_suspended, suspend_generation);
...
void WebAudioSession::set_media_element_audio_source_streams(Vector<MediaElementAudioSourceStreamDescriptor>&& streams)
{
    ...
    for (size_t i = 0; i < streams.size(); ++i) {
        ...
        if (Web::WebAudio::should_log_media_element_bridge()) {
            ...
            dbgln("cid={}: WebAudio session bind provider {} stream: sr={}Hz ch={} cap_ch={} cap_frames={} read={} write={} eos={} timeline_gen={} timeline_sr={}",
                m_client_id,
                binding.provider_id,
...
{
    return a.device_handle == b.device_handle
...
void WebAudioSession::request_media_stream_binding_async(u64 provider_id, Web::WebAudio::Render::AudioInputStreamMetadata metadata, u64 binding_generation)
{
    auto weak_self = make_weak_ptr();
    auto begin_or_error = create_audio_input_stream_async(metadata.device_handle, metadata.capacity_frames, [weak_self, provider_id, metadata, binding_generation](Optional<AudioServer::InputStreamDescriptor> maybe_descriptor) {
        ...
        AudioServer::InputStreamDescriptor descriptor = maybe_descriptor.release_value();
        ...
        if (view_or_error.is_error()) {
            warnln("cid={}: WebAudio session rejected media stream source for provider {}: {}", self->m_client_id, provider_id, view_or_error.error());
        ...
        int notify_read_fd = descriptor.notify_fd.take_fd();
        if (Web::WebAudio::should_log_media_element_bridge())
            dbgln("cid={}: WebAudio session bind media stream provider {} stream_id={} notify_read_fd={}", self->m_client_id, provider_id, descriptor.stream_id, notify_read_fd);

        auto provider = Web::WebAudio::MediaElementAudioSourceProvider::create_for_remote_consumer(provider_id, view_or_error.release_value(), move(ring_descriptor.shared_memory), notify_read_fd);
...
ErrorOr<void> WebAudioSession::create_audio_input_stream_async(AudioServer::DeviceHandle device_handle, u64 capacity_frames, InputStreamReadyCallback on_ready)
{
    if (device_handle == 0)
        return Error::from_string_literal("Invalid input device handle");

    auto client = AudioServer::SessionClientOfAudioServer::default_client();
    if (!client)
    ...
    auto request_or_error = client->create_input_stream(device_handle, capacity_frames, [request_state](AudioServer::InputStreamDescriptor descriptor) {
            if (request_state->completed)
                return;
            request_state->completed = true;
            request_state->ready_callback(move(descriptor)); }, [request_state]([[maybe_unused]] ByteString const& error) {
            if (request_state->completed)
                return;
            request_state->completed = true;
...
void WebAudioSession::destroy_audio_input_stream(u64 stream_id)
{
    ...
    if (auto client = AudioServer::SessionClientOfAudioServer::default_client(); client)
...
void WebAudioSession::shutdown()
{
    ...
    auto run_control_cleanup = [retired_graphs, media_stream_ids = move(media_stream_ids)]() mutable {
