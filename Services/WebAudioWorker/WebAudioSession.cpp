...
static bool defer_to_main_event_loop(Function<void()> callback)
{
    auto event_loop = Web::WebAudio::main_event_loop();
    if (!event_loop)
        return false;

    auto strong_loop = event_loop->take();
    if (!strong_loop)
        return false;

    strong_loop->deferred_invoke(move(callback));
    return true;
}

static HashMap<u64, RefPtr<MediaElementAudioSourceProvider>> s_media_stream_destination_provider_registry;

static void register_media_stream_destination_provider(u64 provider_id, MediaElementAudioSourceProvider& provider)
{
    ASSERT_CONTROL_THREAD();
    if (provider_id == 0)
        return;
    s_media_stream_destination_provider_registry.set(provider_id, provider);
}

static void unregister_media_stream_destination_provider(u64 provider_id)
{
    ASSERT_CONTROL_THREAD();
    if (provider_id == 0)
        return;
    s_media_stream_destination_provider_registry.remove(provider_id);
}

static RefPtr<MediaElementAudioSourceProvider> lookup_media_stream_destination_provider(u64 provider_id)
{
    ASSERT_CONTROL_THREAD();
    auto provider = s_media_stream_destination_provider_registry.get(provider_id);
    if (!provider.has_value())
        return {};
    return provider.release_value();
}

static bool resolve_media_stream_source_providers_from_destination_registry(GraphDescription const& description,
    GraphResources& resources, int client_id, u64 session_id)
{
    ASSERT_CONTROL_THREAD();
    ...
    for (auto const& it : description.nodes) {
        if (!it.value.has<MediaStreamAudioSourceGraphNode>())
            continue;

        u64 const provider_id = it.value.get<MediaStreamAudioSourceGraphNode>().provider_id;
        if (provider_id == 0)
            continue;
        if (resources.media_stream_audio_sources.contains(provider_id))
            continue;

        auto provider = lookup_media_stream_destination_provider(provider_id);
        if (!provider)
            continue;

        resources.media_stream_audio_sources.set(provider_id, NonnullRefPtr { *provider });
        resolved_any = true;

        if (Web::WebAudio::should_log_webaudio(LOG_MEDIA)) {
            dbgln("cid={}: WebAudio session {} resolved media stream source provider {} from destination registry",
                client_id, session_id, provider_id);
        }
    }
...
struct StreamState::DynamicsCompressorStreamMap
    : public AtomicRefCounted<StreamState::DynamicsCompressorStreamMap> {
public:
...
WebAudioSession::WebAudioSession(u64 session_id, u32 device_sample_rate_hz, u32 device_channel_count,
    Core::AnonymousBuffer timing_buffer, int timing_notify_write_fd,
    int client_id)
    : m_session_id(session_id)
    , m_client_id(client_id)
    , m_device_sample_rate_hz(device_sample_rate_hz)
    , m_device_channel_count(device_channel_count)
    , m_playback_policy(move(timing_buffer), timing_notify_write_fd, device_sample_rate_hz,
          device_channel_count)
{
    m_requested_suspended.store(1, AK::MemoryOrder::memory_order_release);
    m_requested_suspend_generation.store(0, AK::MemoryOrder::memory_order_release);
...
WebAudioSession::~WebAudioSession() { shutdown(); }
...
bool WebAudioSession::render_one_quantum(u64 output_playback_head_frames, u64 output_tail_frames)
{
    ensure_render_thread_scratch_initialized();

    bool const suspend_requested = m_requested_suspended.load(AK::MemoryOrder::memory_order_acquire) != 0;
    u64 const suspend_generation = m_requested_suspend_generation.load(AK::MemoryOrder::memory_order_acquire);

    if (!suspend_requested) {
        m_thread_state.pending_suspend_output_tail_frame.clear();
        m_thread_state.pending_suspend_generation = 0;
    } else if (suspend_generation != m_thread_state.acknowledged_suspend_generation
        && (!m_thread_state.pending_suspend_output_tail_frame.has_value()
            || suspend_generation != m_thread_state.pending_suspend_generation)) {
        m_thread_state.pending_suspend_output_tail_frame = output_tail_frames;
        m_thread_state.pending_suspend_generation = suspend_generation;
    }

    if (m_thread_state.pending_suspend_output_tail_frame.has_value()
        && output_playback_head_frames >= m_thread_state.pending_suspend_output_tail_frame.value()) {
        m_thread_state.acknowledged_suspend_generation = m_thread_state.pending_suspend_generation;
        m_thread_state.pending_suspend_output_tail_frame.clear();
    }

    bool const timing_page_is_suspended = suspend_requested
        && suspend_generation == m_thread_state.acknowledged_suspend_generation;
    ...
    bool const should_render_audio = !suspend_requested;
    ...
    if (!timing_page_is_suspended || !m_thread_state.last_timing_page_was_suspended)
        update_timing_page_and_notify(timing_page_is_suspended, suspend_generation);
    m_thread_state.last_timing_page_was_suspended = timing_page_is_suspended;

    return did_render;
}

ReadonlySpan<f32> WebAudioSession::interleaved_output() const { return m_scratch.interleaved.span(); }
...
void WebAudioSession::publish_analyser_snapshots(ThreadLoopState const& state) const
{
    ...
    if (!analyser_streams_index->streams.is_empty()) {
        ...
        for (size_t analyser_index = 0; analyser_index < analyser_count; ++analyser_index) {
            ...
            auto expected_used_size = analyser_feedback_page_size(stream_state.fft_size);
            if (block.size() < expected_used_size) {
                (void)stream_state.stream.try_release_block_index(block_index.value());
                continue;
            }

            auto* header = reinterpret_cast<AnalyserFeedbackHeader*>(block.data());
            ...
            if (!stream_state.stream.try_submit_ready_block(block_index.value(),
                    static_cast<u32>(expected_used_size)))
                (void)stream_state.stream.try_release_block_index(block_index.value());
        }
    }
...
void WebAudioSession::publish_dynamics_compressor_snapshots(ThreadLoopState const& state) const
{
    ...
    if (!compressor_streams_index->streams.is_empty()) {
        for (auto& it : compressor_streams_index->streams) {
            ...
            auto block = stream_state.stream.block_bytes(block_index.value());
            size_t expected_used_size = sizeof(Web::WebAudio::Render::CompressorFeedbackPage);
            if (block.size() < expected_used_size) {
                (void)stream_state.stream.try_release_block_index(block_index.value());
                continue;
            }

            auto* header = reinterpret_cast<CompressorFeedbackPage*>(block.data());
            header->compressor_node_id = compressor_node_id;
            header->rendered_frames_total = m_scratch.rendered_frames;

            f32 reduction_db = 0.0f;
            (void)state.current_graph->executor->try_copy_dynamics_compressor_reduction(
                Web::WebAudio::NodeID { compressor_node_id }, reduction_db);
...
void WebAudioSession::update_timing_page_and_notify(bool quantum_is_suspended, u64 suspend_generation)
{
    ...
    (void)m_playback_policy.publish_timing_update(context_sample_rate_hz, m_scratch.rendered_frames,
        underruns, graph_generation, quantum_is_suspended, suspend_generation);
}
...
void WebAudioSession::update_and_maybe_log_output_levels(ThreadLoopState& state)
{
    ...
    if (sample_rate > 0 && m_scratch.frames_written - state.last_level_log_frame >= sample_rate) {
        auto rms = state.level_sample_count > 0
            ? AK::sqrt(state.level_sum_squares / static_cast<f64>(state.level_sample_count))
            : 0.0;
        dbgln("cid={}: WebAudio session={} output level peak={:.6f} rms={:.6f}", m_client_id, m_session_id,
            state.level_peak, rms);
...
void WebAudioSession::maybe_swap_graph(ThreadLoopState& state, u64 generation)
{
    ...
    if (Web::WebAudio::should_log_webaudio(LOG_GENERAL)) {
        dbgln("cid={}: WebAudio session={} swapped engine graph (nodes={} connections={})", m_client_id,
            m_session_id, state.current_graph->build.description.nodes.size(),
...
void WebAudioSession::maybe_log_script_processor_never_ran(ThreadLoopState& state)
{
    ...
    if (frames_since_swap >= static_cast<u64>(sample_rate_hz / 2) && processed_blocks == 0) {
        if (Web::WebAudio::should_log_webaudio(LOG_OUTPUT) || Web::WebAudio::should_log_webaudio(LOG_SCRIPT_PROCESSOR))
            dbgln("cid={}: WebAudio session={} ScriptProcessor has not run after {} frames", m_client_id,
                m_session_id, frames_since_swap);
...
void WebAudioSession::set_analyser_stream(u64 analyser_node_id, u32 fft_size, SharedBufferStream stream)
{
    if (!stream.is_valid())
        return;
    ...
    new_index->streams.set(analyser_node_id,
        StreamState::Analyser { .fft_size = fft_size, .stream = move(stream) });
...
void WebAudioSession::set_dynamics_compressor_stream(u64 compressor_node_id, SharedBufferStream stream)
{
    if (!stream.is_valid())
        return;
...
{
    if (Web::WebAudio::should_log_webaudio(LOG_GENERAL))
        dbgln("cid={}: WebAudio session={} received worklet module id={} '{}' ({} bytes)", m_client_id,
            m_session_id, module_id, url, source_text.length());
...
void WebAudioSession::ensure_worklet_host()
{
    ...
    Vector<WorkletPortBinding> port_bindings;
    {
        ...
        port_bindings.ensure_capacity(m_worklet.processor_port_handles.size());
        for (auto& it : m_worklet.processor_port_handles) {
            port_bindings.append(WorkletPortBinding {
                .node_id = Web::WebAudio::NodeID { it.key },
                .processor_port_handle = move(it.value),
            });
        }
        m_worklet.processor_port_handles.clear();
    }
    ...
    if (Web::WebAudio::should_log_webaudio(LOG_GENERAL)) {
        dbgln(
            "cid={}: WebAudio session={} creating worklet host (modules={} node_definitions={} port_bindings={})",
            m_client_id, m_session_id, modules.size(), node_definitions.size(), port_bindings.size());
        for (auto const& binding : port_bindings)
            dbgln("cid={}: WebAudio session={} worklet host port binding node_id={}", m_client_id, m_session_id,
                binding.node_id.value());
    }
    ...
    m_worklet.host = make<RealtimeAudioWorkletProcessorHost>(
        m_scratch.rendered_frames, static_cast<float>(context_sample_rate_hz), move(modules),
        move(node_definitions), move(port_bindings));
    ...
    m_worklet.host->set_processor_registration_callback(
        [weak_self = make_weak_ptr()](String const& name,
    ...
    m_worklet.host->set_worklet_module_evaluation_callback(
        [weak_self = make_weak_ptr()](u64 module_id, u64 required_generation, bool success,
            String const& error_name, String const& error_message,
            Vector<String> failed_processor_registrations) {
            if (auto self = weak_self.strong_ref())
                self->notify_worklet_module_evaluated_from_render_thread(module_id, required_generation, success,
...
void WebAudioSession::set_worklet_processor_registration_callback(
    Function<void(String const&, Vector<Web::WebAudio::Render::AudioParamDescriptor> const&, u64)>
        callback)
{
    m_worklet.processor_registration_callback = move(callback);
}

void WebAudioSession::set_worklet_module_evaluation_callback(
...
void WebAudioSession::notify_worklet_processor_error_from_render_thread(Web::WebAudio::NodeID node_id)
{
    ...
    bool expected = false;
    if (!m_worklet.error_task_scheduled.compare_exchange_strong(expected, true,
            AK::MemoryOrder::memory_order_acq_rel))
        return;

    if (!defer_to_main_event_loop([weak_self = make_weak_ptr()] {
            if (auto self = weak_self.strong_ref())
...
void WebAudioSession::notify_worklet_processor_registered_from_render_thread(
    String const& name, Vector<Web::WebAudio::Render::AudioParamDescriptor> const& descriptors,
    u64 generation)
{
    (void)m_worklet.registration_queue.try_push(
        WorkletState::ProcessorRegistration { name, descriptors, generation });

    bool expected = false;
    if (!m_worklet.registration_task_scheduled.compare_exchange_strong(expected, true,
            AK::MemoryOrder::memory_order_acq_rel))
        return;
...
void WebAudioSession::notify_worklet_module_evaluated_from_render_thread(
    u64 module_id, u64 required_generation, bool success, String const& error_name,
    String const& error_message, Vector<String> failed_processor_registrations)
{
    (void)m_worklet.module_evaluation_queue.try_push(
        WorkletState::ModuleEvaluation { module_id, required_generation, success, error_name, error_message,
            move(failed_processor_registrations) });

    bool expected = false;
    if (!m_worklet.module_evaluation_task_scheduled.compare_exchange_strong(
            expected, true, AK::MemoryOrder::memory_order_acq_rel))
        return;
...
void WebAudioSession::flush_worklet_module_evaluations()
{
    ...
    while (m_worklet.module_evaluation_queue.try_pop(evaluation))
        m_worklet.module_evaluation_callback(evaluation.module_id, evaluation.required_generation,
            evaluation.success, evaluation.error_name, evaluation.error_message,
            move(evaluation.failed_processor_registrations));
...
void WebAudioSession::flush_worklet_processor_registrations()
{
    ...
    while (m_worklet.registration_queue.try_pop(registration))
        m_worklet.processor_registration_callback(registration.name, registration.descriptors,
...
void WebAudioSession::set_render_graph(WireGraphBuildResult graph)
{
    if (!Web::WebAudio::current_thread_is_control_thread()) {
        ...
        (void)defer_to_main_event_loop([weak_self = make_weak_ptr()] {
            if (auto self = weak_self.strong_ref())
...
void WebAudioSession::set_media_element_audio_source_streams(
    Vector<MediaElementAudioSourceStreamDescriptor>&& streams)
{
    // Called on the control thread.
    if (Web::WebAudio::should_log_webaudio(LOG_MEDIA))
        dbgln("cid={}: WebAudio session received {} media element stream binding(s)", m_client_id,
            streams.size());
    ...
    for (size_t i = 0; i < streams.size(); ++i) {
        ...
        if (view_or_error.is_error()) {
            warnln("cid={}: WebAudio session rejected media element stream for provider {}: {}", m_client_id,
                binding.provider_id, view_or_error.error());
            continue;
        }
        ...
        if (Web::WebAudio::should_log_webaudio(LOG_MEDIA)) {
            ...
            u32 end_of_stream = AK::atomic_load(&header.end_of_stream, AK::MemoryOrder::memory_order_relaxed);
            ...
            dbgln("cid={}: WebAudio session bind provider {} stream: sr={}Hz ch={} cap_ch={} cap_frames={} read={} "
                  "write={} eos={} timeline_gen={} timeline_sr={}",
                m_client_id, binding.provider_id, header.sample_rate_hz, header.channel_count,
                header.channel_capacity, header.capacity_frames, read_frame, write_frame, end_of_stream,
                timeline_generation, timeline_sample_rate);
        }
        int notify_read_fd = descriptor.notify_fd.take_fd();
        if (Web::WebAudio::should_log_webaudio(LOG_MEDIA))
            dbgln("cid={}: WebAudio session bind provider {} notify_read_fd={}", m_client_id, binding.provider_id,
                notify_read_fd);
        auto provider = Web::WebAudio::MediaElementAudioSourceProvider::create_for_remote_consumer(
            binding.provider_id, view, move(descriptor.shared_memory), notify_read_fd);
    ...
    rebuild_graph_with_current_stream_bindings();
}

static bool media_stream_metadata_matches(Web::WebAudio::Render::AudioInputStreamMetadata const& a,
    Web::WebAudio::Render::AudioInputStreamMetadata const& b)
{
    return a.device_handle == b.device_handle && a.sample_rate_hz == b.sample_rate_hz && a.channel_count == b.channel_count && a.capacity_frames == b.capacity_frames;
}

void WebAudioSession::synchronize_media_stream_destination_streams(GraphDescription const& description,
    GraphResources& resources)
{
    ASSERT_CONTROL_THREAD();

    HashTable<u64> seen_provider_ids;
    for (auto const& it : description.nodes) {
        if (!it.value.has<MediaStreamAudioDestinationGraphNode>())
            continue;
        u64 const provider_id = it.value.get<MediaStreamAudioDestinationGraphNode>().provider_id;
        if (provider_id == 0)
            continue;
        seen_provider_ids.set(provider_id);
    }

    Vector<u64> provider_ids_to_remove;
    for (auto const& it : m_streams.media_stream_destination_streams) {
        if (!seen_provider_ids.contains(it.key))
            provider_ids_to_remove.append(it.key);
    }
    for (u64 provider_id : provider_ids_to_remove) {
        unregister_media_stream_destination_provider(provider_id);
        m_streams.media_stream_destination_streams.remove(provider_id);
    }
    ...
    for (auto const& it : description.nodes) {
        ...
        auto provider_it = m_streams.media_stream_destination_streams.find(provider_id);
        if (provider_it == m_streams.media_stream_destination_streams.end()) {
            auto provider = Web::WebAudio::MediaElementAudioSourceProvider::create(8,
                static_cast<size_t>(RENDER_QUANTUM_SIZE * 64));
            provider->set_debug_connection_info(m_client_id, m_session_id);
            m_streams.media_stream_destination_streams.set(provider_id, StreamState::MediaElement {
                                                                            .provider = move(provider),
                                                                        });
            provider_it = m_streams.media_stream_destination_streams.find(provider_id);
        }
        register_media_stream_destination_provider(provider_id, *provider_it->value.provider);
        resources.media_stream_audio_sources.set(provider_id, provider_it->value.provider);
    }
}

void WebAudioSession::rebuild_graph_with_current_stream_bindings()
{
    u32 device_sample_rate_hz = m_device_sample_rate_hz;
    ...
    WireGraphBuildResult build {
        ...
        .resources = make<GraphResources>(*base_graph->build.resources),
    };
    build.resources->media_element_audio_sources.clear();
    {
        Threading::MutexLocker locker(m_streams.media_element_streams_mutex);
        for (auto const& it : m_streams.media_element_streams)
            build.resources->media_element_audio_sources.set(it.key, it.value.provider);
    }

    build.resources->media_stream_audio_sources.clear();
    {
        Threading::MutexLocker locker(m_streams.media_stream_streams_mutex);
        for (auto const& it : m_streams.media_stream_streams)
            build.resources->media_stream_audio_sources.set(it.key, it.value.provider);
    }
    synchronize_media_stream_destination_streams(build.description, *build.resources);
    (void)resolve_media_stream_source_providers_from_destination_registry(build.description, *build.resources,
        m_client_id, m_session_id);

    build.resources->script_processor_host = m_script_processor_host.ptr();

    auto executor = make<GraphExecutor>(build.description, static_cast<f32>(context_sample_rate_hz),
        static_cast<size_t>(RENDER_QUANTUM_SIZE), *build.resources);
...
void WebAudioSession::request_media_stream_binding_async(
    u64 provider_id, Web::WebAudio::Render::AudioInputStreamMetadata metadata, u64 binding_generation)
{
    auto weak_self = make_weak_ptr();
    auto begin_or_error = create_audio_input_stream_async(
        metadata.device_handle, metadata.capacity_frames,
        [weak_self, provider_id, metadata,
            binding_generation](Optional<Audio::InputStreamDescriptor> maybe_descriptor) {
            auto self = weak_self.strong_ref();
            if (!self)
                return;

            {
                Threading::MutexLocker pending_locker(self->m_media_stream_pending_mutex);
                auto generation_it = self->m_pending_media_stream_binding_generations.find(provider_id);
                if (generation_it == self->m_pending_media_stream_binding_generations.end() || generation_it->value != binding_generation) {
                    if (maybe_descriptor.has_value())
                        self->destroy_audio_input_stream(maybe_descriptor->stream_id);
                    return;
                }

                auto desired_it = self->m_desired_media_stream_bindings.find(provider_id);
                if (desired_it == self->m_desired_media_stream_bindings.end() || !media_stream_metadata_matches(desired_it->value, metadata)) {
                    self->m_pending_media_stream_binding_generations.remove(provider_id);
                    if (maybe_descriptor.has_value())
                        self->destroy_audio_input_stream(maybe_descriptor->stream_id);
                    return;
                }
            ...
            Audio::InputStreamDescriptor descriptor = maybe_descriptor.release_value();
            if (descriptor.stream_id == 0)
                return;

            RingStreamDescriptor ring_descriptor;
            ring_descriptor.stream_id = descriptor.stream_id;
            ring_descriptor.format.sample_rate_hz = descriptor.sample_rate_hz;
            ring_descriptor.format.channel_count = descriptor.channel_count;
            ring_descriptor.format.channel_capacity = descriptor.channel_capacity;
            ring_descriptor.format.capacity_frames = descriptor.capacity_frames;
            ring_descriptor.shared_memory = descriptor.shared_memory;

            auto view_or_error = validate_ring_stream_descriptor(ring_descriptor);
            if (view_or_error.is_error()) {
                warnln("cid={}: WebAudio session rejected media stream source for provider {}: {}",
                    self->m_client_id, provider_id, view_or_error.error());
                self->destroy_audio_input_stream(descriptor.stream_id);
                return;
            }

            int notify_read_fd = descriptor.notify_fd.take_fd();
            if (Web::WebAudio::should_log_webaudio(LOG_MEDIA))
                dbgln("cid={}: WebAudio session bind media stream provider {} stream_id={} notify_read_fd={}",
                    self->m_client_id, provider_id, descriptor.stream_id, notify_read_fd);

            auto provider = Web::WebAudio::MediaElementAudioSourceProvider::create_for_remote_consumer(
                provider_id, view_or_error.release_value(), move(ring_descriptor.shared_memory), notify_read_fd);
            provider->set_debug_connection_info(self->m_client_id, self->m_session_id);
            ...
            Optional<u64> replaced_stream_id;
            {
                Threading::MutexLocker streams_locker(self->m_streams.media_stream_streams_mutex);
                auto existing_it = self->m_streams.media_stream_streams.find(provider_id);
                if (existing_it != self->m_streams.media_stream_streams.end())
                    replaced_stream_id = existing_it->value.stream_id;
                self->m_streams.media_stream_streams.set(provider_id, StreamState::MediaStream {
                                                                          .metadata = metadata,
            ...
            if (replaced_stream_id.has_value() && replaced_stream_id.value() != descriptor.stream_id)
                self->destroy_audio_input_stream(replaced_stream_id.value());

            self->rebuild_graph_with_current_stream_bindings();
        });

    if (begin_or_error.is_error()) {
        Threading::MutexLocker pending_locker(m_media_stream_pending_mutex);
        m_pending_media_stream_binding_generations.remove(provider_id);
    }
}

void WebAudioSession::set_media_stream_audio_source_streams(
    Vector<MediaStreamAudioSourceStreamDescriptor> const& streams)
{
    // Called on the control thread.
    if (Web::WebAudio::should_log_webaudio(LOG_MEDIA))
        dbgln("cid={}: WebAudio session received {} media stream source binding(s)", m_client_id, streams.size());

    HashMap<u64, Web::WebAudio::Render::AudioInputStreamMetadata> desired_bindings;
    for (auto const& binding : streams) {
        if (binding.provider_id == 0)
            continue;
        if (binding.metadata.device_handle == 0)
            continue;
        desired_bindings.set(binding.provider_id, binding.metadata);
    }
    ...
    for (auto const& desired_binding : desired_bindings) {
        u64 provider_id = desired_binding.key;
        auto const& metadata = desired_binding.value;

        auto existing = old_streams.find(provider_id);
        if (existing != old_streams.end() && media_stream_metadata_matches(existing->value.metadata, metadata)) {
            new_streams.set(provider_id, move(existing->value));
    ...
    Vector<u64> providers_to_request;
    u64 binding_generation = 0;
    {
        Threading::MutexLocker pending_locker(m_media_stream_pending_mutex);
        ++m_media_stream_binding_generation;
        binding_generation = m_media_stream_binding_generation;
        m_desired_media_stream_bindings = desired_bindings;

        Vector<u64> stale_provider_ids;
        for (auto const& pending : m_pending_media_stream_binding_generations) {
            if (!desired_bindings.contains(pending.key))
                stale_provider_ids.append(pending.key);
        }
        for (u64 stale_provider_id : stale_provider_ids)
            m_pending_media_stream_binding_generations.remove(stale_provider_id);

        for (auto const& desired_binding : desired_bindings) {
            if (new_streams.contains(desired_binding.key)) {
                m_pending_media_stream_binding_generations.remove(desired_binding.key);
                continue;
            }

            m_pending_media_stream_binding_generations.set(desired_binding.key, binding_generation);
            providers_to_request.append(desired_binding.key);
            bindings_changed = true;
        }
    }
    ...
    for (u64 provider_id : providers_to_request) {
        auto metadata = desired_bindings.get(provider_id).release_value();
        request_media_stream_binding_async(provider_id, metadata, binding_generation);
    }
}
...
void WebAudioSession::apply_render_graph(WireGraphBuildResult graph)
{
    ...
    if (active_graph && active_graph->executor) {
        active_graph->build.resources->script_processor_host = m_script_processor_host.ptr();
        ...
        // Keep the active graph's external providers up to date.
        {
            ...
            if (!m_streams.media_element_streams.is_empty()) {
                for (auto const& it : m_streams.media_element_streams)
                    active_graph->build.resources->media_element_audio_sources.set(it.key, it.value.provider);
            }
        }
        {
            Threading::MutexLocker locker(m_streams.media_stream_streams_mutex);
            if (!m_streams.media_stream_streams.is_empty()) {
                for (auto const& it : m_streams.media_stream_streams)
                    active_graph->build.resources->media_stream_audio_sources.set(it.key, it.value.provider);
            }
        }
        synchronize_media_stream_destination_streams(active_graph->build.description,
            *active_graph->build.resources);
        bool const resolved_sources_from_registry = resolve_media_stream_source_providers_from_destination_registry(
            active_graph->build.description, *active_graph->build.resources, m_client_id, m_session_id);
        ...
        case GraphUpdateKind::None:
            applied_incrementally = !resolved_sources_from_registry;
            break;
        case GraphUpdateKind::Parameter:
            applied_incrementally = !resolved_sources_from_registry
                && active_graph->executor->enqueue_parameter_update(graph.description);
            break;
        case GraphUpdateKind::Topology:
            applied_incrementally = !resolved_sources_from_registry
                && active_graph->executor->enqueue_topology_update(graph.description);
            break;
        ...
        if (Web::WebAudio::should_log_webaudio(LOG_GENERAL)) {
            dbgln("cid={}: WebAudio session {} update kind={} incremental={} nodes={} conns={} pconns={} "
                  "automation={}",
                m_client_id, m_session_id, static_cast<u32>(update_kind), applied_incrementally,
                graph.description.nodes.size(), graph.description.connections.size(),
                graph.description.param_connections.size(), graph.description.param_automations.size());
            ...
            if (graph.description.nodes.size() <= 8) {
                for (auto const& it : graph.description.nodes) {
                    ...
                    if (node.has<ScriptProcessorGraphNode>()) {
                        ...
                        dbgln("cid={}: WebAudio session {} node id={} type={} bs={} in_ch={} out_ch={}", m_client_id,
                            m_session_id, it.key.value(), graph_node_type_name(GraphNodeType::ScriptProcessor),
                            sp.buffer_size, sp.input_channel_count, sp.output_channel_count);
                        continue;
                    }

                    if (node.has<MediaElementAudioSourceGraphNode>()) {
                        auto const& source = node.get<MediaElementAudioSourceGraphNode>();
                        dbgln("cid={}: WebAudio session {} node id={} type={} provider_id={} ch={}", m_client_id,
                            m_session_id, it.key.value(), graph_node_type_name(GraphNodeType::MediaElementAudioSource),
                            source.provider_id, source.channel_count);
                        continue;
                    }

                    if (node.has<MediaStreamAudioSourceGraphNode>()) {
                        auto const& source = node.get<MediaStreamAudioSourceGraphNode>();
                        dbgln("cid={}: WebAudio session {} node id={} type={} provider_id={}", m_client_id, m_session_id,
                            it.key.value(), graph_node_type_name(GraphNodeType::MediaStreamAudioSource),
                            source.provider_id);
                        continue;
                    }
                    ...
                    if (node.has<DestinationGraphNode>()) {
                        ...
                        dbgln("cid={}: WebAudio session {} node id={} type={} ch={}", m_client_id, m_session_id,
                            it.key.value(), graph_node_type_name(GraphNodeType::Destination), dest.channel_count);
                        continue;
                    }

                    dbgln("cid={}: WebAudio session {} node id={} type={}", m_client_id, m_session_id, it.key.value(),
                ...
                if (graph.description.connections.size() <= 16) {
                    for (auto const& connection : graph.description.connections) {
                        dbgln("cid={}: WebAudio session {} conn {}:{} -> {}:{}", m_client_id, m_session_id,
                            connection.source.value(), connection.source_output_index, connection.destination.value(),
...

    {
        ...
        if (!m_streams.media_element_streams.is_empty()) {
            for (auto const& it : m_streams.media_element_streams)
                graph.resources->media_element_audio_sources.set(it.key, it.value.provider);
        }
    }
    {
        Threading::MutexLocker locker(m_streams.media_stream_streams_mutex);
        if (!m_streams.media_stream_streams.is_empty()) {
            for (auto const& it : m_streams.media_stream_streams)
                graph.resources->media_stream_audio_sources.set(it.key, it.value.provider);
        }
    }
    synchronize_media_stream_destination_streams(graph.description, *graph.resources);
    (void)resolve_media_stream_source_providers_from_destination_registry(graph.description, *graph.resources,
        m_client_id, m_session_id);

    graph.resources->script_processor_host = m_script_processor_host.ptr();

    auto executor = make<GraphExecutor>(graph.description, static_cast<f32>(context_sample_rate_hz),
        static_cast<size_t>(RENDER_QUANTUM_SIZE), *graph.resources);
...
void WebAudioSession::retire_graph_on_control_thread(PreparedGraph* graph)
{
    ...
    do {
        ...
        !m_retired_graphs.compare_exchange_strong(expected_node, node, AK::MemoryOrder::memory_order_release));

    bool expected_scheduled = false;
    if (!m_retired_graph_task_scheduled.compare_exchange_strong(expected_scheduled, true,
            AK::MemoryOrder::memory_order_acq_rel))
        return;

    auto main_event_loop = Web::WebAudio::main_event_loop();
    if (!main_event_loop) {
        m_retired_graph_task_scheduled.store(false, AK::MemoryOrder::memory_order_release);
        return;
    }

    auto strong_loop = main_event_loop->take();
    if (!strong_loop) {
        m_retired_graph_task_scheduled.store(false, AK::MemoryOrder::memory_order_release);
        return;
    }
...
void WebAudioSession::drain_retired_graphs_on_control_thread()
{
    ...
    if (retired_count > 0 && Web::WebAudio::should_log_webaudio(LOG_GENERAL)) {
...
void WebAudioSession::set_script_processor_streams(Vector<ScriptProcessorStreamDescriptor> streams)
{
    ...
    if (Web::WebAudio::should_log_webaudio(LOG_SCRIPT_PROCESSOR))
        dbgln("cid={}: WebAudio session received {} ScriptProcessor stream binding(s) for session={}",
            m_client_id, streams.size(), m_session_id);
    ...
    for (auto& stream : streams) {
        ...
        auto request_stream_or_error = SharedBufferStream::attach(move(request_pool), move(request_ready), move(request_free));
        if (request_stream_or_error.is_error()) {
            if (Web::WebAudio::should_log_webaudio(LOG_SCRIPT_PROCESSOR))
                dbgln("cid={}: WebAudio session={} failed to attach ScriptProcessor request stream node={} error={}",
                    m_client_id, m_session_id, stream.node_id, request_stream_or_error.error());
            continue;
        }
        ...
        auto response_stream_or_error = SharedBufferStream::attach(move(response_pool), move(response_ready), move(response_free));
        if (response_stream_or_error.is_error()) {
            if (Web::WebAudio::should_log_webaudio(LOG_SCRIPT_PROCESSOR))
                dbgln("cid={}: WebAudio session={} failed to attach ScriptProcessor response stream node={} error={}",
                    m_client_id, m_session_id, stream.node_id, response_stream_or_error.error());
            continue;
        }

        if (Web::WebAudio::should_log_webaudio(LOG_SCRIPT_PROCESSOR)) {
            dbgln("cid={}: WebAudio session={} attached ScriptProcessor streams node={} req(blocks={}, "
                  "block_size={}) resp(blocks={}, block_size={})",
                m_client_id, m_session_id, stream.node_id, request_stream_or_error.value().block_count(),
                request_stream_or_error.value().block_size(), response_stream_or_error.value().block_count(),
...
void WebAudioSession::set_worklet_node_ports(Vector<WorkletNodePortDescriptor> ports)
{
    if (Web::WebAudio::should_log_webaudio(LOG_GENERAL))
        dbgln("cid={}: WebAudio session={} received {} worklet port binding(s)", m_client_id, m_session_id,
            ports.size());
    ...
    HashMap<u64, IPC::TransportHandle> new_handles;
    if (!had_worklet_host)
        new_handles.ensure_capacity(ports.size());

    for (auto& port : ports) {
        if (had_worklet_host) {
            host_port_bindings.append(Web::WebAudio::Render::WorkletPortBinding {
                .node_id = Web::WebAudio::NodeID { port.node_id },
                .processor_port_handle = move(port.processor_port_handle),
            });
        } else {
            new_handles.set(port.node_id, move(port.processor_port_handle));
        }

        if (Web::WebAudio::should_log_webaudio(LOG_GENERAL))
            dbgln("cid={}: WebAudio session={} bind worklet port node_id={}", m_client_id, m_session_id,
                port.node_id);
    }

    if (!had_worklet_host) {
        Threading::MutexLocker locker(m_worklet.ports_mutex);
        m_worklet.processor_port_handles = move(new_handles);
    }
    ...
    if (had_worklet_host) {
        Threading::MutexLocker locker(m_worklet.host_mutex);
        if (m_worklet.host)
            m_worklet.host->enqueue_port_bindings(move(host_port_bindings));
    }
}
...
{
    if (Web::WebAudio::should_log_webaudio(LOG_GENERAL))
        dbgln("cid={}: WebAudio session={} received {} worklet node definition(s)", m_client_id, m_session_id,
            definitions.size());
...
ErrorOr<void> WebAudioSession::create_audio_input_stream_async(Audio::DeviceHandle device_handle,
    u64 capacity_frames,
    InputStreamReadyCallback on_ready)
{
    if (device_handle == 0)
        return Error::from_string_literal("Invalid input device handle");

    auto client = Audio::SessionClientOfAudioServer::default_client();
    if (!client)
        return Error::from_string_literal("WebAudioWorker has no AudioServer client");

    struct InputStreamRequestState : public RefCounted<InputStreamRequestState> {
        explicit InputStreamRequestState(InputStreamReadyCallback&& callback)
            : ready_callback(move(callback))
        {
        }

        InputStreamReadyCallback ready_callback;
        bool completed { false };
    };

    auto request_state = adopt_ref(*new InputStreamRequestState(move(on_ready)));

    auto request_or_error = client->create_input_stream(
        device_handle, capacity_frames,
        [request_state](Audio::InputStreamDescriptor descriptor) {
            if (request_state->completed)
                return;
            request_state->completed = true;
            request_state->ready_callback(move(descriptor));
        },
        [request_state]([[maybe_unused]] ByteString const& error) {
            if (request_state->completed)
                return;
            request_state->completed = true;
            request_state->ready_callback({});
        });
...
void WebAudioSession::destroy_audio_input_stream(u64 stream_id)
{
    if (stream_id == 0)
        return;

    if (auto client = Audio::SessionClientOfAudioServer::default_client(); client)
        (void)client->destroy_input_stream(stream_id, [] { });
}
...
void WebAudioSession::set_suspended(bool suspended, u64 generation)
{
    ...
    m_requested_suspend_generation.store(generation, AK::MemoryOrder::memory_order_release);
    m_requested_suspended.store(suspended ? 1 : 0, AK::MemoryOrder::memory_order_release);
}
...
void WebAudioSession::initialize_render_state()
{
    ...
    if (channel_count > max_supported_output_channels) {
        warnln("cid={}: WebAudio session clamping output channels {} -> {}", m_client_id, channel_count,
            max_supported_output_channels);
        channel_count = max_supported_output_channels;
    }
    if (sample_rate_hz == 0 || channel_count == 0) {
        warnln("cid={}: WebAudio session invalid sample specification {} Hz, {} channels", m_client_id,
...
void WebAudioSession::shutdown()
{
    ...
    m_retired_graph_task_scheduled.store(false, AK::MemoryOrder::memory_order_release);
    RetiredGraphNode* retired_graphs = m_retired_graphs.exchange(nullptr, AK::MemoryOrder::memory_order_acq_rel);

    Vector<u64> media_stream_ids;
    Vector<u64> media_stream_destination_provider_ids;
    {
        Threading::MutexLocker locker(m_streams.media_stream_streams_mutex);
        media_stream_ids.ensure_capacity(m_streams.media_stream_streams.size());
        for (auto const& it : m_streams.media_stream_streams)
            media_stream_ids.unchecked_append(it.value.stream_id);
        m_streams.media_stream_streams.clear();
    }

    {
        Threading::MutexLocker locker(m_streams.media_stream_destination_streams_mutex);
        media_stream_destination_provider_ids.ensure_capacity(m_streams.media_stream_destination_streams.size());
        for (auto const& it : m_streams.media_stream_destination_streams)
            media_stream_destination_provider_ids.unchecked_append(it.key);
        m_streams.media_stream_destination_streams.clear();
    }

    auto run_control_cleanup = [retired_graphs, media_stream_ids = move(media_stream_ids), media_stream_destination_provider_ids = move(media_stream_destination_provider_ids)]() mutable {
        ASSERT_CONTROL_THREAD();

        while (retired_graphs) {
            RetiredGraphNode* next = retired_graphs->next;
            if (retired_graphs->graph)
                retired_graphs->graph->unref();
            delete retired_graphs;
            retired_graphs = next;
        }

        for (u64 provider_id : media_stream_destination_provider_ids)
            unregister_media_stream_destination_provider(provider_id);

        if (auto client = Audio::SessionClientOfAudioServer::default_client(); client) {
            for (auto stream_id : media_stream_ids) {
                if (stream_id != 0)
                    (void)client->destroy_input_stream(stream_id, [] { });
            }
        }
    ...
    bool queued_control_cleanup = false;
    if (!Web::WebAudio::current_thread_is_control_thread()) {
        if (auto main_event_loop = Web::WebAudio::main_event_loop(); main_event_loop) {
            if (auto control_loop = main_event_loop->take(); control_loop) {
                control_loop->deferred_invoke(move(run_control_cleanup));
                queued_control_cleanup = true;
            }
        }
    }

    if (!queued_control_cleanup)
        run_control_cleanup();
    ...
    {
        Threading::MutexLocker pending_locker(m_media_stream_pending_mutex);
        m_desired_media_stream_bindings.clear();
        m_pending_media_stream_binding_generations.clear();
    }
}
