...
Messages::WebAudioServer::CreateWebaudioSessionResponse WebAudioConnection::create_webaudio_session(u32 target_latency_ms)
{
    RefPtr<AudioServerClient::Client> audio_server = AudioServerClient::Client::default_client();
    if (!audio_server) {
        warnln("client_cid={}: WebAudioWorker has no AudioServer client", m_owner_client_id);
    ...
    if (output_session_or_error.is_error()) {
        warnln("client_cid={}: failed to create AudioOutputSession for WebAudio: {}", m_owner_client_id, output_session_or_error.error());
        return { 0, 0, 0, Core::AnonymousBuffer {}, IPC::File {} };
    }
    ...
    auto session = adopt_ref(*new WebAudioSession(session_id, output_session.session_id, sample_rate, channel_count, move(output_session.ring), timing_buffer, timing_notify_write_fd, m_owner_client_id));
    ...
    m_webaudio_sessions.set(session_id, move(session));
...
void WebAudioConnection::webaudio_session_add_worklet_module(u64 session_id, ByteString url, ByteString source_text)
{
    auto session_or_error = m_webaudio_sessions.get(session_id);
    if (!session_or_error.has_value())
        return;

    session_or_error.value()->add_worklet_module(move(url), move(source_text));
}
