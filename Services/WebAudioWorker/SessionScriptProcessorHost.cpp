...
bool SessionScriptProcessorHost::process_script_processor(
    Web::WebAudio::NodeID node_id, Web::WebAudio::Render::RenderContext& render_process_context,
    double playback_time_seconds, size_t buffer_size, size_t input_channel_count, size_t output_channel_count,
    Span<ReadonlySpan<f32>> input_channels, Span<Span<f32>> output_channels)
{
    ...
    if (Web::WebAudio::should_log_webaudio(LOG_SCRIPT_PROCESSOR)) {
        dbgln("client_cid={}: ScriptProcessor request session={} node={} t={} bs={} in={} out={}",
            m_session.m_client_id, m_session.m_session_id, node_id.value(), playback_time_seconds, buffer_size,
    ...
    if (!index) {
        if (Web::WebAudio::should_log_webaudio(LOG_SCRIPT_PROCESSOR))
            dbgln("client_cid={}: ScriptProcessor missing stream index session={} node={}", m_session.m_client_id,
                m_session.m_session_id, node_id.value());
    ...
    if (it == index->streams.end()) {
        index->unref();
        if (Web::WebAudio::should_log_webaudio(LOG_SCRIPT_PROCESSOR))
            dbgln("client_cid={}: ScriptProcessor missing stream state session={} node={}", m_session.m_client_id,
                m_session.m_session_id, node_id.value());
        for (auto span : output_channels)
            span.fill(0);
        return false;
    ...
    if (!request_stream.is_valid() || !response_stream.is_valid() || notify_write_fd < 0) {
        if (Web::WebAudio::should_log_webaudio(LOG_SCRIPT_PROCESSOR))
            dbgln("cid={}: ScriptProcessor missing streams session={} node={} notify_fd={}", m_session.m_client_id,
                m_session.m_session_id, node_id.value(), notify_write_fd);
        for (auto span : output_channels)
            span.fill(0);
        return false;
    }
    ...
    if (buffer_size != expected_buffer_size || input_channel_count != expected_input_channel_count || output_channel_count != expected_output_channel_count) {
        if (Web::WebAudio::should_log_webaudio(LOG_SCRIPT_PROCESSOR)) {
            dbgln("cid={}: ScriptProcessor mismatch session={} node={} got bs={} in={} out={} expected bs={} in={} "
                  "out={}",
                m_session.m_client_id, m_session.m_session_id, node_id.value(), buffer_size, input_channel_count,
                output_channel_count, expected_buffer_size, expected_input_channel_count,
    ...
    if (!request_block_index.has_value()) {
        if (Web::WebAudio::should_log_webaudio(LOG_SCRIPT_PROCESSOR))
            dbgln("cid={}: ScriptProcessor no free request block session={} node={}", m_session.m_client_id,
                m_session.m_session_id, node_id.value());
        for (auto span : output_channels)
            span.fill(0);
        return false;
    ...
    if (request_block.size() < request_bytes) {
        if (Web::WebAudio::should_log_webaudio(LOG_SCRIPT_PROCESSOR))
            dbgln("cid={}: ScriptProcessor request block too small session={} node={} have={} need={}",
                m_session.m_client_id, m_session.m_session_id, node_id.value(), request_block.size(),
                request_bytes);
    ...
    *request_header = Web::WebAudio::Render::ScriptProcessorRequestHeader {
    ...
    if (!request_stream.try_submit_ready_block(*request_block_index, static_cast<u32>(request_bytes))) {
        if (Web::WebAudio::should_log_webaudio(LOG_SCRIPT_PROCESSOR))
            dbgln("cid={}: ScriptProcessor failed to submit request session={} node={}", m_session.m_client_id,
                m_session.m_session_id, node_id.value());
        (void)request_stream.try_release_block_index(*request_block_index);
        for (auto span : output_channels)
            span.fill(0);
    ...
    (void)Web::WebAudio::Render::try_signal_event_fd(notify_write_fd);
    ...
    while (true) {
        ...
        if (!response_block.is_empty() && response_desc->used_size >= response_bytes && response_block.size() >= response_bytes) {
            auto const* response_header = reinterpret_cast<Web::WebAudio::Render::ScriptProcessorResponseHeader const*>(
                response_block.data());
            if (response_header->magic == Web::WebAudio::Render::script_processor_response_magic && response_header->version == Web::WebAudio::Render::script_processor_stream_version && response_header->node_id == node_id.value() && response_header->buffer_size == buffer_size && response_header->output_channel_count == output_channel_count) {
                auto const* output_f32 = reinterpret_cast<f32 const*>(
                    response_block.data() + Web::WebAudio::Render::script_processor_response_fixed_bytes);
    ...
    if (Web::WebAudio::should_log_webaudio(LOG_SCRIPT_PROCESSOR)) {
        dbgln("cid={}: ScriptProcessor response {} session={} node={} drained={} valid={} invalid={}",
            m_session.m_client_id, have_response ? "ok" : "missing", m_session.m_session_id, node_id.value(),
            drained_responses, valid_responses, invalid_responses);
    }
