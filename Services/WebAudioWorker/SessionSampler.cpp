...
#include <LibWebAudio/Engine/SincResampler.h>
...
void ensure_resampler_initialized(RenderState& scratch, size_t device_channel_count,
    u32 context_sample_rate_hz, u32 device_sample_rate_hz,
    double input_frames_per_output_frame)
{
    if (scratch.resampler_initialized && scratch.resampler_last_context_sample_rate == context_sample_rate_hz && scratch.resampler_last_device_sample_rate == device_sample_rate_hz && scratch.resampler_last_channel_count == device_channel_count)
        return;
...
void append_context_quantum_to_resampler_input(ResampleRenderContext& ctx)
{
    ...
    for (size_t ch = 0; ch < ctx.device_channel_count; ++ch) {
        ...
        __builtin_memcpy(ctx.scratch.resample_input_channels[ch].data() + write_index, input_channel.data(),
            first_part * sizeof(f32));
        if (second_part > 0)
            __builtin_memcpy(ctx.scratch.resample_input_channels[ch].data(), input_channel.data() + first_part,
                second_part * sizeof(f32));
    }

    ctx.scratch.resample_input_available_frames += RENDER_QUANTUM_SIZE;
}

void ensure_resampler_input_frames_available(ResampleRenderContext& ctx, size_t required_frames,
    size_t max_context_quanta_per_output_quantum,
    size_t& context_quanta_appended)
{
...
void build_resampler_input_spans(RenderState& scratch, size_t device_channel_count)
{
    ...
    for (size_t ch = 0; ch < device_channel_count; ++ch) {
        ...
        if (scratch.resample_input_available_frames <= contiguous) {
            scratch.resample_input_spans[ch] = ReadonlySpan<f32> { scratch.resample_input_channels[ch].data() + scratch.resample_input_read_index,
            ...
            __builtin_memcpy(scratch_channel.data(),
                scratch.resample_input_channels[ch].data() + scratch.resample_input_read_index,
                first_part * sizeof(f32));
            __builtin_memcpy(scratch_channel.data() + first_part, scratch.resample_input_channels[ch].data(),
                second_part * sizeof(f32));
...
void render_at_device_sample_rate(ResampleRenderContext& ctx)
{
    ...
    copy_planar_to_interleaved(ctx.scratch.planar_spans.span(), ctx.scratch.interleaved.span(),
        RENDER_QUANTUM_SIZE);
...
void render_with_resampler(ResampleRenderContext& ctx)
{
    ...
    ensure_resampler_initialized(ctx.scratch, ctx.device_channel_count, ctx.context_sample_rate_hz,
        ctx.device_sample_rate_hz, input_frames_per_output_frame);
    ...
    ensure_resampler_input_frames_available(ctx, estimated_input, max_context_quanta_per_output_quantum,
        context_quanta_appended);
    ...
    auto resample_result = sample_rate_converter_process(ctx.scratch.resampler, ctx.scratch.resample_input_spans.span(),
        ctx.scratch.resample_output_spans.span(), false);
    ...
    if (resample_result.output_frames_produced == 0 && context_quanta_appended < max_context_quanta_per_output_quantum) {
        ...
        resample_result = sample_rate_converter_process(ctx.scratch.resampler, ctx.scratch.resample_input_spans.span(),
            ctx.scratch.resample_output_spans.span(), false);
    }
    ...
    if (resample_result.output_frames_produced < RENDER_QUANTUM_SIZE) {
        ctx.scratch.underrun_frames.fetch_add(RENDER_QUANTUM_SIZE - resample_result.output_frames_produced,
