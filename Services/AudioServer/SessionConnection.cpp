...
static ErrorOr<SharedCircularBuffer> create_output_ring(u32 sample_rate, u32 channel_count, u32 target_latency_ms)
{
...
void SessionConnection::create_input_stream(u64 request_id, ByteString grant_id, DeviceHandle device_handle, u64 capacity_frames)
{
    verify_thread_affinity();
    ...
    if (stream_or_error.is_error()) {
        async_request_error(request_id, ByteString::formatted("Failed to create input stream: {}", stream_or_error.error()));
        return;
    }
...
void SessionConnection::create_session(u64 request_id, ByteString grant_id, u32 target_latency_ms, DeviceHandle device_handle)
{
    verify_thread_affinity();
    (void)grant_id;
