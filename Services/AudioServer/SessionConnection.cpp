...
SessionConnection::SessionConnection(NonnullOwnPtr<IPC::Transport> transport)
    : IPC::ConnectionFromClient<ToSessionClientFromAudioServerEndpoint,
          ToAudioServerFromSessionClientEndpoint>(
          *this, move(transport), Server::the().allocate_session_client_id())
{
...
void SessionConnection::die()
{
    ...
    Vector<u64> output_sink_ids;
    output_sink_ids.ensure_capacity(m_output_sinks.size());
    for (auto const& it : m_output_sinks)
        output_sink_ids.unchecked_append(it.key);

    for (u64 session_id : output_sink_ids)
...
static ErrorOr<SharedCircularBuffer> create_output_ring(u32 sample_rate, u32 channel_count,
    u32 target_latency_ms)
{
    ...
    size_t desired_latency_frames = static_cast<size_t>(
        (static_cast<double>(sample_rate) * static_cast<double>(target_latency_ms)) / 1000.0);
    ...
    return SharedCircularBuffer::create(ring_capacity_bytes);
}
...
void SessionConnection::get_devices(u64 request_id)
{
    verify_thread_affinity();
    auto devices = Server::the().enumerate_devices();

    async_did_get_devices(request_id, move(devices));
}

void SessionConnection::create_input_stream(u64 request_id, ByteString grant_id, DeviceHandle device_handle,
    u64 capacity_frames)
{
    verify_thread_affinity();
    if (!Server::the().is_grant_active(grant_id)) {
        warnln("cid={}: create_input_stream denied (inactive or mismatched grant)", client_id());
        async_request_error(request_id, ByteString("Audio input grant is inactive or invalid"));
        return;
    }
    ...
    if (!Server::the().can_grant_use_mic(grant_id)) {
        ...
        async_request_error(request_id, ByteString("Audio input grant does not allow microphone access"));
        return;
    }

    if (device_handle == 0) {
        async_request_error(request_id, ByteString("Invalid input device handle"));
        return;
    }

    auto device_info = Server::the().get_device(device_handle);
    if (!device_info.has_value() || device_info->type != DeviceInfo::Type::Input) {
        async_request_error(request_id, ByteString("Unknown audio input device handle"));
        return;
    }
    ...
    if (should_log_audio_server())
        dbgln("cid={}: create_input_stream(device_handle={}, sample_rate_hz={}, channel_count={}, "
              "capacity_frames={})",
            client_id(), device_handle, sample_rate_hz, channel_count, capacity_frames);
    ...
    if (stream_or_error.is_error()) {
        async_request_error(request_id,
            ByteString::formatted("Failed to create input stream: {}", stream_or_error.error()));
        return;
    }
    ...
    if (descriptor_or_error.is_error()) {
        async_request_error(request_id, ByteString::formatted("Failed to prepare input stream descriptor: {}", descriptor_or_error.error()));
        return;
    }
    ...
    async_did_create_input_stream(request_id, descriptor);
}

void SessionConnection::start_input_stream(u64 request_id, u64 stream_id)
{
    verify_thread_affinity();
    auto stream = m_input_streams.get(stream_id);
    if (!stream.has_value()) {
        async_request_error(request_id, ByteString("Unknown audio input stream id"));
        return;
    }
    ...
    stream->started = true;
    async_did_start_input_stream(request_id);
}

void SessionConnection::stop_input_stream(u64 request_id, u64 stream_id)
{
    verify_thread_affinity();
    auto stream = m_input_streams.get(stream_id);
    if (!stream.has_value()) {
    ...
    async_did_stop_input_stream(request_id);
}

void SessionConnection::destroy_input_stream(u64 request_id, u64 stream_id)
{
    verify_thread_affinity();
    if (should_log_audio_server())
    ...
    destroy_input_stream_internal(stream_id);
    async_did_destroy_input_stream(request_id);
}

void SessionConnection::set_output_sink_volume(u64 request_id, u64 session_id, double gain)
{
    verify_thread_affinity();
    auto session = m_output_sinks.get(session_id);
    if (!session.has_value()) {
        async_request_error(request_id, ByteString("Unknown audio output session id"));
        return;
    }
    ...
    async_did_set_output_sink_volume(request_id);
}

void SessionConnection::create_session(u64 request_id, ByteString grant_id, u32 target_latency_ms,
    DeviceHandle device_handle)
{
    ...
    if (should_log_audio_server())
        dbgln("cid={}: create_output_sink(target_latency_ms={}, device_handle={})", client_id(),
            target_latency_ms, device_handle);
    ...
    if (device_handle != 0 && !Server::the().get_device(device_handle).has_value()) {
        ...
        async_request_error(request_id, ByteString("Unknown audio output device handle"));
        return;
    }
    ...
    Server::the().when_output_device_ready(device_handle,
        [weak_self = make_weak_ptr<SessionConnection>(), session_id] {
            if (auto self = weak_self.strong_ref())
                self->finalize_output_sink(session_id);
        });

    async_did_create_session(request_id, session_id);
}
...
void SessionConnection::finalize_output_sink(u64 session_id)
{
    ...
    if (!maybe_device.has_value()) {
        warnln("cid={}: async audio output session={} unknown output device handle {}", client_id(), session_id,
    ...
    u32 channel_count = device_info.channel_count;
    ChannelMap channel_layout = device_info.channel_layout;
    if (sample_rate == 0 || channel_count == 0) {
        warnln("cid={}: async audio output session={} invalid sample specification {} Hz, {} channels",
            client_id(), session_id, sample_rate, channel_count);
        fail_output_sink(session_id, ByteString("Invalid sample specification"));
        return;
    }
    if (channel_layout.channel_count() != channel_count) {
        warnln("cid={}: async audio output session={} invalid channel layout (layout_count={}, channel_count={})",
            client_id(), session_id, channel_layout.channel_count(), channel_count);
    ...
    if (ring_or_error.is_error()) {
        warnln("cid={}: async audio output session={} failed to create audio output ring: {}", client_id(),
            session_id, ring_or_error.error());
    ...
    if (timing_buffer_or_error.is_error()) {
        warnln("cid={}: async audio output session={} failed to create audio timing buffer: {}", client_id(),
            session_id, timing_buffer_or_error.error());
    ...
    Server::the().register_output_producer(session.device_handle, session_id, ring, timing_buffer,
        session.state->bytes_per_frame);

    if (should_log_audio_server())
        dbgln("cid={}: async audio output session {} ready ({} Hz, {} channels)", client_id(), session_id,
...
void SessionConnection::destroy_session(u64 request_id, u64 session_id)
{
    verify_thread_affinity();
    ...
    destroy_output_sink_internal(session_id);
    async_did_destroy_session(request_id);
}
...
ErrorOr<IPC::TransportHandle> SessionConnection::connect_new_client_for_broker()
{
    auto paired = TRY(IPC::Transport::create_paired());
    auto handle = move(paired.remote_handle);

    (void)adopt_ref(*new SessionConnection(move(paired.local)));
