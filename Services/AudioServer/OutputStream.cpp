...
void OutputStream::publish_timing(TimingInfo& storage, u64 device_played_frames, u64 server_monotonic_ns,
    u64 additional_ring_read_frames, u64 additional_underruns)
{
...
void OutputStream::OutputDriverDeleter::operator()(OutputDriver* driver) { delete driver; }
...
void OutputStream::ensure_started(Core::ThreadEventQueue& control_event_queue, u32 target_latency_ms)
{
    ...
    auto sample_specification_callback = [this, control_event_queue_ptr](SampleSpecification spec) {
    ...
    auto audio_data_request_callback = [this](Span<float> buffer) {
        ...
        for (auto& producer : snapshot->producers) {
            ...
            if (producer.muted) {
                ...
                if (bytes_read % producer.bytes_per_frame != 0) {
                    if (should_log_audio_server())
                        warnln(
                            "AudioServer: muted producer ring returned misaligned read: bytes_read={} bytes_per_frame={}",
                            bytes_read, producer.bytes_per_frame);
                }
                if (bytes_read > 0)
                    producers_with_data++;
            ...
            if (!have_written_anything) {
                ...
                if (bytes_read != aligned_bytes_read) {
                    if (should_log_audio_server())
                        warnln("AudioServer: producer ring returned misaligned read: bytes_read={} bytes_per_frame={} "
                               "(dropping tail)",
                            bytes_read, producer.bytes_per_frame);
                ...
                if (auto* timing_storage = timing_storage_from_buffer(producer.timing_buffer); timing_storage) {
                    ...
                    publish_timing(*timing_storage, session_played_frames, server_monotonic_ns, read_frames,
                        additional_underruns);
                }
        ...
        if (should_log_audio_server()) {
            ...
            if (m_last_debug_log_time.is_zero() || (now - m_last_debug_log_time) > AK::Duration::from_seconds(1)) {
                ...
                dbgln("AudioServer: mixed callback (samples={}, producers={}, producers_with_data={}, bytes_read={}, "
                      "peak={})",
    ...
    auto stream_or_error = create_platform_output_driver(m_device_handle, initial_output_state, target_latency_ms,
        move(sample_specification_callback), move(audio_data_request_callback));
...
void OutputStream::register_producer(u64 producer_id, SharedCircularBuffer ring,
