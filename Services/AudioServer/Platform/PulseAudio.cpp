...
static ChannelMap pulse_output_channel_layout(pa_channel_map const& channel_map)
{
    ...
    auto pulse_position_to_channel = [](pa_channel_position_t position) -> Channel {
        switch (position) {
...
static void ensure_pulse_device_change_notifications_registered()
{
    ...
    monitor_thread = Threading::Thread::construct("AudioDeviceMon"sv, [] -> intptr_t {
        ...
        pa_context_set_subscribe_callback(
            context,
            [](pa_context*, pa_subscription_event_type_t event_type, [[maybe_unused]] u32 index,
                [[maybe_unused]] void*) {
        ...
        pa_operation* subscribe_op = pa_context_subscribe(context, notification_mask, nullptr, nullptr);
...
Vector<DeviceInfo> Server::enumerate_platform_devices()
{
    ...
    auto sink_info_callback = [](pa_context*, pa_sink_info const* info, int eol, void* userdata) {
        ...
        devices.append(DeviceInfo {
            ...
            .dom_device_id = is_default ? ByteString("default"sv)
                                        : Server::the().generate_dom_device_id("audiooutput"sv, backend_id,
                                              static_cast<u64>(info->index)),
            .group_id = is_default ? ByteString("default"sv)
                                   : Server::the().generate_dom_device_id("group"sv, backend_id,
                                         static_cast<u64>(info->index)),
    ...
    auto source_info_callback = [](pa_context*, pa_source_info const* info, int eol, void* userdata) {
        ...
        devices.append(DeviceInfo {
            ...
            .label = info->description ? ByteString(info->description) : backend_id,
            .dom_device_id = is_default ? ByteString("default"sv)
                                        : Server::the().generate_dom_device_id("audioinput"sv, backend_id,
                                              static_cast<u64>(info->index)),
            .group_id = is_default ? ByteString("default"sv)
                                   : Server::the().generate_dom_device_id("group"sv, backend_id,
                                         static_cast<u64>(info->index)),
