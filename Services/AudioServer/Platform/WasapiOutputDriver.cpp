...
public:
    ...
    static ErrorOr<NonnullOwnPtr<OutputDriver>> create(OutputState initial_output_state, u32 target_latency_ms,
        SampleSpecificationCallback&&,
...
ALWAYS_INLINE AK::Duration
WasapiOutputDriver::total_time_played_with_com_initialized(WasapiOutputDriver::AudioState& state)
{
    UINT64 position;
...
ErrorOr<NonnullOwnPtr<OutputDriver>>
create_platform_output_driver(DeviceHandle device_handle, OutputState initial_output_state,
    u32 target_latency_ms,
    OutputDriver::SampleSpecificationCallback&& specification_callback,
    OutputDriver::AudioDataRequestCallback&& data_callback)
{
    if (device_handle != 0)
        return Error::from_string_literal("WASAPI output supports only the default output device");

    return WasapiOutputDriver::create(initial_output_state, target_latency_ms, move(specification_callback),
        move(data_callback));
}
...
static void print_audio_format(WAVEFORMATEXTENSIBLE& format)
{
    ...
    dbgln("wFormatTag: {:x}\n"
        ...
        format.Format.cbSize, format.Samples.wValidBitsPerSample, format.dwChannelMask,
        Span<u8> { reinterpret_cast<char*>(&format.SubFormat), 16 });
}

ErrorOr<NonnullOwnPtr<OutputDriver>>
WasapiOutputDriver::create(OutputState initial_output_state, u32 target_latency_ms,
    SampleSpecificationCallback&& sample_specification_callback,
    AudioDataRequestCallback&& data_request_callback)
{
    ...
    hr = state->audio_client->IsFormatSupported(AUDCLNT_SHAREMODE_SHARED, &state->wave_format.Format,
    ...
    TRY_HR(state->audio_client->Initialize(AUDCLNT_SHAREMODE_SHARED, stream_flags, 0, 0,
        &state->wave_format.Format, &PlaybackSessionGUID));
    ...
    auto audio_thread = Threading::Thread::construct(
        "Audio Render"sv, [state] { return AudioState::render_thread_loop(*state); });
...
NonnullRefPtr<Core::ThreadedPromise<void>> WasapiOutputDriver::set_volume(double volume)
{
    ...
    hr = m_state->audio_stream_volume->SetAllVolumes(m_state->channel_volumes.size(),
        m_state->channel_volumes.data());
    if (FAILED(hr)) [[unlikely]] {
        promise->reject(Error::from_windows_error(hr));
    } else [[likely]] {
        promise->resolve();
