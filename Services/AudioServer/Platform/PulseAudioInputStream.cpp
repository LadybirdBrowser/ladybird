...
static ByteString resolve_pulse_source_name(DeviceHandle device_handle)
{
    ...
    auto* operation = pa_context_get_source_info_by_index(
        context, static_cast<u32>(device_handle),
...
public:
    static ErrorOr<NonnullRefPtr<InputStream>> create(ByteString device_name, u32 sample_rate_hz,
    ...
    ~PulseAudioInputStream() override { shutdown(); }
...
private:
    ...
    ErrorOr<void> initialize_stream(u32 sample_rate_hz, u32 channel_count)
    {
        ...
        pa_stream_set_read_callback(
            m_stream,
            [](pa_stream* stream, size_t bytes, void* user_data) {
                auto& input_stream = *static_cast<PulseAudioInputStream*>(user_data);
                input_stream.read_callback(stream, bytes);
            },
            this);
    ...
    [[nodiscard]] ScopeGuard<Function<void()>> lock_mainloop() const
    {
        ...
        return ScopeGuard<Function<void()>>(
            Function<void()>([this]() { pa_threaded_mainloop_unlock(m_mainloop); }));
    }
