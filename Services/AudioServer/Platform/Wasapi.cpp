...
ByteString wide_string_to_utf8(wchar_t const* wide_string)
{
    ...
    int bytes_written = WideCharToMultiByte(
        CP_UTF8, 0, wide_string, -1, reinterpret_cast<char*>(buffer.data()), utf8_bytes, nullptr, nullptr);
    if (bytes_written <= 0)
        return {};
...
ErrorOr<ByteString> endpoint_id_for_device(IMMDevice& device)
{
    ...
    if (FAILED(hr))
        return Error::from_windows_error(hr);
    ScopeGuard free_endpoint_id = [&] { CoTaskMemFree(endpoint_id); };
...
static ByteString friendly_name_for_device(IMMDevice& device)
{
    ...
    ScopeGuard clear_property_variant = [&] { PropVariantClear(&value); };
...
public:
    ...
    virtual HRESULT STDMETHODCALLTYPE OnDeviceStateChanged([[maybe_unused]] LPCWSTR device_id,
        [[maybe_unused]] DWORD new_state) override
    {
    ...
    OnDefaultDeviceChanged([[maybe_unused]] EDataFlow flow, [[maybe_unused]] ERole role,
        [[maybe_unused]] LPCWSTR default_device_id) override
    {
        Server::the().update_devices();
        return S_OK;
    }

    virtual HRESULT STDMETHODCALLTYPE OnPropertyValueChanged([[maybe_unused]] LPCWSTR device_id,
        [[maybe_unused]] PROPERTYKEY key) override
    {
        Server::the().update_devices();
        return S_OK;
    }
...
static void ensure_wasapi_device_change_notifications_registered()
{
    ...
    if (FAILED(hr)) {
        warnln("WASAPI device notifications: failed to create device enumerator: {}",
            Error::from_windows_error(hr));
        return;
    }
    ...
    for (UINT index = 0; index < device_count; ++index) {
        ...
        devices.append(DeviceInfo {
            ...
            .dom_device_id = is_default ? ByteString("default"sv)
                                        : Server::the().generate_dom_device_id(kind, endpoint_id, backend_handle),
            .group_id = is_default ? ByteString("default"sv)
                                   : Server::the().generate_dom_device_id("group"sv, endpoint_id, backend_handle),
        ...
        if (should_log_audio_server())
            dbgln("WASAPI enumerate {}: label='{}', channels={}, sample_rate={}, default={}", kind, label,
                channel_count, sample_rate_hz, is_default);
    }
