...
public:
    ...
    static ErrorOr<NonnullOwnPtr<OutputDriver>> create(DeviceHandle, OutputState, u32 target_latency_ms,
...
private:
    struct MainLoopUnlocker {
        ...
        void operator()() { driver.unlock_main_loop(); }
    };

    explicit PulseAudioOutputDriver() = default;

    ErrorOr<void> initialize(DeviceHandle, OutputState, u32 target_latency_ms, SampleSpecificationCallback&&,
        AudioDataRequestCallback&&);
...
{
    DeviceHandle raw_handle = (device_handle == 0) ? 0
                                                   : static_cast<DeviceHandle>(Server::device_handle_to_os_device_id(device_handle));
    return PulseAudioOutputDriver::create(raw_handle, initial_output_state, target_latency_ms,
        move(sample_specification_callback), move(data_request_callback));
}

ErrorOr<NonnullOwnPtr<OutputDriver>>
PulseAudioOutputDriver::create(DeviceHandle device_handle, OutputState initial_state, u32 target_latency_ms,
    SampleSpecificationCallback&& sample_specification_selected_callback,
    AudioDataRequestCallback&& data_request_callback)
{
    ...
    TRY(output_driver->initialize(device_handle, initial_state, target_latency_ms,
        move(sample_specification_selected_callback), move(data_request_callback)));
    return output_driver;
}

PulseAudioOutputDriver::~PulseAudioOutputDriver() { shutdown(); }
...
ErrorOr<void>
PulseAudioOutputDriver::initialize(DeviceHandle device_handle, OutputState initial_state,
    u32 target_latency_ms,
    SampleSpecificationCallback&& sample_specification_selected_callback,
    AudioDataRequestCallback&& data_request_callback)
{
...
ErrorOr<void> PulseAudioOutputDriver::resume_stream()
{
    ...
    TRY(wait_for_operation(pa_stream_cork(
                               m_stream, 0,
                               [](pa_stream*, int, void* user_data) {
                                   static_cast<PulseAudioOutputDriver*>(user_data)->signal_to_wake();
                               },
                               this),
        "Uncorking PulseAudio stream failed"sv));
...
ErrorOr<void> PulseAudioOutputDriver::drain_and_suspend_stream()
{
    ...
    TRY(wait_for_operation(pa_stream_drain(
                               m_stream,
                               [](pa_stream*, int, void* user_data) {
                                   static_cast<PulseAudioOutputDriver*>(user_data)->signal_to_wake();
                               },
                               this),
        "Draining PulseAudio stream failed"sv));
    ...
    TRY(wait_for_operation(pa_stream_cork(
                               ...
                               this),
        "Corking PulseAudio stream after drain failed"sv));
...
ErrorOr<void> PulseAudioOutputDriver::flush_and_suspend_stream()
{
    ...
    TRY(wait_for_operation(pa_stream_flush(
                               m_stream,
                               [](pa_stream*, int, void* user_data) {
                                   static_cast<PulseAudioOutputDriver*>(user_data)->signal_to_wake();
                               },
                               this),
        "Flushing PulseAudio stream failed"sv));
    ...
    TRY(wait_for_operation(pa_stream_cork(
                               ...
                               this),
        "Corking PulseAudio stream after flush failed"sv));
...
ErrorOr<void> PulseAudioOutputDriver::set_stream_volume(double volume)
{
    ...
    auto* operation = pa_context_set_sink_input_volume(
        m_context, index, &per_channel_volumes,
        [](pa_context*, int, void* user_data) {
            static_cast<PulseAudioOutputDriver*>(user_data)->signal_to_wake();
        },
        this);
...
ErrorOr<void> PulseAudioOutputDriver::resolve_selected_sink_name(DeviceHandle device_handle)
{
    ...
    auto* operation = pa_context_get_sink_info_by_index(
        m_context, static_cast<u32>(device_handle),
...
ErrorOr<void> PulseAudioOutputDriver::request_device_sample_specification()
{
    ...
    auto* operation = pa_context_get_server_info(
        m_context,
        [](pa_context*, pa_server_info const* info, void* user_data) {
            ...
            auto* operation = pa_context_get_sink_info_by_name(
                state.m_context, info->default_sink_name,
                [](pa_context*, pa_sink_info const* sink_info, int status, void* user_data) {
                    auto& state = *static_cast<PulseAudioOutputDriver*>(user_data);
...
ErrorOr<void> PulseAudioOutputDriver::create_stream(OutputState initial_state, u32 target_latency_ms)
{
    ...
    pa_stream_set_write_callback(
        m_stream,
        [](pa_stream* stream, size_t bytes_to_write, void* user_data) {
            auto& state = *static_cast<PulseAudioOutputDriver*>(user_data);
            VERIFY(state.m_stream == stream);
            state.on_write_requested(bytes_to_write);
        },
        this);

    pa_stream_set_started_callback(
        m_stream,
        [](pa_stream* stream, void* user_data) {
            auto& state = *static_cast<PulseAudioOutputDriver*>(user_data);
    ...
    if (auto error = pa_stream_connect_playback(m_stream, sink_name, &buffer_attributes, flags, nullptr, nullptr);
        error != 0)
...
void PulseAudioOutputDriver::on_write_requested(size_t bytes_to_write)
{
    ...
    while (bytes_to_write > 0) {
        ...
        if (pa_stream_write(m_stream, requested_data.data(), requested_data.size(), nullptr, 0,
                PA_SEEK_RELATIVE)
            != 0)
            return;
