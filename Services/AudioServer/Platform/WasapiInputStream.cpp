...
static ErrorOr<ComPtr<IMMDevice>> resolve_input_device(IMMDeviceEnumerator& enumerator,
    DeviceHandle backend_handle)
{
    if (backend_handle == 0) {
...
public:
    static ErrorOr<NonnullRefPtr<InputStream>> create(DeviceHandle backend_handle, u32 sample_rate_hz,
        u32 channel_count, u64 capacity_frames)
    {
    ...
    ~WasapiInputStream() override { shutdown(); }
...
private:
    ...
    ErrorOr<void> initialize_stream(DeviceHandle backend_handle, u32 sample_rate_hz, u32 channel_count)
    {
        ...
        if (SUCCEEDED(hr) && mix_format != nullptr) {
            ScopeGuard free_mix_format = [&] { CoTaskMemFree(mix_format); };
        ...
        TRY_HR(m_audio_client->Initialize(AUDCLNT_SHAREMODE_SHARED, stream_flags, 0, 0, &m_wave_format.Format,
            nullptr));
        ...
        m_capture_thread = Threading::Thread::construct("Audio Capture"sv, [this] { return capture_thread_loop(); });
...
ErrorOr<NonnullRefPtr<InputStream>> create_platform_input_stream(DeviceHandle device_handle,
    u32 sample_rate_hz, u32 channel_count,
    u64 capacity_frames)
{
    DeviceHandle backend_handle = (device_handle == 0) ? 0
                                                       : static_cast<DeviceHandle>(Server::device_handle_to_os_device_id(device_handle));
