...
struct AudioTask {
    ...
    void reject(OSStatus error)
    {
        ...
        promise.visit([](Empty) { VERIFY_NOT_REACHED(); },
            [error](auto& promise) { promise->reject(Error::from_errno(error)); });
    }

    Type type;
    Variant<Empty, NonnullRefPtr<Core::ThreadedPromise<void>>,
        NonnullRefPtr<Core::ThreadedPromise<AK::Duration>>>
        promise;
...
public:
    ...
    ~CoreAudioPropertyValue() { free(m_ptr); }
...
template<typename T>
static ErrorOr<CoreAudioPropertyValue<T>> get_audio_unit_property(AudioComponentInstance& instance,
    u32 property)
{
    u32 size = 0;
    AU_TRY(AudioUnitGetPropertyInfo(instance, property, kAudioUnitScope_Output, AUDIO_UNIT_OUTPUT_BUS, &size,
        nullptr));
    VERIFY(size >= sizeof(T));

    auto result = TRY(CoreAudioPropertyValue<T>::create(size));
    AU_TRY(AudioUnitGetProperty(instance, property, kAudioUnitScope_Output, AUDIO_UNIT_OUTPUT_BUS, result.ptr(),
        &size));
...
template<typename T>
static ErrorOr<void> set_audio_unit_property(AudioComponentInstance& instance, u32 property,
    CoreAudioPropertyValue<T> const& value)
{
    AU_TRY(AudioUnitSetProperty(instance, property, kAudioUnitScope_Input, AUDIO_UNIT_OUTPUT_BUS, value.ptr(),
        value.size()));
    return {};
}
...
public:
    static ErrorOr<NonnullRefPtr<AudioState>>
    create(OutputDriver::SampleSpecificationCallback sample_specification_callback,
        OutputDriver::AudioDataRequestCallback data_request_callback, OutputState initial_output_state)
    {
        auto state = TRY(adopt_nonnull_ref_or_enomem(
            new (nothrow) AudioState(move(data_request_callback), initial_output_state)));
        ...
        auto description = TRY(get_audio_unit_property<AudioStreamBasicDescription>(
            state->m_audio_unit, kAudioUnitProperty_StreamFormat));
        ...
        auto layout = TRY(get_audio_unit_property<AudioChannelLayout>(state->m_audio_unit,
            kAudioUnitProperty_AudioChannelLayout));
        auto channel_map = device_channel_layout(*layout, layout.size(), description->mChannelsPerFrame);
        ...
        AU_TRY(AudioUnitSetProperty(state->m_audio_unit, kAudioUnitProperty_SetRenderCallback,
            kAudioUnitScope_Global, AUDIO_UNIT_OUTPUT_BUS, &callbackStruct,
    ...
    AK::Duration last_sample_time() const { return AK::Duration::from_milliseconds(m_last_sample_time.load()); }

private:
    AudioState(OutputDriver::AudioDataRequestCallback data_request_callback, OutputState initial_output_state)
    ...
    {
        // OPTIMIZATION: We can avoid taking a lock in the audio decoder thread if there are no queued commands,
        // which
    ...
    static OSStatus on_audio_unit_buffer_request(void* user_data, AudioUnitRenderActionFlags*,
        AudioTimeStamp const* time_stamp, UInt32 element,
        UInt32 frames_to_render, AudioBufferList* output_buffer_list)
    {
        ...
        if (auto task = state.dequeue_task(); task.has_value()) {
            ...
            case AudioTask::Type::Volume:
                ...
                error = AudioUnitSetParameter(state.m_audio_unit, kHALOutputParam_Volume, kAudioUnitScope_Global, 0,
                    static_cast<float>(*task->data), 0);
                break;
            }
    ...
    OutputDriver::AudioDataRequestCallback m_data_request_callback;
    Atomic<i64> m_last_sample_time { 0 };
};

class CoreAudioOutputDriver final : public OutputDriver {
public:
    ...
    virtual ~CoreAudioOutputDriver() override;

private:
    explicit CoreAudioOutputDriver(NonnullRefPtr<AudioState>);

    NonnullRefPtr<AudioState> m_state;
};

ErrorOr<NonnullOwnPtr<OutputDriver>>
create_platform_output_driver(DeviceHandle, OutputState initial_output_state, u32 target_latency_ms,
...
{
    return CoreAudioOutputDriver::create(initial_output_state, target_latency_ms,
        move(sample_specification_callback), move(data_request_callback));
}

ErrorOr<NonnullOwnPtr<OutputDriver>>
CoreAudioOutputDriver::create(OutputState initial_output_state, u32,
    SampleSpecificationCallback&& sample_specification_callback,
    AudioDataRequestCallback&& data_request_callback)
{
    auto state = TRY(AudioState::create(move(sample_specification_callback), move(data_request_callback),
        initial_output_state));
    return TRY(adopt_nonnull_own_or_enomem(new (nothrow) CoreAudioOutputDriver(move(state))));
}

CoreAudioOutputDriver::CoreAudioOutputDriver(NonnullRefPtr<AudioState> impl)
    : m_state(move(impl))
{
}

CoreAudioOutputDriver::~CoreAudioOutputDriver() = default;

void CoreAudioOutputDriver::set_underrun_callback(Function<void()>)
{
    // FIXME: Implement this.
}

NonnullRefPtr<Core::ThreadedPromise<AK::Duration>> CoreAudioOutputDriver::resume()
{
...
NonnullRefPtr<Core::ThreadedPromise<void>> CoreAudioOutputDriver::drain_buffer_and_suspend()
{
...
NonnullRefPtr<Core::ThreadedPromise<void>> CoreAudioOutputDriver::discard_buffer_and_suspend()
{
    auto promise = Core::ThreadedPromise<void>::create();
...
AK::Duration CoreAudioOutputDriver::device_time_played() const { return m_state->last_sample_time(); }

NonnullRefPtr<Core::ThreadedPromise<void>> CoreAudioOutputDriver::set_volume(double volume)
{
    auto promise = Core::ThreadedPromise<void>::create();
