...
public:
    static ErrorOr<NonnullRefPtr<AudioState>> create(OutputDriver::SampleSpecificationCallback sample_specification_callback, OutputDriver::AudioDataRequestCallback data_request_callback, OutputState initial_output_state)
    {
        ...
        auto channel_map = AudioServer::device_channel_layout(*layout, layout.size(), description->mChannelsPerFrame);
...
ErrorOr<NonnullOwnPtr<OutputDriver>> create_platform_output_driver(DeviceHandle, OutputState initial_output_state, u32 target_latency_ms, OutputDriver::SampleSpecificationCallback&& sample_specification_callback, OutputDriver::AudioDataRequestCallback&& data_request_callback)
{
    return CoreAudioOutputDriver::create(initial_output_state, target_latency_ms, move(sample_specification_callback), move(data_request_callback));
}

ErrorOr<NonnullOwnPtr<OutputDriver>> CoreAudioOutputDriver::create(OutputState initial_output_state, u32, SampleSpecificationCallback&& sample_specification_callback, AudioDataRequestCallback&& data_request_callback)
{
...
AK::Duration CoreAudioOutputDriver::device_time_played() const
{
