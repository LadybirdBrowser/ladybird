...
static OSStatus coreaudio_device_change_listener(
    [[maybe_unused]] AudioObjectID in_object_id, [[maybe_unused]] UInt32 in_number_addresses,
    [[maybe_unused]] AudioObjectPropertyAddress const in_addresses[], [[maybe_unused]] void* in_client_data)
{
...
static void ensure_coreaudio_device_change_notifications_registered()
{
    ...
    OSStatus devices_status = AudioObjectAddPropertyListener(kAudioObjectSystemObject, &devices_address,
        coreaudio_device_change_listener, nullptr);
    OSStatus output_status = AudioObjectAddPropertyListener(kAudioObjectSystemObject, &default_output_address,
        coreaudio_device_change_listener, nullptr);
    OSStatus input_status = AudioObjectAddPropertyListener(kAudioObjectSystemObject, &default_input_address,
        coreaudio_device_change_listener, nullptr);
...
{
    if (channel_count == 1)
        return ChannelMap(Vector<Channel, ChannelMap::capacity()> { Channel::FrontCenter });

    if (channel_count == 2)
        return ChannelMap(Vector<Channel, ChannelMap::capacity()> { Channel::FrontLeft, Channel::FrontRight });
...
ChannelMap device_channel_layout(AudioChannelLayout const& layout, u32 layout_size,
    u32 expected_channel_count)
{
    check_audio_channel_layout_size(layout, layout_size);

    if (should_log_audio_server())
        dbgln("CoreAudio layout input: tag={}, bitmap={:#x}, descriptions={}, expected_channels={}",
            layout.mChannelLayoutTag, layout.mChannelBitmap, layout.mNumberChannelDescriptions,
            expected_channel_count);

    if (layout.mChannelLayoutTag != kAudioChannelLayoutTag_UseChannelBitmap && layout.mChannelLayoutTag != kAudioChannelLayoutTag_UseChannelDescriptions) {
        u32 explicit_layout_size = 0;
        OSStatus status = AudioFormatGetPropertyInfo(kAudioFormatProperty_ChannelLayoutForTag, sizeof(AudioChannelLayoutTag),
            &layout.mChannelLayoutTag, &explicit_layout_size);
        ...
        status = AudioFormatGetProperty(kAudioFormatProperty_ChannelLayoutForTag, sizeof(AudioChannelLayoutTag),
            &layout.mChannelLayoutTag, &explicit_layout_size, explicit_layout);
        if (status != noErr) {
            if (should_log_audio_server())
                dbgln("CoreAudio layout output: {}", create_unknown_channel_layout(expected_channel_count));
            return create_unknown_channel_layout(expected_channel_count);
    ...
    if (layout.mChannelLayoutTag == kAudioChannelLayoutTag_UseChannelBitmap) {
        ...
        for (u32 i = 0; i < layout.mNumberChannelDescriptions; ++i) {
            ...
            case kAudioChannelLabel_LeftSideSurround:
                channels.append(Channel::SideLeft);
                break;
            case kAudioChannelLabel_RightSideSurround:
                channels.append(Channel::SideRight);
                break;
            case kAudioChannelLabel_RearSurroundLeft:
            case kAudioChannelLabel_LeftBackSurround:
                channels.append(Channel::BackLeft);
                break;
            case kAudioChannelLabel_RearSurroundRight:
            case kAudioChannelLabel_RightBackSurround:
                channels.append(Channel::BackRight);
                break;
            case kAudioChannelLabel_Mono:
                channels.append(Channel::FrontCenter);
                break;
            default:
...
static ChannelMap device_channel_layout(AudioObjectID device_id, AudioObjectPropertyScope scope,
    u32 channel_count)
{
        ...
        if (!streams_buffer_or_error.is_error()) {
            ...
            if (streams_status == noErr) {
                ...
                for (size_t i = 0; i < stream_count; ++i) {
                    if (should_log_audio_server())
                        dbgln("CoreAudio: Stream probe: {}, device_id={}, object_id={}, scope={}",
                            device_name(device_id).value_or(ByteString()), device_id, stream_ids[i], scope);
                    ...
                    OSStatus stream_status = AudioObjectGetPropertyData(stream_ids[i], &stream_format_address, 0,
                        nullptr, &stream_format_size, &stream_format);
...
Vector<DeviceInfo> Server::enumerate_platform_devices()
{
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        if (output_channels > 0) {
            ...
            if (should_log_audio_server())
                dbgln("Enumerating output device \"{}\", channels: {}, sample rate: {}, default: {}", label,
                    output_channels, sample_rate_hz, is_default);
            ...
            devices.append(DeviceInfo {
                ...
                .dom_device_id = is_default ? ByteString("default"sv)
                                            : Server::the().generate_dom_device_id("audiooutput"sv, persistent_id,
                                                  static_cast<u64>(device_id)),
                .group_id = is_default ? ByteString("default"sv)
                                       : Server::the().generate_dom_device_id("group"sv, persistent_id,
                                             static_cast<u64>(device_id)),
        ...
        if (input_channels > 0) {
            ...
            if (should_log_audio_server())
                dbgln("Enumerating input device \"{}\", channels: {}, sample rate: {}, default: {}", label,
                    input_channels, sample_rate_hz, is_default);
            ...
            devices.append(DeviceInfo {
                ...
                .label = move(label),
                .dom_device_id = is_default ? ByteString("default"sv)
                                            : Server::the().generate_dom_device_id("audioinput"sv, persistent_id,
                                                  static_cast<u64>(device_id)),
                .group_id = is_default ? ByteString("default"sv)
                                       : Server::the().generate_dom_device_id("group"sv, persistent_id,
                                             static_cast<u64>(device_id)),
