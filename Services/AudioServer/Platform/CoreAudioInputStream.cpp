...
public:
    static ErrorOr<NonnullRefPtr<InputStream>> create(DeviceHandle device_handle, u32 sample_rate_hz,
    ...
    ErrorOr<void> initialize_unit(DeviceHandle device_handle, u32 sample_rate_hz, u32 channel_count)
    {
        ...
        status = AudioUnitSetProperty(m_unit, kAudioOutputUnitProperty_EnableIO, kAudioUnitScope_Input, 1,
            &enable_input, sizeof(enable_input));
        if (status != noErr)
            return Error::from_errno(status);

        UInt32 disable_output = 0;
        status = AudioUnitSetProperty(m_unit, kAudioOutputUnitProperty_EnableIO, kAudioUnitScope_Output, 0,
        ...
        status = AudioUnitSetProperty(m_unit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Output, 1, &format,
            sizeof(format));
        if (status != noErr)
            return Error::from_errno(status);
        ...
        status = AudioUnitSetProperty(m_unit, kAudioOutputUnitProperty_SetInputCallback, kAudioUnitScope_Global,
    ...
    static OSStatus input_callback(void* user_data, AudioUnitRenderActionFlags* flags,
        AudioTimeStamp const* timestamp, UInt32 bus_number, UInt32 frames,
        [[maybe_unused]] AudioBufferList* io_data)
    {
