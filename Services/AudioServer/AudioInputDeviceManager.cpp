...
static Optional<AudioObjectID> default_input_device_id()
{
...
static u32 input_channel_count(AudioObjectID device_id)
{
    AudioObjectPropertyAddress address {
...
Vector<AudioInputDeviceInfo> AudioInputDeviceManager::enumerate_devices()
{
    Vector<AudioInputDeviceInfo> devices;
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        devices.append(AudioInputDeviceInfo {
            .device_id = static_cast<AudioInputDeviceID>(device_id),
            .label = move(label),
    ...
    auto* server_info_op = pa_context_get_server_info(context, server_info_callback, &default_source);
    ...
    auto source_info_callback = [](pa_context*, pa_source_info const* info, int eol, void* userdata) {
        ...
        devices.append(AudioInputDeviceInfo {
            .device_id = static_cast<AudioInputDeviceID>(info->index),
    ...
    auto* source_op = pa_context_get_source_info_list(context, source_info_callback, &data);
    while (pa_operation_get_state(source_op) == PA_OPERATION_RUNNING)
        pa_mainloop_iterate(mainloop, 1, nullptr);
