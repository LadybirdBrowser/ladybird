...
class SessionConnection final : public IPC::ConnectionFromClient<ToSessionClientFromAudioServerEndpoint, ToAudioServerFromSessionClientEndpoint> {
...
public:
    ...
    static ErrorOr<IPC::TransportHandle> connect_new_client_for_broker();
...
private:
    ...
    void get_devices(u64 request_id) override;
    void create_input_stream(u64 request_id, ByteString grant_id, DeviceHandle device_handle,
        u64 capacity_frames) override;
    void start_input_stream(u64 request_id, u64 stream_id) override;
    void stop_input_stream(u64 request_id, u64 stream_id) override;
    void create_session(u64 request_id, ByteString grant_id, u32 target_latency_ms,
        DeviceHandle device_handle) override;
    void destroy_session(u64 request_id, u64 session_id) override;
    void destroy_input_stream(u64 request_id, u64 stream_id) override;
    void set_output_sink_volume(u64 request_id, u64 session_id, double gain) override;

    struct OutputSink {
        struct State : public RefCounted<State> {
            Atomic<bool> ring_ready { false };
            Optional<SharedCircularBuffer> ring;
