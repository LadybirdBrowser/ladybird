...
int Server::allocate_session_client_id() { return m_session_client_ids.allocate(); }

void Server::release_session_client_id(int client_id) { m_session_client_ids.deallocate(client_id); }
...
void Server::unregister_session_connection(int client_id) { m_session_connections.remove(client_id); }
...
void Server::ensure_output_device_started(DeviceHandle device_handle,
    Core::ThreadEventQueue& control_event_queue,
...
void Server::register_output_producer(DeviceHandle device_handle, u64 producer_id, SharedCircularBuffer ring,
    Core::AnonymousBuffer timing_buffer, size_t bytes_per_frame)
{
    output_stream(device_handle)
        .register_producer(producer_id, move(ring), move(timing_buffer), bytes_per_frame);
}
...
static Optional<DeviceInfo> find_device_by_handle(Vector<DeviceInfo> const& devices,
...
void Server::ensure_channel_layouts(Vector<DeviceInfo>& devices)
{
    for (auto& device : devices) {
        ...
        for (int i = 0; i < device.channel_layout.channel_count(); i++) {
            if (device.channel_layout.channel_at(i) == Channel::Unknown)
        ...
        case 1:
            device.channel_layout = ChannelMap::mono();
            continue;
        case 2:
            device.channel_layout = ChannelMap::stereo();
            continue;
        case 4:
            device.channel_layout = ChannelMap::quadrophonic();
            continue;
        case 6:
            device.channel_layout = ChannelMap::surround_5_1();
            continue;
        case 8:
            device.channel_layout = ChannelMap::surround_7_1();
            continue;
        default:
            continue;
        }
...
ByteString Server::generate_dom_device_id(StringView kind, ByteString const& backend_persistent_id,
    u64 device_handle)
{
...
bool Server::is_grant_active(ByteString const& grant_id) const { return m_grants.contains(grant_id); }
