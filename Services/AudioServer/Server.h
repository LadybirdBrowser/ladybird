...
public:
    ...
    void ensure_output_device_started(DeviceHandle device_handle, Core::ThreadEventQueue& control_event_queue,
        u32 target_latency_ms);
    void when_output_device_ready(DeviceHandle device_handle, Function<void()>);
    void register_output_producer(DeviceHandle device_handle, u64 producer_id, SharedCircularBuffer ring,
    ...
    static ChannelMap common_channel_layout(u32 channel_count);
    static u64 device_handle_to_os_device_id(DeviceHandle device_handle);

private:
    static ByteString generate_dom_device_id(StringView kind, ByteString const& backend_persistent_id,
        u64 device_handle);
