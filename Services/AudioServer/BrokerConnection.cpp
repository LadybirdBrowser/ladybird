...
ErrorOr<Vector<CreateClientResponse>> BrokerConnection::create_new_clients(Vector<CreateClientRequest> requests)
{
...
void BrokerConnection::connect_new_clients_async(u64 request_id, Vector<CreateClientRequest> requests)
{
    ...
    auto responses_or_error = create_new_clients(move(requests));
    if (responses_or_error.is_error()) {
        async_did_fail_to_connect_new_clients(request_id, ByteString("AudioServer: connect_new_clients_async failed"));
        return;
    }
