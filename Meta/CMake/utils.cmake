...
function (serenity_testjs_test test_src sub_dir)
    ...
    serenity_test(${test_src} ${sub_dir} CUSTOM_MAIN "${SERENITY_TEST_CUSTOM_MAIN}" LIBS ${SERENITY_TEST_LIBS})
endfunction ()
...
function (invoke_generator name generator primary_source header implementation)
    ...
    set(CURRENT_LIB_GENERATED
        ${CURRENT_LIB_GENERATED}
        PARENT_SCOPE
    )
endfunction ()
...
function (
    ...
    add_custom_command(
        ...
        COMMAND $<TARGET_FILE:${generator}> -h "${header}.tmp" -c "${implementation}.tmp" -i "${idl}.tmp"
                ${invoke_idl_generator_arguments}
        COMMAND "${CMAKE_COMMAND}" -E copy_if_different "${header}.tmp" "${header}"
        COMMAND "${CMAKE_COMMAND}" -E copy_if_different "${implementation}.tmp" "${implementation}"
...
function (download_file url path)
    ...
    # If the timestamp doesn't match exactly, the Web Archive should redirect to the closest archived file
    # automatically.
    download_file_multisource(
        "${url};https://web.archive.org/web/99991231235959/${url}" "${path}" SHA256 "${DOWNLOAD_SHA256}"
    )
endfunction ()
...
function (add_lagom_library_install_rules target_name)
    ...
    # Don't make alias when we're going to import a previous build for Tools FIXME: Is there a better way to write this?
