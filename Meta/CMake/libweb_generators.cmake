function (generate_css_implementation)
    ...
    embed_as_string(
        "DefaultStyleSheetSource.cpp" "${LIBWEB_INPUT_FOLDER}/CSS/Default.css" "CSS/DefaultStyleSheetSource.cpp"
        "default_stylesheet_source" NAMESPACE "Web::CSS"
    )

    embed_as_string(
        "QuirksModeStyleSheetSource.cpp" "${LIBWEB_INPUT_FOLDER}/CSS/QuirksMode.css"
        "CSS/QuirksModeStyleSheetSource.cpp" "quirks_mode_stylesheet_source" NAMESPACE "Web::CSS"
    )

    embed_as_string(
        "MathMLStyleSheetSource.cpp" "${LIBWEB_INPUT_FOLDER}/MathML/Default.css" "MathML/MathMLStyleSheetSource.cpp"
        "mathml_stylesheet_source" NAMESPACE "Web::CSS"
    )

    embed_as_string(
        "SVGStyleSheetSource.cpp" "${LIBWEB_INPUT_FOLDER}/SVG/Default.css" "SVG/SVGStyleSheetSource.cpp"
        "svg_stylesheet_source" NAMESPACE "Web::CSS"
    )
    ...
    set(LIBWEB_ALL_GENERATED_HEADERS
        ${LIBWEB_ALL_GENERATED_HEADERS}
        PARENT_SCOPE
    )

    set(CSS_GENERATED_IDL "GeneratedCSSStyleProperties.idl")
    list(APPEND LIBWEB_ALL_GENERATED_IDL ${CSS_GENERATED_IDL})
    set(LIBWEB_ALL_GENERATED_IDL
        ${LIBWEB_ALL_GENERATED_IDL}
        PARENT_SCOPE
    )
endfunction ()
...
function (generate_html_implementation)
    ...
    set(HTML_GENERATED_HEADERS "HTML/Parser/NamedCharacterReferences.h")
...
function (generate_js_bindings target)
    ...
    function (libweb_js_bindings class)
        ...
        if (LIBWEB_BINDINGS_NAMESPACE)
            set(BINDINGS_SOURCES "Bindings/${basename}Namespace.h" "Bindings/${basename}Namespace.cpp")
        else ()
            set(BINDINGS_SOURCES "Bindings/${basename}Constructor.h" "Bindings/${basename}Constructor.cpp"
                                 "Bindings/${basename}Prototype.h" "Bindings/${basename}Prototype.cpp"
            )
        endif ()

        if (LIBWEB_BINDINGS_ITERABLE)
            list(APPEND BINDINGS_SOURCES "Bindings/${basename}IteratorPrototype.h"
                 "Bindings/${basename}IteratorPrototype.cpp"
            )
        endif ()

        if (LIBWEB_BINDINGS_ASYNC_ITERABLE)
            list(APPEND BINDINGS_SOURCES "Bindings/${basename}AsyncIteratorPrototype.h"
                 "Bindings/${basename}AsyncIteratorPrototype.cpp"
            )
        endif ()

        if (LIBWEB_BINDINGS_GLOBAL)
            list(APPEND BINDINGS_SOURCES "Bindings/${basename}GlobalMixin.h" "Bindings/${basename}GlobalMixin.cpp")
        endif ()
        ...
        # Ninja expects the target name in depfiles to be relative to CMAKE_BINARY_DIR, but ${bindings_src} is relative
        # to CMAKE_CURRENT_BINARY_DIR. CMake >= 3.20 can do the rewriting transparently (CMP0116).
        ...
        add_custom_command(
            ...
            COMMAND
                "$<TARGET_FILE:Lagom::BindingsGenerator>" -o "Bindings" --depfile "Bindings/${basename}.d"
    ...
    function (generate_exposed_interface_files)
        set(exposed_interface_sources
            ...
            DedicatedWorkerExposedInterfaces.cpp
            DedicatedWorkerExposedInterfaces.h
            SharedWorkerExposedInterfaces.cpp
            SharedWorkerExposedInterfaces.h
            ShadowRealmExposedInterfaces.cpp
            ShadowRealmExposedInterfaces.h
            WindowExposedInterfaces.cpp
            WindowExposedInterfaces.h
        )
        ...
        if (WIN32)
            ...
            file(
                GENERATE
                OUTPUT "${CMAKE_CURRENT_BINARY_DIR}/all_idl_files.txt"
                CONTENT "${idl_file_list}" NEWLINE_STYLE UNIX
            )
        ...
        add_custom_command(
            ...
            COMMAND $<TARGET_FILE:Lagom::GenerateWindowOrWorkerInterfaces> -o "${CMAKE_CURRENT_BINARY_DIR}/tmp" -b
                    "${LIBWEB_INPUT_FOLDER}" -b "${CMAKE_CURRENT_BINARY_DIR}" ${LIBWEB_ALL_IDL_FILES_ARGUMENT}
            COMMAND "${CMAKE_COMMAND}" -E copy_if_different tmp/IntrinsicDefinitions.cpp
                    "Bindings/IntrinsicDefinitions.cpp"
            COMMAND "${CMAKE_COMMAND}" -E copy_if_different tmp/DedicatedWorkerExposedInterfaces.h
                    "Bindings/DedicatedWorkerExposedInterfaces.h"
            COMMAND "${CMAKE_COMMAND}" -E copy_if_different tmp/DedicatedWorkerExposedInterfaces.cpp
                    "Bindings/DedicatedWorkerExposedInterfaces.cpp"
            COMMAND "${CMAKE_COMMAND}" -E copy_if_different tmp/SharedWorkerExposedInterfaces.h
                    "Bindings/SharedWorkerExposedInterfaces.h"
            COMMAND "${CMAKE_COMMAND}" -E copy_if_different tmp/SharedWorkerExposedInterfaces.cpp
                    "Bindings/SharedWorkerExposedInterfaces.cpp"
            COMMAND "${CMAKE_COMMAND}" -E copy_if_different tmp/ShadowRealmExposedInterfaces.h
                    "Bindings/ShadowRealmExposedInterfaces.h"
            COMMAND "${CMAKE_COMMAND}" -E copy_if_different tmp/ShadowRealmExposedInterfaces.cpp
                    "Bindings/ShadowRealmExposedInterfaces.cpp"
            COMMAND "${CMAKE_COMMAND}" -E copy_if_different tmp/WindowExposedInterfaces.h
                    "Bindings/WindowExposedInterfaces.h"
            COMMAND "${CMAKE_COMMAND}" -E copy_if_different tmp/WindowExposedInterfaces.cpp
