...
function (embed_as_string name source_file output source_variable_name)
    ...
    add_custom_command(
        OUTPUT "${output}"
        COMMAND "${Python3_EXECUTABLE}" "${SerenityOS_SOURCE_DIR}/Meta/embed_as_string.py" "${source_file}" -o
                "${output}.tmp" -n "${source_variable_name}" ${namespace_arg}
...
function (generate_clang_module_map target_name)
    ...
    add_custom_command(
        ...
        COMMAND
            "${Python3_EXECUTABLE}" "${SerenityOS_SOURCE_DIR}/Meta/generate_clang_module_map.py"
            "${MODULE_MAP_DIRECTORY}" --module-name "${module_name}" --module-map "${module_map_file}" --vfs-map
            ${vfs_overlay_file} --exclude-files ${MODULE_MAP_EXCLUDE_FILES} --generated-files
    ...
    target_compile_options(
        ${target_name} PUBLIC "SHELL:$<$<COMPILE_LANGUAGE:Swift>:-Xcc -ivfsoverlay${vfs_overlay_file}>"
    )
endfunction ()
...
function (compile_ipc source output)
    ...
    cmake_path(
        RELATIVE_PATH CMAKE_CURRENT_SOURCE_DIR BASE_DIRECTORY ${SerenityOS_SOURCE_DIR} OUTPUT_VARIABLE
        current_source_dir_relative
    )
    if (ENABLE_INSTALL_HEADERS)
        install(
            FILES ${CMAKE_CURRENT_BINARY_DIR}/${output}
            DESTINATION "${CMAKE_INSTALL_INCLUDEDIR}/${current_source_dir_relative}"
            OPTIONAL
        )
    endif ()
endfunction ()
