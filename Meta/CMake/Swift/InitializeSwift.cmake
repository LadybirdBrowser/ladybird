...
# Compute the name of the architecture directory on Windows from the CMake system processor name.
...
# Compute flags and search paths NOTE: This logic will eventually move to CMake
function (_setup_swift_paths)
    # If we haven't set the swift library search paths, do that now
    if (NOT SWIFT_LIBRARY_SEARCH_PATHS
        OR NOT SWIFT_INCLUDE_PATHS
        OR (APPLE AND NOT SWIFT_TARGET_TRIPLE)
    )
        if (APPLE)
            set(SDK_FLAGS "-sdk" "${CMAKE_OSX_SYSROOT}")
        endif ()

        # Note: This does not handle cross-compiling correctly. To handle it correctly, we would need to pass the target
        # triple and flags to this compiler invocation.
        execute_process(
            COMMAND ${CMAKE_Swift_COMPILER} ${SDK_FLAGS} -print-target-info OUTPUT_VARIABLE SWIFT_TARGET_INFO
        )

        # FIXME: https://gitlab.kitware.com/cmake/cmake/-/issues/26174
        if (APPLE)
            if (CMAKE_OSX_DEPLOYMENT_TARGET)
                set(SWIFT_TARGET_TRIPLE
                    "${CMAKE_SYSTEM_PROCESSOR}-apple-macosx${CMAKE_OSX_DEPLOYMENT_TARGET}"
                    CACHE STRING "Swift target triple"
                )
            else ()
        ...
        set(SWIFT_INCLUDE_PATHS
            ${SWIFT_TOOLCHAIN_INCLUDE_DIR}
            CACHE FILEPATH "Swift interop include paths"
        )
    endif ()
