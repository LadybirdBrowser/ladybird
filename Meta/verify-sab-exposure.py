...
ARRAYBUFFER_MEMBERS = {
    ...
    "shared_buffer": "RAW",
    # Hands out the whole block by const reference — and DataBlock::data_at() is public, so this is a pointer surface
    # too. Structured clone reads it to alias a same-process clone onto the source's storage (the block's handle and
    # owner, not its bytes).
    "data_block": "RAW",
