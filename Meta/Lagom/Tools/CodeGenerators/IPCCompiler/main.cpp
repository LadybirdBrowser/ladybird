...
{
    ASSERT(!parameters.is_empty() && parameters[0].name == "ipc_request_id");
...
ByteString resolver_alias(ReadonlySpan<Parameter const> parameters)
{
    ASSERT(!parameters.is_empty() && parameters[0].name == "ipc_request_id");
    auto parameters_without_seqid = parameters.slice(1);
    StringBuilder builder;
    builder.append("using Resolver = Function<void("sv);
    for (auto const& [i, arg] : enumerate(parameters_without_seqid)) {
        builder.append("const "sv);
...
void do_message(bool is_response, SourceGenerator message_generator, ByteString const& name, Vector<Parameter> const& parameters, ByteString const& response_type = {})
{
    ...
    message_generator.set("message.resolver", resolver_alias(parameters));
...
public:)~~~");
   ...
   using Resolver = ResponseType::Resolver;)~~~");
    ...
    auto parameters_without_seqid = ReadonlySpan<Parameter const> { parameters }.slice(1);
...
void generate_proxy_method(SourceGenerator& message_generator, Endpoint const& endpoint, Message const& message, ByteString const& name, ReadonlySpan<const Parameter> parameters, bool is_synchronous, bool is_try, bool is_utf8_string_overload);
...
public:
    ...
    static auto create_resolver(NonnullRefPtr<IPC::ConnectionBase> conn, u64 seq_id) {
        return [state=IPC::MessageResolverState::create(seq_id), conn](ResponseArguments&&... args) {
            VERIFY(state->can_resolve());
            state->set_resolved();
            if constexpr(sizeof...(ResponseArguments) == 0) {
                auto res = ResponseType::static_encode(state->request_id());
                ...
                auto res = ResponseType::static_encode(state->request_id(), std::forward<ResponseArguments>(args)...);
                if (auto const post_result = conn->post_message(res.release_value()); post_result.is_error()) {
                    dbgln("IPC::ConnectionBase::handle_messages: {}", post_result.error());
                }
            }
    ...
    for (auto const& message : endpoint.messages) {
        auto do_handle_message = [&](ByteString const& name, ReadonlySpan<const Parameter> parameters, ReadonlySpan<const Parameter> response_args, bool returns_something) {
            auto message_generator = generator.fork();
            ...
            for (auto const& [i, parameter] : enumerate(parameters_without_seqid)) {
                ...
                // always add ',' if IPC message returns a value, becaise it will be prepended to create_resolver call
                if (i != parameters_without_seqid.size() - 1 || returns_something) {
                    argument_generator.append(", "sv);
                }
            }
        ...
        case (int)Messages::@endpoint.name@::MessageID::@message.pascal_name@: {)~~~");
            ...
            return @handler_name@(@arguments@create_resolver<Messages::@endpoint.name@::@message.response_type@@arguments_types@>(std::move(conn), message->ipc_request_id()));)~~~");
            } else {
                message_generator.append(R"~~~(
            [[maybe_unused]] auto& request = static_cast<Messages::@endpoint.name@::@message.pascal_name@&>(*message);
    ...
    virtual void @handler_name@()~~~");
            ...
            for (size_t i = 1; i < parameters.size(); ++i) {
                ...
                argument_generator.set("argument.name", parameter.name);
                argument_generator.append("@argument.type@ @argument.name@");
                if (i != parameters.size() - 1 || message.is_synchronous)
                    argument_generator.append(", ");
            }
            if (message.is_synchronous) {
                message_generator.append("Messages::@endpoint.name@::@message.response_type@::Resolver resolver");
            }
