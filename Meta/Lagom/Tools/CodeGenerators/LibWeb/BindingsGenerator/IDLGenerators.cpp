...
void @class_name@::initialize(JS::Realm& realm)
{
    ...
    StringBuilder window_exposed_defines;
    SourceGenerator window_exposed_defines_generator { window_exposed_defines, MUST(generator.mapping().clone()) };
    auto get_correct_generator = [&](auto const& name, auto& extended_attributes) -> SourceGenerator {
        if (auto maybe_exposed = extended_attributes.get("Exposed"); maybe_exposed.has_value()) {
            ...
            if (exposed_to & IDL::ExposedTo::Window && !(exposed_to & IDL::ExposedTo::AllWorkers)) {
                return window_exposed_defines_generator.fork();
            }
        }
    ...
    for (auto& attribute : interface.attributes) {
        ...
        auto attribute_generator = get_correct_generator(attribute.name, attribute.extended_attributes);
        if (attribute.extended_attributes.contains("FIXME")) {
            attribute_generator.set("attribute.name", attribute.name);
            attribute_generator.append(R"~~~(
    ...
    for (auto& function : interface.functions) {
        ...
        if (function.extended_attributes.contains("FIXME")) {
            auto function_generator = get_correct_generator(function.name, function.extended_attributes);
            function_generator.set("function.name", function.name);
            function_generator.append(R"~~~(
    ...
    if (interface.has_stringifier && should_generate_stringifier) {
        ...
        auto stringifier_generator = interface.stringifier_extended_attributes.has_value()
            ? get_correct_generator("stringifier"sv, *interface.stringifier_extended_attributes)
            : generator.fork();
        stringifier_generator.append(R"~~~(
    ...
    if (!window_exposed_defines.is_empty()) {
        auto window_exposed_defines_generator = generator.fork();
        window_exposed_defines_generator.set("defines", window_exposed_defines.to_byte_string());
        window_exposed_defines_generator.append(R"~~~(
