...
ErrorOr<Client::AudioOutputSession> Client::create_audio_output_session(u32 target_latency_ms)
{
    auto response = send_sync_but_allow_failure<Messages::AudioServerServer::CreateAudioOutputSession>(target_latency_ms);
    if (!response)
...
ErrorOr<u64> Client::create_audio_output_session_async(u32 target_latency_ms)
{
    auto response = send_sync_but_allow_failure<Messages::AudioServerServer::CreateAudioOutputSessionAsync>(target_latency_ms);
    if (!response)
