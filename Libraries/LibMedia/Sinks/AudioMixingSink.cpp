...
void AudioMixingSink::set_tap(TapCallback callback, Optional<Audio::SampleSpecification> override_sample_specification)
{
...

    {
        ...
        tap_sample_specification = override_sample_specification.value_or(default_sample_specification);

        if (callback)
            m_tap_state = adopt_ref(*new TapState(move(callback), tap_sample_specification));
        else
            m_tap_state = nullptr;

        Audio::SampleSpecification target_sample_specification = m_tap_state ? m_tap_state->sample_specification : m_device_sample_specification;
