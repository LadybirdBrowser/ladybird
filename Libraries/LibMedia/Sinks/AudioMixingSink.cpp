...
static Audio::SampleSpecification sample_specification_for_output_sink(Audio::OutputSink const& session)
{
...
struct AudioMixingSink::AudioServerOutput {
    explicit AudioServerOutput(NonnullRefPtr<Audio::SingleSinkSessionClient> single_sink_session)
        : single_sink_session(move(single_sink_session))
    {
    }

    NonnullRefPtr<Audio::SingleSinkSessionClient> single_sink_session;
    Optional<Audio::OutputSink> stream;
...
AudioMixingSink::AudioMixingSink(NonnullRefPtr<Audio::SingleSinkSessionClient> single_sink_session)
...
{
    auto single_sink_session = TRY(Audio::SingleSinkSessionClient::try_create());
    auto sink = TRY(try_make_ref_counted<AudioMixingSink>(move(single_sink_session)));
...
intptr_t AudioMixingSink::sink_thread_main()
{
    ...
    while (!output.should_exit.load(AK::MemoryOrder::memory_order_acquire)) {
        ...
        Optional<Audio::SharedCircularBuffer*> ring;
        ...
        Optional<Audio::TimingReader::Snapshot> timing_snapshot;
        {
        ...
        AK::Duration buffer_start_time = AK::Duration::from_time_units(m_next_sample_to_write.load(MemoryOrder::memory_order_acquire), 1, current_spec.sample_rate());
...
{
    Audio::SampleSpecification tap_sample_specification;
    bool should_apply_sample_specification = false;
    Optional<Audio::SampleSpecification> effective_override_sample_specification;

    if (override_sample_specification.has_value() && override_sample_specification->is_valid())
        effective_override_sample_specification = override_sample_specification;

    {
        Threading::MutexLocker locker { m_sample_spec_mutex };
        Audio::SampleSpecification default_sample_specification = m_device_sample_specification;
        tap_sample_specification = effective_override_sample_specification.value_or(default_sample_specification);

        if (callback)
            m_tap_state = adopt_ref(*new TapState(move(callback), effective_override_sample_specification));
        else
            m_tap_state = nullptr;

        Audio::SampleSpecification target_sample_specification = m_device_sample_specification;
        if (m_tap_state)
            target_sample_specification = m_tap_state->override_sample_specification.value_or(m_device_sample_specification);
        should_apply_sample_specification = target_sample_specification.is_valid() && m_sample_specification != target_sample_specification;
        tap_sample_specification = target_sample_specification;
    }

    if (should_apply_sample_specification)
        apply_sample_specification(tap_sample_specification);
...
{
    Audio::SampleSpecification device_sample_specification;
    bool should_apply_sample_specification = false;

    {
        Threading::MutexLocker locker { m_sample_spec_mutex };
        m_tap_state = nullptr;
        device_sample_specification = m_device_sample_specification;
        should_apply_sample_specification = device_sample_specification.is_valid() && m_sample_specification != device_sample_specification;
    }

    if (should_apply_sample_specification)
        apply_sample_specification(device_sample_specification);
...
ReadonlySpan<float> AudioMixingSink::write_audio_data_to_playback_stream(Span<float> buffer, bool* did_mix_audio, bool advance_cursor)
{
    ...
    for (auto& [track, track_data] : m_track_mixing_datas) {
        if (!buffering) {
            ...
            m_main_thread_event_loop.deferred_invoke([weak_self, track] {
...
void AudioMixingSink::request_output_sink_if_needed()
{
    ...
    m_main_thread_event_loop.deferred_invoke([weak_self, single_sink_session = move(single_sink_session)] {
        ...
        auto request_result = single_sink_session->request_output_sink(
            [weak_self](Audio::OutputSink const& output_sink) {
                auto self = weak_self.strong_ref();
                if (!self || !self->m_audio_server_output)
                    return;
                ...
                if (!device_specification.is_valid()) {
                    if (self->on_audio_output_error)
                        self->on_audio_output_error(Error::from_string_literal("Audio output initialization failed"));
                ...
                Audio::SampleSpecification target_sample_specification = device_specification;
                {
                    ...
                    if (self->m_tap_state)
                        target_sample_specification = self->m_tap_state->override_sample_specification.value_or(device_specification);
            ...
            [weak_self](u64, ByteString const& error) {
                auto self = weak_self.strong_ref();
                if (!self || !self->m_audio_server_output)
                    return;

                dbgln("AudioMixingSink({}): request_output_sink fail: {}", (void*)self.ptr(), error);
                {
                    Threading::MutexLocker session_locker { self->m_audio_server_output->session_mutex };
        ...
        if (request_result.is_error()) {
            dbgln("AudioMixingSink({}): request_output_sink sync fail: {}", (void*)self.ptr(), request_result.error());
            if (self->on_audio_output_error)
                self->on_audio_output_error(request_result.release_error());
        }
    });
}
...
void AudioMixingSink::apply_time_change(AK::Duration time)
{
    ...
    i64 current_session_start_sample = next_sample_to_write;
    {
        Threading::MutexLocker session_locker { output.session_mutex };
        if (output.stream.has_value()) {
            Optional<Audio::TimingReader::Snapshot> timing_snapshot = output.stream->timing.read_snapshot();
