...
DecoderErrorOr<Optional<AK::Duration>> FFmpegDemuxer::seek_to_most_recent_keyframe(Track const& track, AK::Duration timestamp, DemuxerSeekOptions)
{
    ...
    do {
        if (track_context.peeked_packet_already) {
            av_packet_unref(&packet);
            track_context.peeked_packet_already = false;
        }
        ...
        track_context.peeked_packet_already = true;
    } while (packet.stream_index != stream.index);

    auto keyframe_time = time_units_to_duration(packet.pts, stream.time_base);
