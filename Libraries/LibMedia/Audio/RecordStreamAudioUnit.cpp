...
ErrorOr<NonnullRefPtr<RecordStream>> RecordStream::create(SampleSpecification const& specification, u32, StringView device_id, RecordCallback callback)
{
    auto stream = TRY(RecordStreamAudioUnit::create(specification, device_id, move(callback)));
    return stream;
}
