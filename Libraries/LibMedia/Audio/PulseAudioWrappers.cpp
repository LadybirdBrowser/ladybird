        ...
        m_context, [](pa_context*, pa_server_info const* info, void* user_data) {
            ...
            auto* operation = pa_context_get_sink_info_by_name(
                context.m_context, info->default_sink_name, [](pa_context*, pa_sink_info const* info, int status, void* user_data) {
                    ...
                    // If any channel position is one we don't enumerate (e.g. PA_CHANNEL_POSITION_AUX*,
                    // which is common on Apple Silicon Linux setups where the speakers map to AUX0..3),
                    // converting the map back to PulseAudio would produce PA_CHANNEL_POSITION_INVALID
                    // and stream creation would fail. Fall back to a stereo map; PulseAudio will remix
                    ...
                    for (u8 i = 0; i < channel_map.channel_count(); ++i) {
                        if (channel_map.channel_at(i) == Channel::Unknown) {
                            has_unknown_channel = true;
                            break;
                        }
                    }
                    if (has_unknown_channel)
                        channel_map = ChannelMap::stereo();

                    context.m_device_sample_specification = SampleSpecification(info->sample_spec.rate, channel_map);
                    context.signal_to_wake();
                },
                &context);
