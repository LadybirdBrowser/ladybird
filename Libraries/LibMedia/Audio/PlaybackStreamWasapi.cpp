...
struct AudioState : public AtomicRefCounted<AudioState> {
    AudioState();
    ~AudioState();
...
ErrorOr<NonnullRefPtr<PlaybackStream>> PlaybackStreamWasapi::create(OutputState initial_output_state, u32 sample_rate, u8 channels, u32, AudioDataRequestCallback&& data_request_callback)
{
    ...
    dbgln("Mixing engine audio format:\n"
          ...
          "dwChannelMask: {:b}\n"
          "SubFormat: {}",
        ideal_format->Format.wFormatTag,
        ideal_format->Format.nChannels,
        ideal_format->Format.nSamplesPerSec,
        ideal_format->Format.nAvgBytesPerSec,
        ideal_format->Format.nBlockAlign,
        ideal_format->Format.wBitsPerSample,
        ideal_format->Format.cbSize,
        ideal_format->Samples.wValidBitsPerSample,
        ideal_format->dwChannelMask,
        Span<u8> { reinterpret_cast<char*>(&ideal_format->SubFormat), 16 });
    ...
    state->wave_format->SubFormat = KSDATAFORMAT_SUBTYPE_PCM;
    ...
    if (hr == S_FALSE) {
        dbgln("Audio format not supported. Current format:\n"
              ...
              "dwChannelMask: {:b}\n"
              "SubFormat: {}",
            state->wave_format->Format.wFormatTag,
            state->wave_format->Format.nChannels,
            state->wave_format->Format.nSamplesPerSec,
            state->wave_format->Format.nAvgBytesPerSec,
            state->wave_format->Format.nBlockAlign,
            state->wave_format->Format.wBitsPerSample,
            state->wave_format->Format.cbSize,
            state->wave_format->Samples.wValidBitsPerSample,
            state->wave_format->dwChannelMask,
            Span<u8> { reinterpret_cast<char*>(&closest_match->SubFormat), 16 });
        dbgln("Supported audio format:\n"
              "wFormatTag: {:x}\n"
              "nChannels: {}\n"
              "nSamplesPerSec: {}\n"
              "nAvgBytesPerSec: {}\n"
              ...
              "dwChannelMask: {:b}\n"
              "SubFormat: {}",
            closest_match->Format.wFormatTag,
            closest_match->Format.nChannels,
            closest_match->Format.nSamplesPerSec,
            closest_match->Format.nAvgBytesPerSec,
            closest_match->Format.nBlockAlign,
            closest_match->Format.wBitsPerSample,
            closest_match->Format.cbSize,
            closest_match->Samples.wValidBitsPerSample,
            closest_match->dwChannelMask,
            Span<u8> { reinterpret_cast<char*>(&closest_match->SubFormat), 16 });
    ...
    hr = state->audio_client->Initialize(AUDCLNT_SHAREMODE_SHARED, stream_flags, 0, 0, &state->wave_format->Format, &PlaybackSessionGUID);
    if (FAILED(hr))
        return Error::from_windows_error(hr);
    dbgln("Initialized audio client");
...
int AudioState::render_thread_loop(AudioState* state)
{
    ...
    while (!state->exit_requested.load(MemoryOrder::memory_order_acquire)) {
        ...
        // TODO: Verify the memory order here
        if (state->exit_requested.load(MemoryOrder::memory_order_acquire))
            break;
        ...
        // TODO: Have sane action on failing. This is for testing only.
        if (FAILED(hr)) {
            dbgln("GetCurrentPadding failed with: {}", Error::from_windows_error(hr));
            VERIFY_NOT_REACHED();
        }
        ...
        if (FAILED(hr)) {
            dbgln("GetBuffer failed with: {}", Error::from_windows_error(hr));
            VERIFY_NOT_REACHED();
        }

        u32 buffer_size = frames_available * block_align;
        dbgln("Request buffer of size: {}\n, frames_available: {}", buffer_size, frames_available);
        ...
        hr = state->render_client->ReleaseBuffer(frames_available, buffer_flags);
        if (FAILED(hr)) {
...
NonnullRefPtr<Core::ThreadedPromise<AK::Duration>> PlaybackStreamWasapi::resume()
{
    ...
    if (m_state->playing.load(MemoryOrder::memory_order_relaxed)) {
        m_state->resume_promise->resolve(total_time_played());
        return m_state->resume_promise.release_nonnull();
    }
...
NonnullRefPtr<Core::ThreadedPromise<void>> PlaybackStreamWasapi::drain_buffer_and_suspend()
{
    ...
    Threading::MutexLocker locker(m_state->mutex);
    m_state->suspend_promise = promise;
    if (m_state->playing.load(MemoryOrder::memory_order_relaxed)) {
...
AK::Duration PlaybackStreamWasapi::total_time_played() const
{
    ...
    return AK::Duration::from_nanoseconds(static_cast<i64>(position * 1'000'000'000ull / frequency));
}
...
NonnullRefPtr<Core::ThreadedPromise<void>> PlaybackStreamWasapi::set_volume(double volume)
{
    ...
    auto channels = m_state->wave_format->Format.nChannels;
