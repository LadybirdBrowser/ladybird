...
int AudioState::render_thread_loop(void* context)
{
    ...
    WORD block_align = state->wave_format->nBlockAlign;

    ScopeGuard uninitialize_com = [] { CoUninitialize(); };

    DWORD task_index = 0;
    // Maybe this should be done on thread spawn and not on each loop.
    ...
    while (!state->exit_requested.load(MemoryOrder::memory_order_acquire)) {
        ...
        if (bytes_written.is_empty()) {
            ...
            // TODO: Is making sure the audio is silent all that is needed if we have no data?
