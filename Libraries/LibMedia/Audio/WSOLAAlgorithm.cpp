...
void copy_partial_frames(AudioBuffer const& source, size_t source_offset, size_t frame_count, size_t destination_offset, Media::AudioBlock& destination)
{
    ...
    , m_search_block_center_offset((m_num_candidate_blocks / 2) + ((m_ola_window_size / 2) - 1))
{
    ...
    m_wsola_output = create_buffer(m_ola_window_size + m_ola_hop_size);

    m_optimal_block = create_buffer(m_ola_window_size);
    m_search_block = create_buffer(m_num_candidate_blocks + (m_ola_window_size - 1));
    m_target_block = create_buffer(m_ola_window_size);
}

WSOLAAlgorithm::~WSOLAAlgorithm() = default;

AudioBuffer WSOLAAlgorithm::create_buffer(size_t frame_count) const
{
    return AudioBuffer { m_sample_specification, frame_count };
}
...
void WSOLAAlgorithm::peek_audio_with_zero_prepend(i64 read_offset_frames, AudioBuffer& destination)
{
    ...
    if (read_offset_frames < 0) {
        ...
        destination.zero_frames(0, num_zero_frames_appended);
    }
    if (frames_to_read > 0)
