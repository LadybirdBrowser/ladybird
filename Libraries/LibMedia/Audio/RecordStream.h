...
public:
    ...
    // Creates a capture stream on the platform audio backend. The device id is a
    // dom_device_id produced by Media::AudioDevices, or an empty string to capture from the
    // default input device. Backends honor the requested sample specification by converting
    // where they can; the specification passed to the callback is authoritative.
    static ErrorOr<NonnullRefPtr<RecordStream>> create(SampleSpecification const&, u32 fragment_size_bytes, StringView device_id, RecordCallback);
