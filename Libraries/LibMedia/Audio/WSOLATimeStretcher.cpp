...
Media::DecoderErrorOr<void> WSOLATimeStretcher::retrieve_block(Media::AudioBlock& into)
{
    auto const target_output_count = m_impl->output_chunk_frames();

    into.initialize(m_impl->sample_specification, m_impl->next_output_frame_index, target_output_count);
    auto const rendered = m_impl->algorithm->fill_buffer(into, 0, target_output_count, m_impl->rate);
    ...
    into.set_media_time_start(AK::Duration::from_time_units(media_start_in_frames, 1, m_impl->sample_specification.sample_rate()));
    into.set_media_time_duration(AK::Duration::from_time_units(media_duration_in_frames, 1, m_impl->sample_specification.sample_rate()));

    m_impl->next_output_frame_index = saturating_add(m_impl->next_output_frame_index, AK::clamp_to<i64>(into.frame_count()));
