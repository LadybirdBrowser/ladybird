...
void AudioDataProvider::ThreadData::resolve_seek()
{
    {
        auto locker = take_lock();
...
bool AudioDataProvider::ThreadData::handle_seek()
{
    ...
    auto reject_promise = [&](DecoderError&& error) {
        {
            auto locker = take_lock();
            m_queue.clear();
        }
    ...
    while (true) {
        ...
        auto demuxer_seek_result = m_demuxer->seek_to_most_recent_keyframe(m_track, timestamp);
        if (demuxer_seek_result.is_error()) {
            reject_promise(demuxer_seek_result.release_error());
            return true;
        }
        ...
        while (new_seek_id == seek_id) {
            ...
            if (coded_frame_result.is_error()) {
                reject_promise(coded_frame_result.release_error());
                return true;
            }
            ...
            if (decode_result.is_error()) {
                reject_promise(decode_result.release_error());
                return true;
            }
            ...
            while (new_seek_id == seek_id) {
                ...
                if (block_result.is_error()) {
                    if (block_result.error().category() == DecoderErrorCategory::NeedsMoreInput)
                        break;
                    reject_promise(block_result.release_error());
                    return true;
                }
