...
public:
    ...
    static DecoderErrorOr<Reader> from_stream(IncrementallyPopulatedStream&);
    static bool sniff_webm(IncrementallyPopulatedStream&);
    ...
    void abort_blocking_reads() const { m_stream->abort_blocking_reads(); }

private:
    Reader(IncrementallyPopulatedStream& stream)
        : m_stream(stream)
    {
    }
...
class Streamer {
public:
    Streamer(NonnullRefPtr<IncrementallyPopulatedStream::Seekable> stream)
        : m_stream(stream)
    {
    }
    ...
    size_t position() const { return m_stream->position(); }
