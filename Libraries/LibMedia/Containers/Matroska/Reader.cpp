...
DecoderErrorOr<Reader> Reader::from_stream(NonnullRefPtr<IncrementallyPopulatedStream> stream)
{
...
DecoderErrorOr<void> Reader::parse_initial_data()
{
    ...
    dbgln_if(MATROSKA_TRACE_DEBUG, "Segment is at {} with size {}, available size is {}", m_segment_contents_position, m_segment_contents_size, m_stream->size() - m_segment_contents_position);
    return {};
}
...
{
    u8 byte = TRY(m_stream->read_octet(m_position));
    m_octets_read.last()++;
    m_position++;
    return byte;
}
...
{
    auto result = TRY(m_stream->read_octets(m_position, num_octets));
