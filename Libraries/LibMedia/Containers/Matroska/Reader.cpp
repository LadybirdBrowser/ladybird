...
DecoderErrorOr<Reader> Reader::from_stream(IncrementallyPopulatedStream& stream)
{
...
bool Reader::sniff_webm(IncrementallyPopulatedStream& stream)
{
    auto result = [&] -> DecoderErrorOr<bool> {
        Reader reader(stream);
        Streamer streamer { stream.create_seekable() };
...
{
    Streamer streamer { m_stream->create_seekable() };
    auto first_element_id = TRY(streamer.read_variable_size_integer(false));
    ...
    dbgln_if(MATROSKA_TRACE_DEBUG, "Segment is at {} with size {}, available size is {}", m_segment_contents_position, m_segment_contents_size, m_stream->total_size() - m_segment_contents_position);
    return {};
}
...
DecoderErrorOr<SampleIterator> Reader::create_sample_iterator(u64 track_number)
{
    ...
    return SampleIterator(m_stream, TRY(track_for_track_number(track_number)), TRY(segment_information()).timestamp_scale(), m_segment_contents_position, position);
}
...
DecoderErrorOr<u8> Streamer::read_octet()
{
    ...
    auto count = TRY(m_stream->read_bytes(bytes));
    VERIFY(count == 1);
    m_octets_read.last()++;
    return bytes[0];
}
...
{
    ByteBuffer result;
    MUST(result.try_resize(num_octets));
    auto bytes = result.bytes();
    auto count = TRY(m_stream->read_bytes(bytes));
    VERIFY(count == num_octets);
...
{
    return m_stream->seek(position, IncrementallyPopulatedStream::Seekable::SeekMode::SetPosition);
}
