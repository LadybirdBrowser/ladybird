...
static constexpr size_t BITRATE_SCAN_BYTE_COUNT = 4096;
...
{
    TRY(m_cursor->seek_to_position(frame.position));
    return TRY(m_cursor->read_bytes(frame.header.frame_byte_size));
}
...
static Optional<FrameScan> scan_frames_at(NonnullRefPtr<MediaStreamCursor> const& cursor, size_t position)
{
    ...
    FrameScan scan;
    while (scan.frame_count < FRAME_COUNT_RULING_OUT_A_FALSE_SYNC_CODE || scan.byte_count < BITRATE_SCAN_BYTE_COUNT) {
...
// Bitrate and padding differ from one frame to the next, and so does the mode extension of a joint
// stereo stream, but the rest of what a header describes holds for all the frames of a stream.
...
// A stream carries its frames from where its identification tags end, but one that was cut can have
// junk in between, so the frames are searched for when they do not begin there.
...
static Optional<FrameScan> scan_first_frames(NonnullRefPtr<MediaStreamCursor> const& cursor)
{
    auto position = find_first_frame_position(cursor);
    if (!position.has_value())
        return {};
    return scan_frames_at(cursor, *position);
}

static Optional<StreamInfo> read_stream_info(MediaStreamCursor& cursor, FrameScan const& scan)
{
    Vector<u8> frame;
    frame.resize(scan.header.frame_byte_size);
    if (!read_exact(cursor, scan.position, frame))
        return {};
    return StreamInfo::parse(frame, scan.header);
}

Optional<Reader> Reader::from_stream(NonnullRefPtr<MediaStreamCursor> const& cursor)
{
    auto scan = scan_first_frames(cursor);
    if (!scan.has_value())
        return {};

    Reader reader;
    reader.m_frame_header = scan->header;
    reader.m_first_audio_frame_position = scan->position;
    reader.m_stream_info = read_stream_info(*cursor, scan.value());

    // A frame describing the stream carries no audio, so it counts towards neither the timeline nor
    // the bitrate the remaining frames establish.
    if (reader.m_stream_info.has_value()) {
        reader.m_first_audio_frame_position += scan->header.frame_byte_size;
        scan->frame_count--;
        scan->sample_count -= scan->header.sample_count;
        scan->byte_count -= scan->header.frame_byte_size;
    }

    auto sample_rate = scan->header.sample_rate;
    if (reader.m_stream_info.has_value()) {
        if (auto sample_count = reader.m_stream_info->total_sample_count(scan->header); sample_count.has_value()) {
            reader.m_duration = AK::Duration::from_time_units(static_cast<i64>(*sample_count), 1, sample_rate);
            return reader;
        }
    }

    reader.m_duration_is_estimated = true;
    if (scan->byte_count == 0)
        return reader;

    auto audio_byte_count = scan->byte_count;
    if (auto stream_byte_count = cursor->size(); stream_byte_count.has_value() && stream_byte_count.value() > reader.m_first_audio_frame_position)
        audio_byte_count = max(audio_byte_count, static_cast<size_t>(*stream_byte_count) - reader.m_first_audio_frame_position);

    // Prioritize the size indicated by the metadata, as that will exclude trailing garbage.
    if (reader.m_stream_info.has_value() && reader.m_stream_info->byte_count.has_value()) {
        auto declared_byte_count = *reader.m_stream_info->byte_count;
        if (declared_byte_count > scan->header.frame_byte_size)
            audio_byte_count = max(scan->byte_count, declared_byte_count - scan->header.frame_byte_size);
    }

    auto estimated_sample_count = static_cast<u64>(audio_byte_count) * scan->sample_count / scan->byte_count;
    reader.m_duration = AK::Duration::from_time_units(static_cast<i64>(estimated_sample_count), 1, sample_rate);
    return reader;
}
