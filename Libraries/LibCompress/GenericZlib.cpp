...
GenericZlibDecompressor::GenericZlibDecompressor(MaybeOwned<Stream> stream, z_stream* zstream)
...
ErrorOr<Bytes> GenericZlibDecompressor::read_some(Bytes bytes)
{
    ...
    if (m_zstream->avail_in == 0) {
        auto in = TRY(m_stream->read_some(Bytes { m_buffer_in, sizeof(m_buffer_in) }));
        m_zstream->avail_in = in.size();
        m_zstream->next_in = m_buffer_in;
    }
...
GenericZlibCompressor::GenericZlibCompressor(MaybeOwned<Stream> stream, z_stream* zstream)
    : m_stream(move(stream))
    , m_zstream(zstream)
{
}
...
ErrorOr<size_t> GenericZlibCompressor::write_some(ReadonlyBytes bytes)
{
    ...
    do {
        m_zstream->avail_out = sizeof(m_buffer_out);
        m_zstream->next_out = m_buffer_out;

        auto ret = deflate(m_zstream, Z_NO_FLUSH);
        if (ret != Z_OK && ret != Z_BUF_ERROR)
            return handle_zlib_error(ret);

        auto have = sizeof(m_buffer_out) - m_zstream->avail_out;
        TRY(m_stream->write_until_depleted(ReadonlyBytes { m_buffer_out, have }));
