...
struct DNSKEY {
    ...
    ReadonlyBytes public_key_rsa_exponent() const [[clang::lifetimebound]] { return public_key.bytes().slice(1, public_key_rsa_exponent_length()); }
    ReadonlyBytes public_key_rsa_modulus() const [[clang::lifetimebound]] { return public_key.bytes().slice(1 + public_key_rsa_exponent_length()); }
