...
ErrorOr<NonnullOwnPtr<MappedTextureBuffer>> Texture::Impl::map_buffer()
{
    ...
    return make<MappedTextureBuffer>(*this, static_cast<u8*>(mapped_buffer), buffer_size);
}
...
MappedTextureBuffer::MappedTextureBuffer(Texture::Impl& texture_impl, u8* buffer, size_t buffer_size)
