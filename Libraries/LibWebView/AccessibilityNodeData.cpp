...
ErrorOr<void> IPC::encode(Encoder& encoder, WebView::AccessibilityNodeData const& node)
{
    ...
    TRY(encoder.encode(node.hierarchical_level));
    ...
    TRY(encoder.encode(node.is_password));
    ...
    TRY(encoder.encode(to_underlying(node.pressed_state)));
...
ErrorOr<WebView::AccessibilityNodeData> IPC::decode(Decoder& decoder)
{
    ...
    auto hierarchical_level = TRY(decoder.decode<i32>());
    ...
    auto is_password = TRY(decoder.decode<bool>());
    ...
    auto pressed_state = static_cast<WebView::AccessibilityNodeData::PressedState>(TRY(decoder.decode<u8>()));
