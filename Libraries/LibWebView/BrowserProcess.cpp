...
NonnullRefPtr<Messages::UIProcessServer::CreateNewTab::Promise> UIProcessConnectionFromClient::create_new_tab(Vector<ByteString> urls)
{
    auto promise = Messages::UIProcessServer::CreateNewTab::Promise::construct();
    if (on_new_tab)
        on_new_tab(sanitize_urls(urls, Application::settings().new_tab_page_url()));

    promise->resolve(Messages::UIProcessServer::CreateNewTabResponse {});
    return promise;
}

NonnullRefPtr<Messages::UIProcessServer::CreateNewWindow::Promise> UIProcessConnectionFromClient::create_new_window(Vector<ByteString> urls)
{
    auto promise = Messages::UIProcessServer::CreateNewWindow::Promise::construct();
    if (on_new_window)
        on_new_window(sanitize_urls(urls, Application::settings().new_tab_page_url()));

    promise->resolve(Messages::UIProcessServer::CreateNewWindowResponse {});
    return promise;
}
