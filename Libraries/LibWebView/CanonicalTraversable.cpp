...
void CanonicalTraversable::traverse_the_history(TraversableSessionHistory::TraversalTarget const& target, CheckForCancelation check_for_cancelation, Function<void()> on_ready, NonnullRefPtr<Core::Promise<Empty>> promise)
{
    run_browser_history_traversal_at_queue_position(
        ...
        allocate_navigation_or_traversal_sequence_number(),
        move(on_ready),
        nullptr,
        move(promise));
}
...
struct CanonicalTraversable::HistoryOperation {
    HistoryOperation(u64 operation_id, Web::HistoryOperationParameters parameters, Optional<u64> initiation_id, RefPtr<WebContentClient> initiating_client, u64 initiating_page_id, Optional<i32> resolved_step, Optional<u64> traversal_sequence_number, OnHistoryOperationComplete on_complete)
...
void CanonicalTraversable::queue_browser_history_traversal(Optional<i32> target_step, Optional<int> delta, CheckForCancelation check_for_cancelation, Function<void()> on_ready)
{
    if (m_pending_browser_history_traversal.has_value()) {
        ...
        m_pending_browser_history_traversal->traversal_sequence_number = allocate_navigation_or_traversal_sequence_number();
        if (target_step.has_value()) {
    ...
    m_pending_browser_history_traversal = PendingBrowserHistoryTraversal {
        ...
        .traversal_sequence_number = allocate_navigation_or_traversal_sequence_number(),
        .deltas = {},
...
void CanonicalTraversable::run_pending_browser_history_traversal(TraversableSessionHistory::TraversalTarget target, NonnullRefPtr<Core::Promise<Empty>> promise)
{
    ...
    run_browser_history_traversal_at_queue_position(
        ...
        check_for_cancelation == CheckForCancelation::Yes,
        m_pending_browser_history_traversal->traversal_sequence_number,
        move(on_ready),
        nullptr,
        move(promise));
}
...
bool CanonicalTraversable::select_changing_navigable_history_step_job_endpoint(HistoryOperation& operation, ApplyHistoryStepJobs::ChangingNavigableHistoryStepJob& job)
{
    ...
    if (job.navigation_type == Web::Bindings::NavigationType::Traverse
        && operation.traversal_sequence_number.has_value()) {
        if (auto& ongoing_navigation = navigable->ongoing_navigation(); ongoing_navigation.has_value()) {
            if (ongoing_navigation->sequence_number > *operation.traversal_sequence_number) {
                // A newer navigation takes precedence over this traversal.
                job.superseded_by_newer_navigation = true;
            } else {
                // Supersede the older navigation so its late callbacks are rejected.
                navigable->clear_ongoing_navigation();
            }
        }
    }
...
void CanonicalTraversable::dispatch_changing_navigable_history_step_job(HistoryOperation& operation, Web::HTML::CrossProcessId navigable_id)
{
    ...
    endpoint->client->async_run_changing_navigable_history_job(
        ...
        pending_job.value()->job.superseded_by_newer_navigation, initiation_id);
}
...
void CanonicalTraversable::run_history_operation_at_queue_position(u64 initiation_id, Web::HistoryOperationParameters request, WebContentClient& requesting_client, u64 requesting_page_id, Optional<i32> resolved_step, Optional<u64> traversal_sequence_number, OnHistoryOperationComplete on_complete, NonnullRefPtr<Core::Promise<Empty>> promise)
{
    auto operation_id = m_next_history_operation_id++;
    m_history_operations.set(operation_id, make<HistoryOperation>(operation_id, move(request), initiation_id, &requesting_client, requesting_page_id, resolved_step, traversal_sequence_number, move(on_complete)));
    auto* operation = find_history_operation(operation_id);
    VERIFY(operation);
...
{
    auto operation_id = m_next_history_operation_id++;
    auto owned_operation = make<HistoryOperation>(operation_id, Web::HistoryOperationParameters { move(parameters) }, Optional<u64> {}, nullptr, 0, Optional<i32> {}, traversal_sequence_number, move(on_complete));
...
void CanonicalTraversable::enqueue_history_operation(u64 initiation_id, Web::HistoryOperationParameters request, WebContentClient& requesting_client, u64 requesting_page_id, Optional<u64> traversal_sequence_number, OnHistoryOperationComplete on_complete)
{
    ...
    auto steps = [this, initiation_id, request = move(request), requesting_client = move(requesting_client_ref), requesting_page_id, traversal_sequence_number, on_complete = move(on_complete)](NonnullRefPtr<Core::Promise<Empty>> promise) mutable {
        run_history_operation_at_queue_position(initiation_id, move(request), *requesting_client, requesting_page_id, {}, traversal_sequence_number, move(on_complete), move(promise));
    };
...
{
    auto traversal_sequence_number = allocate_navigation_or_traversal_sequence_number();
    auto steps = [this, parameters = move(parameters), check_for_cancelation, traversal_sequence_number, on_complete = move(on_complete)](NonnullRefPtr<Core::Promise<Empty>> promise) mutable {
        run_browser_history_traversal_at_queue_position(move(parameters), check_for_cancelation, traversal_sequence_number, nullptr, move(on_complete), move(promise));
    };
    m_history_traversal_queue.append_session_history_traversal_steps(move(steps));
}
        ...
        if (child_navigable.has_value() && current_step.has_value()) {
            if (auto const* target_entry = m_session_history.get_the_target_history_entry(*child_navigable, *current_step)) {
                ...
                auto ongoing_navigation = CanonicalNavigable::OngoingNavigation {
                    ...
                    .sequence_number = allocate_navigation_or_traversal_sequence_number(),
                    .has_started = true,
                    .is_uncommitted = true,
...
void CanonicalTraversable::did_receive_history_step_unload_cancelation_result(WebContentClient& source_client, u64 source_page_id, u64 operation_id, Web::HTML::HistoryStepResult result)
{
