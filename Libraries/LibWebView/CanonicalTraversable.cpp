...
struct CanonicalTraversable::HistoryOperation {
    HistoryOperation(Web::HTML::CrossProcessId operation_id, Web::HistoryOperationParameters parameters, RefPtr<WebContentClient> initiating_client, u64 initiating_page_id, Optional<i32> resolved_step, u64 sequence_number, OnHistoryOperationComplete on_complete)
...
void CanonicalTraversable::run_history_operation_at_queue_position(Web::HTML::CrossProcessId operation_id, Web::HistoryOperationParameters request, WebContentClient& requesting_client, u64 requesting_page_id, Optional<i32> resolved_step, u64 sequence_number, OnHistoryOperationComplete on_complete, NonnullRefPtr<Core::Promise<Empty>> promise)
{
    ...
    m_history_operations.set(operation_id, make<HistoryOperation>(operation_id, move(request), &requesting_client, requesting_page_id, resolved_step, sequence_number, move(on_complete)));
    auto* operation = find_history_operation(operation_id);
    VERIFY(operation);
...
void CanonicalTraversable::run_browser_history_traversal_at_queue_position(Web::TraverseToStepHistoryOperationParameters parameters, bool check_for_cancelation, u64 sequence_number, Function<void()> on_ready, OnHistoryOperationComplete on_complete, NonnullRefPtr<Core::Promise<Empty>> promise)
{
    ...
    auto owned_operation = make<HistoryOperation>(operation_id, Web::HistoryOperationParameters { move(parameters) }, nullptr, 0, Optional<i32> {}, sequence_number, move(on_complete));
...
void CanonicalTraversable::enqueue_history_operation(Web::HTML::CrossProcessId operation_id, Web::HistoryOperationParameters request, WebContentClient& requesting_client, u64 requesting_page_id, u64 sequence_number, OnHistoryOperationComplete on_complete)
{
    ...
    auto steps = [this, operation_id, request = move(request), requesting_client = move(requesting_client_ref), requesting_page_id, sequence_number, on_complete = move(on_complete)](NonnullRefPtr<Core::Promise<Empty>> promise) mutable {
        run_history_operation_at_queue_position(operation_id, move(request), *requesting_client, requesting_page_id, {}, sequence_number, move(on_complete), move(promise));
    };
