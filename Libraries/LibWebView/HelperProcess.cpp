...
ErrorOr<NonnullRefPtr<::Web::WebAudio::BrokerOfWebAudioWorker>> launch_webaudio_renderer_process()
{
    ...
    auto client = TRY(launch_server_process<::Web::WebAudio::BrokerOfWebAudioWorker>("WebAudioWorker"sv, arguments));
    auto audio_server_client = TRY(connect_new_audio_server_client());
    client->async_connect_to_audio_server(audio_server_client.handle, audio_server_client.grant_id);
    return client;
}
