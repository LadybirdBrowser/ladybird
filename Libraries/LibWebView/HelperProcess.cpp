...
ErrorOr<NonnullRefPtr<::Web::WebAudio::BrokerOfWebAudioWorker>> launch_webaudio_renderer_process()
{
    Vector<ByteString> arguments;
    if (auto server = mach_server_name(); server.has_value()) {
        arguments.append("--mach-server-name"sv);
    ...
    arguments.append("--audio-server-socket"sv);
    arguments.append(ByteString::number(audio_server_client.socket.fd()));
    arguments.append("--audio-grant-id"sv);
    arguments.append(audio_server_client.grant_id);

    return launch_server_process<::Web::WebAudio::BrokerOfWebAudioWorker>("WebAudioWorker"sv, arguments);
}
