...
Application::~Application()
{
    ...
    // Explicitly delete the observers first, as the observer destructor will refer to Application::the().
...
{
    if (!ProcessPolicyRouter::should_maintain_spare_web_content_process(browser_options()))
        return;
...
ErrorOr<void> Application::launch_audio_server()
{
    ...
    m_audio_server->on_death = [this]() {
        ...
        WebContentClient::for_each_client([new_clients = new_clients_or_error.release_value()](WebContentClient& client) mutable {
            ...
            client.async_connect_to_audio_server(new_client.handle, new_client.grant_id);
...
void Application::process_did_exit(Process&& process)
{
    ...
    case ProcessType::AudioServer:
        ...
        if (auto client = process.client<Audio::BrokerOfAudioServer>(); client.has_value()) {
            if (auto on_death = move(client->on_death))
                on_death();
        }
        break;
