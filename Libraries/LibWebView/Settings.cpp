...
static constexpr auto site_settings_enabled_globally_key = "enabledGlobally"sv;
static constexpr auto site_settings_site_filters_key = "siteFilters"sv;
...
Settings Settings::create(Badge<Application>)
{
    ...
    auto init_site_settings = [&](SiteSetting& site_settings, StringView key) {
        auto saved_settings = settings_json.value().get_object(key);
        if (!saved_settings.has_value())
            return;

        if (auto enabled_globally = saved_settings->get_bool(site_settings_enabled_globally_key); enabled_globally.has_value())
            site_settings.enabled_globally = *enabled_globally;

        if (auto site_filters = saved_settings->get_array(site_settings_site_filters_key); site_filters.has_value()) {
            site_settings.site_filters.clear();

            site_filters->for_each([&](auto const& site_filter) {
                if (site_filter.is_string())
                    site_settings.site_filters.set(site_filter.as_string());
            });
        }
    };

    init_site_settings(settings.m_autoplay, autoplay_key);
...
JsonValue Settings::serialize_json() const
{
    ...
    JsonArray autoplay_site_filters;
    autoplay_site_filters.ensure_capacity(m_autoplay.site_filters.size());

    for (auto const& site_filter : m_autoplay.site_filters)
        autoplay_site_filters.must_append(site_filter);

    JsonObject autoplay;
    autoplay.set("enabledGlobally"sv, m_autoplay.enabled_globally);
    autoplay.set("siteFilters"sv, move(autoplay_site_filters));
    settings.set("autoplay"sv, move(autoplay));
