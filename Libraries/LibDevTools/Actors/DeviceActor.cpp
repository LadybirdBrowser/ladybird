...
void DeviceActor::handle_message(Message const& message)
{
    if (message.type == "getDescription"sv) {
        ...
        static auto version_label = MUST(String::formatted("Version {}", browser_version));
        ...
        value.set("appbuildid"sv, version_label);
        value.set("platformbuildid"sv, version_label);
