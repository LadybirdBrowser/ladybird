...
ErrorOr<void> WebAudioClient::webaudio_session_add_worklet_module(u64 session_id, ByteString url, ByteString source_text)
{
    TRY(ensure_connection(0));
    (void)m_connection->post_message(Messages::WebAudioServer::WebaudioSessionAddWorkletModule(session_id, move(url), move(source_text)));
    return {};
}
