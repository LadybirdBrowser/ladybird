...
PatternErrorOr<Init> ConstructorStringParser::parse(Utf8View const& input)
{
    ...
    while (parser.m_token_index < parser.m_token_list.size()) {
        ...
        // Spec-Note: On every iteration of the parse loop the parser’s token index will be incremented by its token
        ...
        if (parser.m_token_list[parser.m_token_index].type == Token::Type::End) {
            ...
            if (parser.m_state == State::Initial) {
                // Spec-Note: If we reached the end of the string in the "init" state, then we failed to find a protocol
                //            terminator and this has to be a relative URLPattern constructor string.

                // 1. Run rewind given parser.
                parser.rewind();

                // Spec-Note: We next determine at which component the relative pattern begins. Relative pathnames are
            ...
            if (parser.m_state == State::Authority) {
                // Spec-Note: If we reached the end of the string in the "authority" state, then we failed to find an
        ...
        if (parser.is_a_group_open()) {
            // Spec-Note: We ignore all code points within "{ ... }" pattern groupings. It would not make sense to allow
        ...
        case State::Protocol: {
            ...
            if (parser.is_a_protocol_suffix()) {
                ...
                // Spec-Note: We need to eagerly compile the protocol component to determine if it matches any special
    ...
    // Spec-Note: This is special-cased because when an author does not specify a port, they usually intend the default
    // port. If any port is acceptable, the author can specify it as a wildcard explicitly. For example,
    // "https://example.com/*" does not match URLs beginning with "https://example.com:8443/", which is a different origin.
...
void ConstructorStringParser::change_state(State new_state, u32 skip)
{
    ...
    if (m_state != State::Initial && new_state != State::Done) {
        ...
        //    to the empty string.
        if ((m_state == State::Protocol || m_state == State::Authority || m_state == State::Username || m_state == State::Password)
            && (new_state == State::Port || new_state == State::Pathname || new_state == State::Search || new_state == State::Hash)
        ...
        if ((m_state == State::Protocol || m_state == State::Authority || m_state == State::Username || m_state == State::Password
                || m_state == State::Hostname || m_state == State::Port)
        ...
        //    to the empty string.
        if ((m_state == State::Protocol || m_state == State::Authority || m_state == State::Username || m_state == State::Password
                || m_state == State::Hostname || m_state == State::Port || m_state == State::Pathname)
