...
PatternErrorOr<void> PatternParser::consume_a_required_token(Token::Type type)
{
    ...
    auto const* result = try_to_consume_a_token(type);
...
String PatternParser::consume_text()
{
    ...
    while (true) {
        ...
        auto const* token = try_to_consume_a_token(Token::Type::Char);
...
PatternErrorOr<void> PatternParser::add_a_part(String const& prefix, Token const* name_token,
    Token const* regexp_or_wildcard_token, String const& suffix, Token const* modifier_token)
{
    ...
    if (!name_token && !regexp_or_wildcard_token && modifier == Part::Modifier::None) {
...
Token const* PatternParser::try_to_consume_a_modifier_token()
{
    // 1. Let token be the result of running try to consume a token given parser and "other-modifier".
    auto const* token = try_to_consume_a_token(Token::Type::OtherModifier);
...
Token const* PatternParser::try_to_consume_a_regexp_or_wildcard_token(Token const* name_token)
{
    // 1. Let token be the result of running try to consume a token given parser and "regexp".
    auto const* token = try_to_consume_a_token(Token::Type::Regexp);
...
Token const* PatternParser::try_to_consume_a_token(Token::Type type)
{
...
PatternErrorOr<Vector<Part>> PatternParser::parse(Utf8View const& input, Options const& options, EncodingCallback encoding_callback)
{
    ...
    while (parser.m_index < parser.m_token_list.size()) {
        ...
        auto const* char_token = parser.try_to_consume_a_token(Token::Type::Char);

        // 2. Let name token be the result of running try to consume a token given parser and "name".
        auto const* name_token = parser.try_to_consume_a_token(Token::Type::Name);
        ...
        auto const* regexp_or_wildcard_token = parser.try_to_consume_a_regexp_or_wildcard_token(name_token);
        ...
        if (name_token || regexp_or_wildcard_token) {
            ...
            auto const* modifier_token = parser.try_to_consume_a_modifier_token();
        ...
        auto const* fixed_token = char_token;
        ...
        auto const* open_token = parser.try_to_consume_a_token(Token::Type::Open);
