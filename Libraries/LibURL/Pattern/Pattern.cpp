...
PatternErrorOr<Optional<Result>> Pattern::match(Variant<String, Init, URL> const& input, Optional<String> const& base_url_string) const
{
    ...
    // 10. If input is a URL:
    if (input.has<URL>()) {
        // 1. Append the serialization of input to inputs.
        inputs.append(input.get<URL>().serialize());
    }
    // 11. Otherwise:
    else {
        // 1. Append input to inputs.
    ...
    if (input.has<Init>()) {
        ...
        auto apply_result_or_error = process_a_url_pattern_init(input.get<Init>(), PatternProcessType::URL,
    ...
    // 12. Otherwise:
        ...
        if (input.has<String>()) {
            ...
            auto maybe_url = Parser::basic_parse(input.get<String>(), base_url);
