...
void AudioFrameFanout::deliver(float const* samples, size_t frame_count, u8 channel_count, u32 sample_rate)
{
    ...
    for (auto const& sink : sinks) {
        if (sink->on_frames)
            sink->on_frames(samples, frame_count, channel_count, sample_rate);
    }
}
...
void MediaStreamTrack::ensure_audio_capture_started()
{
    ...
    auto stream_or_error = Audio::RecordStream::create(specification, fragment_size_bytes, device_id_string,
    ...
    if (stream_or_error.is_error()) {
        dbgln("MediaStreamTrack: Unable to open a capture stream: {}", stream_or_error.error());
        return;
    }
    m_audio_capture_stream = stream_or_error.release_value();
}
