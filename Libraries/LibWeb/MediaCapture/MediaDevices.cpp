...
static void resolve_media_stream_promise(JS::Realm& realm, WebIDL::Promise& promise, GC::Ref<MediaStream> stream)
{
    WebIDL::resolve_promise(realm, promise, Bindings::wrap(Bindings::host_defined_wrapper_world(realm), realm, stream));
}

static void resolve_media_device_info_list_promise(JS::Realm& realm, WebIDL::Promise& promise, GC::RootVector<GC::Ref<MediaDeviceInfo>> const& result_list)
{
...
void MediaDevices::queue_get_user_media_task(GC::Ref<WebIDL::Promise> promise, Optional<Vector<String>> requested_device_ids)
{
    ...
    Platform::EventLoopPlugin::the().deferred_invoke(GC::create_function(GC::Heap::the(), [promise = GC::Root(promise), media_devices, requested_device_ids = move(requested_device_ids), audio_input_devices = move(audio_input_devices)] mutable {
        ...
        auto reject_not_found = [&](Utf16String const& message) {
            WebIDL::reject_promise(realm, *promise, WebIDL::NotFoundError::create(message));
        };

        auto reject_permission_failure = [&] {
            WebIDL::reject_promise(realm, *promise, WebIDL::NotAllowedError::create("Permission denied"_utf16));
        };
        ...
        if (!final_candidate.has_value()) {
            WebIDL::reject_promise(realm, *promise, WebIDL::NotReadableError::create("No readable audio input devices available"_utf16));
            return;
        }
        ...
        resolve_media_stream_promise(realm, *promise, stream);
...
void MediaDevices::queue_enumerate_devices_task(GC::Ref<WebIDL::Promise> promise)
{
    ...
    Platform::EventLoopPlugin::the().deferred_invoke(GC::create_function(GC::Heap::the(), [promise = GC::Root(promise), media_devices] {
        ...
        resolve_media_device_info_list_promise(realm, *promise, result_list);
    }));
}
...
// https://w3c.github.io/mediacapture-main/#dom-mediadevices-getusermedia
void MediaDevices::get_user_media(JS::Realm& realm, Optional<MediaStreamConstraints> const& constraints, GC::Ref<WebIDL::Promise> promise)
{
    ...
    if (!constraints.has_value()) {
        WebIDL::reject_promise(realm, promise, JS::TypeError::create(realm, "getUserMedia requires constraints"sv));
        return;
    }
    ...
    if (!audio_requested && !video_requested) {
        WebIDL::reject_promise(realm, promise, JS::TypeError::create(realm, "No media types requested"sv));
        return;
    }
    ...
    if (!document.is_fully_active()) {
        WebIDL::reject_promise(realm, promise, WebIDL::InvalidStateError::create("Document is not fully active"_utf16));
        return;
    }
    ...
    if (video_requested) {
        WebIDL::reject_promise(realm, promise, WebIDL::NotSupportedError::create("Video capture is not supported"_utf16));
        return;
    }
