...
static void resolve_haptics_promise(WebIDL::Promise const& promise, GamepadHapticsResult result)
{
    ...
    WebIDL::resolve_promise(promise, result_string);
}
...
void GamepadHapticActuator::play_effect(GamepadHapticEffectType type, GamepadEffectParameters const& params, GC::Ref<WebIDL::Promise> promise)
{
    ...
    if (!is_valid_effect(type, params)) {
        WebIDL::reject_promise(promise, JS::TypeError::create(WebIDL::promise_realm(promise), "Invalid effect"sv));
        return;
    }
    ...
    if (!can_play_effect_with_type(type)) {
        WebIDL::reject_promise(promise, WebIDL::NotSupportedError::create("Gamepad does not support this effect"_utf16));
        return;
    }

    play_effect(type, params, create_queued_haptics_completion_steps(WebIDL::promise_realm(promise).global_object(), promise));
}

// https://w3c.github.io/gamepad/#dom-gamepadhapticactuator-reset
void GamepadHapticActuator::reset(GC::Ref<WebIDL::Promise> promise)
{
