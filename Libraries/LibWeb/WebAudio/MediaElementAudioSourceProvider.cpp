...
void MediaElementAudioSourceProvider::declare_discontinuity()
{
    ...
    AK::atomic_store(&local.header.end_of_stream, 0u, AK::MemoryOrder::memory_order_relaxed);
    ...
    if (should_log_media_element_bridge()) {
        ...
        if ((now_ms - last_ms) >= 250 && m_last_discontinuity_log_ms.compare_exchange_strong(last_ms, now_ms, AK::MemoryOrder::memory_order_relaxed)) {
            u32 const end_of_stream = AK::atomic_load(&local.header.end_of_stream, AK::MemoryOrder::memory_order_relaxed);
            ...
            WA_MEDIA_DBGLN("[WebAudio] media-tap discontinuity: cid={} session={} provider={} read={} write={} gen={} eos={} timeline_sr={}",
                debug_client_id(),
                debug_session_id(),
                m_provider_id,
                read_frame,
    ...
    if (m_stream_transport_producer_view.has_value()) {
        ...
        AK::atomic_store(&header.end_of_stream, 0u, AK::MemoryOrder::memory_order_relaxed);

        if (m_stream_transport_notify_write_fd >= 0)
            (void)Render::try_signal_event_fd(m_stream_transport_notify_write_fd);
    }
}
...
void MediaElementAudioSourceProvider::declare_end_of_stream()
{
    ...
    if (should_log_media_element_bridge()) {
        u32 const eos_before = AK::atomic_load(&local.header.end_of_stream, AK::MemoryOrder::memory_order_relaxed);
        bool const was_eos = eos_before != 0;
        if (!was_eos) {
            i64 now_ms = AK::MonotonicTime::now().milliseconds();
            ...
            if ((now_ms - last_ms) >= 250 && m_last_eos_log_ms.compare_exchange_strong(last_ms, now_ms, AK::MemoryOrder::memory_order_relaxed)) {
                ...
                u64 const generation = AK::atomic_load(&local.header.timeline_generation, AK::MemoryOrder::memory_order_relaxed);
                u32 const timeline_sample_rate = AK::atomic_load(&local.header.timeline_sample_rate, AK::MemoryOrder::memory_order_relaxed);
                WA_MEDIA_DBGLN("[WebAudio] media-tap declare-eos: cid={} session={} provider={} read={} write={} gen={} eos_before={} timeline_sr={}",
                    debug_client_id(),
                    debug_session_id(),
                    m_provider_id,
                    read_frame,
    ...
    AK::atomic_store(&local.header.end_of_stream, 1u, AK::MemoryOrder::memory_order_relaxed);
    ...
    if (m_stream_transport_producer_view.has_value()) {
        ...
        if (should_log_media_element_bridge()) {
            u32 const eos_before = AK::atomic_load(&header.end_of_stream, AK::MemoryOrder::memory_order_relaxed);
            bool const was_eos = eos_before != 0;
            if (!was_eos) {
                i64 now_ms = AK::MonotonicTime::now().milliseconds();
                i64 last_ms = m_last_eos_log_ms.load(AK::MemoryOrder::memory_order_relaxed);
                if ((now_ms - last_ms) >= 250 && m_last_eos_log_ms.compare_exchange_strong(last_ms, now_ms, AK::MemoryOrder::memory_order_relaxed)) {
                    ...
                    WA_MEDIA_DBGLN("[WebAudio] media-tap declare-eos (transport): cid={} session={} provider={} read={} write={} gen={} eos_before={} timeline_sr={}",
                        debug_client_id(),
                        debug_session_id(),
                        m_provider_id,
                        read_frame,
        ...
        AK::atomic_store(&header.end_of_stream, 1u, AK::MemoryOrder::memory_order_relaxed);
        if (m_stream_transport_notify_write_fd >= 0)
            (void)Render::try_signal_event_fd(m_stream_transport_notify_write_fd);
    }
}
...
bool MediaElementAudioSourceProvider::wait_for_frames(size_t min_frames, int timeout_ms)
{
    ...
    if (pfd.revents & POLLIN)
        Render::drain_event_fd(fd);

    auto peek_after = peek_with_timing();
    return peek_after.available_frames >= min_frames;
}

void MediaElementAudioSourceProvider::set_stream_transport_producer(Render::RingStreamView view, int notify_write_fd)
{
    ASSERT_CONTROL_THREAD();
    if (!view.header)
        return;
    ...
    LocalConsumerState& local = *m_consumer.get<LocalConsumerPtr>();
    RingHeader& local_header = local.header;
    RingHeader& transport_header = *view.header;
    ...
    AK::atomic_store(&transport_header.end_of_stream, AK::atomic_load(&local_header.end_of_stream, AK::MemoryOrder::memory_order_relaxed), AK::MemoryOrder::memory_order_relaxed);
...
MediaElementAudioSourceProvider::PeekResult MediaElementAudioSourceProvider::peek_with_timing()
{
    ...
    if (m_consumer.has<TransportConsumerState>()) {
        ...
        RingHeader& header = *view.header;
        result.end_of_stream = AK::atomic_load(&header.end_of_stream, AK::MemoryOrder::memory_order_relaxed) != 0;
    ...
    result.end_of_stream = AK::atomic_load(&local.header.end_of_stream, AK::MemoryOrder::memory_order_relaxed) != 0;
    return result;
}
...
void MediaElementAudioSourceProvider::push_interleaved(ReadonlySpan<float> interleaved_samples, u32 sample_rate, u32 channel_count)
{
    ...
    Render::RingStreamProducer local_producer(local_view);
    ...
    if (m_stream_transport_producer_view.has_value()) {
        ...
        Render::RingStreamProducer transport_producer(view);
...
void MediaElementAudioSourceProvider::push_interleaved(ReadonlySpan<float> interleaved_samples, u32 sample_rate, u32 channel_count, AK::Duration media_time)
{
    ...
    auto reset_stream_transport_timeline = [&](RingHeader& header, u64 start_media_frame, u64 stream_write_frame) {
    ...
    auto update_stream_transport_timeline_mapping = [&](RingHeader& header, u64 start_media_frame, u64 stream_write_frame) {
        AK::atomic_store(&header.timeline_media_start_frame, start_media_frame, AK::MemoryOrder::memory_order_relaxed);
        AK::atomic_store(&header.timeline_media_start_at_ring_frame, stream_write_frame, AK::MemoryOrder::memory_order_relaxed);
        AK::atomic_store(&header.timeline_sample_rate, sample_rate, AK::MemoryOrder::memory_order_relaxed);
    };
...
MediaElementAudioSourceProvider::PopResult MediaElementAudioSourceProvider::pop_planar_with_timing(Span<Span<f32>> out_channels, size_t requested_frames, u32 expected_channel_count)
{
    ...
    if (m_consumer.has<TransportConsumerState>()) {
        ...
        if (frames_to_read == 0) {
            if (should_log_media_element_bridge()) {
                ...
                if ((now_ms - last_ms) >= 250 && m_last_empty_log_ms.compare_exchange_strong(last_ms, now_ms, AK::MemoryOrder::memory_order_relaxed)) {
                    u32 const timeline_sample_rate = AK::atomic_load(&header.timeline_sample_rate, AK::MemoryOrder::memory_order_relaxed);
                    WA_MEDIA_DBGLN("[WebAudio] media-tap empty (remote): cid={} session={} provider={} want={} avail={} read={} write={} gen={} eos={} header_sr={} header_ch={} timeline_sr={}",
                        debug_client_id(),
                        debug_session_id(),
                        m_provider_id,
                        requested_frames,
        ...
        if (should_log_media_element_bridge() && frames_to_read < requested_frames) {
            ...
            if ((now_ms - last_ms) >= 250 && m_last_short_read_log_ms.compare_exchange_strong(last_ms, now_ms, AK::MemoryOrder::memory_order_relaxed)) {
                u32 const timeline_sample_rate = AK::atomic_load(&header.timeline_sample_rate, AK::MemoryOrder::memory_order_relaxed);
                WA_MEDIA_DBGLN("[WebAudio] media-tap short-read (remote): cid={} session={} provider={} need={} got={} avail={} read={} write={} gen={} eos={} header_sr={} header_ch={} timeline_sr={}",
                    debug_client_id(),
                    debug_session_id(),
                    m_provider_id,
                    requested_frames,
    ...
    Render::RingStreamView view { .header = &local.header, .interleaved_frames = { local.ring.data(), local.ring.size() } };
    RingHeader& header = local.header;
    ...
    if (frames_to_read == 0) {
        if (should_log_media_element_bridge()) {
            ...
            if ((now_ms - last_ms) >= 250 && m_last_empty_log_ms.compare_exchange_strong(last_ms, now_ms, AK::MemoryOrder::memory_order_relaxed)) {
                u32 const timeline_sample_rate = AK::atomic_load(&header.timeline_sample_rate, AK::MemoryOrder::memory_order_relaxed);
                WA_MEDIA_DBGLN("[WebAudio] media-tap empty (local): cid={} session={} provider={} want={} avail={} read={} write={} gen={} eos={} timeline_sr={}",
                    debug_client_id(),
                    debug_session_id(),
                    m_provider_id,
                    requested_frames,
    ...
    if (should_log_media_element_bridge() && frames_to_read < requested_frames) {
        ...
        if ((now_ms - last_ms) >= 250 && m_last_short_read_log_ms.compare_exchange_strong(last_ms, now_ms, AK::MemoryOrder::memory_order_relaxed)) {
            u32 const timeline_sample_rate = AK::atomic_load(&header.timeline_sample_rate, AK::MemoryOrder::memory_order_relaxed);
            WA_MEDIA_DBGLN("[WebAudio] media-tap short-read (local): cid={} session={} provider={} need={} got={} avail={} read={} write={} gen={} eos={} timeline_sr={}",
                debug_client_id(),
                debug_session_id(),
                m_provider_id,
                requested_frames,
