...
static Optional<float> sample_rate_from_cached_output_device(Variant<String, GC::Ref<AudioSinkInfo>> const& sink_id);
...
static bool document_allows_audiocontext_operations(DOM::Document const& document);
static void queue_statechange_event(AudioContext& context, Bindings::AudioContextState expected_state);
...
WebIDL::ExceptionOr<GC::Ref<AudioContext>> AudioContext::construct_impl(JS::Realm& realm, Optional<AudioContextOptions> const& context_options)
{
    ...
    if (!settings.responsible_document() || !document_allows_audiocontext_operations(*settings.responsible_document())) {
    ...
    if (context_options.has_value()) {
        ...
        // Otherwise, follow these substeps:
            ...
            if (auto sample_rate = sample_rate_from_cached_output_device(context->m_sink_id_at_construction); sample_rate.has_value())
                default_sample_rate = sample_rate.value();
    ...
    if (context->m_allowed_to_start) {
        u64 const generation = context->m_next_suspend_generation++;
        context->queue_control_message(ResumeContext { .generation = generation });
        context->set_rendering_state(Bindings::AudioContextState::Running);

        if (!context->m_audio_service_client_id.has_value() && !context->start_rendering_audio_graph()) {
            context->set_rendering_state(Bindings::AudioContextState::Suspended);
        } else {
            context->m_pending_render_thread_state_acks.append({
                .promise = {},
                .generation = generation,
                .suspended = false,
            });

            if (context->m_audio_service_client_id.has_value())
                EngineController::the().set_client_suspended(*context->m_audio_service_client_id, false, generation);

            context->ensure_render_thread_state_ack_timer_running();
        }
    }
...
WebIDL::ExceptionOr<GC::Ref<WebIDL::Promise>> AudioContext::set_sink_id(Variant<String, AudioSinkOptions> const& sink_id)
{
    ...
    queue_a_media_element_task(GC::create_function(heap(), [self = GC::Ref { *this }, promise, sink_id_copy]() mutable {
...
WebIDL::ExceptionOr<GC::Ref<WebIDL::Promise>> AudioContext::resume()
{
    ...
    if (!document_allows_audiocontext_operations(associated_document))
        return WebIDL::InvalidStateError::create(realm, "Document is not fully active"_utf16);
    ...
    u64 const generation = m_next_suspend_generation++;
    ...
    if (!m_audio_service_client_id.has_value() && !start_rendering_audio_graph()) {
        ...
        queue_a_media_element_task(GC::create_function(heap(), [self = GC::Ref { *this }]() {
            auto& callback_realm = self->realm();
            HTML::TemporaryExecutionContext context(callback_realm, HTML::TemporaryExecutionContext::CallbacksEnabled::Yes);

            // 7.4.1: Reject all promises from [[pending resume promises]] in order, then clear [[pending resume promises]].
            for (auto const& pending_promise : self->m_pending_resume_promises) {
                if (!self->take_pending_promise(pending_promise))
                    continue;
                WebIDL::reject_promise(callback_realm, pending_promise, JS::js_null());
            }
...
WebIDL::ExceptionOr<GC::Ref<WebIDL::Promise>> AudioContext::suspend()
{
    ...
    // The promise resolves when the frame buffer is empty (has been handed off to
    // the hardware), or immediately (with no other effect) if the context is
    // already suspended.
    //
    // A control-thread state of "suspended" is not sufficient on its own here:
    // the rendering thread can still be transitioning to running (for example,
    // due to the test-mode auto-resume during construction). In that case an
    // explicit suspend() must still enqueue a suspend request instead of being
    // treated as a no-op.
    bool const backend_is_suspended = !m_audio_service_client_id.has_value()
        || render_thread_suspended_atomic().load(AK::MemoryOrder::memory_order_acquire) != 0;
    if (state() == Bindings::AudioContextState::Suspended && m_pending_render_thread_state_acks.is_empty() && backend_is_suspended) {
        WebIDL::resolve_promise(realm, promise, JS::js_undefined());
        return promise;
    }
    ...
    if (!m_audio_service_client_id.has_value()) {
        queue_a_media_element_task(GC::create_function(heap(), [promise, self = GC::Ref { *this }]() {
            ...
            WebIDL::resolve_promise(callback_realm, promise, JS::js_undefined());
            queue_statechange_event(*self, Bindings::AudioContextState::Suspended);
        }));
        return promise;
    }
...
WebIDL::ExceptionOr<GC::Ref<WebIDL::Promise>> AudioContext::close()
{
    ...
    // 2. If the [[control thread state]] flag on the AudioContext is closed, reject the promise with InvalidStateError.
    if (state() == Bindings::AudioContextState::Closed)
        return WebIDL::create_rejected_promise(realm, WebIDL::InvalidStateError::create(realm, "Audio context is already closed."_utf16));
    ...
    queue_a_media_element_task(GC::create_function(heap(), [promise, self = GC::Ref { *this }]() {
        ...
        WebIDL::resolve_promise(realm, promise, JS::js_undefined());
        queue_statechange_event(*self, Bindings::AudioContextState::Closed);
    }));
...
void AudioContext::process_render_thread_state_acks()
{
    ...
    bool const is_suspended = render_thread_suspended_atomic().load(AK::MemoryOrder::memory_order_acquire) != 0;
    u64 const generation = render_thread_suspend_generation_atomic().load(AK::MemoryOrder::memory_order_acquire);
    ...
    while (!m_pending_render_thread_state_acks.is_empty()) {
        ...
        bool const target_state_matches = pending.suspended == is_suspended;
        bool const was_superseded = generation > pending.generation;

        if (!target_state_matches && !was_superseded)
            break;
        ...
        queue_a_media_element_task(GC::create_function(heap(), [promise, self, target_suspended]() {
            ...
            if (!target_suspended) {
                for (auto const& pending_resume_promise : self->m_pending_resume_promises) {
                    ...
                    if (promise.has_value() && pending_resume_promise == *promise)
            ...
            if (!promise_resolved && promise.has_value()) {
                if (!self->take_pending_promise(*promise))
                    return;
                WebIDL::resolve_promise(realm, *promise, JS::js_undefined());
            }
            ...
            if (promise.has_value()) {
                queue_statechange_event(*self, desired_state);
                return;
            }
...
void AudioContext::snapshot_render_graph_and_prepare_resources(Render::GraphResources& resources, Render::GraphDescription& out_graph_description)
{
    ...
    resources.script_processor_host = m_script_processor_host.ptr();

    resources.script_processor_transport_metadata.clear();
    for (auto const& it : out_graph_description.nodes) {
        ...
        resources.script_processor_transport_metadata.set(node_id, {
...
{
    if (m_output_device_open_in_progress)
        return;
    ...
    if (!m_audio_service_client_id.has_value()) {
        ...
        m_audio_service_client_id = EngineController::the().register_client(*this, control_message_queue(), associated_task_queue(), current_frame_atomic(), render_thread_suspended_atomic(), render_thread_suspend_generation_atomic(), underrun_frames_total_atomic());

        m_output_device_open_in_progress = true;
        u64 const client_id = *m_audio_service_client_id;
        auto self = GC::Ref { *this };
        auto open_result = EngineController::the().open_webaudio_session(client_id, m_target_latency_ms, page_id, [self, client_id](ErrorOr<EngineController::DeviceFormat> const& device_format_or_error) {
            ASSERT_CONTROL_THREAD();

            if (!self->m_audio_service_client_id.has_value() || *self->m_audio_service_client_id != client_id)
                return;

            self->m_output_device_open_in_progress = false;
            if (device_format_or_error.is_error()) {
                ...
                EngineController::the().unregister_client(client_id);
                self->m_audio_service_client_id.clear();
                return;
            }

            self->m_base_latency = static_cast<double>(self->m_target_latency_ms) / 1000.0;

            self->on_audio_graph_changed();
        });

        if (open_result.is_error()) {
            m_output_device_open_in_progress = false;
            if (should_log_webaudio(LOG_GENERAL))
                WA_DBGLN("[WebAudio] Failed to start async output open for worklet graph: {}", open_result.error());
    ...
    auto resources = make<Render::GraphResources>();
    ...
    for (auto const& it : graph_description.nodes) {
        ...
        worklet_nodes.append(Render::WorkletNodeDefinition {
            ...
            .channel_count_mode = static_cast<u8>(aw.channel_count_mode),
            .channel_interpretation = static_cast<u8>(aw.channel_interpretation),
    ...
    auto global_handle = worklet->take_realtime_global_port_handle();
    worklet_port_bindings.ensure_capacity(worklet_node_ids.size() + (global_handle.has_value() ? 1 : 0));

    if (global_handle.has_value()) {
        worklet_port_bindings.unchecked_append(Render::WorkletPortBinding {
            .node_id = NodeID { 0 },
            .processor_port_handle = move(*global_handle),
        });
    }

    for (auto node_id : worklet_node_ids) {
        auto handle = worklet->take_realtime_processor_port_handle(node_id);
        if (!handle.has_value())
            continue;
        worklet_port_bindings.unchecked_append(Render::WorkletPortBinding {
            .node_id = node_id,
            .processor_port_handle = move(*handle),
        });
    }

    auto encoded_graph = encoded_or_error.release_value();
    EngineController::the().update_client_render_graph(*m_audio_service_client_id, sample_rate(), move(encoded_graph.encoded_graph), move(encoded_graph.shared_audio_buffers), move(resources), move(worklet_modules), move(worklet_nodes), move(worklet_port_bindings));
}
...
static Optional<float> sample_rate_from_cached_output_device(Variant<String, GC::Ref<AudioSinkInfo>> const& sink_id)
{
    ...
    Media::AudioDeviceInfo const* selected_device = nullptr;

    if (sink_id.has<String>() && !sink_id.get<String>().is_empty()) {
        auto const& requested_sink_id = sink_id.get<String>();
        for (auto const& device : output_devices) {
            if (String::from_utf8_with_replacement_character(device.dom_device_id.view()) != requested_sink_id)
                continue;
    ...
    if (!selected_device || selected_device->sample_rate_hz == 0)
        return {};

    return static_cast<float>(selected_device->sample_rate_hz);
}
...
static bool document_allows_audiocontext_operations(DOM::Document const& document)
{
    ...
    if (!navigable)
        return false;
    return !navigable->has_been_destroyed();
}
