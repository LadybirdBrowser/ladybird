...
bool AudioWorklet::is_processor_registered(String const& name)
{
    ...
    return global_scope.parameter_descriptors(name) != nullptr;
}
...
void AudioWorklet::register_processor_from_worker(String const& name, Vector<AudioParamDescriptor> const& descriptors)
{
    auto& worklet_settings_object = ensure_worklet_environment_settings_object();
    auto& global_scope = as<AudioWorkletGlobalScope>(worklet_settings_object.global_object());
    if (global_scope.is_processor_registration_failed(name))
        return;
    if (global_scope.parameter_descriptors(name))
        return;
...
Vector<Render::WorkletModule> AudioWorklet::loaded_modules() const
{
    ...
    for (auto const& it : m_loaded_module_sources)
        modules.unchecked_append(Render::WorkletModule { .url = it.key.to_byte_string(), .source_text = it.value });
    return modules;
}
...
// https://webaudio.github.io/web-audio-api/#dom-audioworklet-addmodule
GC::Ref<WebIDL::Promise> AudioWorklet::add_module(String const& module_url)
{
    ...
    auto on_complete = HTML::create_on_fetch_script_complete(vm.heap(), [this, promise, outside_global, worklet_realm, resolved_url](GC::Ptr<HTML::Script> result) mutable {
        ...
        // Cache the fetched module source so the realtime renderer can mirror it.
        if (!module_script.source_text().is_empty())
            m_loaded_module_sources.set(resolved_url, module_script.source_text());

        // For realtime AudioContext, ensure the AudioWorkletGlobalScope's shared port is bound in the
        // renderer before the module's top-level code runs (some tests install handlers immediately).
        if (!is<OfflineAudioContext>(*self->m_context) && self->m_realtime_global_port_fd < 0)
            (void)self->port();

        // Ensure the renderer sees newly loaded modules (and the global port binding, if any).
        if (!is<OfflineAudioContext>(*self->m_context))
