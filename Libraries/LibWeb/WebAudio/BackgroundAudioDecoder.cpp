...
static void decode_audio_data_async(GC::Weak<DOM::Document> document, u64 request_id, ByteBuffer encoded_audio_data, Optional<u32> target_sample_rate)
{
    ...
    auto on_decode_complete = AK::Function<void(DecodeResult)>(
        [document = move(document), request_id](DecodeResult decoded_audio_data) mutable -> void {
...
GC::Ref<WebIDL::Promise> BackgroundAudioDecoder::decode_audio_data(BaseAudioContext& context, GC::Root<JS::ArrayBuffer> const& audio_data, GC::Ptr<WebIDL::CallbackType> success_callback, GC::Ptr<WebIDL::CallbackType> error_callback)
{
    ...
    auto encoded_or_error = WebIDL::get_buffer_source_copy(*audio_data);
