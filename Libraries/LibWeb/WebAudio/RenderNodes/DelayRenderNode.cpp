...
void DelayRenderNode::process(RenderContext& context, Vector<Vector<AudioBus const*>> const& inputs, Vector<Vector<AudioBus const*>> const& param_inputs)
{
    ...
    // Determine a stable output channel count for this quantum.
    // For WPT compatibility, when the delay reads from the unfilled portion of its history buffer,
    // the node should output a single channel of silence.
    f32 delay_seconds_for_shape = has_delay_time_param_input ? delay_time_in[0] : m_delay_time_seconds;
    if (!__builtin_isfinite(delay_seconds_for_shape) || __builtin_isnan(delay_seconds_for_shape))
        delay_seconds_for_shape = 0.0f;
    delay_seconds_for_shape = clamp(delay_seconds_for_shape, 0.0f, m_max_delay_time_seconds);

    f64 const delay_frames_for_shape = static_cast<f64>(delay_seconds_for_shape) * static_cast<f64>(context.sample_rate);

    bool const reading_from_unfilled_history = delay_frames_for_shape > static_cast<f64>(m_frames_written);
    size_t const output_channels_this_quantum = reading_from_unfilled_history ? 1 : mixed_input_channels;
