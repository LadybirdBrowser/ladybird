...
struct GraphNodeBuildContext {
    ...
    Render::GraphResources* resources_out;
};
    ...
    Render::GraphResources* resources_out)
{
    ...
    GraphNodeBuildContext node_context { graph, context_sample_rate, analyser_nodes_out, audio_worklet_nodes_out, script_processor_nodes_out, resources_out };
...
{
    NodeID const node_id = node->node_id();
    Render::GraphResources* resources_out = context.resources_out;
    ...
    if (is<AudioDestinationNode>(*node)) {
        ...
        if (resources_out)
            resources_out->media_element_audio_sources.set(provider->provider_id(), move(provider));
        ...
        if (resources_out) {
            ...
            if (track->ready_state() != Bindings::MediaStreamTrackState::Ended) {
                auto device_handle = track->device_handle();
                if (device_handle.has_value()) {
                    Render::AudioInputStreamMetadata metadata;
                    metadata.device_handle = device_handle.value();
                    metadata.sample_rate_hz = track->sample_rate_hz();
                    metadata.channel_count = track->channel_count();
                    metadata.capacity_frames = 0;
                    resources_out->media_stream_audio_source_metadata.set(source_node.provider_id(), metadata);
                }
            }
        }
        ...
        MediaStreamAudioDestinationNode const& destination_node = static_cast<MediaStreamAudioDestinationNode const&>(*node);

        MediaStreamAudioDestinationGraphNode dest_desc;
        dest_desc.provider_id = destination_node.provider_id();
        node_description = dest_desc;
    } else if (is<ConstantSourceNode>(*node)) {
...
static Render::GraphNodeDescription build_audio_buffer_source_node_description(AudioBufferSourceNode const& buffer_source, GraphNodeBuildContext& context)
{
    ...
    if (auto buffer = buffer_source.buffer()) {
        ...
        if (resources_out) {
            Render::ResourceID const buffer_id = buffer->render_resource_id();
            if (!resources_out->audio_buffers.contains(buffer_id)) {
                ...
                resources_out->audio_buffers.set(buffer_id, move(shared));
            }
