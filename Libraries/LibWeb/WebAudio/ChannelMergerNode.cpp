...
ChannelMergerNode::ChannelMergerNode(GC::Ref<BaseAudioContext> context, ChannelMergerOptions const& options)
    : AudioNode(context)
...
WebIDL::ExceptionOr<GC::Ref<ChannelMergerNode>> ChannelMergerNode::create(GC::Ref<BaseAudioContext> context, ChannelMergerOptions const& options)
{
...
WebIDL::ExceptionOr<void> ChannelMergerNode::validate_options(ChannelMergerOptions const& options)
{
...
WebIDL::ExceptionOr<GC::Ref<ChannelMergerNode>> ChannelMergerNode::create_for_constructor(GC::Ref<BaseAudioContext> context, ChannelMergerOptions const& options)
{
...
WebIDL::ExceptionOr<void> ChannelMergerNode::set_channel_count(WebIDL::UnsignedLong channel_count)
{
    ...
    if (channel_count != 1)
        return WebIDL::InvalidStateError::create("Channel count cannot be changed"_utf16);

    return Base::set_channel_count(channel_count);
}

WebIDL::ExceptionOr<void> ChannelMergerNode::set_channel_count_mode(ChannelCountMode channel_count_mode)
{
    ...
    if (channel_count_mode != ChannelCountMode::Explicit)
        return WebIDL::InvalidStateError::create("Channel count mode cannot be changed"_utf16);
