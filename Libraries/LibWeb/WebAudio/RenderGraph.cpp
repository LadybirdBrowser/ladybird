...
RenderGraph::RenderGraph(GraphDescription const& description, f32 sample_rate, GraphResourceResolver const* resources)
    : m_sample_rate(sample_rate)
    , m_resources(resources)
{
    m_impl = new GraphExecutor(description, sample_rate, resources);
}
...
void RenderGraph::enqueue_full_rebuild(GraphDescription const& description)
{
    ...
    auto* new_impl = new GraphExecutor(description, m_sample_rate, m_resources);
...
GraphUpdateKind RenderGraph::enqueue_update(GraphDescription const& description)
{
    ...
    WA_GR_DBGLN("[WebAudio] enqueue_update: kind={} nodes={} conns={} param_conns={} param_autos={} dest_id={}",
    ...
    if (classification == GraphUpdateKind::None) {
        ...
        if (auto* pending = m_pending_impl.exchange(nullptr, AK::MemoryOrder::memory_order_acq_rel)) {
            WA_GR_DBGLN("[WebAudio] enqueue_update: canceled stale pending rebuild (classification=None)");
            delete pending;
        }
    ...
    if (classification == GraphUpdateKind::Parameter) {
        if (enqueue_parameter_update(description)) {
            if (auto* pending = m_pending_impl.exchange(nullptr, AK::MemoryOrder::memory_order_acq_rel)) {
                WA_GR_DBGLN("[WebAudio] enqueue_update: canceled stale pending rebuild (applied ParameterUpdate)");
                delete pending;
            }
            WA_GR_DBGLN("[WebAudio] enqueue_update: applied ParameterUpdate");
            return classification;
        }
    } else if (classification == GraphUpdateKind::Topology) {
        if (enqueue_topology_update(description)) {
            if (auto* pending = m_pending_impl.exchange(nullptr, AK::MemoryOrder::memory_order_acq_rel)) {
                WA_GR_DBGLN("[WebAudio] enqueue_update: canceled stale pending rebuild (applied TopologyUpdate)");
                delete pending;
            }
            WA_GR_DBGLN("[WebAudio] enqueue_update: applied TopologyUpdate");
            return classification;
        }
    }

    WA_GR_DBGLN("[WebAudio] enqueue_update: fell back to full rebuild");
...
void RenderGraph::try_commit_pending_update()
{
    ...
    if (free_slot_index == retired_slot_count) {
        ...
        if ((now_ms - last_ms) > 1000 && s_last_log_ms.compare_exchange_strong(last_ms, now_ms, AK::MemoryOrder::memory_order_relaxed))
            WA_GR_DBGLN("[WebAudio] commit stalled: rebuild retired slots full");
        return;
    }
    ...
    WA_GR_DBGLN("[WebAudio] committed full rebuild: generation={} retired_slot={}", new_generation, free_slot_index);
}
