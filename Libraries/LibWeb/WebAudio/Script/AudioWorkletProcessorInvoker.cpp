...
#include <LibWeb/WebAudio/Script/AudioWorkletProcessorInvoker.h>
...
JS::ThrowCompletionOr<bool> invoke_audio_worklet_processor_process(
    JS::Realm& worklet_realm, JS::Object& processor_instance, Vector<Vector<AudioBus const*>> const& inputs,
    Span<AudioBus*> outputs, Span<AudioWorkletProcessorHost::ParameterSpan const> parameters,
    size_t quantum_size)
{
    ...
    for (size_t input_index = 0; input_index < inputs.size(); ++input_index) {
        ...
        TRY(inputs_array->create_data_property_or_throw(JS::PropertyKey { static_cast<u32>(input_index) },
            channels_array));
    }
    ...
    for (size_t output_index = 0; output_index < outputs.size(); ++output_index) {
        ...
        TRY(outputs_array->create_data_property_or_throw(JS::PropertyKey { static_cast<u32>(output_index) },
            channels_array));
    ...
    for (auto const& param : parameters) {
        ...
        TRY(parameters_object->set(JS::PropertyKey { Utf16String::from_utf8(param.name) }, values,
            JS::Object::ShouldThrowExceptions::Yes));
    }
    TRY(parameters_object->set_integrity_level(JS::Object::IntegrityLevel::Frozen));

    auto result_value = TRY(
        JS::call(vm, process_function, &processor_instance, inputs_array, outputs_array, parameters_object));
