...
#include <LibWeb/WebAudio/Script/OfflineAudioWorkletProcessorHost.h>
#include <LibWebAudio/Debug.h>

namespace Web::WebAudio::Render {

OfflineAudioWorkletProcessorHost::OfflineAudioWorkletProcessorHost(
    GC::Ref<JS::Realm> worklet_realm, HashMap<NodeID, GC::Root<JS::Object>>& processor_instances)
    : m_worklet_realm(worklet_realm)
    , m_processor_instances(processor_instances)
{
}

bool OfflineAudioWorkletProcessorHost::process_audio_worklet(
    NodeID node_id, RenderContext& process_context, String const&, size_t, size_t, Span<size_t const>,
    ...
    Span<AudioWorkletProcessorHost::ParameterSpan const> parameters)
{
    ...
    HTML::TemporaryExecutionContext execution_context(*m_worklet_realm,
        HTML::TemporaryExecutionContext::CallbacksEnabled::Yes);
    auto result = invoke_audio_worklet_processor_process(*m_worklet_realm, *processor, inputs, outputs,
        parameters, process_context.quantum_size);
    if (result.is_error()) {
...
bool OfflineAudioWorkletProcessorHost::has_errors() const { return !m_errors.is_empty(); }
