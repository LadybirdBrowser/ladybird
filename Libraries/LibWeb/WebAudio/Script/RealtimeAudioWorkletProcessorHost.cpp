...
#include <LibWeb/WebAudio/Script/AudioWorkletPageClient.h>
...
static Vector<AudioParamDescriptor>
to_wire_audio_param_descriptors(Vector<Web::WebAudio::AudioParamDescriptor> const& descriptors)
{
    Vector<AudioParamDescriptor> wire_descriptors;
    wire_descriptors.ensure_capacity(descriptors.size());
    for (auto const& descriptor : descriptors) {
        wire_descriptors.unchecked_append(AudioParamDescriptor {
            .name = descriptor.name.to_string(),
            .default_value = descriptor.default_value,
            .min_value = descriptor.min_value,
            .max_value = descriptor.max_value,
            .is_k_rate = descriptor.automation_rate == Bindings::AutomationRate::KRate,
        });
    }
    return wire_descriptors;
}

static bool processor_instance_defines_process(JS::Object& instance, JS::FunctionObject& constructor,
    JS::VM& vm)
{
...
RealtimeAudioWorkletProcessorHost::RealtimeAudioWorkletProcessorHost(
    u64 initial_current_frame, float initial_sample_rate, Vector<WorkletModule> modules,
    Vector<WorkletNodeDefinition> node_definitions, Vector<WorkletPortBinding> port_bindings)
    : m_modules(move(modules))
    , m_initial_current_frame(initial_current_frame)
    , m_initial_sample_rate(initial_sample_rate)
{
    m_processor_port_handles.ensure_capacity(port_bindings.size());
    for (auto& binding : port_bindings) {
        m_processor_port_handles.set(binding.node_id, move(binding.processor_port_handle));
    }
    ...
    for (auto& def : node_definitions) {
        ...
        node->node_id = NodeID { def.node_id };
        ...
        node->channel_count_mode = static_cast<ChannelCountMode>(def.channel_count_mode);
        node->channel_interpretation = static_cast<ChannelInterpretation>(def.channel_interpretation);
...
void RealtimeAudioWorkletProcessorHost::synchronize_node_definitions(
    Vector<WorkletNodeDefinition> const& definitions)
{
    ...
    for (auto const& def : definitions) {
        auto node_id = NodeID { def.node_id };
        auto it = m_nodes_by_id.find(node_id);
        ...
        existing->channel_count_mode = static_cast<ChannelCountMode>(def.channel_count_mode);
        existing->channel_interpretation = static_cast<ChannelInterpretation>(def.channel_interpretation);
...
void RealtimeAudioWorkletProcessorHost::enqueue_port_bindings(Vector<WorkletPortBinding> port_bindings)
{
    {
        Threading::MutexLocker locker(m_update_mutex);
        m_pending_port_bindings.ensure_capacity(m_pending_port_bindings.size() + port_bindings.size());
        for (auto& binding : port_bindings) {
            m_pending_port_bindings.append(move(binding));
        }
    }
}
...
void RealtimeAudioWorkletProcessorHost::set_processor_registration_callback(
    Function<void(String const&, Vector<AudioParamDescriptor> const&, u64)> callback)
{
    Threading::MutexLocker locker(m_callback_mutex);
    m_processor_registration_callback = move(callback);
}

void RealtimeAudioWorkletProcessorHost::set_worklet_module_evaluation_callback(
...
void RealtimeAudioWorkletProcessorHost::notify_processor_registered(
    String const& name, Vector<AudioParamDescriptor> const& descriptors)
{
...
void RealtimeAudioWorkletProcessorHost::notify_module_evaluated(
    ...
    String const& error_message, Vector<String> failed_processor_registrations)
{
    ...
    if (m_module_evaluation_callback)
        m_module_evaluation_callback(module_id, required_generation, success, error_name, error_message,
...
void RealtimeAudioWorkletProcessorHost::consume_pending_updates(
    Vector<WorkletModule>& out_new_modules, Vector<WorkletNodeDefinition>& out_node_definitions,
...
struct RealtimeAudioWorkletProcessorHost::RenderThreadState {
    ...
    RefPtr<JS::VM> vm;
    GC::Ptr<HTML::EventLoop> html_event_loop;
    Optional<u64> last_processed_frame;
    u64 latest_current_frame { 0 };
    float latest_sample_rate { 44100.0f };
...
bool RealtimeAudioWorkletProcessorHost::has_pending_worklet_tasks(RenderThreadState& state)
{
    ...
    if (state.html_event_loop->task_queue().has_runnable_tasks())
        return true;
    if (!state.html_event_loop->microtask_queue_empty())
        return true;
    ...
    if (agent->event_loop->task_queue().has_runnable_tasks())
        return true;
    if (!agent->event_loop->microtask_queue_empty())
        return true;

    return false;
}

RealtimeAudioWorkletProcessorHost::RenderThreadState&
RealtimeAudioWorkletProcessorHost::ensure_render_thread_state()
{
    ASSERT_RENDER_THREAD();
    ...
    for (auto const& it : m_processor_port_handles)
...
bool RealtimeAudioWorkletProcessorHost::process_audio_worklet(
    NodeID node_id, RenderContext& process_context, [[maybe_unused]] String const& processor_name,
    [[maybe_unused]] size_t number_of_inputs, [[maybe_unused]] size_t number_of_outputs,
    [[maybe_unused]] Span<size_t const> output_channel_count, Vector<Vector<AudioBus const*>> const& inputs,
    Span<AudioBus*> outputs, Span<ParameterSpan const> parameters)
{
    ASSERT_RENDER_THREAD();
    ...
    state.latest_current_frame = process_context.current_frame;
    state.latest_sample_rate = process_context.sample_rate;
    ...
    auto process_execution_context = JS::ExecutionContext::create(0, ReadonlySpan<JS::Value> {}, 0);
    process_execution_context->realm = state.realm;
    state.vm->push_execution_context(*process_execution_context);

    auto keep_alive_or_error = invoke_audio_worklet_processor_process(*state.realm, *processor, inputs, outputs,
...
RealtimeAudioWorkletProcessorHost::RenderThreadState
RealtimeAudioWorkletProcessorHost::create_render_thread_state()
{
    ASSERT_RENDER_THREAD();
    ...
    worklet_vm.host_enqueue_promise_job =
        [&worklet_vm, event_loop](GC::Ref<GC::Function<JS::ThrowCompletionOr<JS::Value>()>> job,
            JS::Realm* realm) {
            VERIFY(realm);
            JS::ScriptOrModule script_or_module = worklet_vm.get_active_script_or_module();
            auto& heap = realm->heap();
            HTML::queue_a_task(HTML::Task::Source::Microtask, event_loop, nullptr,
                GC::create_function(heap, [realm, job, script_or_module = move(script_or_module)] {
                    HTML::TemporaryExecutionContext temporary_context(
                        *realm, HTML::TemporaryExecutionContext::CallbacksEnabled::Yes);
                    auto& realm_execution_context = HTML::principal_realm_settings_object(*realm).realm_execution_context();
                    JS::ScriptOrModule previous_script_or_module = realm_execution_context.script_or_module;
                    ScopeGuard restore_script_or_module = [&] {
                        realm_execution_context.script_or_module = move(previous_script_or_module);
                    };
                    realm_execution_context.script_or_module = move(script_or_module);
    ...
    auto realm_execution_context = Bindings::create_a_new_javascript_realm(
        *vm,
        ...
        [&](JS::Realm&) -> JS::Object* { return global_scope.ptr(); });
    ...
    HTML::SerializedEnvironmentSettingsObject outside_settings {
        ...
        .time_origin = 0,
        .global = HTML::SerializedWorkerGlobalScope {},
    };

    (void)AudioWorkletEnvironmentSettingsObject::setup(page_client->page_ref(), move(realm_execution_context),
        *global_scope, outside_settings, global_scope_url);
    ...
    global_scope->set_processor_registration_callback(
        [this](String const& name, Vector<Web::WebAudio::AudioParamDescriptor> const& descriptors) {
            notify_processor_registered(name, to_wire_audio_param_descriptors(descriptors));
        });

    return RenderThreadState {
        .core_event_loop = s_core_event_loop.ptr(),
        .vm = move(vm),
        .html_event_loop = event_loop,
        .last_processed_frame = {},
        .latest_current_frame = m_initial_current_frame,
        .latest_sample_rate = m_initial_sample_rate,
        .last_pump_time = {},
...
void RealtimeAudioWorkletProcessorHost::service_render_thread_state(u64 current_frame, float sample_rate)
{
    ...
    state.latest_current_frame = current_frame;
    state.latest_sample_rate = sample_rate;
...
void RealtimeAudioWorkletProcessorHost::evaluate_modules(RenderThreadState& state,
    Vector<WorkletModule> const& modules)
{
    ASSERT_RENDER_THREAD();
    ...
    for (auto const& module : modules) {
        if (evaluated_urls.contains(module.url)) {
            if (auto* self = weak_this.ptr()) {
                ...
                if (auto it = self->m_failed_processor_registrations_by_url.find(module.url);
                    it != self->m_failed_processor_registrations_by_url.end())
                    failed_registrations = it->value;
                self->notify_module_evaluated(
                    module.module_id,
                    self->m_processor_registration_generation.load(AK::MemoryOrder::memory_order_relaxed), true, {},
                    {}, move(failed_registrations));
            }
            continue;
        }
        ...
        if (parse_result.is_error()) {
            if (auto* self = weak_this.ptr())
                self->notify_module_evaluated(
                    module.module_id,
                    self->m_processor_registration_generation.load(AK::MemoryOrder::memory_order_relaxed), false,
                    "OperationError"_string, "AudioWorklet module parse error"_string, {});
            continue;
        }

        auto record = parse_result.release_value();
        auto module_execution_context = JS::ExecutionContext::create(0, ReadonlySpan<JS::Value> {}, 0);
        ...
        if (link_or_error.is_error()) {
            ...
            if (auto* self = weak_this.ptr())
                self->notify_module_evaluated(
                    module.module_id,
                    self->m_processor_registration_generation.load(AK::MemoryOrder::memory_order_relaxed), false,
                    "OperationError"_string, "AudioWorklet module link error"_string, {});
            continue;
        }
        ...
        if (promise_or_error.is_error()) {
            ...
            if (auto* self = weak_this.ptr())
                self->notify_module_evaluated(
                    module.module_id,
                    self->m_processor_registration_generation.load(AK::MemoryOrder::memory_order_relaxed), false,
                    "OperationError"_string, "AudioWorklet module evaluation failed"_string, {});
            continue;
        }
        ...
        auto on_fulfilled_steps = [weak_this, module_id, module_url,
                                      state_ptr](JS::VM&) -> JS::ThrowCompletionOr<JS::Value> {
            if (auto* self = weak_this.ptr()) {
        ...
        auto on_rejected_steps = [weak_this, module_id, module_url,
                                     state_ptr](JS::VM& vm) -> JS::ThrowCompletionOr<JS::Value> {
            ...
            if (auto* self = weak_this.ptr()) {
                ...
                self->notify_module_evaluated(module_id, stable_generation, false, name, message,
                    move(failed_registrations));
            }
            return JS::js_undefined();
        };

        auto on_fulfilled = JS::NativeFunction::create(*state.realm, move(on_fulfilled_steps), 1,
            Utf16FlyString {}, state.realm.ptr());
        auto on_rejected = JS::NativeFunction::create(*state.realm, move(on_rejected_steps), 1, Utf16FlyString {},
            state.realm.ptr());
        as<JS::Promise>(evaluation_promise->promise().ptr())->perform_then(on_fulfilled, on_rejected, nullptr);
...
void RealtimeAudioWorkletProcessorHost::ensure_node_exists(RenderThreadState& state,
    WorkletNodeDefinition const& def)
{
    ASSERT_RENDER_THREAD();
    ...
    if (existing && processor_changed) {
        auto node_id = NodeID { def.node_id };
        state.instances.remove(node_id);
        state.instance_processor_names.remove(node_id);
        state.instance_has_process.remove(node_id);
    }

    auto node_id = NodeID { def.node_id };
    if (!state.processor_ports.contains(node_id)) {
        auto port = HTML::MessagePort::create(*state.realm);
        port->set_task_source(HTML::Task::Source::AudioWorklet);
        port->enable();
        state.processor_ports.set(node_id, GC::make_root(port));
    }
}
...
void RealtimeAudioWorkletProcessorHost::try_attach_port_transport(RenderThreadState& state, NodeID node_id)
{
    ...
    auto handle_it = m_processor_port_handles.find(node_id);
    if (handle_it == m_processor_port_handles.end())
        return;

    if (node_id == NodeID { 0 }) {
        MUST(attach_message_port_transport(*state.shared_port, move(handle_it->value)));
        state.ports_with_transport.set(node_id);
        m_processor_port_handles.remove(handle_it);
        return;
    }
    ...
    MUST(attach_message_port_transport(*port_it->value, move(handle_it->value)));
    state.ports_with_transport.set(node_id);
    m_processor_port_handles.remove(handle_it);
}
...
void RealtimeAudioWorkletProcessorHost::ensure_ready_processor_instances(RenderThreadState& state)
{
    ...
    for (auto* node : nodes_to_check) {
        ...
        auto processor_ctor = state.global_scope->processor_constructor(
            String::from_byte_string(node->processor_name).release_value_but_fixme_should_propagate_errors());
        if (!processor_ctor.is_function())
            continue;

        state.global_scope->set_current_frame(state.latest_current_frame);
        state.global_scope->set_sample_rate(state.latest_sample_rate);
        ...
        MUST(node_options_object->create_data_property_or_throw(
            JS::PropertyKey { "numberOfInputs"_utf16_fly_string }, JS::Value(node->number_of_inputs)));
        MUST(node_options_object->create_data_property_or_throw(
            JS::PropertyKey { "numberOfOutputs"_utf16_fly_string }, JS::Value(node->number_of_outputs)));
        ...
        if (node->output_channel_count.has_value()) {
            ...
            for (size_t i = 0; i < node->output_channel_count->size(); ++i)
                MUST(output_channel_count_array->create_data_property_or_throw(
                    JS::PropertyKey { static_cast<u32>(i) }, JS::Value((*node->output_channel_count)[i])));
            MUST(node_options_object->create_data_property_or_throw(
                JS::PropertyKey { "outputChannelCount"_utf16_fly_string }, output_channel_count_array));
        }
        ...
        if (node->parameter_data.has_value()) {
            ...
            for (auto const& entry : node->parameter_data.value())
                MUST(parameter_data_object->create_data_property_or_throw(
                    JS::PropertyKey { Utf16String::from_utf8(entry.name) }, JS::Value(entry.value)));
            MUST(node_options_object->create_data_property_or_throw(
                JS::PropertyKey { "parameterData"_utf16_fly_string }, parameter_data_object));
        }
        ...
        if (node->serialized_processor_options.has_value()) {
            ...
            if (!processor_options_value_or_error.is_error())
                MUST(node_options_object->create_data_property_or_throw(
                    JS::PropertyKey { "processorOptions"_utf16_fly_string },
                    processor_options_value_or_error.release_value()));
        }

        auto ctor_execution_context = JS::ExecutionContext::create(0, ReadonlySpan<JS::Value> {}, 0);
        ...
        if (should_log_webaudio())
            dbgln("[WebAudio] AudioWorklet: node {} has_process={} (preconstructed)", node->node_id.value(),
                has_process);
    }
}
    ...
    for (auto& module : new_modules) {
        ...
        if (already_have) {
            ...
            if (!matched_url.is_empty()) {
                if (auto it = m_failed_processor_registrations_by_url.find(matched_url);
                    it != m_failed_processor_registrations_by_url.end())
                    failed_registrations = it->value;
            } else if (auto it = m_failed_processor_registrations_by_url.find(module.url);
                it != m_failed_processor_registrations_by_url.end()) {
                failed_registrations = it->value;
            }
            notify_module_evaluated(module.module_id,
                m_processor_registration_generation.load(AK::MemoryOrder::memory_order_relaxed),
                true, {}, {}, move(failed_registrations));
            continue;
        }
...
JS::Object* RealtimeAudioWorkletProcessorHost::ensure_processor_instance(RenderThreadState& state,
    SharedNode& shared)
{
    ASSERT_RENDER_THREAD();
    ...
    if (it != state.instances.end()) {
        ...
        if (name_it != state.instance_processor_names.end() && name_it->value != shared.processor_name) {
            ...
            if (!state.instance_has_process.contains(shared.node_id)) {
                auto processor_ctor = state.global_scope->processor_constructor(String::from_byte_string(shared.processor_name)
                ...
                if (processor_ctor.is_function()) {
                    ...
                    if (should_log_webaudio())
                        dbgln("[WebAudio] AudioWorklet: node {} has_process={} (late)", shared.node_id.value(),
                            has_process);
                }
            }
        }
    ...
    auto processor_ctor = state.global_scope->processor_constructor(
        String::from_byte_string(shared.processor_name).release_value_but_fixme_should_propagate_errors());
    if (!processor_ctor.is_function()) {
        if (should_log_webaudio())
    ...
    auto node_options_object = JS::Object::create(*state.realm, state.realm->intrinsics().object_prototype());
    MUST(node_options_object->create_data_property_or_throw(JS::PropertyKey { "numberOfInputs"_utf16_fly_string },
        JS::Value(shared.number_of_inputs)));
    MUST(node_options_object->create_data_property_or_throw(JS::PropertyKey { "numberOfOutputs"_utf16_fly_string },
        JS::Value(shared.number_of_outputs)));
    ...
    if (shared.output_channel_count.has_value()) {
        ...
        for (size_t i = 0; i < shared.output_channel_count->size(); ++i)
            MUST(output_channel_count_array->create_data_property_or_throw(
                JS::PropertyKey { static_cast<u32>(i) }, JS::Value((*shared.output_channel_count)[i])));
        MUST(node_options_object->create_data_property_or_throw(
            JS::PropertyKey { "outputChannelCount"_utf16_fly_string }, output_channel_count_array));
    }
    ...
    if (shared.parameter_data.has_value()) {
        ...
        for (auto const& entry : shared.parameter_data.value())
            MUST(parameter_data_object->create_data_property_or_throw(
                JS::PropertyKey { Utf16String::from_utf8(entry.name) }, JS::Value(entry.value)));
        MUST(node_options_object->create_data_property_or_throw(JS::PropertyKey { "parameterData"_utf16_fly_string },
            parameter_data_object));
    }
