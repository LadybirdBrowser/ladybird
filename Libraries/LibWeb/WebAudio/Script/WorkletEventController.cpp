...
static Vector<Web::WebAudio::AudioParamDescriptor>
from_wire_audio_param_descriptors(Vector<Web::WebAudio::Render::AudioParamDescriptor> const& descriptors)
{
    Vector<Web::WebAudio::AudioParamDescriptor> local_descriptors;
    local_descriptors.ensure_capacity(descriptors.size());
    for (auto const& descriptor : descriptors) {
        local_descriptors.unchecked_append({
            .name = descriptor.name,
            .default_value = descriptor.default_value,
            .min_value = descriptor.min_value,
            .max_value = descriptor.max_value,
            .automation_rate = descriptor.is_k_rate ? Bindings::AutomationRate::KRate : Bindings::AutomationRate::ARate,
        });
    }
...
void clear_all_worklet_contexts() { s_context_by_session.clear(); }
...
static void handle_worklet_processor_error(u64 session_id, NodeID node_id)
{
    ...
    for (auto& node : nodes_to_notify) {
        ...
        target_context->queue_a_media_element_task(GC::create_function(target_context->heap(), [node] {
            HTML::TemporaryExecutionContext execution_context(
                node->realm(), HTML::TemporaryExecutionContext::CallbacksEnabled::Yes);
            HTML::ErrorEventInit event_init;
            event_init.error = JS::js_undefined();
            node->dispatch_event(
                HTML::ErrorEvent::create(node->realm(), HTML::EventNames::processorerror, event_init));
        }));
    }
}
...
static void
handle_worklet_processor_registration(u64 session_id, String const& name,
    Vector<Web::WebAudio::AudioParamDescriptor> const& descriptors,
    u64 generation)
{
...
static void
enqueue_worklet_processor_registration_task(u64 session_id, String const& name,
    Vector<Web::WebAudio::AudioParamDescriptor> const& descriptors,
    u64 generation)
{
    auto context = context_for_session(session_id);
    ...
    HTML::queue_a_task(
        HTML::Task::Source::AudioWorklet, HTML::main_thread_event_loop(), nullptr,
...
static void handle_worklet_module_evaluated(u64 session_id, u64 module_id, u64 required_generation,
    bool success, String const& error_name,
    ...
    Vector<String> const& failed_processor_registrations)
{
    ...
    if (required_generation > worklet->registration_generation() && (worklet->has_loaded_any_module() || worklet->has_pending_module_promises())) {
        ...
        if (get_worklet_replay_state_for_generation(session_id, required_generation, registered_descriptors,
                failed_registrations, last_generation)) {
    ...
    context->queue_a_media_element_task(GC::create_function(
        context->heap(), [worklet, local_module_id, required_generation, success, error_name_copy = move(error_name_copy), message_copy = move(message_copy)] {
            worklet->handle_module_evaluated(local_module_id, required_generation, success, error_name_copy,
                message_copy);
        }));
}

static void enqueue_worklet_module_evaluated_task(u64 session_id, u64 module_id, u64 required_generation,
    bool success, String const& error_name,
    String const& error_message,
    Vector<String> const& failed_processor_registrations)
{
    ...
    HTML::queue_a_task(HTML::Task::Source::AudioWorklet, HTML::main_thread_event_loop(), nullptr,
        GC::create_function(HTML::main_thread_event_loop().heap(),
            [session_id, module_id, required_generation, success, error_name,
                error_message, failed_processor_registrations] {
                handle_worklet_module_evaluated(
                    session_id, module_id, required_generation, success,
                    error_name, error_message, failed_processor_registrations);
            }));
}
...
void install_worklet_event_callbacks(SessionClientOfWebAudioWorker& client)
{
    ...
    client.on_worklet_processor_registered =
        [](u64 session_id, String const& name,
            Vector<Web::WebAudio::Render::AudioParamDescriptor> const& descriptors, u64 generation) {
            enqueue_worklet_processor_registration_task(
                session_id, name, from_wire_audio_param_descriptors(descriptors), generation);
        };
    client.on_worklet_module_evaluated = [](u64 session_id, u64 module_id, u64 required_generation,
                                             bool success, String const& error_name, String const& error_message,
                                             Vector<String> const& failed_processor_registrations) {
        enqueue_worklet_module_evaluated_task(session_id, module_id, required_generation, success, error_name,
            error_message, failed_processor_registrations);
    };
}

void note_worklet_processor_registration(u64 session_id, String const& name,
    Vector<Web::WebAudio::AudioParamDescriptor> const& descriptors,
    u64 generation)
{
...
void note_worklet_failed_processor_registrations(u64 session_id,
    Vector<String> const& failed_processor_registrations)
{
    if (failed_processor_registrations.is_empty())
        return;
...
bool get_worklet_replay_state_for_generation(
    u64 session_id, u64 required_generation,
    HashMap<String, Vector<Web::WebAudio::AudioParamDescriptor>>& out_registered_descriptors,
    HashTable<String>& out_failed_registrations, u64& out_last_generation)
{
...
void clear_all_worklet_session_state() { s_registration_state_by_session.clear(); }
