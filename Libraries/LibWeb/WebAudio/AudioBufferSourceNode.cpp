...
WebIDL::ExceptionOr<void> AudioBufferSourceNode::start(Optional<double> when, Optional<double> offset, Optional<double> duration)
{
    ...
    double const requested_start_time = when.value_or(0.0);
    m_start_when = when;
    m_start_offset = offset;
    m_start_duration = duration;
    latch_start_time_for_rendering(requested_start_time);
    ...
    context()->queue_control_message(StartSource { .node_id = node_id(), .when = requested_start_time });

    context()->notify_audio_graph_changed();

    if (m_buffer) {
        double const effective_start_time = start_when_for_rendering().value();
        ...
        if (effective_rate > 0.0 && (has_explicit_duration || !m_loop)) {
            ...
            double end_time = effective_start_time + playback_duration;
