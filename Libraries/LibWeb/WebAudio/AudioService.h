...
// Process-global WebAudio realtime service.
//
// Today this owns a single output device stream and a single render thread.
// AudioContexts register as clients and provide control/task queues and a RenderGraph.
//
// NOTE: This is an interim design. The device callback thread lives in LibMedia.
//       We intentionally keep the WebAudio render thread separate for now.
class AudioService final : public Weakable<AudioService> {
public:
    using ClientId = u64;

    static AudioService& the();
    ...
    void set_client_render_graph(ClientId,
