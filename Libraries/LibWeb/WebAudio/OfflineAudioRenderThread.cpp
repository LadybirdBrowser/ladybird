...
OfflineAudioRenderThread::OfflineAudioRenderThread(OfflineAudioRenderRequest request,
    CompletionDispatcher completion_dispatcher,
    int suspend_write_fd)
...
void OfflineAudioRenderThread::start() { m_thread->start(); }
...
Optional<OfflineAudioAnalyserSnapshot>
OfflineAudioRenderThread::take_analyser_snapshot(u32 expected_frame_index)
{
    Threading::MutexLocker const locker { m_mutex };
...
void OfflineAudioRenderThread::rendering_thread_loop()
{
    ...
    WA_DBGLN("[WebAudio] offline render thread loop start length={} sr={}", m_request.length_in_sample_frames,
        m_request.sample_rate);
    ...
    u32 const quantum_size = m_request.render_quantum_size > 0 ? m_request.render_quantum_size
    ...
    RenderGraph graph(current_graph_description, m_request.sample_rate,
        m_request.render_quantum_size > 0 ? m_request.render_quantum_size
                                          : static_cast<u32>(RENDER_QUANTUM_SIZE),
        *m_request.resources);
    ...
    while (frame_index < length_in_frames) {
        ...
        // Shall we suspend?
        if (next_suspend_index < m_request.suspend_frame_indices.size() && frame_index == m_request.suspend_frame_indices[next_suspend_index]) {
            ...
            snapshot_analysers(graph, current_graph_description, analyser_snapshot.analyser_time_domain_data,
                analyser_snapshot.analyser_frequency_data_db);
            {
                Threading::MutexLocker const locker { m_mutex };
    ...
    snapshot_analysers(graph, current_graph_description, result.analyser_time_domain_data,
        result.analyser_frequency_data_db);
    {
        Threading::MutexLocker const locker { m_mutex };
        if (!m_abort_requested)
            m_result = move(result);
...
static OfflineAudioRenderResult render_offline_audio_graph(OfflineAudioRenderRequest const& request)
{
    ...
    u32 const quantum_size = request.render_quantum_size > 0 ? request.render_quantum_size : static_cast<u32>(RENDER_QUANTUM_SIZE);
    ...
    RenderGraph graph(request.graph, request.sample_rate,
        request.render_quantum_size > 0 ? request.render_quantum_size
                                        : static_cast<u32>(RENDER_QUANTUM_SIZE),
        *request.resources);
    ...
    snapshot_analysers(graph, request.graph, result.analyser_time_domain_data,
        result.analyser_frequency_data_db);
