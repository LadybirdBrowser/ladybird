...
WebIDL::ExceptionOr<void> AudioScheduledSourceNode::start(double when)
{
    ...
    if (source_started())
        return WebIDL::InvalidStateError::create("AudioScheduledSourceNode source has already started"_utf16);
...
WebIDL::ExceptionOr<void> AudioScheduledSourceNode::stop(double when)
{
    ...
    if (!m_source_started)
        return WebIDL::InvalidStateError::create("AudioScheduledSourceNode source has not been started"_utf16);
