...
void EngineController::update_client_render_graph(ClientId client_id, f32 graph_sample_rate, ByteBuffer encoded_graph, Vector<Render::SharedAudioBufferBinding> shared_audio_buffer_bindings, NonnullOwnPtr<Render::GraphResources> resources, Vector<Render::WorkletModule> worklet_modules, Vector<Render::WorkletNodeDefinition> worklet_node_definitions, Vector<Render::WorkletPortBinding> worklet_port_bindings)
{
    ASSERT_CONTROL_THREAD();
    if (!m_engine) {
        return;
