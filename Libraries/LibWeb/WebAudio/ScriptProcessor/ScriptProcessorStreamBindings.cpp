    ...
    if (!m_publish_retry_timer) {
        m_publish_retry_timer = Core::Timer::create_single_shot(Render::SCRIPT_PROCESSOR_PUBLISH_RETRY_INTERVAL_MS, [this, weak_engine = engine.make_weak_ptr()] {
            ...
            if (m_publish_retry_attempts++ > Render::SCRIPT_PROCESSOR_PUBLISH_RETRY_MAX_ATTEMPTS) {
                WA_GR_DBGLN("[WebAudio] ScriptProcessor stream publish retry giving up (session={})", m_session_id);
                if (m_pending_graph_update_for_retry.has_value())
                    close_worklet_port_binding_fds(m_pending_graph_update_for_retry->worklet_port_bindings);
                m_pending_graph_update_for_retry.clear();
                m_retry_graph_update = {};
            ...
            if (m_pending_graph_update_for_retry.has_value()) {
                ...
                WA_GR_DBGLN("[WebAudio] ScriptProcessor stream publish retry firing (session={} attempt={} kind=graph_update)", m_session_id, m_publish_retry_attempts);
                if (retry_cb)
                    retry_cb(move(pending));
                return;
            }

            WA_GR_DBGLN("[WebAudio] ScriptProcessor stream publish retry firing (session={} attempt={} kind=publish_only)", m_session_id, m_publish_retry_attempts);
...
TransactionalPublishOutcome ScriptProcessorStreamBindings::try_publish_bindings_for_nodes(Vector<NodeID> const& node_ids)
{
    ...
    for (auto node_id : node_ids) {
        ...
        if (notify_write_fd_or_error.is_error()) {
            skipped_any = true;
            WA_GR_DBGLN("[WebAudio] ScriptProcessor notify fd clone failed (session={} node={} error={})", m_session_id, node_id, notify_write_fd_or_error.error());
            continue;
        }
    ...
    auto outcome = transactional_publish_bindings(
        ...
        [this](Vector<ScriptProcessorStreamDescriptor>&& bindings) {
            ...
            if (result.is_error()) {
                WA_GR_DBGLN("[WebAudio] Failed to send ScriptProcessor stream bindings to AudioServer: {}", result.error());
                return false;
            }
            return true;
        });
    ...
    Function<void(PendingClientRenderGraphUpdate&&)>&& retry_graph_update)
{
    ...
    if (::Web::WebAudio::should_log_graph_updates()) {
        WA_GR_DBGLN("[WebAudio] ScriptProcessor stream bindings state: nodes_in_graph={} publish_requested={} published_already={}",
    ...
    for (auto const& it : script_processors) {
        ...
        if (!request_buffers.has_value()) {
            WA_GR_DBGLN("[WebAudio] ScriptProcessor stream buffers not available yet (session={} node={} req_block_bytes={} blocks={})", m_session_id, node_id, request_block_bytes, script_processor_stream_block_count);
            continue;
        }
        ...
        if (!response_buffers.has_value()) {
            WA_GR_DBGLN("[WebAudio] ScriptProcessor stream buffers not available yet (session={} node={} resp_block_bytes={} blocks={})", m_session_id, node_id, response_block_bytes, script_processor_stream_block_count);
            continue;
        }

        auto notify_fds_or_error = create_nonblocking_stream_notify_fds();
        if (notify_fds_or_error.is_error()) {
            WA_GR_DBGLN("[WebAudio] ScriptProcessor notify fds not available yet (session={} node={} error={})", m_session_id, node_id, notify_fds_or_error.error());
            continue;
        }
