...
{
    size_t required = static_cast<size_t>(block_count) * sizeof(SharedBufferStream::Descriptor);
    size_t capacity = 64;
...
static Optional<SharedBufferStreamBuffers> create_shared_buffer_stream_buffers(u32 block_size, u32 block_count, char const* debug_name)
{
    ...
    auto ready_ring_or_error = SharedCircularBuffer::create(ring_capacity_bytes);
    ...
    auto free_ring_or_error = SharedCircularBuffer::create(ring_capacity_bytes);
    ...
    auto stream_or_error = SharedBufferStream::attach(pool_buffer, ready_ring_buffer, free_ring_buffer);
...
void ScriptProcessorStreamBindings::set_webaudio_session(NonnullRefPtr<::Web::WebAudio::SessionClientOfWebAudioWorker> const& client, u64 session_id)
{
    m_client = client;
...
void ScriptProcessorStreamBindings::schedule_publish_retry_with_graph_update(PendingClientRenderGraphUpdate&& update, Function<void(PendingClientRenderGraphUpdate&&)>&& retry_graph_update)
{
    ...
    Function<void(PendingClientRenderGraphUpdate&&)>&& retry_graph_update)
{
    ...
    for (auto const& it : script_processors) {
        ...
        if (pipe_fds_or_error.is_error()) {
            WA_DBGLN("[WebAudio] ScriptProcessor notify fds not available yet (session={} node={} error={})", m_session_id, node_id, pipe_fds_or_error.error());
            continue;
        }
