...
using Audio::RingHeader;

static Function<ErrorOr<IPC::TransportHandle>(u64 page_id)> s_webaudio_transport_provider;

static bool validate_analyser_snapshot_request(u32 fft_size, Span<f32> out_time_domain,
...
void SessionRouter::set_webaudio_session(
    NonnullRefPtr<::Web::WebAudio::SessionClientOfWebAudioWorker> const& client, u64 session_id)
{
    ...
    if (::Web::WebAudio::should_log_webaudio(LOG_GENERAL))
        WA_DBGLN(
            "[WebAudio] WebAudioWorker: set_audio_server_session old_session={} new_session={} have_client={}",
            m_session_id, session_id, true);
    m_webaudio_client = client;
    ...
    if (m_pending_suspend.has_value()) {
        ...
        (void)m_webaudio_client->set_suspended(m_session_id, pending.suspended, pending.generation);
    }
}
...
ErrorOr<NonnullRefPtr<::Web::WebAudio::SessionClientOfWebAudioWorker>> SessionRouter::ensure_webaudio_client(u64 page_id)
{
    ASSERT_CONTROL_THREAD();

    if (m_webaudio_client)
        return NonnullRefPtr { *m_webaudio_client };

    if (!s_webaudio_transport_provider)
        return Error::from_string_literal("WebAudio: WebAudioWorker transport provider is not available");

    auto handle = TRY(s_webaudio_transport_provider(page_id));
    auto transport = TRY(handle.create_transport());
    NonnullRefPtr<::Web::WebAudio::SessionClientOfWebAudioWorker> client = ::Web::WebAudio::SessionClientOfWebAudioWorker::create(move(transport));
...
static Optional<RingStreamView> view_for_ring_stream_buffer(Core::AnonymousBuffer& buffer)
{
    ...
    return RingStreamView { .header = header,
        .interleaved_frames = Span<f32> { data_f32, data_bytes / sizeof(f32) } };
}
...
bool SessionRouter::update_media_element_stream_bindings(GraphResources const& resources,
    Vector<WorkletPortBinding>& worklet_port_bindings)
{
    ...
    bool removed_any = remove_entries_not_in(m_remote_media_element_streams, seen_provider_ids,
        [](RemoteMediaElementStream& state) {
            if (state.provider)
    ...
    if (::Web::WebAudio::should_log_webaudio(LOG_MEDIA)) {
        WA_MEDIA_DBGLN("[WebAudio] media-stream-bindings: providers_in_graph={} publishable={} "
                       "publish_requested={} published_already={} remote_streams={}",
            sources.size(), publish_provider_ids.size(), should_publish_media_stream_bindings,
            m_published_media_element_stream_bindings, m_remote_media_element_streams.size());
    }
    ...
    auto result = m_webaudio_client->set_media_element_audio_source_streams(m_session_id, move(media_stream_bindings));
    if (result.is_error()) {
...
void SessionRouter::update_media_stream_audio_source_bindings(GraphResources const& resources)
{
    ASSERT_CONTROL_THREAD();
    ...
    for (auto const& it : metadata_sources) {
        ...
        if (existing != m_media_stream_source_metadata.end()) {
            if (existing->value.device_handle == it.value.device_handle && existing->value.sample_rate_hz == it.value.sample_rate_hz && existing->value.channel_count == it.value.channel_count && existing->value.capacity_frames == it.value.capacity_frames)
                continue;
        }
    ...
    bool removed_any = remove_entries_not_in(m_media_stream_source_metadata, seen_provider_ids,
        [](AudioInputStreamMetadata&) { });
    if (removed_any)
        should_publish = true;
    ...
    auto result = m_webaudio_client->set_media_stream_audio_source_streams(m_session_id, move(bindings));
    if (result.is_error())
...
ErrorOr<void>
SessionRouter::open_session_transport(u32 target_latency_ms, u64 page_id,
    Function<void(ErrorOr<OutputSessionFormat> const&)> on_complete)
{
    ...
    NonnullRefPtr<::Web::WebAudio::SessionClientOfWebAudioWorker> client = TRY(ensure_webaudio_client(page_id));

    auto weak_this = make_weak_ptr();
    TRY(client->create_session_async(
        target_latency_ms,
        [weak_this, client, on_complete = move(on_complete)](
            ErrorOr<::Web::WebAudio::SessionClientOfWebAudioWorker::WebAudioSession>&& session_or_error) mutable {
            ...
            this_session->set_webaudio_session(client, this_session->m_webaudio_session->session_id);
            on_complete(format);
        }));
...
void SessionRouter::close_session_transport()
{
    ...
    if (m_webaudio_client && m_session_id != 0)
        (void)m_webaudio_client->destroy_session(m_session_id);
...
void SessionRouter::start_time_sync_notifier_if_needed()
{
    ...
    if (m_time_sync_notifier)
        return;
    if (!m_webaudio_session.has_value()) {
        if (should_log_webaudio(LOG_OUTPUT))
    ...
    if (m_timing_buffer.size() < sizeof(Web::WebAudio::Render::TimingFeedbackPage)) {
        if (should_log_webaudio(LOG_OUTPUT))
            WA_OUT_DBGLN("[WebAudio] WebAudioWorker time sync: timing buffer too small ({} bytes)",
                m_timing_buffer.size());
        return;
    }
...
void SessionRouter::update_client_render_graph(f32 graph_sample_rate, ByteBuffer encoded_graph,
    ...
    Vector<WorkletPortBinding> worklet_port_bindings)
{
    ...
    if (::Web::WebAudio::should_log_webaudio(LOG_GENERAL)) {
        WA_DBGLN("[WebAudio] WebAudioWorker update: session={} have_client={} client_id={} graph_sr={} "
                 "encoded_graph_bytes={} published_sp={} published_media={} event_loop_running={}",
            m_session_id, m_webaudio_client != nullptr, m_client_id, graph_sample_rate, encoded_graph.size(),
            m_script_processor_stream_bindings.published_bindings(),
            m_published_media_element_stream_bindings, Core::EventLoop::is_running());
    }
    ...
    if (m_session_id == 0 || !m_webaudio_client) {
        if (::Web::WebAudio::should_log_webaudio(LOG_GENERAL))
    ...
    bool did_reschedule = m_script_processor_stream_bindings.update_stream_bindings_and_maybe_reschedule(
        update, [this](PendingClientRenderGraphUpdate&& retry_update) {
            update_client_render_graph(retry_update.graph_sample_rate, move(retry_update.encoded_graph),
                move(retry_update.shared_audio_buffer_bindings),
                move(retry_update.resources), move(retry_update.worklet_modules),
    ...
    {
        if (::Web::WebAudio::should_log_webaudio(LOG_GENERAL)) {
            WA_DBGLN("[WebAudio] Publishing {} worklet node definition(s) to AudioServer",
                update.worklet_node_definitions.size());
            for (auto const& def : update.worklet_node_definitions)
                WA_DBGLN("[WebAudio]  - worklet node definition node_id={} processor='{}'", def.node_id,
                    def.processor_name);
        }

        auto defs_result = m_webaudio_client->set_worklet_node_definitions(m_session_id, update.worklet_node_definitions);
    ...
    // worklet VM can attach the processor MessagePort when the graph becomes active.
    {
        ...
        for (auto& binding : update.worklet_port_bindings) {
            port_descriptors.append(WorkletNodePortDescriptor {
                ...
                .processor_port_handle = move(binding.processor_port_handle),
            });
        }
        ...
        auto port_result = m_webaudio_client->set_worklet_node_ports(m_session_id, move(port_descriptors));
    ...
    for (auto const& module : update.worklet_modules) {
        ...
        if (::Web::WebAudio::should_log_webaudio(LOG_GENERAL))
            WA_DBGLN("[WebAudio] Publishing worklet module id={} '{}' ({} bytes)", ipc_module_id, module.url,
                module.source_text.length());
        auto result = m_webaudio_client->add_worklet_module(m_session_id, ipc_module_id, module.url, module.source_text);
        if (result.is_error())
            WA_DBGLN("[WebAudio] Failed to send worklet module to AudioServer: {}", result.error());
    }

    auto graph_result = m_webaudio_client->set_render_graph(m_session_id, move(update.encoded_graph),
        move(update.shared_audio_buffer_bindings));
    if (graph_result.is_error())
        WA_DBGLN("[WebAudio] Failed to send render graph to AudioServer: {}", graph_result.error());
}

bool SessionRouter::try_copy_analyser_snapshot(NodeID analyser_node_id, u32 fft_size,
    Span<f32> out_time_domain, Span<f32> out_frequency_db,
    u64& out_render_quantum_index)
{
...

    {
        ...
        if (it == m_remote_analyser_streams.end() || it->value.fft_size != fft_size || !it->value.stream.is_valid()) {
            auto stream_or_error = m_webaudio_client->create_analyser_stream(m_session_id, analyser_node_id.value(), fft_size, 4);

            if (stream_or_error.is_error())
                return false;

            m_remote_analyser_streams.set(
                analyser_node_id,
                RemoteAnalyserStream { .fft_size = fft_size, .stream = stream_or_error.release_value() });
        }
    }
...
bool SessionRouter::try_copy_dynamics_compressor_reduction(NodeID compressor_node_id, f32& out_reduction_db,
    u64& out_render_quantum_index)
{
    ASSERT_CONTROL_THREAD();
    start_time_sync_notifier_if_needed();
...

    {
        ...
        if (it == m_remote_dynamics_compressor_streams.end() || !it->value.stream.is_valid()) {
            auto stream_or_error = m_webaudio_client->create_dynamics_compressor_stream(m_session_id, compressor_node_id.value(), 4);

            if (stream_or_error.is_error())
                return false;

            m_remote_dynamics_compressor_streams.set(
                compressor_node_id, RemoteDynamicsCompressorStream { .stream = stream_or_error.release_value() });
        }
    }
...
SessionRouter::SessionRouter(u64 client_id, Atomic<u64>* current_frame, Atomic<u8>* suspended,
...
void SessionRouter::set_webaudio_transport_provider(
    Function<ErrorOr<IPC::TransportHandle>(u64 page_id)> provider)
{
    s_webaudio_transport_provider = move(provider);
}

bool SessionRouter::has_session_transport_open() const { return m_webaudio_session.has_value(); }

u64 SessionRouter::session_id() const { return m_session_id; }
...
void SessionRouter::set_client_suspended(bool suspended, u64 generation)
{
    ...
    m_pending_suspend = PendingSuspendState { .suspended = suspended, .generation = generation };
    (void)m_webaudio_client->set_suspended(m_session_id, suspended, generation);
}
