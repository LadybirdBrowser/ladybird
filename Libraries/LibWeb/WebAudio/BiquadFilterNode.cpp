...
BiquadFilterNode::BiquadFilterNode(GC::Ref<BaseAudioContext> context, BiquadFilterOptions const& options)
    : AudioNode(context)
    , m_type(options.type)
    , m_frequency(AudioParam::create(context, this, options.frequency, 0, context->nyquist_frequency(), AutomationRate::ARate))
    ...
    , m_q(AudioParam::create(context, this, options.q, NumericLimits<float>::lowest(), NumericLimits<float>::max(), AutomationRate::ARate))
    , m_gain(AudioParam::create(context, this, options.gain, NumericLimits<float>::lowest(), 40 * AK::log10(NumericLimits<float>::max()), AutomationRate::ARate))
{
}
...
WebIDL::ExceptionOr<void> BiquadFilterNode::get_frequency_response(GC::Ref<JS::Float32Array> frequency_hz, GC::Ref<JS::Float32Array> mag_response, GC::Ref<JS::Float32Array> phase_response)
{
    ...
    if (float32_array_length(*mag_response) != length || float32_array_length(*phase_response) != length)
        return WebIDL::InvalidAccessError::create(context()->relevant_settings_object().realm(), "Arrays must have the same length"_utf16);
