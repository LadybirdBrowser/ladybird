...
{
    auto& settings_object = HTML::relevant_settings_object(*this);
    auto task = HTML::Task::create(vm(), m_media_element_event_task_source.source, settings_object.responsible_document(), steps);
...
void BaseAudioContext::schedule_source_end(AudioScheduledSourceNode& node, double end_time_seconds)
{
    ...
    end_time_seconds = AK::max(end_time_seconds, 0.0);
