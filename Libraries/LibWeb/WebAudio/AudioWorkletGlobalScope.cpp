...
static WebIDL::ExceptionOr<AudioParamDescriptor> convert_parameter_descriptor(JS::VM& vm, JS::Value value)
{
    ...
    auto read_float_member = [&](Utf16FlyString const& member_name, float fallback) -> JS::ThrowCompletionOr<float> {
        ...
        if (member_value.is_undefined())
            return fallback;
        return static_cast<float>(TRY(member_value.to_double(vm)));
    };
...
void AudioWorkletGlobalScope::pump(NodeID node_id, Rendering::AudioWorkletPipe& pipe)
{
    ...
    if (slot.pipe->state() == Rendering::AudioWorkletPipe::State::ShutDown) {
    ...
    auto const& config = slot.pipe->config();

    while (true) {
        if (slot.pipe->output_occupancy() == config.ring_capacity)
            break;
        bool processed_one = false;
        bool pump_failed = false;
        bool output_full = false;

        (void)slot.pipe->try_pop_input([&](Rendering::AudioWorkletPipe::InputSlotReader const& input_slot) {
            processed_one = true;
            ...
            for (size_t output = 0; output < config.output_count; ++output) {
                auto channels = build_channel_array(config.output_channel_capacity[output], [&](size_t, auto&) { });
            ...
            for (size_t param_index = 0; param_index < config.param_count; ++param_index) {
                ...
                MUST(parameters->create_data_property(JS::PropertyKey { Utf16FlyString(slot.parameter_names[param_index]) }, float32_array));
            }

            // Look up process on the instance each call (spec); a non-callable value fails the node.
            auto process_value_or_error = slot.processor->get("process"_utf16_fly_string);
            ...
            auto result = JS::call(vm, process_value.as_function(), JS::Value { slot.processor.ptr() },
                ReadonlySpan<JS::Value> { process_arguments });
            if (result.is_error()) {
                pump_failed = true;
                if (auto node = slot.node.ptr())
                    node->fire_processor_error(result.release_error().value());
                return;
            }
            ...
            if (!slot.pipe->try_push_output([&](Rendering::AudioWorkletPipe::OutputSlotWriter& output_slot) {
                    output_slot.start_frame() = input_slot.start_frame();
                    output_slot.set_processor_active(processor_active);
                    for (size_t output = 0; output < config.output_count; ++output) {
                        auto capacity = config.output_channel_capacity[output];
                        output_slot.actual_channel_count(output) = capacity;
                        for (size_t channel = 0; channel < capacity; ++channel) {
        ...
        if (pump_failed) {
            slot.failed = true;
            slot.pipe->set_state(Rendering::AudioWorkletPipe::State::Failed);
            return;
        }
        if (output_full)
            break;
