...
void AudioWorkletGlobalScope::initialize_web_interfaces()
{
    ...
    auto register_processor_steps = [](JS::VM& vm) -> JS::ThrowCompletionOr<JS::Value> {
        ...
        if (name.is_empty())
            return vm.throw_completion<JS::TypeError>("Processor name must not be empty"sv);

        if (!processor_constructor.is_function())
            return vm.throw_completion<JS::TypeError>("Processor constructor must be a function"sv);
