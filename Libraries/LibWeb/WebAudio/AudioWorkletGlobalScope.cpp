...
void AudioWorkletGlobalScope::initialize(JS::Realm& realm)
{
...
void AudioWorkletGlobalScope::initialize_web_interfaces()
{
    ...
    AK::Function<JS::ThrowCompletionOr<JS::Value>(JS::VM&)> register_processor_steps = [](JS::VM& vm) -> JS::ThrowCompletionOr<JS::Value> {
        auto& realm = *vm.current_realm();
...
void AudioWorkletGlobalScope::visit_edges(Cell::Visitor& visitor)
{
    ...
    visitor.visit(m_pending_processor_port);
    visitor.visit(m_shared_port);
}
