...
struct WebAudioWorkerSession::Impl {
public:
    bool has_output_open() const { return m_webaudio_session.has_value(); }
    ErrorOr<void> ensure_output_open(WebAudioClientRegistry& engine, u32 target_latency_ms, u64 page_id);
    void shutdown_output(WebAudioClientRegistry& engine);
    ...
    void set_webaudio_session(NonnullRefPtr<WebAudioWorkerClient::WebAudioClient> const& client, u64 session_id)
    {
        if (::Web::WebAudio::should_log_graph_updates())
            WA_GR_DBGLN("[WebAudio] WebAudioWorker: set_audio_server_session old_session={} new_session={} have_client={}", m_session_id, session_id, true);
        m_client = client;
    ...
    void update_client_render_graph(
        ...
        NonnullOwnPtr<GraphResourceRegistry>,
        Vector<WorkletModule> worklet_modules,
        Vector<WorkletNodeDefinition>,
        Vector<WorkletPortBinding>);
    ...
    void set_client_suspended(WebAudioClientRegistry&, u64, bool suspended, u64 generation)
    {
...
bool WebAudioWorkerSession::Impl::update_media_element_stream_bindings(GraphResourceRegistry const& resources, Vector<WorkletPortBinding>& worklet_port_bindings)
{
    ...
    auto publish_outcome = transactional_publish_bindings(
        ...
        [this](Vector<MediaElementAudioSourceStreamDescriptor>&& bindings) {
            ...
            if (result.is_error()) {
                WA_GR_DBGLN("[WebAudio] Failed to send media element stream bindings to AudioServer: {}", result.error());
                return false;
            }
            return true;
        });
...
ErrorOr<void> WebAudioWorkerSession::Impl::ensure_output_open(WebAudioClientRegistry& engine, u32 target_latency_ms, u64 page_id)
{
    ASSERT_CONTROL_THREAD();
    ...
    {
        Threading::MutexLocker locker { engine.m_device_format_mutex };
        engine.m_device_format = format;
        engine.m_device_format_selected.signal();
    }
...
void WebAudioWorkerSession::Impl::shutdown_output(WebAudioClientRegistry&)
{
    ASSERT_CONTROL_THREAD();
    ...
    if (m_webaudio_session.has_value() && client)
        (void)client->destroy_webaudio_session(m_webaudio_session->session_id);
...
void WebAudioWorkerSession::Impl::start_time_sync_notifier_if_needed(WebAudioClientRegistry& engine)
{
...
void WebAudioWorkerSession::Impl::update_current_frames_from_timing_page(WebAudioClientRegistry& engine)
{
    if (!m_timing_buffer.is_valid())
        return;
...
void WebAudioWorkerSession::Impl::stop_time_sync_notifier()
{
    if (!m_time_sync_notifier)
        return;
...
void WebAudioWorkerSession::Impl::update_client_render_graph(
    WebAudioClientRegistry& engine,
    u64 client_id,
    ...
    Vector<WorkletPortBinding> worklet_port_bindings)
{
    ...
    if (::Web::WebAudio::should_log_graph_updates()) {
        WA_GR_DBGLN("[WebAudio] WebAudioWorker update: session={} have_client={} client_id={} graph_sr={} encoded_graph_bytes={} published_sp={} published_media={} event_loop_running={}",
            m_session_id,
            m_client != nullptr,
            client_id,
            graph_sample_rate,
    ...
    if (m_session_id == 0 || !m_client) {
        if (::Web::WebAudio::should_log_graph_updates())
            WA_GR_DBGLN("[WebAudio] WebAudioWorker update: skipping (no active AudioServer session)");
        return;
    }
    ...
    {
        if (::Web::WebAudio::should_log_graph_updates()) {
            WA_GR_DBGLN("[WebAudio] Publishing {} worklet node definition(s) to AudioServer", update.worklet_node_definitions.size());
            for (auto const& def : update.worklet_node_definitions)
                WA_GR_DBGLN("[WebAudio]  - worklet node definition node_id={} processor='{}'", def.node_id.value(), def.processor_name);
        }
        ...
        if (defs_result.is_error())
            WA_GR_DBGLN("[WebAudio] Failed to send worklet node definitions to AudioServer: {}", defs_result.error());
    }
    ...
    // worklet VM can attach the processor MessagePort when the graph becomes active.
    {
        ...
        if (::Web::WebAudio::should_log_graph_updates()) {
            WA_GR_DBGLN("[WebAudio] Publishing {} worklet port binding(s) to AudioServer", port_descriptors.size());
            for (auto const& port : port_descriptors)
                WA_GR_DBGLN("[WebAudio]  - worklet port binding node_id={}", port.node_id);
        }
        ...
        if (port_result.is_error())
            WA_GR_DBGLN("[WebAudio] Failed to send worklet port bindings to AudioServer: {}", port_result.error());
    }

    for (auto const& module : update.worklet_modules) {
        if (::Web::WebAudio::should_log_graph_updates())
            WA_GR_DBGLN("[WebAudio] Publishing worklet module '{}' ({} bytes)", module.url, module.source_text.length());
        auto result = m_client->webaudio_session_add_worklet_module(m_session_id, module.url, module.source_text);
        if (result.is_error())
            WA_GR_DBGLN("[WebAudio] Failed to send worklet module to AudioServer: {}", result.error());
    }
    ...
    if (graph_result.is_error())
        WA_GR_DBGLN("[WebAudio] Failed to send render graph to AudioServer: {}", graph_result.error());
}

bool WebAudioWorkerSession::Impl::try_copy_analyser_snapshot(
    WebAudioClientRegistry& engine,
    u64,
    NodeID analyser_node_id,
    u32 fft_size,
...
bool WebAudioWorkerSession::Impl::try_copy_dynamics_compressor_reduction(
    WebAudioClientRegistry& engine,
    u64,
    NodeID compressor_node_id,
    f32& out_reduction_db,
...
WebAudioWorkerSession::WebAudioWorkerSession()
    : m_impl(make<Impl>())
{
}
...
{
    return m_impl->has_output_open();
}

ErrorOr<void> WebAudioWorkerSession::ensure_output_open(WebAudioClientRegistry& engine, u32 target_latency_ms, u64 page_id)
{
    return m_impl->ensure_output_open(engine, target_latency_ms, page_id);
}

void WebAudioWorkerSession::shutdown_output(WebAudioClientRegistry& engine)
{
    m_impl->shutdown_output(engine);
}
...
void WebAudioWorkerSession::update_client_render_graph(
    ...
    Vector<WorkletPortBinding> worklet_port_bindings)
{
    m_impl->update_client_render_graph(engine, client_id, graph_sample_rate, move(encoded_graph), move(resources), move(worklet_modules), move(worklet_node_definitions), move(worklet_port_bindings));
}
...
{
    m_impl->set_client_suspended(engine, client_id, suspended, generation);
}
...
bool WebAudioWorkerSession::try_copy_analyser_snapshot(
    ...
    u64& out_render_quantum_index)
{
    return m_impl->try_copy_analyser_snapshot(engine, client_id, analyser_node_id, fft_size, out_time_domain, out_frequency_db, out_render_quantum_index);
}
...
bool WebAudioWorkerSession::try_copy_dynamics_compressor_reduction(
    ...
    u64& out_render_quantum_index)
{
    return m_impl->try_copy_dynamics_compressor_reduction(engine, client_id, compressor_node_id, out_reduction_db, out_render_quantum_index);
}
