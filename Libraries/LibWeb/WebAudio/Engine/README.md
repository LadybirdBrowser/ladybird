...
- AudioContextRegistry owns one SessionRouter per AudioContext; timing pages and worklet registration caches are per-context.
- Notify fd signaling and draining helpers live in LibWebAudio.h/cpp; tunables live in Policy.h.
...
- Real-time readback for analyser data and dynamics compressor reduction is done via SharedBufferStream snapshots in SessionRouter.
