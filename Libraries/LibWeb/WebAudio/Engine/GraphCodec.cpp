...
static ErrorOr<void> append_buffer_table_section(Encoder& encoder, GraphDescription const& graph, Vector<u64> const& sorted_node_ids, GraphResourceResolver const& resources, u32& flags, Vector<SharedAudioBufferBinding>& shared_audio_buffer_bindings)
{
...
ErrorOr<EncodedRenderGraph> encode_render_graph_for_media_server(GraphDescription const& graph, f32 context_sample_rate, GraphResourceResolver const& resources)
{
    Encoder encoder;
    ...
    TRY(append_buffer_table_section(encoder, graph, sorted_node_ids, resources, flags, shared_audio_buffer_bindings));
...
ErrorOr<WireGraphBuildResult> decode_render_graph_wire_format(ReadonlyBytes bytes, Span<SharedAudioBufferBinding const> shared_audio_buffers)
{
    ...
    HashMap<u64, Core::AnonymousBuffer> shared_audio_buffer_descriptor_map;
