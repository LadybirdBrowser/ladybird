...
static OfflineAudioRenderResult render_offline_audio_graph(OfflineAudioRenderRequest const& request)
{
    ...
    RenderGraph graph(request.graph, request.sample_rate, request.resources.ptr());
...
OfflineAudioRenderThread::OfflineAudioRenderThread(OfflineAudioRenderRequest request, int completion_write_fd, int suspend_write_fd)
...
{
    mark_current_thread_as_render_thread();
    ...
    RenderGraph graph(current_graph_description, m_request.sample_rate, m_request.resources.ptr());
