...
static void apply_fourier_transform_in_place(Span<f32> real, Span<f32> imaginary)
{
    // This is the Cooley-Tukey FFT with bit-reversal adapted from pseudocode at:
    ...
    for (size_t n = N, s = 1; n > 1; n >>= 1, ++s) {
        ...
        for (size_t k = 0; k < N; k += m) {
            ...
            for (size_t j = 0; j < half_m; ++j) {
                ...
                f32 const t_imag = (real[v_index] * w_sin) + (imaginary[v_index] * w_cos);

                f32 const u_real = real[u_index];
                f32 const u_imag = imaginary[u_index];
...
void compute_frequency_data_db_in_place(ReadonlySpan<f32> time_domain_data, size_t fft_size, f32 smoothing_time_constant, Vector<f32>& previous_block, Vector<f32>& output_db, FrequencyAnalysisScratch& scratch)
{
    ...
    apply_fourier_transform_in_place(real, imaginary);
