...
WebIDL::ExceptionOr<GC::Ref<AudioWorkletNode>> AudioWorkletNode::construct_impl(JS::Realm& realm, GC::Ref<BaseAudioContext> context, String const& name, AudioWorkletNodeOptions const& options)
{
    ...
    Optional<IPC::TransportHandle> realtime_processor_port_handle;
    if (!is_offline) {
        ...
        node_port = pair.port;
        realtime_processor_port_handle = move(pair.peer_handle);
    }
    ...
    if (!is_offline) {
        ...
        if (realtime_processor_port_handle.has_value())
            worklet->set_realtime_processor_port(node->node_id(), move(*realtime_processor_port_handle));
        ...
        worklet->register_realtime_node_definition(Render::WorkletNodeDefinition {
            ...
            .channel_count_mode = static_cast<u8>(to_render_channel_count_mode(node->channel_count_mode())),
            .channel_interpretation = static_cast<u8>(to_render_channel_interpretation(node->channel_interpretation())),
    ...
    if (processor_error.has_value()) {
        ...
        context->queue_a_media_element_task(GC::create_function(realm.heap(), [node, error_value]() {
