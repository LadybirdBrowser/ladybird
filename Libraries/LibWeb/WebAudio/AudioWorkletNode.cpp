...
{
    if (name.is_empty())
        return WebIDL::SimpleException { WebIDL::SimpleExceptionType::TypeError, "Processor name must not be empty"sv };
    ...
    node_port->set_message_generation_gate([worklet](u64 generation) {
        return generation > worklet->registration_generation();
    });
    ...
    if (options.parameter_data.has_value()) {
        for (auto const& it : options.parameter_data.value()) {
            if (auto param = parameters->get(it.key))
                param->set_value(static_cast<float>(it.value));
        }
    }
