...
WebIDL::ExceptionOr<GC::Ref<AudioWorkletNode>> AudioWorkletNode::construct_impl(JS::Realm& realm, GC::Ref<BaseAudioContext> context, Utf16String const& name, AudioWorkletNodeOptions const& options)
{
    ...
    auto const* definition = worklet->find_definition(name);
    if (!definition)
    ...
    auto& worklet_scope = *worklet->audio_worklet_global_scope();
    ...
    // 5. Let serializedProcessorOptions be StructuredSerialize(optionsObject's processorOptions) — with
    //    shared array buffers allowed: this clone never leaves the agent, and processors legitimately
    //    receive SABs through processorOptions (the COI gate's threat model does not apply).
    ...
    if (options.processor_options) {
        auto record = TRY(HTML::structured_serialize(vm, options.processor_options, HTML::AllowSharedArrayBuffers::SameAgentAlways));
    ...
    for (WebIDL::UnsignedLong i = 0; i < options.number_of_inputs; ++i)
        pipe_config.input_channel_capacity.append(max(node->channel_count(), 2u));
    for (WebIDL::UnsignedLong i = 0; i < options.number_of_outputs; ++i) {
        if (options.output_channel_count.has_value())
            ...
            pipe_config.output_channel_capacity.append(max(node->channel_count(), 2u));
    }
...
void AudioWorkletNode::invoke_processor_constructor(GC::Ref<AudioWorkletGlobalScope> scope, GC::Ref<HTML::MessagePort> processor_port, GC::Ref<JS::Object> options_object)
{
    ...
    if (auto const* synced = context()->audio_worklet()->find_definition(m_name)) {
