...
struct RealtimeAudioWorkletProcessorHost::RenderThreadState {
    OwnPtr<Core::EventLoop> core_event_loop;
    RefPtr<JS::VM> vm;
...
{
    auto core_event_loop = make<Core::EventLoop>();
    auto vm = JS::VM::create();
    vm->set_agent(HTML::WorkerAgent::create(vm->heap(), JS::Agent::CanBlock::Yes));
    ...
    return RenderThreadState {
        .core_event_loop = move(core_event_loop),
        .vm = move(vm),
...
void RealtimeAudioWorkletProcessorHost::prepare_render_thread_state(u64 current_frame, float sample_rate, bool pump_event_loop)
{
    auto& state = ensure_render_thread_state();
    process_pending_updates(state);
    ensure_ready_processor_instances(state);
    ...
    if (should_log_all())
        dbgln("[WebAudio] AudioWorklet: pump event loop frame={} sr={}", current_frame, sample_rate);
...
void RealtimeAudioWorkletProcessorHost::service_worklet_event_loop(u64 current_frame, float sample_rate)
{
    auto& state = ensure_render_thread_state();
    process_pending_updates(state);
    ...
    if (!state.last_processed_frame.has_value() || current_frame != state.last_processed_frame.value())
        pump_event_loops(state);
...
void RealtimeAudioWorkletProcessorHost::initialize_ports(RenderThreadState& state)
{
    ...
    for (auto const& node : m_nodes) {
        if (!state.processor_ports.contains(node->node_id)) {
            ...
            port->enable();
            port->set_message_generation_provider([this]() {
                return m_processor_registration_generation.load(AK::MemoryOrder::memory_order_relaxed);
            });
    ...
    for (auto& binding : new_port_bindings) {
        ...
        if (binding.node_id != NodeID { 0 }) {
            if (state.ports_with_transport.contains(binding.node_id)) {
                ...
                state.processor_ports.set(binding.node_id, GC::make_root(HTML::MessagePort::create(*state.realm)));
...
JS::Object* RealtimeAudioWorkletProcessorHost::ensure_processor_instance(RenderThreadState& state, SharedNode& shared)
{
    ...
    if (port_it == state.processor_ports.end()) {
        ...
        processor_port->set_message_generation_provider([this]() {
            return m_processor_registration_generation.load(AK::MemoryOrder::memory_order_relaxed);
        });
