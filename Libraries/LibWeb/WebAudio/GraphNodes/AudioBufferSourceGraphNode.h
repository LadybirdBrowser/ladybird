...
struct WEB_API AudioBufferSourceGraphNode {
    ...
    {
        set_state(BufferSourceParamIndex::playback_rate, playback_rate, 0.0f, AK::NumericLimits<f32>::max());
        set_state(BufferSourceParamIndex::detune, detune_cents, -AK::NumericLimits<f32>::max(), AK::NumericLimits<f32>::max());
    }

    template<typename UpdateIntrinsic>
    void update_intrinsic_values(UpdateIntrinsic&& update_intrinsic) const
    {
        update_intrinsic(BufferSourceParamIndex::playback_rate, playback_rate);
        update_intrinsic(BufferSourceParamIndex::detune, detune_cents);
    }
};
