/*
 * Copyright (c) 2026, The Ladybird developers
 *
 * SPDX-License-Identifier: BSD-2-Clause
 */

#include <AK/OwnPtr.h>
#include <LibWeb/WebAudio/Debug.h>
#include <LibWeb/WebAudio/GraphNodes/BiquadFilterGraphNode.h>
#include <LibWeb/WebAudio/RenderNodes/BiquadFilterRenderNode.h>

namespace Web::WebAudio::Render {

ErrorOr<void> BiquadFilterGraphNode::encode_wire_payload(WireEncoder& encoder) const
{
    ASSERT_CONTROL_THREAD();
    TRY(encoder.append_u8(static_cast<u8>(type)));
    TRY(encoder.append_f32(frequency_hz));
    TRY(encoder.append_f32(detune_cents));
    TRY(encoder.append_f32(q));
    TRY(encoder.append_f32(gain_db));
    TRY(encoder.append_u32(static_cast<u32>(channel_count)));
    TRY(encoder.append_u8(static_cast<u8>(channel_count_mode)));
    TRY(encoder.append_u8(static_cast<u8>(channel_interpretation)));
    return {};
}

ErrorOr<BiquadFilterGraphNode> BiquadFilterGraphNode::decode_wire_payload(WireDecoder& decoder)
{
    ASSERT_CONTROL_THREAD();
    BiquadFilterGraphNode node;
    node.type = static_cast<BiquadFilterType>(TRY(decoder.read_u8()));
    node.frequency_hz = TRY(decoder.read_f32());
    node.detune_cents = TRY(decoder.read_f32());
    node.q = TRY(decoder.read_f32());
    node.gain_db = TRY(decoder.read_f32());
    node.channel_count = TRY(decoder.read_u32());
    node.channel_count_mode = static_cast<ChannelCountMode>(TRY(decoder.read_u8()));
    node.channel_interpretation = static_cast<ChannelInterpretation>(TRY(decoder.read_u8()));
    return node;
}

OwnPtr<RenderNode> BiquadFilterGraphNode::make_render_node(NodeID node_id, size_t quantum_size, GraphResourceResolver const&) const
{
    ASSERT_CONTROL_THREAD();
    return make<BiquadFilterRenderNode>(node_id, *this, quantum_size);
}

GraphUpdateKind BiquadFilterGraphNode::classify_update(BiquadFilterGraphNode const& new_desc) const
{
    ASSERT_CONTROL_THREAD();
    if (channel_count != new_desc.channel_count)
        return GraphUpdateKind::Topology;
    if (channel_count_mode != new_desc.channel_count_mode)
        return GraphUpdateKind::Topology;
    if (channel_interpretation != new_desc.channel_interpretation)
        return GraphUpdateKind::Topology;

    if (type != new_desc.type)
        return GraphUpdateKind::Parameter;
    if (frequency_hz != new_desc.frequency_hz)
        return GraphUpdateKind::Parameter;
    if (detune_cents != new_desc.detune_cents)
        return GraphUpdateKind::Parameter;
    if (q != new_desc.q)
        return GraphUpdateKind::Parameter;
    if (gain_db != new_desc.gain_db)
        return GraphUpdateKind::Parameter;

    return GraphUpdateKind::None;
}

}
