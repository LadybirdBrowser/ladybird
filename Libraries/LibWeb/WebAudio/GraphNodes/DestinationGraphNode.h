...
struct WEB_API DestinationGraphNode {
    size_t channel_count { 2 };

    ErrorOr<void> encode_wire_payload(WireEncoder&) const;
    static ErrorOr<DestinationGraphNode> decode_wire_payload(WireDecoder&);

    OwnPtr<RenderNode> make_render_node(NodeID, size_t quantum_size, GraphResourceResolver const&) const;
    GraphUpdateKind classify_update(DestinationGraphNode const& new_desc) const;
};
