...
{
    return current_thread_role() == WebAudioThreadRole::Control;
}

ALWAYS_INLINE bool current_thread_is_render_thread()
{
    return current_thread_role() == WebAudioThreadRole::Render;
}
...
ALWAYS_INLINE bool should_log_graph_updates()
{
    if (should_log_all())
        return true;
    static bool enabled = Core::Environment::has("WEBAUDIO_GRAPH_LOG"sv);
    return enabled;
}
