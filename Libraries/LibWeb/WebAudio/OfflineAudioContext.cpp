...
u32 OfflineAudioContext::quantum_aligned_frame_index_for_time(double time_seconds) const
{
    ...
    u32 const quantum = static_cast<u32>(BaseAudioContext::render_quantum_size());
...
void OfflineAudioContext::begin_offline_rendering(GC::Ref<WebIDL::Promise> promise)
{
    ...
    auto graph = Web::WebAudio::snapshot_render_graph(destination(), sample_rate(), &analyser_nodes, &audio_worklet_nodes, &script_processor_nodes, &resources);
    ...
    if (!audio_worklet_nodes.is_empty() || !script_processor_nodes.is_empty()) {
        ...
        m_worklet_render_state->graph = make<RenderGraph>(m_worklet_render_state->graph_description, static_cast<f32>(sample_rate()), &m_worklet_render_state->resources);
    ...
    *request.resources = move(resources);
    ...
    auto completion_pipe_fds = MUST(Core::System::pipe2(O_CLOEXEC | O_NONBLOCK));
    ...
    m_render_thread = make<Render::OfflineAudioRenderThread>(move(request), completion_write_fd, suspend_write_fd);
...
void OfflineAudioContext::render_worklet_step()
{
    ...
    u32 const quantum_size = static_cast<u32>(BaseAudioContext::render_quantum_size());
    ...
    auto dispatch_worklet_processor_errors = [this]() {
        ...
        for (auto& entry : errors) {
            ...
            queue_a_media_element_task(GC::create_function(heap(), [node, error_value = entry.error]() {
...
void OfflineAudioContext::handle_offline_render_completion()
{
    ...
    // Drain and tear down the completion signal immediately; this is a one-shot wakeup.
    if (m_render_completion_read_fd >= 0) {
        u8 buffer[64];
        for (;;) {
            auto nread_or_error = Core::System::read(m_render_completion_read_fd, { buffer, sizeof(buffer) });
            if (nread_or_error.is_error()) {
                auto code = nread_or_error.error().code();
                if (code == EAGAIN)
                    break;
            ...
            if (nread_or_error.value() == 0)
                break;
            if (nread_or_error.value() < sizeof(buffer))
                break;
        }
    }
    ...
    if (!render_result.has_value()) {
        ...
        queue_a_media_element_task(GC::create_function(heap(), [&realm, promise] {
            HTML::TemporaryExecutionContext context(realm, HTML::TemporaryExecutionContext::CallbacksEnabled::Yes);
...
void OfflineAudioContext::handle_offline_render_suspended(u32 frame_index)
{
    ...
    WA_GR_DBGLN("[WebAudio] offline suspended at frame={} worklet_state={} render_thread={}", frame_index, m_worklet_render_state ? 1 : 0, m_render_thread ? 1 : 0);
...
WebIDL::ExceptionOr<GC::Ref<WebIDL::Promise>> OfflineAudioContext::resume()
{
    ...
    if (!m_rendering_started || !m_is_suspended || (!m_render_thread && !m_worklet_render_state)) {
        WebIDL::reject_promise(realm, promise, WebIDL::InvalidStateError::create(realm, "OfflineAudioContext is not suspended"_utf16));
        return promise;
    }

    WA_GR_DBGLN("[WebAudio] offline resume requested (worklet_state={} render_thread={})", m_worklet_render_state ? 1 : 0, m_render_thread ? 1 : 0);
    ...
    if (m_worklet_render_state) {
        ...
        auto updated_graph = Web::WebAudio::snapshot_render_graph(destination(), sample_rate(), nullptr, nullptr, nullptr, &updated_resources);
