...
static void queue_offline_statechange_event(OfflineAudioContext& context, Bindings::AudioContextState expected_state)
{
    context.queue_a_media_element_task(GC::create_function(context.heap(), [self = GC::Ref { context }, expected_state]() {
        auto& realm = self->realm();
        HTML::TemporaryExecutionContext execution_context(realm, HTML::TemporaryExecutionContext::CallbacksEnabled::Yes);
        self->set_control_state(expected_state);
...
void OfflineAudioContext::finalize()
{
    Base::finalize();
...
void OfflineAudioContext::begin_offline_rendering(GC::Ref<WebIDL::Promise> promise)
{
    ...
    queue_offline_statechange_event(*this, Bindings::AudioContextState::Running);
...
void OfflineAudioContext::render_worklet_step()
{
    ...
    queue_a_media_element_task(GC::create_function(heap(), [&realm, promise, this]() {
        ...
        queue_offline_statechange_event(*this, Bindings::AudioContextState::Closed);
...
void OfflineAudioContext::handle_offline_render_suspended(u32 frame_index)
{
    ...
    for (auto& request : m_suspend_requests) {
        ...
        queue_offline_statechange_event(*this, Bindings::AudioContextState::Suspended);
        break;
    }
}
...
void OfflineAudioContext::dispatch_scheduled_source_end_event(AudioScheduledSourceNode& node)
{
    ...
    queue_a_media_element_task(GC::create_function(heap(), [node_ref] {
...
WebIDL::ExceptionOr<GC::Ref<WebIDL::Promise>> OfflineAudioContext::resume()
{
    ...
    if (m_worklet_render_state) {
        ...
        Render::GraphResources updated_resources;
...
void OfflineAudioContext::visit_edges(Cell::Visitor& visitor)
{
    ...
    for (auto const& suspend_request : m_suspend_requests)
        suspend_request.visit_edges(visitor);
