...
void XRSession::shut_down()
{
    ...
    for (auto promise : m_outstanding_promises) {
        WebIDL::reject_promise(promise, WebIDL::InvalidStateError::create("Session ended."_utf16));
    }
...
GC::Ref<WebIDL::Promise> XRSession::end()
{
    ...
    auto promise = WebIDL::create_promise_for(m_xr_system->relevant_global_object());

    // 2. If the ended value of this is true, reject promise with a "InvalidStateError" DOMException and return promise.
    if (ended())
        WebIDL::reject_promise(promise, WebIDL::InvalidStateError::create("Session already ended."_utf16));

    // 3. Shut down this.
    shut_down();

    // 4. Queue a task to perform the following steps:
    HTML::queue_a_task(HTML::Task::Source::Unspecified, nullptr, nullptr, GC::create_function(GC::Heap::the(), [promise]() {
