...
void XRSystem::is_session_supported(XRSessionMode mode, GC::Ref<WebIDL::Promise> promise) const
{
    WebIDL::resolve_promise(promise, JS::Value(is_session_mode_supported(mode)));
}

// https://immersive-web.github.io/webxr/#dom-xrsystem-requestsession
void XRSystem::request_session(XRSessionMode mode, XRSessionInit const& options, GC::Ref<WebIDL::Promise> promise)
{
    auto resolve = GC::create_function(GC::Heap::the(), [promise = GC::Root(promise)](GC::Ref<WebXR::XRSession> session) {
        auto& realm = WebIDL::promise_realm(*promise);
        HTML::TemporaryExecutionContext execution_context { realm, HTML::TemporaryExecutionContext::CallbacksEnabled::Yes };
        WebIDL::resolve_promise(*promise, Bindings::wrap(Bindings::host_defined_wrapper_world(realm), realm, session));
    });
    auto reject = GC::create_function(GC::Heap::the(), [promise = GC::Root(promise)](WebIDL::Exception exception) {
        auto& realm = WebIDL::promise_realm(*promise);
        HTML::TemporaryExecutionContext execution_context { realm, HTML::TemporaryExecutionContext::CallbacksEnabled::Yes };
        WebIDL::reject_promise_with_exception(*promise, move(exception));
    });
