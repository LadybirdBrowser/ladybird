...
static SnapAreaReference snap_area_reference_for(Paintable const& snap_area)
{
    auto const& layout_node = snap_area.layout_node();
    if (auto pseudo_element = layout_node.generated_for_pseudo_element(); pseudo_element.has_value())
        return { layout_node.pseudo_element_generator(), pseudo_element };
    return { as_if<DOM::Element>(snap_area.dom_node().ptr()), {} };
}

static CSS::ComputedValues const* computed_values_for_snap_container(Paintable const& snap_container)
{
    if (snap_container.is_viewport_paintable()) {
        auto const* document_element = snap_container.document().document_element();
        if (!document_element)
            return nullptr;
        return document_element->computed_values().ptr();
    }
    return &snap_container.computed_values();
}
...
static bool is_captured_by_snap_container(Paintable const& snap_area, Paintable const& snap_container)
{
    for (auto containing_block = snap_area.containing_block(); containing_block; containing_block = containing_block->containing_block()) {
        if (containing_block.ptr() == &snap_container)
            return true;
        ...
        if (containing_block->layout_node().is_scroll_container())
            return false;
    }
    return false;
}
...
static void for_each_descendant_snap_area(Paintable const& parent, Paintable const& snap_container, Callback const& callback)
{
    parent.for_each_child([&](Paintable const& child) {
        ...
        if (has_snap_alignment(child.computed_values().scroll_snap_align()) && is_captured_by_snap_container(child, snap_container))
            callback(child);
...
static CSSPixelRect snap_area_rect(Paintable const& snap_area, Paintable const& snap_container)
{
    ...
    auto rect = apply_css_transform_to_rect(snap_area, snap_area.absolute_border_box_rect());
    for (auto containing_block = snap_area.containing_block(); containing_block && containing_block.ptr() != &snap_container; containing_block = containing_block->containing_block())
        rect = apply_css_transform_to_rect(*containing_block, rect);

    auto const& scroll_margin = snap_area.computed_values().scroll_margin();
    rect.inflate(
...
static PhysicalSnapAlignment physical_snap_alignment(CSS::ScrollSnapAlignData alignment, CSS::ComputedValues const& writing_mode_source)
{
...
static Optional<SnapPositionCandidate> snap_position_candidate_for_axis(CSS::ScrollSnapAlign alignment, CSSPixels area_start, CSSPixels area_size, SnapAxisGeometry const& geometry)
{
    ...
    SnapPositionCandidate candidate { .offset = clamp(offset, CSSPixels { 0 }, geometry.max_offset), .covering_ranges = {} };

    if (area_size > geometry.snapport_size) {
        candidate.covering_ranges.append({
            .start = clamp(area_start - geometry.snapport_start, CSSPixels { 0 }, geometry.max_offset),
            .end = clamp(area_start + area_size - (geometry.snapport_start + geometry.snapport_size), CSSPixels { 0 }, geometry.max_offset),
        });
    }
...
SnapAxes snap_axes_of_scroll_container(Paintable const& snap_container)
{
    auto const* computed_values = computed_values_for_snap_container(snap_container);
    if (!computed_values)
        return {};

    auto snap_type = computed_values->scroll_snap_type();
    if (snap_type.strictness == CSS::ScrollSnapStrictness::None)
        return {};

    bool horizontal_writing_mode = computed_values->writing_mode() == CSS::WritingMode::HorizontalTb;
...
static Optional<SnapCandidateCollection> collect_snap_position_candidates(Paintable const& snap_container, bool collect_x, bool collect_y)
{
    auto const* computed_values = computed_values_for_snap_container(snap_container);
    if (!computed_values)
        return {};
    ...
    auto scrollable_overflow_rect = snap_container.scrollable_overflow_rect();
    ...
    auto snapport = snap_container.scroll_snapport_rect();

    auto max_scroll_offset = snap_container.maximum_scroll_offset();
    ...
    for_each_descendant_snap_area(snap_container, snap_container, [&](Paintable const& snap_area) {
        auto const& area_computed_values = snap_area.computed_values();
        auto always_stop = area_computed_values.scroll_snap_stop() == CSS::ScrollSnapStop::Always;
        ...
        bool area_is_larger_than_snapport = area_computed_values.writing_mode() == CSS::WritingMode::HorizontalTb
            ? area_rect.width() > snapport.width()
            : area_rect.height() > snapport.height();
        auto alignment = physical_snap_alignment(area_computed_values.scroll_snap_align(), area_is_larger_than_snapport ? area_computed_values : *computed_values);
...
bool is_scroll_snap_container(Paintable const& paintable)
{
    if (!paintable.layout_node().is_scroll_container())
        return false;
    return !snap_axes_of_scroll_container(paintable).is_empty();
}

// https://drafts.csswg.org/css-scroll-snap-1/#choosing
SnapDestination adjust_scroll_destination_for_snapping(Paintable const& snap_container, CSSPixelPoint destination, SnapSelectionStrategy const& strategy)
{
...
SnapDestination select_resnap_destination(Paintable const& snap_container, CSSPixelPoint current_offset, ResnapSelection const& selection)
{
    auto snap_axes = snap_axes_of_scroll_container(snap_container);
    if (snap_axes.is_empty())
    ...
    if (x_offset.has_value() && y_offset.has_value() && !chosen_offsets_are_mutually_visible(candidates, *x_offset, *y_offset)) {
        auto const* computed_values = computed_values_for_snap_container(snap_container);
        bool block_axis_is_y = computed_values->writing_mode() == CSS::WritingMode::HorizontalTb;
