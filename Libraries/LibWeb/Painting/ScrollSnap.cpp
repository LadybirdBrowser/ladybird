...
static SnapAreaReference snap_area_reference_for(Paintable const& snap_area)
{
    ...
    if (auto pseudo_element = layout_node.generated_for_pseudo_element(); pseudo_element.has_value())
        return { layout_node.pseudo_element_generator().ptr(), pseudo_element };
    return { as_if<DOM::Element>(snap_area.dom_node().ptr()), {} };
}

static Layout::NodeWithStyle const* style_source_for_snap_container(Paintable const& snap_container)
{
    if (snap_container.layout_node().is_viewport()) {
        auto const* document_element = snap_container.document().document_element();
        if (!document_element)
            return nullptr;
        return document_element->unsafe_layout_node();
    }
    return &snap_container.layout_node();
}
...
static void for_each_descendant_snap_area(Paintable const& parent, Paintable const& snap_container, Callback const& callback)
{
    parent.for_each_child([&](Paintable const& child) {
        ...
        if (has_snap_alignment(child.layout_node().scroll_snap_align()) && is_captured_by_snap_container(child, snap_container))
            callback(child);
...
static CSSPixelRect snap_area_rect(Paintable const& snap_area, Paintable const& snap_container)
{
    ...
    auto const& scroll_margin = snap_area.layout_node().scroll_margin();
    rect.inflate(
...
static Optional<SnapCandidateCollection> collect_snap_position_candidates(Paintable const& snap_container, bool collect_x, bool collect_y)
{
    ...
    auto min_scroll_offset = snap_container.minimum_scroll_offset();
    ...
    for_each_descendant_snap_area(snap_container, snap_container, [&](Paintable const& snap_area) {
        auto const& area_layout_node = snap_area.layout_node();
        auto always_stop = area_layout_node.scroll_snap_stop() == CSS::ScrollSnapStop::Always;
        ...
        bool area_is_larger_than_snapport = area_layout_node.writing_mode() == CSS::WritingMode::HorizontalTb
            ? area_rect.width() > snapport.width()
            : area_rect.height() > snapport.height();
        auto alignment = physical_snap_alignment(area_layout_node.scroll_snap_align(), area_is_larger_than_snapport ? area_layout_node : *style_source);
