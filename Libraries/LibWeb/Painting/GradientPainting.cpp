    ...
    auto resolved_color_stops = resolve_color_stop_positions(
        node, linear_gradient.color_stop_list(), [&](auto const& position) -> float {
            if (position.is_length())
                return position.as_length().length().to_px(*context.length_resolution_context) / gradient_length_px;
            if (position.is_percentage())
                return position.as_percentage().percentage().as_fraction();
            return position.as_calculated().resolve_length(context)->to_px(*context.length_resolution_context) / gradient_length_px;
        },
...
ConicGradientData resolve_conic_gradient_data(Layout::NodeWithStyle const& node, CSS::ConicGradientStyleValue const& conic_gradient)
{
    ...
    auto from_angle = conic_gradient.angle_degrees(context);

    CSS::Angle const one_turn { 360.0f, CSS::AngleUnit::Deg };
    context.percentage_basis = one_turn;
    auto resolved_color_stops = resolve_color_stop_positions(
    ...
    return { from_angle, resolved_color_stops, conic_gradient.interpolation_method() };
}
...
RadialGradientData resolve_radial_gradient_data(Layout::NodeWithStyle const& node, CSSPixelSize gradient_size, CSS::RadialGradientStyleValue const& radial_gradient)
{
    ...
    auto resolved_color_stops = resolve_color_stop_positions(
        node, radial_gradient.color_stop_list(), [&](auto const& position) -> float {
            if (position.is_length())
                return position.as_length().length().to_px(*context.length_resolution_context) / gradient_size.width().to_float();
            if (position.is_percentage())
                return position.as_percentage().percentage().as_fraction();
            return position.as_calculated().resolve_length(context)->to_px(*context.length_resolution_context) / gradient_size.width().to_float();
        },
