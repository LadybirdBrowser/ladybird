...
LengthPercentage ComputedProperties::length_percentage_or_fallback(PropertyID id, Layout::NodeWithStyle const& layout_node, DisallowNegativeLengths disallow_negative_lengths, LengthPercentage const& fallback) const
{
    return length_percentage(id, layout_node, disallow_negative_lengths).value_or(fallback);
}

Optional<LengthPercentage> ComputedProperties::length_percentage(PropertyID id, Layout::NodeWithStyle const& layout_node, DisallowNegativeLengths disallow_negative_lengths) const
{
    auto const& value = property(id);
    ...
    if (value.is_percentage()) {
        ...
        //        once we do that.
        if (disallow_negative_lengths == DisallowNegativeLengths::Yes && percentage.as_fraction() < 0)
            return {};
    ...
    if (value.is_length()) {
        ...
        // FIXME: This value can be negative as interpolation does not yet clamp values to allowed ranges - remove this
        //        once we do that.
        if (disallow_negative_lengths == DisallowNegativeLengths::Yes && length.to_px(layout_node) < 0)
            return {};
...
LengthBox ComputedProperties::length_box(PropertyID left_id, PropertyID top_id, PropertyID right_id, PropertyID bottom_id, Layout::NodeWithStyle const& layout_node, DisallowNegativeLengths disallow_negative_lengths, Length const& default_value) const
{
    LengthBox box;
