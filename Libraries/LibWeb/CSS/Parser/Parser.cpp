...
RefPtr<PositionStyleValue> Parser::parse_position_value(TokenStream<ComponentValue>& tokens, PositionParsingMode position_parsing_mode)
{
    ...
    auto make_edge_style_value = [](PositionEdge position_edge) -> NonnullRefPtr<EdgeStyleValue> {
        return EdgeStyleValue::create(position_edge, {});
    };
    ...
    auto alternative_1 = [&]() -> RefPtr<PositionStyleValue> {
        ...
        if (auto maybe_edge = parse_position_edge(token); maybe_edge.has_value()) {
            ...
            if (is_horizontal(edge, false))
                return PositionStyleValue::create(make_edge_style_value(edge), make_edge_style_value(PositionEdge::Center));

            // [ top | bottom ]
            if (is_vertical(edge, false))
                return PositionStyleValue::create(make_edge_style_value(PositionEdge::Center), make_edge_style_value(edge));

            // [ center ]
            VERIFY(edge == PositionEdge::Center);
            return PositionStyleValue::create(make_edge_style_value(PositionEdge::Center), make_edge_style_value(PositionEdge::Center));
        }

        // [ <length-percentage> ]
        if (auto maybe_percentage = parse_length_percentage(token); maybe_percentage.has_value()) {
            transaction.commit();
            return PositionStyleValue::create(EdgeStyleValue::create({}, *maybe_percentage), make_edge_style_value(PositionEdge::Center));
        }
    ...
    auto alternative_2 = [&]() -> RefPtr<PositionStyleValue> {
        ...
        if (is_horizontal(first_edge, true) && is_vertical(second_edge, true)) {
            transaction.commit();
            return PositionStyleValue::create(make_edge_style_value(first_edge), make_edge_style_value(second_edge));
        }
    ...
    auto alternative_3 = [&]() -> RefPtr<PositionStyleValue> {
        ...
        auto parse_position_or_length = [&](bool as_horizontal) -> RefPtr<EdgeStyleValue> {
            ...
            if (auto maybe_position = parse_position_edge(token); maybe_position.has_value()) {
                ...
                if (!valid)
                    return nullptr;
                return make_edge_style_value(position);
            }
