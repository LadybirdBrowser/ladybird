...
RefPtr<RadialGradientStyleValue const> Parser::parse_radial_gradient_function(TokenStream<ComponentValue>& outer_tokens)
{
    ...
    auto parse_size = [&]() -> Optional<Size> {
        ...
        if (first_radius->is_length() || (first_radius->is_calculated() && first_radius->as_calculated().resolves_to_length()))
            return commit_value(CircleSize { first_radius->as_length() }, transaction_size);
        return {};
    };
