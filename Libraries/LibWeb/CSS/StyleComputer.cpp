...
CSS::RequiredInvalidationAfterStyleChange StyleComputer::update_element_inherited_computed_style(Web::DOM::Element& element)
{
    ...
    for (auto i = to_underlying(CSS::first_longhand_property_id); i <= to_underlying(CSS::last_longhand_property_id); ++i) {
        ...
        RefPtr<CSS::StyleValue const> new_animated_value = CSS::StyleComputer::get_animated_inherit_value(property_id, { element }).map([&](auto& value) { return value.ptr(); }).value_or({});
        ...
        RefPtr new_value = get_inherit_value(property_id, { element });
