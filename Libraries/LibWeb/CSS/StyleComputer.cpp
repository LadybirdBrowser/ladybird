            ...
            for_each_property_expanding_shorthands(property_id, *style_value, [&](PropertyID longhand_id, StyleValue const& longhand_value) {
                ...
                auto const& specified_value_with_css_wide_keywords_applied = [&]() -> StyleValue const& {
                    if (longhand_value.is_inherit() || (longhand_value.is_unset() && is_inherited_property(longhand_id))) {
                        ...
                        return get_non_animated_inherit_value(longhand_id, &element);
                    }
...
NonnullRefPtr<StyleValue const> StyleComputer::get_non_animated_inherit_value(CSS::PropertyID property_id, DOM::Element const* element, Optional<CSS::PseudoElement> pseudo_element)
{
...
{
    // FIXME: We round to int here as that is what is expected by our font infrastructure below expects
    auto width = round_to<int>(font_width.value());

    // FIXME: We round to int here as that is what is expected by our font infrastructure below expects
    // FIXME: This can end up negative due to interpolation - we can remove the max() once we correctly handle
    //        interpolation clamping
    auto weight = round_to<int>(max(0.0, font_weight));

    // FIXME: Implement the full font-matching algorithm: https://www.w3.org/TR/css-fonts-4/#font-matching-algorithm

    // FIXME: Font size can end up being negative due to interpolation - we can remove this max() once we correctly
    //        handle interpolation clamping.
    float const font_size_in_pt = max(0.0f, font_size * 0.75f);
...
{
    auto const& inheritance_parent = element.has_value() ? element->element_to_inherit_style_from() : OptionalNone {};
...
GC::Ref<ComputedProperties> StyleComputer::compute_properties(DOM::Element& element, Optional<PseudoElement> pseudo_element, CascadedProperties& cascaded_properties) const
{
    ...
    for (auto i = to_underlying(first_longhand_property_id); i <= to_underlying(last_longhand_property_id); ++i) {
        ...
        if (should_inherit) {
            value = get_non_animated_inherit_value(property_id, &element, pseudo_element);
...
NonnullRefPtr<StyleValue const> StyleComputer::compute_font_size(NonnullRefPtr<StyleValue const> const& specified_value, int computed_math_depth, CSSPixels inherited_font_size, int inherited_math_depth, Length::ResolutionContext const& parent_length_resolution_context)
{
    ...
    if (specified_value->is_calculated())
        return LengthStyleValue::create(specified_value->as_calculated().resolve_length({ .percentage_basis = Length(1, Length::Type::Em), .length_resolution_context = parent_length_resolution_context }).value());
...
NonnullRefPtr<StyleValue const> StyleComputer::compute_font_width(NonnullRefPtr<StyleValue const> const& specified_value, Length::ResolutionContext const& parent_length_resolution_context)
{
    ...
    // ultra-condensed 50%
    if (specified_value->to_keyword() == Keyword::UltraCondensed)
        return PercentageStyleValue::create(Percentage(50));

    // extra-condensed 62.5%
    if (specified_value->to_keyword() == Keyword::ExtraCondensed)
        return PercentageStyleValue::create(Percentage(62.5));

    // condensed 75%
    if (specified_value->to_keyword() == Keyword::Condensed)
        return PercentageStyleValue::create(Percentage(75));

    // semi-condensed 87.5%
    if (specified_value->to_keyword() == Keyword::SemiCondensed)
    ...
    // semi-expanded 112.5%
    if (specified_value->to_keyword() == Keyword::SemiExpanded)
        return PercentageStyleValue::create(Percentage(112.5));

    // expanded 125%
    if (specified_value->to_keyword() == Keyword::Expanded)
        return PercentageStyleValue::create(Percentage(125));

    // extra-expanded 150%
    if (specified_value->to_keyword() == Keyword::ExtraExpanded)
        return PercentageStyleValue::create(Percentage(150));

    // ultra-expanded 200%
    if (specified_value->to_keyword() == Keyword::UltraExpanded)
