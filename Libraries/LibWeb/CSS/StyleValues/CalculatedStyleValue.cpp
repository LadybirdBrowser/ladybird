...
NonnullRefPtr<CalculationNode const> RandomCalculationNode::with_simplified_children(CalculationContext const& context, CalculationResolutionContext const& resolution_context) const
{
    ...
    return RandomCalculationNode::create(simplified_random_value_sharing.release_nonnull(), simplified_minimum, simplified_maximum, simplified_step);
}
...
{
    // NOTE: We don't want to resolve this before computation time even if it's possible
    ...
    // Spec Note: As usual for math functions, if any argument calculation is NaN, the result is NaN.
