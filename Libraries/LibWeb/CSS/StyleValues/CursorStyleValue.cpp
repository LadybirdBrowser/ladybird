...
Optional<Gfx::ImageCursor> CursorStyleValue::make_image_cursor(Layout::NodeWithStyle const& layout_node) const
{
    ...
    auto const gfx_image_orientation = to_gfx_image_orientation(image_orientation);
    ...
    if (!m_cached_bitmap.has_value()) {
        ...
        auto cursor_css_size = run_default_sizing_algorithm({}, {}, image.intrinsic_width(gfx_image_orientation), image.intrinsic_height(gfx_image_orientation), image.intrinsic_aspect_ratio(gfx_image_orientation), default_cursor_size);
    ...
    if (m_cache_key != cache_key) {
        ...
        image.resolve_for_size(layout_node, CSSPixelSize { bitmap.size(gfx_image_orientation) });
        image.paint(paint_context, DevicePixelRect { bitmap.rect(gfx_image_orientation) }, ImageRendering::Auto, image_orientation);
