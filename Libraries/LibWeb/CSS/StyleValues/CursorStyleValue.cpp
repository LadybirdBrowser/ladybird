...
Optional<Gfx::ImageCursor> CursorStyleValue::make_image_cursor(Layout::NodeWithStyle const& layout_node) const
{
    ...
    if (!m_cached_bitmap.has_value()) {
        ...
        auto cursor_css_size = run_default_sizing_algorithm({}, {}, image.natural_width(image_orientation), image.natural_height(image_orientation), image.natural_aspect_ratio(image_orientation), default_cursor_size);
