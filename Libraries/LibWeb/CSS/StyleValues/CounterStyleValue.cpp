...
String CounterStyleValue::resolve(DOM::Element& element, bool& return_element_needs_reversed_counters_fixup) const
{
    ...
    auto generate_counter_string = [&](Counter const& counter) {
        auto* originating_node = DOM::Node::from_unique_id(counter.originating_element_id);
        VERIFY(originating_node);
        auto& originating_element = as<DOM::Element>(*originating_node);
        VERIFY(originating_element.counters_set().has_value());
        auto const& originating_counter = originating_element.counters_set().value().last_counter_with_name(m_properties.counter_name);
        VERIFY(originating_counter.has_value());

        if (originating_counter.value().reversed) {
            VERIFY(originating_counter.value().value.has_value());
            auto const& originating_counter_value = originating_counter.value().value.value().value();
