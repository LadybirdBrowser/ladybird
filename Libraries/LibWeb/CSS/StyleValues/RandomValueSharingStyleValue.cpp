...
ValueComparingNonnullRefPtr<StyleValue const> RandomValueSharingStyleValue::absolutized(ComputationContext const& computation_context) const
{
    ...
    DOM::Document::RandomCachingKey random_caching_key {
        ...
        // NOTE: This is implicit since the cache is stored on the document
    };

    auto random_base_value = computation_context.abstract_element->document().ensure_cached_css_random_base_value(random_caching_key);
