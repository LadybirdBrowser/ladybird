...
String ConicGradientStyleValue::to_string(SerializationMode mode) const
{
    ...
    bool has_color_space = m_properties.interpolation_method.has_value() && m_properties.interpolation_method.value().color_space != InterpolationMethod::default_color_space(m_properties.legacy_syntax);
    ...
    if (has_color_space) {
        if (has_from_angle || has_at_position)
            builder.append(' ');
        builder.appendff("{}"sv, m_properties.interpolation_method.value().to_string());
    }
