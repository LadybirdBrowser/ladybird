...
public:
    ...
    static ValueComparingNonnullRefPtr<LinearGradientStyleValue> create(GradientDirection direction, Vector<LinearColorStopListElement> color_stop_list, GradientType type, GradientRepeating repeating, Optional<InterpolationMethod> interpolation_method)
    {
        ...
        bool any_non_legacy = color_stop_list.find_first_index_if([](auto const& stop) { return !stop.color_stop.color->is_keyword() && stop.color_stop.color->as_color().legacy_syntax() == LegacySyntax::No; }).has_value();
        return adopt_ref(*new (nothrow) LinearGradientStyleValue(direction, move(color_stop_list), type, repeating, interpolation_method, any_non_legacy ? LegacySyntax::No : LegacySyntax::Yes));
    }
    ...
    InterpolationMethod interpolation_method() const
    {
        ...
        return InterpolationMethod { .color_space = InterpolationMethod::default_color_space(m_properties.legacy_syntax) };
    }
...
private:
    LinearGradientStyleValue(GradientDirection direction, Vector<LinearColorStopListElement> color_stop_list, GradientType type, GradientRepeating repeating, Optional<InterpolationMethod> interpolation_method, LegacySyntax legacy_syntax)
        : AbstractImageStyleValue(Type::LinearGradient)
        , m_properties { .direction = direction, .color_stop_list = move(color_stop_list), .gradient_type = type, .repeating = repeating, .interpolation_method = interpolation_method, .legacy_syntax = legacy_syntax }
    {
    }
