...
void CSSCounterStyleRule::set_system(FlyString const& system)
{
    ...
    // NB: Since we only allow changing parameters of the of the system, not the algorithm itself (see below), we know
    //     this change can't cause the @counter-style to not define a counter style.
    if (!value)
        return;
