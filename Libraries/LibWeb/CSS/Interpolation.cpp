...
Vector<FilterValue> accumulate_filter_function(FilterValueListStyleValue const& underlying_list, FilterValueListStyleValue const& animated_list)
{
    ...
    auto accumulate_filter = [](FilterValue const& underlying, FilterValue const& animated) -> Optional<FilterValue> {
        return underlying.visit(
            [&](FilterOperation::Blur const& underlying_blur) -> Optional<FilterValue> {
                ...
                return FilterOperation::Blur { .radius = Length { underlying_px + animated_px, underlying_blur.radius.value().unit() } };
            },
            [&](FilterOperation::HueRotate const& underlying_rotate) -> Optional<FilterValue> {
                ...
                auto get_angle_value = [](FilterOperation::HueRotate::AngleOrZero const& angle_or_zero) -> Optional<double> {
                ...
                auto underlying_deg = get_angle_value(underlying_rotate.angle);
                auto animated_deg = get_angle_value(animated_rotate.angle);
                if (!underlying_deg.has_value() || !animated_deg.has_value())
                    return {};

                auto get_unit = [](FilterOperation::HueRotate::AngleOrZero const& angle_or_zero) {
                    return angle_or_zero.visit(
                        [](AngleOrCalculated const& angle) { return angle.is_calculated() ? AngleUnit::Deg : angle.value().unit(); },
                        [](FilterOperation::HueRotate::Zero) { return AngleUnit::Deg; });
                };

                return FilterOperation::HueRotate { .angle = AngleOrCalculated { Angle { *underlying_deg + *animated_deg, get_unit(underlying_rotate.angle) } } };
            },
            ...
            [&](FilterOperation::DropShadow const& underlying_shadow) -> Optional<FilterValue> {
                ...
                auto add_lengths = [](LengthOrCalculated const& a, LengthOrCalculated const& b) -> Optional<LengthOrCalculated> {
                    if (a.is_calculated() || b.is_calculated())
                        return {};
                    return LengthOrCalculated { Length { a.value().raw_value() + b.value().raw_value(), a.value().unit() } };
                };
                ...
                if (underlying_shadow.color && animated_shadow.color) {
                    ...
                    if (underlying_color.has_value() && animated_color.has_value()) {
                        ...
                        accumulated_color = ColorStyleValue::create_from_color(accumulated, ColorSyntax::Modern);
                    }
                }
    ...
    for (size_t i = 0; i < max_size; ++i) {
        ...
        if (!accumulated.has_value())
            return {};
        result.append(accumulated.release_value());
    }
    return result;
}
...
RefPtr<StyleValue const> composite_value(StyleValue const& underlying_value, StyleValue const& animated_value, Bindings::CompositeOperation composite_operation)
{
    ...
    case StyleValue::Type::FilterValueList: {
        ...
        if (composite_operation == Bindings::CompositeOperation::Add) {
            Vector<FilterValue> result;
            result.ensure_capacity(underlying_list.size() + animated_list.size());
            for (auto const& filter : underlying_list.filter_value_list())
                result.unchecked_append(filter);
            for (auto const& filter : animated_list.filter_value_list())
                result.unchecked_append(filter);
