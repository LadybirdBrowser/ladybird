    ...
    StyleValue const& from, StyleValue const& to, float delta, AllowDiscrete)
{
    ...
    auto style_value_to_transformations = [](StyleValue const& style_value)
        ...
        // LB-NOTE: This encompasses both the allowed value "none" and any invalid values.
        ...
        for (auto const& value : style_value.as_value_list().values()) {
            if (value->is_transformation())
                result.append(value->as_transformation());
        }
        return result;
    };
    ...
    auto convert_3d_transform_to_primitive = [&](NonnullRefPtr<TransformationStyleValue const> transform)
        -> NonnullRefPtr<TransformationStyleValue const> {
        // LB-NOTE: Convert to 2D primitive if possible so we don't have to deal with scale/translate X/Y separately.
    ...
    for (; index < from_transformations.size(); ++index) {
        ...
        // FIXME: perspective(none) needs special handling.
        ...
        if (is_2d_transform(from_function) && is_2d_transform(to_function)) {
            ...
            // LB-NOTE: 3D primitives do not need value expansion like their 2D counterparts do, so we check if they are
            //          already a primitive first.
        ...
        } else if (from_function != to_function) {
            // LB-NOTE: We failed to convert both arguments to their primitives. This happens when trying to
            //          interpolate two functions such as matrix() and translate() that cannot be interpolated
            //          between numerically. Break here so the next loop can take care of the remaining functions.
            break;
        }
        from_function = from_transformation->transform_function();
        to_function = to_transformation->transform_function();

        // LB-NOTE: We converted both functions to their primitives. But if they're different primitives or if they have
        //          a different number of values, we can't interpolate numerically between them. Break here so the next
        //          loop can take care of the remaining functions.
        ...
        if (from_function == TransformFunction::Rotate3d) {
            ...
            if (!interpolated_rotation)
                break;
            result.append(*interpolated_rotation);
        } else {
            ...
            if (interpolated.size() != from_values.size())
                break;
            result.append(TransformationStyleValue::create(PropertyID::Transform, from_function, move(interpolated)));
        }
    }
    ...
    if (maybe_interpolated_matrix.has_value()) {
        ...
        for (int i = 0; i < 16; i++)
            values.append(NumberStyleValue::create(interpolated_matrix[i % 4, i / 4]));
