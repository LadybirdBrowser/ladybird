...
static RefPtr<StyleValue const> interpolate_value_impl(DOM::Element& element, CalculationContext const& calculation_context, StyleValue const& from, StyleValue const& to, float delta, AllowDiscrete allow_discrete)
{
    ...
    case StyleValue::Type::BorderRadius: {
        ...
        auto const interpolated_horizontal_radius = interpolate_length_percentage(from_horizontal_radius, to_horizontal_radius, delta);
        auto const interpolated_vertical_radius = interpolate_length_percentage(from_vertical_radius, to_vertical_radius, delta);
