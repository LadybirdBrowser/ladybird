...
using Web::CSS::Parser::SyntaxNode;
...
WebIDL::ExceptionOr<void> register_property(JS::VM& vm, PropertyDefinition definition)
{
    ...
    // 3. If property set already contains an entry with name as its property name (compared codepoint-wise),
    //    throw an InvalidModificationError and exit this algorithm.
    if (document.registered_custom_properties().get(definition.name).has_value())
        return WebIDL::InvalidModificationError::create(realm, "Property already registered"_string);

    // 4. Attempt to consume a syntax definition from syntax. If it returns failure, throw a SyntaxError.
    //    Otherwise, let syntax definition be the returned syntax definition.
    auto syntax_component_values = parse_component_values_list(CSS::Parser::ParsingParams {}, definition.syntax);
    ...
    // 4. Parse initialValue according to the following rules:
    ...
    if (maybe_syntax->type() == Parser::SyntaxNode::NodeType::Universal) {
        if (!definition.initial_value.has_value()) {
            // If initialValue is not present, let parsed initial value be empty.
            ...
            initial_value_maybe = parse_css_value(CSS::Parser::ParsingParams {}, definition.initial_value.value(), PropertyID::Custom);
