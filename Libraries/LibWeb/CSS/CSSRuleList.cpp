...
WebIDL::ExceptionOr<unsigned> CSSRuleList::insert_a_css_rule(JS::Realm& realm, Variant<StringView, CSSRule*> rule, u32 index, Nested nested, HashTable<FlyString> const& declared_namespaces)
{
...
WebIDL::ExceptionOr<void> CSSRuleList::remove_a_css_rule(JS::Realm& realm, u32 index)
{
    // 1. Set length to the number of items in list.
    auto length = m_rules.size();
...
Optional<JS::Value> CSSRuleList::item_value(Bindings::WrapperWorld& wrapper_world, JS::Realm& realm, size_t index) const
{
    if (index >= m_rules.size())
        return {};
    return Bindings::wrap(wrapper_world, realm, m_rules[index]);
}
