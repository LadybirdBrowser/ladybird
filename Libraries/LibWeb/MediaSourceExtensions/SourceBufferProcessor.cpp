...
void SourceBufferProcessor::run_coded_frame_eviction(size_t new_data_size, AK::Duration current_time)
{
    ...
    while (bytes_evicted < bytes_to_evict) {
        ...
        auto evicted_frame = latest_track_buffer->demuxer().take_latest_frame();
        bytes_evicted += evicted_frame.byte_count;
        ...
        if (last_decode_timestamp.has_value() && evicted_frame.decode_timestamp == last_decode_timestamp.value()) {
            // -> If mode equals "segments":
            if (m_mode == AppendMode::Segments) {
                // Set [[group end timestamp]] to presentation timestamp.
                m_group_end_timestamp = evicted_frame.presentation_timestamp;
            }
            // -> If mode equals "sequence":
