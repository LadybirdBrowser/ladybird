...
{
    auto& global_object = this->relevant_global_object();
    set_wrapper_realm_hint(global_object);
    m_source_buffers->set_wrapper_realm_hint(global_object);
    m_active_source_buffers->set_wrapper_realm_hint(global_object);
}
...
WebIDL::ExceptionOr<GC::Ref<SourceBuffer>> MediaSource::add_source_buffer(Utf16String const& type)
{
    ...
    buffer->set_wrapper_realm_hint(relevant_global_object());
