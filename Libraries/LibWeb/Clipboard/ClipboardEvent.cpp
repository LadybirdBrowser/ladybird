...
GC::Ref<ClipboardEvent> ClipboardEvent::create(Utf16FlyString const& event_name, ClipboardEventInit const& event_init, HighResolutionTime::DOMHighResTimeStamp time_stamp)
{
    return GC::Heap::the().allocate<ClipboardEvent>(event_name, event_init, time_stamp);
}
...
ClipboardEvent::ClipboardEvent(Utf16FlyString const& event_name, ClipboardEventInit const& event_init, HighResolutionTime::DOMHighResTimeStamp time_stamp)
    : DOM::Event(event_name, event_init, time_stamp)
    , m_clipboard_data(event_init.clipboard_data)
{
}
