...
template<typename T>
SVGList<T>::SVGList(Vector<T> items, ReadOnlyList read_only)
    : m_items(move(items))
...
template<typename T>
SVGList<T>::SVGList(ReadOnlyList read_only)
    : m_read_only(read_only)
{
}
...
WebIDL::ExceptionOr<void> SVGList<T>::clear()
{
    ...
    if (m_read_only == ReadOnlyList::Yes)
        return WebIDL::NoModificationAllowedError::create(svg_list_realm(), "Cannot modify a read-only list"_utf16);
...
WebIDL::ExceptionOr<T> SVGList<T>::get_item(WebIDL::UnsignedLong index)
{
    ...
    if (index >= m_items.size())
        return WebIDL::IndexSizeError::create(svg_list_realm(), "List index out of bounds"_utf16);
