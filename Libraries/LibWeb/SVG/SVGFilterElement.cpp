...
Optional<Gfx::Filter> SVGFilterElement::gfx_filter(Layout::NodeWithStyle const& referenced_node)
{
    ...
    for_each_child_of_type<DOM::Element>([&](auto& node) {
        if (auto* flood_primitive = as_if<SVGFEFloodElement>(node)) {
            ...
            auto width = image_primitive->width()->base_val()->value();
            if (width == 0)
                width = bitmap->width();

            auto height = image_primitive->height()->base_val()->value();
            if (height == 0)
                height = bitmap->height();

            Gfx::IntRect src_rect {
                image_primitive->x()->base_val()->value(),
                image_primitive->y()->base_val()->value(),
                width,
                height
            };
            ...
            auto scaling_mode = to_gfx_scaling_mode(paintable_box->computed_values().image_rendering(), src_rect, dest_rect);
