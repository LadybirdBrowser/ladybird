...
Gfx::Filter SVGFilterElement::gfx_filter()
{
    ...
    for_each_child([&](auto& node) {
        if (is<SVGFEFloodElement>(node)) {
            ...
            // FIXME: There is no guarantee at the time that the layout node for the filter primitive exits when
            // FIXME: the filter gets resolved by url. When there will be such guarantees, it might be a better idea
            // FIXME: to invalidate the whole filter if layout nodes are missing instead of using fallback values.
            if (flood_primitive.flood_color().has_value() && flood_primitive.flood_opacity().has_value())
                root_filter = Gfx::Filter::flood(*flood_primitive.flood_color(), *flood_primitive.flood_opacity());
            else
                root_filter = Gfx::Filter::flood(Color::Black, 0.0f);
