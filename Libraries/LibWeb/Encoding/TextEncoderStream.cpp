...
WebIDL::ExceptionOr<GC::Ref<TextEncoderStream>> TextEncoderStream::create_for_constructor(JS::Object& relevant_global_object)
{
    ...
    auto transform_algorithm = GC::create_function(GC::Heap::the(), [stream, relevant_global_object = GC::Ref(relevant_global_object), realm = GC::Ref(realm)](JS::Value chunk) -> GC::Ref<WebIDL::Promise> {
        if (auto result = stream->encode_and_enqueue_chunk(*relevant_global_object, chunk); result.is_error())
    ...
    auto flush_algorithm = GC::create_function(GC::Heap::the(), [stream, relevant_global_object = GC::Ref(relevant_global_object), realm = GC::Ref(realm)]() -> GC::Ref<WebIDL::Promise> {
        if (auto result = stream->encode_and_flush(*relevant_global_object); result.is_error())
            return WebIDL::create_rejected_promise_from_exception(realm, result.release_error());
...
WebIDL::ExceptionOr<void> TextEncoderStream::encode_and_enqueue_chunk(JS::Object& relevant_global_object, JS::Value chunk)
{
    ...
    Utf16String input;
    {
        ...
        input = TRY(chunk.to_utf16_string(vm));
    }
...
WebIDL::ExceptionOr<void> TextEncoderStream::encode_and_flush(JS::Object& relevant_global_object)
{
    auto& realm = HTML::relevant_realm(relevant_global_object);
