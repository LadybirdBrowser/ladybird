...
WebIDL::ExceptionOr<u64> Table::grow(u64 delta, Wasm::Reference reference)
{
    auto* table = cache().abstract_machine().store().get(address());
    if (!table)
        return WebIDL::SimpleException { WebIDL::SimpleExceptionType::RangeError, "Could not find the memory table to grow"_utf16 };
...
WebIDL::ExceptionOr<Wasm::Reference> Table::get(u64 index) const
{
    auto* table = cache().abstract_machine().store().get(address());
    if (!table)
        return WebIDL::SimpleException { WebIDL::SimpleExceptionType::RangeError, "Could not find the memory table"_utf16 };
    ...
    if (element_type.kind() == Wasm::ValueType::ExceptionReference)
        return WebIDL::SimpleException { WebIDL::SimpleExceptionType::TypeError, "Cannot get an exnref table element"_utf16 };
    ...
    if (table->elements().size() <= index)
        return WebIDL::SimpleException { WebIDL::SimpleExceptionType::RangeError, "Table element index out of range"_utf16 };
...
WebIDL::ExceptionOr<void> Table::set(u64 index, Wasm::Reference reference)
{
    ...
    // 4. If elementtype matches exnref, throw a TypeError exception.
    if (element_type.kind() == Wasm::ValueType::ExceptionReference)
        return WebIDL::SimpleException { WebIDL::SimpleExceptionType::TypeError, "Cannot set an exnref table element"_utf16 };
