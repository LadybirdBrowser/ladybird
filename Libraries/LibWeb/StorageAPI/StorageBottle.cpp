...
String storage_cache_key(StorageEndpointType endpoint_type, Web::PageId page_id, String const& storage_key)
{
    // Bottles are made one per document, so every bottle for a storage key has to find the same map. Session storage is
    // partitioned further — by the page's top-level traversable, which the owner resolves from the page. So, a session
    // bottle names its page as well.
    if (endpoint_type == StorageEndpointType::SessionStorage)
        return MUST(String::formatted("{}/{}", page_id, storage_key));
    return storage_key;
}
...
NonnullRefPtr<CachedStorageMap> cached_storage_map(StorageEndpointType endpoint_type, String const& cache_key)
{
    ...
    // page, and is never asked for again once that page is gone — so without this, every page ever closed would leave
    // one behind. Sweep them when a new map is made — which is the only moment this registry grows.
