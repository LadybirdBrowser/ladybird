...
String storage_cache_key(StorageEndpointType, Web::PageId page_id, String const& storage_key);
...
protected:
    StorageBottle(Optional<u64> quota, StorageEndpointType endpoint_type, Web::PageId page_id, String const& storage_key)
        : m_quota(quota)
        , m_cache(cached_storage_map(endpoint_type, storage_cache_key(endpoint_type, page_id, storage_key)))
    {
    }
...
private:
    ...
    explicit LocalStorageBottle(GC::Ref<Page> page, StorageEndpointType endpoint_type, StorageKey key, Optional<u64> quota)
        : StorageBottle(quota, endpoint_type, page->client().id(), key.to_string())
        , m_page(move(page))
