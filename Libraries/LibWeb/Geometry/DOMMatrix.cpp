...
WebIDL::ExceptionOr<GC::Ref<DOMMatrix>> DOMMatrix::create_from_constructor(Optional<Variant<Utf16String, Vector<double>>> const& init, DOMMatrixStringContext string_context)
{
    // -> If init is omitted
    if (!init.has_value()) {
        ...
        return GC::Heap::the().allocate<DOMMatrix>(1, 0, 0, 1, 0, 0);
    }
    ...
    if (init_value.has<Utf16String>()) {
        ...
        auto transform_list = init_value.get<Utf16String>().to_utf8();
        auto result = TRY(parse_dom_matrix_init_string(transform_list));
        ...
        if (result.is_2d_transform) {
            ...
            return GC::Heap::the().allocate<DOMMatrix>(m[0, 0], m[1, 0], m[0, 1], m[1, 1], m[0, 3], m[1, 3]);
        }
    ...
    if (double_sequence.size() == 6) {
        ...
        return GC::Heap::the().allocate<DOMMatrix>(double_sequence[0], double_sequence[1], double_sequence[2], double_sequence[3], double_sequence[4], double_sequence[5]);
    }
...
WebIDL::ExceptionOr<GC::Ref<DOMMatrix>> DOMMatrix::create_for_constructor(JS::Object& relevant_global_object, Optional<Variant<Utf16String, Vector<double>>> const& init)
{
    return create_from_constructor(init, dom_matrix_string_context(relevant_global_object));
}
...
DOMMatrix::DOMMatrix(double m11, double m12, double m21, double m22, double m41, double m42)
    : DOMMatrixReadOnly(m11, m12, m21, m22, m41, m42)
{
}

DOMMatrix::DOMMatrix(double m11, double m12, double m13, double m14, double m21, double m22, double m23, double m24, double m31, double m32, double m33, double m34, double m41, double m42, double m43, double m44)
    : DOMMatrixReadOnly(m11, m12, m13, m14, m21, m22, m23, m24, m31, m32, m33, m34, m41, m42, m43, m44)
{
}

DOMMatrix::DOMMatrix(DOMMatrixReadOnly const& read_only_matrix)
    : DOMMatrixReadOnly(read_only_matrix)
{
}
...
{
    ReadonlySpan<float> elements { reinterpret_cast<float const*>(array->viewed_array_buffer()->data_at(array->byte_offset())), array->byte_length().length() / sizeof(float) };
    ...
    if (elements.size() == 6)
        return GC::Heap::the().allocate<DOMMatrix>(elements.at(0), elements.at(1), elements.at(2), elements.at(3), elements.at(4), elements.at(5));
    ...
    if (elements.size() == 16)
        return GC::Heap::the().allocate<DOMMatrix>(elements.at(0), elements.at(1), elements.at(2), elements.at(3),
    ...
    return WebIDL::SimpleException { WebIDL::SimpleExceptionType::TypeError, "Sequence must contain exactly 6 or 16 elements"_utf16 };
}
...
{
    ReadonlySpan<double> elements { reinterpret_cast<double const*>(array->viewed_array_buffer()->data_at(array->byte_offset())), array->byte_length().length() / sizeof(double) };
...
WebIDL::ExceptionOr<GC::Ref<DOMMatrix>> DOMMatrix::set_matrix_value(Utf16String const& transform_list)
{
    ...
    auto transform_list_utf8 = transform_list.to_utf8();
    auto result = TRY(parse_dom_matrix_init_string(transform_list_utf8));
