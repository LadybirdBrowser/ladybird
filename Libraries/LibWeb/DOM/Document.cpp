...
static GC::Ref<HTML::BrowsingContext> obtain_a_browsing_context_to_use_for_a_navigation_response(HTML::NavigationParams const& navigation_params)
{
    ...
    bool swap_group = coop_enforcement_result.needs_a_browsing_context_group_switch;
    ...
    if (!source_origin.is_same_site(destination_origin)) {
        ...
        if (!source_origin.is_opaque() && source_origin.scheme().has_value() && !source_origin.scheme()->starts_with_bytes("http"sv, CaseSensitivity::CaseInsensitive)) {
            swap_group = true;
        }

        if (!destination_origin.is_opaque() && destination_origin.scheme().has_value() && !destination_origin.scheme()->starts_with_bytes("http"sv, CaseSensitivity::CaseInsensitive)) {
            swap_group = true;
        }
    ...
    if (browsing_context->group()->browsing_context_set().size() == 1) {
        swap_group = true;
    }
    ...
    // FIXME: 12. If navigationCOOP's value is "same-origin-plurs-COEP", then set newBrowsingContext's group's
