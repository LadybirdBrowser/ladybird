...
void Document::set_accessibility_focus_target(GC::Ptr<Element> element)
{
    ...
    // The accessibility focus ring is a paint-time browser overlay (see Paintable::paint_outline),
    // so moving it between elements only requires a repaint, not a style recomputation.
    set_needs_repaint();
}
