...
void Document::run_fullscreen_steps()
{
    ...
    Vector<PendingFullscreenEvent> pending_events = move(m_pending_fullscreen_events);
...
void Document::fullscreen_element_within_doc(GC::Ref<Element> element)
{
    ...
    if (hide_until) {
        ...
        HTML::HTMLElement::hide_all_popovers_until(element->owner_document(), HTML::FocusPreviousElement::No, HTML::FireEvents::Yes);
    }
...
{
    GC::Ptr<Element> fullscreen_element { nullptr };

    // https://fullscreen.spec.whatwg.org/#fullscreen-element - get the associated fullscreen element
    for (auto const& el : top_layer_elements().in_reverse()) {
        if (el->is_fullscreen_element()) {
            fullscreen_element = el;
            break;
        }
    }
...
GC::Ref<WebIDL::Promise> Document::exit_fullscreen()
{
    ...
    Platform::EventLoopPlugin::the().deferred_invoke(GC::create_function(heap(), [&realm, doc = GC::Ref { *this }, promise, resize]() {
        ...
        if (!doc->fullscreen_element()) {
            HTML::queue_global_task(HTML::Task::Source::UserInteraction, realm.global_object(), GC::create_function(realm.heap(), [&realm, promise]() {
        ...
        GC::RootVector<GC::Ptr<Document>> descendant_docs(realm.heap());
...
GC::RootVector<GC::Ptr<Document>> Document::collect_documents_to_unfullscreen() const
{
    // 1. Let docs be an ordered set consisting of doc.
    GC::RootVector<GC::Ptr<Document>> docs(heap());
