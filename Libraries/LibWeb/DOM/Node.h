...
public:
    ...
    Node const* find_in_shadow_including_ancestor_chain(Predicate&& predicate) const
    {
        for (auto const* it = this; it && it->is_connected(); it = it->parent_or_shadow_host_node()) {
            if (predicate(*it))
                return it;
        }
        return nullptr;
