...
ErrorOr<String> Node::name_or_description(NameOrDescription target, Document const& document, HashTable<UniqueNodeID>& visited_nodes, IsDescendant is_descendant, ShouldComputeRole shouldComputeRole) const
{
    ...
    if (is_element()) {
        ...
        if (shouldComputeRole == ShouldComputeRole::Yes)
        ...
        if ((role.has_value() && ARIA::allows_name_from_content(role.value())) || element->is_referenced() || is_descendant == IsDescendant::Yes) {
            ...
            for (auto& child_node : child_nodes) {
                ...
                // b. Compute the text alternative of the current node beginning with step 2. Set the result to that text alternative.
                auto result = MUST(current_node->name_or_description(target, document, visited_nodes, IsDescendant::Yes, shouldComputeRole));
...
ErrorOr<String> Node::accessible_name(Document const& document, ShouldComputeRole shouldComputeRole) const
{
    ...
    return name_or_description(NameOrDescription::Name, document, visited_nodes, IsDescendant::No, shouldComputeRole);
}
