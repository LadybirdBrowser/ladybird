...
void Node::build_accessibility_tree(AccessibilityTreeNode& parent)
{
    ...
    if (is_document()) {
        ...
        if (element->include_in_accessibility_tree()) {
            ...
            // elements without a layout_node) would skip them. Therefore, we visit the options directly here — creating
            // an accessibility tree node for each one, regardless of its layout state.
            if (auto const* select = as_if<HTML::HTMLSelectElement>(element)) {
                for (auto const& option : select->list_of_options()) {
                    auto option_node = AccessibilityTreeNode::create(&document(), option.ptr());
                    current_node->append_child(option_node);
                }
            } else if (has_child_nodes()) {
