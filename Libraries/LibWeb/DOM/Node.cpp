...
void Node::live_range_pre_remove_steps()
{
...
WebIDL::ExceptionOr<void> Node::move_node(Node& new_parent, Node* child)
{
    ...
    // Spec-Note: This has the side effect of ensuring that a move is only performed if newParent’s connected is node’s connected.
    ...
    if (is_connected()) {
        if (layout_node() && layout_node()->display().is_contents()) {
    ...
    for_each_shadow_including_inclusive_descendant([this, &new_parent, old_parent](Node& inclusive_descendant) {
        ...
        // Spec-Note: Because the move algorithm is a separate primitive from insert and remove, it does not invoke the traditional insertion steps or removing steps for inclusiveDescendant.
