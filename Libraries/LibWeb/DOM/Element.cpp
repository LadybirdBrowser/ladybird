...
void Element::resolve_counters(CSS::ComputedProperties& style)
{
    ...
    for (auto const& counter : counter_increment) {
        ...
        if (!existing_counter.value().reversed)
            continue;
...
void Element::update_initial_value_for_reversed_counter__after_increment(FlyString const& name, int amount)
{
    ...
    if (!counter.value().value.has_value())
        counter.value().value = increment_negated;

    // 3. If el sets this counter with counter-set, then [...] break this loop. See below for the rest..
    if (counter.value().is_explicitly_set_reversed_counter)
        return;

    // 4. Add incrementNegated to num.
    counter.value().value.value().saturating_add(increment_negated);
}
...
void Element::update_initial_value_for_reversed_counter__after_set(FlyString const& name, int amount)
{
    ...
    counter.value().value.value().saturating_add(amount);

    // FIXME: if counter is set and incremented in the same element
}
