    ...
    if (value()->is_element()) {
        ...
        if (auto parent = text_node.parent_element()) {
            ...
            if (font_values && text_values) {
                ...
                node_data.font_size = MUST(String::formatted("{}", CSSPixels::from_raw(font_values->font_size).to_double()));
                ...
                auto text_slots = Layout::TextOffsetMapping { text_node }.slot_ids();
                ...
                auto line_rect_at_offset = [&](size_t offset) -> Optional<CSSPixelRect> {
                    if (!layout || text_slots.is_empty())
                        return {};
                    auto result = Layout::RustFFI::layout_arena_text_caret_rect_in_dom_range(
                        layout->arena_handle(), text_slots.data(), text_slots.size(), offset);
                    if (!result.has_value)
                        return {};
                    return Painting::from_ffi_css_pixel_rect(result.rect);
                };
