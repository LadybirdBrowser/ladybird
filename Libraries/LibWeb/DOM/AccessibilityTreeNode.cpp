    ...
    if (value()->is_element()) {
        ...
        if (auto parent = text_node.parent_element()) {
            ...
            if (font_values && text_values) {
                ...
                Vector<VisualLine> rendered_lines;
                for (auto const& line : collect_visual_lines(text_node)) {
                    if (line.has_fragments)
                        rendered_lines.append(line);
                }

                auto first_line_rect = rendered_lines.is_empty()
                    ? Optional<CSSPixelRect> {}
                    : line_rect_at_offset(rendered_lines.first().start_offset);

                if (first_line_rect.has_value()) {
                    // Report positions relative to the text's own origin (the top-left of its first line) so that
                    // per-character offsets are independent of scrolling. Caret rects are document-absolute, so anchor
                    // them to the first line's absolute top-left. Using node_data.bounds here would be wrong: it is
                    // viewport-relative (getBoundingClientRect), which would fold the scroll offset into every position.
                    auto text_origin = first_line_rect->top_left();
                    auto text_utf16 = text_node.data().utf16_view();
                    for (auto const& line : rendered_lines) {
                        auto line_rect = line_rect_at_offset(line.start_offset);
                        ...
                        // hold. end_offset excludes whitespace hanging at a soft wrap, which renders on no line.
                        ...
                        node_data.line_break_character_offsets.append(static_cast<i32>(line_character_start));
