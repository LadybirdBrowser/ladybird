...
static WebIDL::ExceptionOr<GC::Ref<DOM::Document>> load_html_document(HTML::NavigationParams const& navigation_params, RefPtr<Core::Promise<Empty>> signal_to_continue_session_history_processing)
{
    ...
    if (document->url_string() == "about:blank"_string
        ...
        // Nothing else is added to the document, so mark it as loaded and resolve the signal_to_continue_session_history_processing.
        if (signal_to_continue_session_history_processing)
    ...
    //    causes a load event to be fired.
        ...
        auto process_body = GC::create_function(document->heap(), [document, signal_to_continue_session_history_processing, url = navigation_params.response->url().value(), mime_type = navigation_params.response->header_list()->extract_mime_type()](ByteBuffer data) {
            Platform::EventLoopPlugin::the().deferred_invoke(GC::create_function(document->heap(), [signal_to_continue_session_history_processing, document = document, data = move(data), url = url, mime_type] {
                // If document is part of a session history entry's traversal, resolve the signal_to_continue_session_history_processing.
                if (signal_to_continue_session_history_processing)
                    signal_to_continue_session_history_processing->resolve({});
...
static WebIDL::ExceptionOr<GC::Ref<DOM::Document>> load_xml_document(HTML::NavigationParams const& navigation_params, MimeSniff::MimeType type, RefPtr<Core::Promise<Empty>> signal_to_continue_session_history_processing)
{
    ...
    auto process_body = GC::create_function(document->heap(), [document, signal_to_continue_session_history_processing, url = navigation_params.response->url().value(), content_encoding = move(content_encoding), mime = type](ByteBuffer data) {
        ...
        if (source.is_error()) {
            ...
            // NOTE: This ensures that the `load` event gets fired for the frame loading this document.
            document->completely_finish_loading();
            // If document is part of session history traversal, resolve the signal_to_continue_session_history_processing.
            if (signal_to_continue_session_history_processing)
                signal_to_continue_session_history_processing->resolve({});
            return;
        }
...
static WebIDL::ExceptionOr<GC::Ref<DOM::Document>> load_text_document(HTML::NavigationParams const& navigation_params, MimeSniff::MimeType type, RefPtr<Core::Promise<Empty>> signal_to_continue_session_history_processing)
{
...
static WebIDL::ExceptionOr<GC::Ref<DOM::Document>> load_media_document(HTML::NavigationParams const& navigation_params, MimeSniff::MimeType type, RefPtr<Core::Promise<Empty>> signal_to_continue_session_history_processing)
{
...
GC::Ptr<DOM::Document> load_document(HTML::NavigationParams const& navigation_params, RefPtr<Core::Promise<Empty>> signal_to_continue_session_history_processing)
{
