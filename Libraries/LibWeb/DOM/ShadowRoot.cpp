...
bool ShadowRoot::has_registered_slots() const
{
    return m_slot_registry && !m_slot_registry->is_empty();
}

GC::Ptr<HTML::HTMLSlotElement> ShadowRoot::first_slot_with_name(StringView name) const
{
    if (!m_slot_registry)
        return nullptr;
