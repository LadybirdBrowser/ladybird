...
wgpu::RequestAdapterOptions GPURequestAdapterOptions::to_wgpu() const
{
    ...
    request_adapter_options.forceFallbackAdapter = force_fallback_adapter;
    // FIXME: Dawn does not expose xrCompatible yet
    return request_adapter_options;
}
...
GC::Ref<WebIDL::Promise> GPUAdapter::request_device(GPUDeviceDescriptor const& options)
{
    ...
    Platform::EventLoopPlugin::the().deferred_invoke(GC::create_function(realm.heap(), [this, adapter, options, &realm, promise]() mutable {
        ...
        m_impl->instance->wgpu().WaitAny(adapter.RequestDevice(&device_descriptor_options, wgpu::CallbackMode::AllowProcessEvents, [this, options, realm = GC::Root(realm), promise = GC::Root(promise)](wgpu::RequestDeviceStatus status, wgpu::Device native_device, char const* message) {
