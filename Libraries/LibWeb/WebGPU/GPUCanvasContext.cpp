...
void GPUCanvasContext::configure(GPUCanvasConfiguration const& config)
{
    ...
    config.device->on_queue_submitted([this]() {
        ...
        Span<u8> const drawing_buffer = mapped_texture_buffer->data();
        for (int y = 0; y < m_size.height(); ++y) {
            for (int x = 0; x < m_size.width(); ++x) {
                size_t const pixel_index = (y * m_size.width() + x) * 4;
                // FIXME: Handle all supported configuration formats, not just RGBA
                u8 const r = drawing_buffer[pixel_index + 0];
                u8 const g = drawing_buffer[pixel_index + 1];
                u8 const b = drawing_buffer[pixel_index + 2];
                u8 const a = drawing_buffer[pixel_index + 3];
                auto const pixel = Color(r, g, b, a);
