...
GC::Ptr<PendingResponse> main_fetch(JS::Realm& realm, Infrastructure::FetchParams const& fetch_params, Recursive recursive)
{
    ...
    auto get_response = GC::create_function(vm.heap(), [&realm, &vm, &fetch_params, request]() -> GC::Ref<PendingResponse> {
        ...
        // AD-HOC: Treat file:// font and worklet loads from file:// pages as same-origin.
        // Fonts and worklets can use CORS mode, but file:// origins are opaque in our implementation,
        // so the standard same-origin check always fails.
        auto is_file_to_file_load = origin && origin->is_opaque_file_origin()
            && request->current_url().scheme() == "file"sv
            && request->destination().has_value()
            && (request->destination().value() == Infrastructure::Request::Destination::Font
                || request->destination().value() == Infrastructure::Request::Destination::AudioWorklet
                || request->destination().value() == Infrastructure::Request::Destination::PaintWorklet);
