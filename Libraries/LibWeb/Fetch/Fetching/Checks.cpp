...
bool cors_check(Infrastructure::Request const& request, Infrastructure::Response const& response)
{
    ...
    auto origin = response.header_list()->get("Access-Control-Allow-Origin"_sv.bytes());
    ...
    if (request.credentials_mode() != Infrastructure::Request::CredentialsMode::Include && origin->span() == "*"_sv.bytes())
        return true;
    ...
    auto credentials = response.header_list()->get("Access-Control-Allow-Credentials"_sv.bytes());

    // 7. If credentials is `true`, then return success.
    if (credentials.has_value() && credentials->span() == "true"_sv.bytes())
        return true;
...
bool tao_check(Infrastructure::Request const& request, Infrastructure::Response const& response)
{
    ...
    auto values = response.header_list()->get_decode_and_split("Timing-Allow-Origin"_sv.bytes());

    // 3. If values contains "*", then return success.
    if (values.has_value() && values->contains_slow("*"_sv))
        return true;
