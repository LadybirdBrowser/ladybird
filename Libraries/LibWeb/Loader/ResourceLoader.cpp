...
// 4.1 Parsing
static Vector<String> parse_clear_site_data_header(HTTP::HeaderMap const& response_headers)
{
...
// https://www.w3.org/TR/clear-site-data/#clear-response
// 4.2 Clear data for response
static void handle_clear_site_data(Page& page, URL::URL const& url, HTTP::HeaderMap const& response_headers)
{
    ...
    // FIXME: 3. "Let origin be response's url's origin."
    ...
    for (auto const& type : types) {
        if (type == "cache"sv) {
            ...
            // FIXME: 4.2.4 Clear cookies for origin. CookieJar lacks domain-scoped API, so this clears ALL cookies globally
            // instead of just cookies for the registered domain of the URL.
            page.client().page_did_clear_all_cookies(url);
...
void ResourceLoader::handle_network_response_headers(LoadRequest const& request, HTTP::HeaderMap const& response_headers)
{
    ...
    if (request.store_set_cookie_headers()) {
        ...
        // From https://www.w3.org/TR/clear-site-data/#fetch-integration (Section 3.2):
