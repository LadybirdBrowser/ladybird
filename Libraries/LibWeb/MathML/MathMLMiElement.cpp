...
bool MathMLMiElement::is_presentational_hint(FlyString const& name) const
{
    if (Base::is_presentational_hint(name))
        return true;
    return first_is_one_of(name, "mathvariant");
}
...
void MathMLMiElement::apply_presentational_hints(GC::Ref<CSS::CascadedProperties> cascaded_properties) const
{
    ...
    // In that case, the user agent is expected to treat the attribute as a presentational hint setting the element's text-transform property to none. Otherwise it has no effects.
    if (auto mathvariant = attribute("mathvariant"_fly_string); mathvariant.has_value() && mathvariant.value().equals_ignoring_ascii_case("normal"sv)) {
