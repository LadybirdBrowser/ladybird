...
GC::Ref<WebIDL::Promise> CredentialsContainer::get(CredentialRequestOptions const& options)
{
    ...
    ASSERT(HTML::is_secure_context(settings));

    // 3. Let document be settings’s relevant global object's associated Document.
    auto const& document = verify_cast<HTML::Window>(settings.global_object()).associated_document();
    ...
    if (!document.is_fully_active()) {
        WebIDL::reject_promise(realm(), p, WebIDL::InvalidStateError::create(realm(), "Document is not fully active yet"_string));
        return p;
    }

    // 5. If options.signal is aborted, then return a promise rejected with options.signal’s abort reason.
    if (options.signal && options.signal->aborted()) {
        WebIDL::reject_promise(realm(), p, options.signal->reason());
        return p;
    }
    ...
    if (interfaces.is_empty()) {
        WebIDL::reject_promise(realm(), p, WebIDL::DOMException::create(realm(), "NotSupportedError"_string, "No credential type is supported"_string));
        return p;
    }
    ...
    // 8. For each interface of interfaces:
    {
        ...
        if (options.mediation == Bindings::CredentialMediationRequirement::Conditional) {
            WebIDL::reject_promise(realm(), p, WebIDL::DOMException::create(realm(), "TypeError"_string, "Interface does not support user mediation"_string));
            return p;
        }
    ...
    Platform::EventLoopPlugin::the().deferred_invoke(GC::create_function(vm().heap(), []() {
    }));
...
Vector<IDL::Interface> relevant_credential_interface_objects(Variant<CredentialCreationOptions, CredentialRequestOptions>)
{
    // FIXME
    // 1. Let settings be the current settings object.

    // 2. Let relevant interface objects be an empty set.
    Vector<IDL::Interface> relevant_interface_objects;
