...
static GC::Ref<WebIDL::DOMException> with_relevant_realm(JS::Object& relevant_global_object, GC::Ref<WebIDL::DOMException> exception)
{
    exception->set_wrapper_realm_hint(relevant_global_object);
    return exception;
}
...
GC::Ref<XMLHttpRequest> XMLHttpRequest::create(GC::Ref<DOM::EventTarget> relevant_global_object)
{
    ...
    auto& global_object = xhr->relevant_global_object();
    xhr->set_wrapper_realm_hint(global_object);
    upload_object->set_wrapper_realm_hint(global_object);
...
WebIDL::ExceptionOr<void> XMLHttpRequest::open(Utf16String const& method, Utf16String const& url, bool async, Optional<Utf16String> const& username, Optional<Utf16String> const& password)
{
    ...
    if (auto const* window = HTML::window_from_global_object(relevant_global_object()); window) {
        if (!window->associated_document().is_fully_active())
            return with_relevant_realm(relevant_global_object(), WebIDL::InvalidStateError::create("Invalid state: Window's associated document is not fully active."_utf16));
    }

    // 2. If method is not a method, then throw a "SyntaxError" DOMException.
    if (!HTTP::is_method(method))
        return with_relevant_realm(relevant_global_object(), WebIDL::SyntaxError::create("An invalid or illegal string was specified."_utf16));

    // 3. If method is a forbidden method, then throw a "SecurityError" DOMException.
    if (HTTP::is_forbidden_method(method))
        return with_relevant_realm(relevant_global_object(), WebIDL::SecurityError::create("Forbidden method, must not be 'CONNECT', 'TRACE', or 'TRACK'"_utf16));
    ...
    if (!parsed_url.has_value())
        return with_relevant_realm(relevant_global_object(), WebIDL::SyntaxError::create("Invalid URL"_utf16));
