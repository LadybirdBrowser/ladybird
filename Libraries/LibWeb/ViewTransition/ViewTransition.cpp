...
ReplacedNamedViewTransitionPseudoElement::ReplacedNamedViewTransitionPseudoElement(CSS::PseudoElement type, FlyString view_transition_name, RefPtr<Gfx::Bitmap> content = {})
...
void ViewTransition::setup_view_transition()
{
    ...
    if (result.is_error()) {
        ...
        skip_the_view_transition(WebIDL::InvalidStateError::create(realm, "Failed to capture old state"_string));
        // and return.
        return;
    }
...
void ViewTransition::activate_view_transition()
{
    ...
    if (!m_initial_snapshot_containing_block_size.has_value() || m_initial_snapshot_containing_block_size.value() != snapshot_containing_block_size) {
...
ErrorOr<void> ViewTransition::update_pseudo_element_styles()
{
    ...
    for (auto [transition_name, captured_element] : m_named_elements) {
        ...
        if (captured_element->new_element) {
            ...
            //    new element.
            new_->m_content = captured_element->old_image;
        }
    }
