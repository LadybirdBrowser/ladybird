...
void OpenGLContext::allocate_d3d11texture_painting_surface()
{
    ...
    if (HRESULT const result = d3d11_shared_texture->get_resource().GetSharedHandle(&shared_handle); FAILED(result)) {
        dbgln("GetSharedHandle failed with HRESULT: 0x{:x}", result);
        return;
    }

    ComPtr<ID3D12Resource> d3d12_resource;
    if (HRESULT const result = d3d_ctx.d12_device().OpenSharedHandle(shared_handle, IID_PPV_ARGS(&d3d12_resource)); FAILED(result)) {
        dbgln("OpenSharedHandle failed with HRESULT: 0x{:x}", result);
        return;
    }
