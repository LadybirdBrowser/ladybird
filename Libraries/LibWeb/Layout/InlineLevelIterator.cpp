    ...
    for (;;) {
        ...
        if (next_item.node->computed_values().text_wrap_mode() == CSS::TextWrapMode::Wrap) {
            ...
            if (next_item.is_collapsible_whitespace)
                break;
            const auto& next_text_node = as<Layout::TextNode>(*(next_item.node));
...
HashMap<StringView, u8> InlineLevelIterator::shape_features_map() const
{
    ...
    const auto& computed_values = m_current_node->computed_values();
...
Gfx::ShapeFeatures InlineLevelIterator::create_and_merge_font_features() const
{
    ...
    const auto& computed_values = m_inline_formatting_context.containing_block().computed_values();
...
Optional<InlineLevelIterator::Item> InlineLevelIterator::next_without_lookahead()
{
    ...
    if (is<Layout::TextNode>(*m_current_node)) {
        const auto& text_node = static_cast<Layout::TextNode const&>(*m_current_node);

        if (!m_text_node_context.has_value())
            enter_text_node(text_node);

        auto chunk_opt = m_text_node_context->chunk_iterator.next();
        const auto is_last_chunk = !m_text_node_context->chunk_iterator.peek(0).has_value();
        auto is_empty_editable = false;
        if (!chunk_opt.has_value()) {
            const auto is_only_chunk = !m_text_node_context->is_last_chunk && is_last_chunk;
            if (is_only_chunk && text_node.text_for_rendering().is_empty()) {
        dbgln("{}", text_node.dom_node().root().node_name());
                auto is_editable = false;
                if (const auto *const shadow_root = as_if<DOM::ShadowRoot>(text_node.dom_node().root())) {
                    dbgln("{}", is<HTML::FormAssociatedTextControlElement>(shadow_root->host()));
                    is_editable = shadow_root->host() && (
                        is<HTML::FormAssociatedTextControlElement>(shadow_root->host()) ||
                        shadow_root->host()->is_editable_or_editing_host());
                }
                if (is_editable) {
                    dbgln("its true");
                    chunk_opt = m_text_node_context->chunk_iterator.create_empty_chunk();
                    is_empty_editable = true;
                }
            }
        ...
        m_text_node_context->is_last_chunk = is_last_chunk;
    ...
    if (m_current_node->is_absolutely_positioned()) {
        const auto& node = *m_current_node;
        skip_to_next();
        return Item {
    ...
    if (is<Layout::ReplacedBox>(*m_current_node)) {
        const auto& replaced_box = static_cast<Layout::ReplacedBox const&>(*m_current_node);
        // FIXME: This const_cast is gross.
        const_cast<Layout::ReplacedBox&>(replaced_box).prepare_for_replaced_layout();
    }

    const auto& box = as<Layout::Box>(*m_current_node);
    const auto& box_state = m_layout_state.get(box);
