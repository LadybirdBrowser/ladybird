...
void FlexFormattingContext::determine_flex_base_size(FlexItem& item)
{
    ...
    auto intrinsic = item.box->intrinsic_content_box_size(); // ?
    if (intrinsic.has_aspect_ratio()) {
        if (!item.used_flex_basis_is_definite && !intrinsic.has_width() && !intrinsic.has_height() && !has_definite_cross_size(item)) {
...
void FlexFormattingContext::determine_hypothetical_cross_size_of_item(FlexItem& item)
{
    ...
    if (item.box->has_preferred_aspect_ratio()) {
        auto intrinsic = item.box->intrinsic_content_box_size();
        if (item.used_flex_basis_is_definite || (intrinsic.has_width() && intrinsic.has_height())) {
