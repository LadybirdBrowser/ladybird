...
GridFormattingContext::PlacementPosition GridFormattingContext::resolve_grid_position(Box const& child_box, GridDimension const dimension)
{
    ...
    if (placement_end.has_identifier()) {
        ...
        size_t line_number = placement_end.has_line_number() ? placement_end.line_number() : 1;
        if (auto area_end_line_index = get_line_index_by_line_name(dimension, area_end_line_name, line_number); area_end_line_index.has_value()) {
            result.end = area_end_line_index.value();
        } else if (auto line_name_index = get_line_index_by_line_name(dimension, placement_end.identifier(), line_number); line_name_index.has_value()) {
    ...
    if (placement_start.has_identifier()) {
        ...
        size_t line_number = placement_start.has_line_number() ? placement_start.line_number() : 1;
        if (auto area_start_line_index = get_line_index_by_line_name(dimension, area_start_line_name, line_number); area_start_line_index.has_value()) {
            result.start = area_start_line_index.value();
        } else if (auto line_name_index = get_line_index_by_line_name(dimension, placement_start.identifier(), line_number); line_name_index.has_value()) {
...
Optional<int> GridFormattingContext::get_line_index_by_line_name(GridDimension dimension, String const& line_name, int line_number)
{
    auto const& lines = dimension == GridDimension::Column ? m_column_lines : m_row_lines;
    size_t line_index = line_number < 0 ? lines.size() + line_number : line_number - 1;
