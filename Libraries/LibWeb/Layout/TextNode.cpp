...
bool TextNode::ChunkIterator::is_at_line_break_opportunity() const
{
    ...
    auto get_previous_code_point = [this]() -> Optional<u32> {
        if (m_current_index == 0)
            return {};
        size_t previous_index = m_current_index - 1;
        if (previous_index > 0 && AK::UnicodeUtils::is_utf16_low_surrogate(m_view.code_unit_at(previous_index)))
            previous_index--;
        auto code_point = m_view.code_point_at(previous_index);
        while (Unicode::line_break_class(code_point) == Unicode::LineBreakClass::CombiningMark && previous_index > 0) {
            --previous_index;
            if (previous_index > 0 && AK::UnicodeUtils::is_utf16_low_surrogate(m_view.code_unit_at(previous_index)))
                previous_index--;
            code_point = m_view.code_point_at(previous_index);
        }
        return code_point;
    };
    ...
    // This property specifies soft wrap opportunities between letters, i.e. where it is “normal” and permissible to
    // break lines of text. Specifically it controls whether a soft wrap opportunity generally exists between adjacent
    // typographic letter units, treating non-letter typographic character units belonging to the NU, AL, AI, or ID
    // Unicode line breaking classes as typographic letter units for this purpose (only). [UAX14] It does not affect
    // rules governing the soft wrap opportunities created by white space (as well as by other space separators) and
    // around punctuation.
