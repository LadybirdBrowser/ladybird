...
fn generate_descriptor_metadata(manifest_dir: &Path, out_dir: &Path) -> Result<(), Box<dyn Error>> {
    ...
    let mut output = String::from(
        "// Generated by build.rs from CSS/Descriptors.json. Do not edit.\n\n\
         ...
         pub(crate) struct DescriptorMetadata { pub syntax: &'static [DescriptorSyntax], pub allow_arbitrary_substitution_functions: bool, pub allow_css_wide_keywords: bool }\n\n\
    ...
    for (at_rule_index, (_at_rule_name, at_rule)) in at_rules.iter().enumerate() {
        ...
        for (name, descriptor) in descriptors {
            ...
            write!(
                output,
                "            if equals(name, b\"{name}\") {{ return Some(DescriptorMetadata {{ syntax: &["
            )?;
        ...
        if let Some(custom) = at_rule.get("custom-descriptors") {
            write!(
                output,
                "            if name.starts_with(&[u16::from(b'-'), u16::from(b'-')]) && name.len() > 2 {{ return Some(DescriptorMetadata {{ syntax: &["
            )?;
