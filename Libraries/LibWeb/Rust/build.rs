...
fn generate_descriptor_metadata(manifest_dir: &Path, out_dir: &Path) -> Result<(), Box<dyn Error>> {
    ...
    let mut descriptor_names = std::collections::BTreeSet::new();
    for at_rule in at_rules.values() {
        let descriptors = at_rule["descriptors"]
            .as_object()
            .ok_or("descriptor table is not an object")?;
        for (name, descriptor) in descriptors {
            if descriptor.get("legacy-alias-for").is_none() {
                descriptor_names.insert(name.clone());
            }
        }
    }
    let descriptor_ids = descriptor_names
        .iter()
        .enumerate()
