...
GC::Ref<CookieChangeEvent> CookieChangeEvent::create(Utf16FlyString const& event_name, CookieChangeEventInit const& event_init, HighResolutionTime::DOMHighResTimeStamp time_stamp)
{
    return GC::Heap::the().allocate<CookieChangeEvent>(event_name, event_init, time_stamp);
}

CookieChangeEvent::CookieChangeEvent(Utf16FlyString const& event_name, CookieChangeEventInit const& event_init, HighResolutionTime::DOMHighResTimeStamp time_stamp)
    : DOM::Event(FlyString { event_name.view().to_utf8_but_should_be_ported_to_utf16() }, event_init, time_stamp)
