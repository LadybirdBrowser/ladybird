    ...
    RefPtr<Core::Promise<Empty>> signal_to_continue_session_history_processing)
{
    ...
    queue_global_task(Task::Source::NavigationAndTraversal, *active_window(), GC::create_function(heap(), [this, entry, signal_to_continue_session_history_processing, navigation_params = move(navigation_params), navigation_id, user_involvement, completion_steps, csp_navigation_type]() mutable {
        ...
        //    loading a document given navigationParams, sourceSnapshotParams, and entry's document state's initiator origin.
            ...
            auto document = load_document(navigation_params.get<GC::Ref<NavigationParams>>(), signal_to_continue_session_history_processing);
    ...
    Platform::EventLoopPlugin::the().deferred_invoke(GC::create_function(heap(), [this, source_snapshot_params, target_snapshot_params, csp_navigation_type, document_resource, url, navigation_id, referrer_policy, initiator_origin_snapshot, response, history_handling, initiator_base_url_snapshot, user_involvement] {
        ...
        populate_session_history_entry_document(history_entry, source_snapshot_params, target_snapshot_params, user_involvement, navigation_id, navigation_params, csp_navigation_type, true, GC::create_function(heap(), [this, signal_to_continue_session_history_processing, history_entry, history_handling, navigation_id, user_involvement] {
            ...
            traversable_navigable()->append_session_history_traversal_steps(GC::create_function(heap(), [this, history_entry, history_handling, navigation_id, user_involvement, signal_to_continue_session_history_processing] {
                ...
                // AD-HOC: If the document isn't active or is still loading session history traversal queue will wait for it to load else resolve the signal_to_continue_session_history_processing.
...
void Navigable::navigate_to_a_fragment(URL::URL const& url, HistoryHandlingBehavior history_handling, UserNavigationInvolvement user_involvement, GC::Ptr<DOM::Element> source_element, Optional<SerializationRecord> navigation_api_state, String navigation_id)
{
    ...
    // Note: Use Core::Promise to signal SessionHistoryTraversalQueue that it can continue to execute next entry.
    auto signal_to_continue_session_history_processing = Core::Promise<Empty>::construct();
    traversable->append_session_history_synchronous_navigation_steps(*this, GC::create_function(heap(), [this, traversable, history_entry, entry_to_replace, navigation_id, history_handling, user_involvement, signal_to_continue_session_history_processing] {
...
GC::Ptr<DOM::Document> Navigable::evaluate_javascript_url(URL::URL const& url, URL::Origin const& new_document_origin, UserNavigationInvolvement user_involvement, String navigation_id)
{
    ...
    return load_document(navigation_params, nullptr);
}
...
void Navigable::navigate_to_a_javascript_url(URL::URL const& url, HistoryHandlingBehavior history_handling, GC::Ref<SourceSnapshotParams> source_snapshot_params, URL::Origin const& initiator_origin, UserNavigationInvolvement user_involvement, ContentSecurityPolicy::Directives::Directive::NavigationType csp_navigation_type, InitialInsertion initial_insertion, String navigation_id)
{
    ...
    // Note: Use Core::Promise to signal SessionHistoryTraversalQueue that it can continue to execute next entry.
    auto signal_to_continue_session_history_processing = Core::Promise<Empty>::construct();
    traversable_navigable()->append_session_history_traversal_steps(GC::create_function(heap(), [this, history_entry, history_handling, user_involvement, signal_to_continue_session_history_processing] {
...
void Navigable::reload(UserNavigationInvolvement user_involvement)
{
    ...
    // Note: Use Core::Promise to signal SessionHistoryTraversalQueue that it can continue to execute next entry.
    auto signal_to_continue_session_history_processing = Core::Promise<Empty>::construct();
    traversable->append_session_history_traversal_steps(GC::create_function(heap(), [traversable, user_involvement, signal_to_continue_session_history_processing] {
...
void perform_url_and_history_update_steps(DOM::Document& document, URL::URL new_url, Optional<SerializationRecord> serialized_data, HistoryHandlingBehavior history_handling)
{
    ...
    // Note: Use Core::Promise to signal SessionHistoryTraversalQueue that it can continue to execute next entry.
    auto signal_to_continue_session_history_processing = Core::Promise<Empty>::construct();
    traversable->append_session_history_synchronous_navigation_steps(*navigable, GC::create_function(document.realm().heap(), [traversable, navigable, new_entry, entry_to_replace, history_handling, signal_to_continue_session_history_processing] {
