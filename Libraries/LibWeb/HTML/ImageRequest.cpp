...
void abort_the_image_request(JS::Realm&, ImageRequest* image_request)
{
    ...
    // AD-HOC: We don't abort the fetch itself, since our SharedResourceRequest may be used by someone else.
    //         Instead, mark the image request as aborted so that its fetch callbacks discard their work.
    image_request->set_was_aborted();
}
