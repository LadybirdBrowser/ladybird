...
WebIDL::ExceptionOr<GC::Ref<Worker>> Worker::create(TrustedTypes::TrustedScriptURLOrString const& script_url, WorkerOptions const& options, DOM::Document& document)
{
    ...
    auto const compliaint_script_url = TRY(TrustedTypes::get_trusted_type_compliant_string(
    ...
    auto url = outside_settings.parse_url(compliaint_script_url.to_utf8_but_should_be_ported_to_utf16());

    // 4. If this fails, throw a "SyntaxError" DOMException.
    if (!url.has_value()) {
        dbgln_if(WEB_WORKER_DEBUG, "WebWorker: Invalid URL loaded '{}'.", compliaint_script_url);
    ...
    auto worker = document.realm().create<Worker>(compliaint_script_url.to_utf8_but_should_be_ported_to_utf16(), options, document);
