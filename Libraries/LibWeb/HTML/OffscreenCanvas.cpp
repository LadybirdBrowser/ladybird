    ...
    WebIDL::UnsignedLong height)
{
    ...
    // TODO: I am not sure if this is already implied by the initialization, since it is all 0s? Ask that in the GitHub PR
    Gfx::Color fill_color = Color::NamedColor(Color::Transparent);
    for (int y = 0; y < bitmap->height(); y++) {
        for (int x = 0; x < bitmap->width(); x++) {
            bitmap->set_pixel(x, y, fill_color);
        }
    }
...
OffscreenCanvas::HasOrCreatedContext OffscreenCanvas::create_2d_context()
{
    if (!m_context.has<Empty>())
        return m_context.has<GC::Ref<OffscreenCanvasRenderingContext2D>>() ? HasOrCreatedContext::Yes : HasOrCreatedContext::No;

    m_context = OffscreenCanvasRenderingContext2D::create(realm(), *this);
