...
WebIDL::ExceptionOr<bool> NavigatorBeaconMixin::send_beacon(String const& url, Optional<Fetch::BodyInit> const& data)
{
    ...
    if (data.has_value()) {
        ...
        if (content_type.has_value()) {
            ...
            auto content_type_header = Fetch::Infrastructure::Header::from_string_pair("Content-Type"_sv, content_type.value());
    ...
    req->set_method(MUST(ByteBuffer::copy("POST"_sv.bytes()))); // method: POST
