    ...
    OnFetchScriptComplete on_complete)
{
    ...
    auto process_response_consume_body = [&module_map, url, module_type, &module_map_realm, on_complete](GC::Ref<Fetch::Infrastructure::Response> response, Fetch::Infrastructure::FetchAlgorithms::BodyBytes body_bytes) {
        ...
        // 7. Otherwise
        {
            ...
            // 2. If mimeType is a JavaScript MIME type and moduleType is "javascript", then set moduleScript to the result of creating a JavaScript module script given sourceText, moduleMapRealm, response's URL, and options.
            // FIXME: Pass options.
            ...
            // 3. If the MIME type essence of mimeType is "text/css" and moduleType is "css", then set moduleScript to the result of creating a CSS module script given sourceText and moduleMapRealm.
            if (mime_type->essence() == "text/css"sv && module_type == "css")
            ...
            // 4. If mimeType is a JSON MIME type and moduleType is "json", then set moduleScript to the result of creating a JSON module script given sourceText and moduleMapRealm.
