...
FlyString module_type_from_module_request(JS::ModuleRequest const&);
WebIDL::ExceptionOr<URL::URL> resolve_module_specifier(Optional<Script&> referring_script, FlyString const& specifier);
WebIDL::ExceptionOr<Optional<URL::URL>> resolve_imports_match(FlyString const& normalized_specifier, Optional<URL::URL> as_url, ModuleSpecifierMap const&);
Optional<URL::URL> resolve_url_like_module_specifier(FlyString const& specifier, URL::URL const& base_url);
...
Fetch::Infrastructure::Request::Destination fetch_destination_from_module_type(Fetch::Infrastructure::Request::Destination, FlyString const&);
