...
void merge_existing_and_new_import_maps(Window& global, ImportMap& new_import_map)
{
    ...
    // FIXME: Spec bug: https://github.com/whatwg/html/issues/10833
    // We need to define 'oldImportMap', which is just a reference to global's existing import map.
            ...
            if (scope_prefix == record.serialized_base_url || (scope_prefix.to_string().ends_with('/') && record.serialized_base_url.has_value() && Infra::is_code_unit_prefix(scope_prefix.to_string(), *record.serialized_base_url))) {
                ...
                scope_imports.remove_all_matching([&](ByteString const& specifier_key, Optional<URL::URL> const&) {
                    ...
                    if (specifier_key.view() == record.specifier
                        ...
                        auto& console = realm.intrinsics().console_object()->console();
                        console.output_debug_message(JS::Console::LogLevel::Warn,
                            MUST(String::formatted("An import map rule for specifier '{}' was ignored as onet was already present in the existing import map", specifier_key)));

                        // 2. Remove scopeImports[specifier].
                        // FIXME: Spec bug: Should be specifier key: https://github.com/whatwg/html/issues/10833
                        return true;
                    }
    ...
    // 5. For each record of global's resolved module set:
    for (auto const& record : global.resolved_module_set()) {
    ...
    // 6. Set oldImportMap's imports to the result of merge module specifier maps, given newImportMapImports and oldImportMap's imports.
