...
public:
    ...
    // There are other methods that are not yet implemented in ladybird.
    ...
    HighResolutionTime::DOMHighResTimeStamp current_wall_time() const
    {
        ...
        HighResolutionTime::DOMHighResTimeStamp unsafeWallTime = HighResolutionTime::wall_clock_unsafe_current_time();

        // 2. Return the result of calling coarsen time with unsafeWallTime and settingsObject's cross-origin isolated capability.
        return HighResolutionTime::coarsen_time(unsafeWallTime, this->cross_origin_isolated_capability() == CanUseCrossOriginIsolatedAPIs::Yes);
    }
