...
WebIDL::ExceptionOr<GC::Ptr<ModuleScript>> ModuleScript::create_a_css_module_script(ByteString const& filename, StringView source, JS::Realm& realm)
{
    ...
    //    the argument.
    auto sheet = CSS::CSSStyleSheet::construct_impl(realm).release_value_but_fixme_should_propagate_errors();
...
JS::Promise* ModuleScript::run(PreventErrorReporting)
{
    ...
    // 7. Otherwise:
        ...
        module_execution_context->script_or_module = record.downcast<GC::Ref<JS::Module>>();
        vm().push_execution_context(*module_execution_context);

        // 2. Set evaluationPromise to record.Evaluate().
        auto elevation_promise_or_error = record.downcast<GC::Ref<JS::Module>>().get<GC::Ref<JS::Module>>()->evaluate(vm());
