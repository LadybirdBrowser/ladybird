...
WebIDL::ExceptionOr<void> NavigableContainer::create_new_child_navigable(GC::Ptr<GC::Function<void()>> after_session_history_update)
{
    ...
    traversable->append_session_history_traversal_steps(GC::create_function(heap(), [traversable, navigable, parent_navigable, history_entry, after_session_history_update, signal_to_continue_session_history_processing] {
...
void NavigableContainer::destroy_the_child_navigable()
{
    ...
    navigable->active_document()->destroy_a_document_and_its_descendants(GC::create_function(heap(), [this, navigable] {
        ...
        // Note: Use Core::Promise to signal SessionHistoryTraversalQueue that it can continue to execute next entry.
        auto signal_to_continue_session_history_processing = Core::Promise<Empty>::construct();
        traversable->append_session_history_traversal_steps(GC::create_function(heap(), [traversable, signal_to_continue_session_history_processing] {
