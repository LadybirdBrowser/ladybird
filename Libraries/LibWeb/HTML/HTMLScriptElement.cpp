...
void HTMLScriptElement::prepare_script()
{
    ...
    else if (script_block_type.equals_ignoring_ascii_case("module"_sv)) {
    ...
    else if (script_block_type.equals_ignoring_ascii_case("importmap"_sv)) {
    ...
    if (m_script_type == ScriptType::Classic && has_attribute(HTML::AttributeNames::event) && has_attribute(HTML::AttributeNames::for_)) {
        ...
        if (!for_.equals_ignoring_ascii_case("window"_sv)) {
        ...
        if (!event.equals_ignoring_ascii_case("onload"_sv)
            && !event.equals_ignoring_ascii_case("onload()"_sv)) {
