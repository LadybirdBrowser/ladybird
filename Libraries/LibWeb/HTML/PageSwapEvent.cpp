...
GC::Ref<PageSwapEvent> PageSwapEvent::create(Utf16String const& event_name, PageSwapEventInit const& event_init, HighResolutionTime::DOMHighResTimeStamp time_stamp)
{
    return GC::Heap::the().allocate<PageSwapEvent>(Utf16FlyString::from_utf16(event_name.utf16_view()), event_init, time_stamp);
}
...
PageSwapEvent::PageSwapEvent(Utf16FlyString const& event_name, PageSwapEventInit const& event_init, HighResolutionTime::DOMHighResTimeStamp time_stamp)
    : DOM::Event(event_name, event_init, time_stamp)
    , m_activation(event_init.activation)
    , m_view_transition(event_init.view_transition)
{
