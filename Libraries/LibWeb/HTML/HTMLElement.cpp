...
String HTMLElement::access_key_label() const
{
    ...
    // NOTE: We mimic Chromium here and treat the attribute value as a single key rather than splitting on whitespace.
    ...
    //    3.2. If the value does not correspond to a key on the system's keyboard, then skip the remainder of these steps for this value.
    //    3.3. If the user agent can find a mix of zero or more modifier keys that, combined with the key that corresponds to
