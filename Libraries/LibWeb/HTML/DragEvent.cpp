...
GC::Ref<DragEvent> DragEvent::create(Utf16FlyString const& event_name, DragEventInit const& event_init, double page_x, double page_y, double offset_x, double offset_y, HighResolutionTime::DOMHighResTimeStamp time_stamp)
{
    return GC::Heap::the().allocate<DragEvent>(event_name, event_init, page_x, page_y, offset_x, offset_y, time_stamp);
}

WebIDL::ExceptionOr<GC::Ref<DragEvent>> DragEvent::create_for_constructor(Utf16FlyString const& event_name, Bindings::DragEventInit const& event_init, HighResolutionTime::DOMHighResTimeStamp time_stamp)
{
...
DragEvent::DragEvent(Utf16FlyString const& event_name, DragEventInit const& event_init, double page_x, double page_y, double offset_x, double offset_y, HighResolutionTime::DOMHighResTimeStamp time_stamp)
