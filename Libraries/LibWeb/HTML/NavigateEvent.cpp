...
GC::Ref<NavigateEvent> NavigateEvent::create(Window& relevant_window, Utf16FlyString const& event_name, NavigateEventInit const& event_init, HighResolutionTime::DOMHighResTimeStamp time_stamp)
{
    auto event = GC::Heap::the().allocate<NavigateEvent>(relevant_window, event_name, event_init, time_stamp);
    event->set_is_trusted(true);
    return event;
...
GC::Ref<NavigateEvent> NavigateEvent::create(JS::Realm& realm, Utf16String const& event_name, Bindings::NavigateEventInit const& event_init)
{
    auto& window = HTML::relevant_window(realm.global_object());
    return create(window, Utf16FlyString::from_utf16(event_name.utf16_view()), {
...
NavigateEvent::NavigateEvent(Window& relevant_window, Utf16FlyString const& event_name, NavigateEventInit const& event_init, HighResolutionTime::DOMHighResTimeStamp time_stamp)
    : DOM::Event(event_name, event_init, time_stamp)
    , m_relevant_window(relevant_window)
    , m_navigation_type(event_init.navigation_type)
    , m_destination(*event_init.destination)
