...
static WebIDL::ExceptionOr<void> serialize_array_buffer(JS::VM& vm, StructuredSerializeWriter& data_holder, JS::ArrayBuffer& array_buffer, bool for_storage, AllowSharedArrayBuffers allow_shared_array_buffers)
{
    ...
    if (array_buffer.is_shared_array_buffer()) {
        ...
        if (!array_buffer.is_fixed_length()) {
            ...
            // FIXME: Growable SharedArrayBuffers are copied rather than shared through the side table below: growth may
            //        resize (and thereby move) the backing primitive storage, so aliases would need to coordinate on
            //        DynamicPrimitiveStorageSize growth instead of pinning a fixed byte length.
...
static Optional<JS::DataBlock> data_block_sharing_storage_with(JS::ArrayBuffer& source_buffer, size_t byte_length)
{
