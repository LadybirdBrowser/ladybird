...
GC::Ref<WebIDL::Promise> Navigator::get_battery()
{
    ...
    if (!m_battery_promise) {
        // FIXME: Don't reject promise here; let steps 2-3 determine resolution
    ...
    // 2. If this's relevant global object's associated Document is not allowed to use the "battery"
    //    policy-controlled feature, then reject this.[[BatteryPromise]] with a "NotAllowedError" DOMException.
    // FIXME: Check if Document is allowed to use "battery" policy-controlled feature

    // 3. Otherwise:
    //    1. If this.[[BatteryManager]] is null, then set it to the result of creating a new BatteryManager
    //       in this's relevant realm.
    // FIXME: Create new BatteryManager in this's relevant realm
