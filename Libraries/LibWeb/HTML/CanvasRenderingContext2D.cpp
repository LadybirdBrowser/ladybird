...
WebIDL::ExceptionOr<GC::Ptr<ImageData>> CanvasRenderingContext2D::get_image_data(int x, int y, int width, int height, Optional<ImageDataSettings> const& settings) const
{
    ...
    if (!surface)
        return image_data;
    auto const snapshot = Gfx::ImmutableBitmap::create(surface->snapshot_bitmap());
    ...
    auto snapshot_frame = Gfx::DecodedImageFrame::create(snapshot->bitmap().release_nonnull());
