...
void EventSource::reestablish_the_connection()
{
    ...
    auto spin_result = HTML::main_thread_event_loop().spin_until(GC::create_function(heap(), [&, delay_start = MonotonicTime::now()]() {
    ...
    if (!initial_task_has_run) {
        auto spin_result = HTML::main_thread_event_loop().spin_until(GC::create_function(heap(), [&]() { return initial_task_has_run; }));
        if (spin_result == HTML::EventLoop::SpinResult::ExitRequested)
            return;
    }
