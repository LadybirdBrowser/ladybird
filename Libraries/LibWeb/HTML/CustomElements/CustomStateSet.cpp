...
void CustomStateSet::add_state(Utf16FlyString const& state)
{
    m_states.set(state, AK::HashSetExistingEntryBehavior::Keep);
}

bool CustomStateSet::remove_state(Utf16FlyString const& state)
{
...
static Utf16FlyString state_from_set_value(JS::Value value)
{
    VERIFY(value.is_string());
    return Utf16FlyString::from_utf16(value.as_string().utf16_string_view());
}

static void add_state_to_set(JS::Set& set_entries, Utf16FlyString const& state)
{
...
static void remove_state_from_set(JS::Set& set_entries, Utf16FlyString const& state)
{
    auto& realm = HTML::relevant_realm(set_entries);
    auto value = GC::make_root(JS::PrimitiveString::create(realm.vm(), state));
...
static GC::Root<JS::Set> create_set_entries(JS::Realm& realm, OrderedHashTable<Utf16FlyString> const& states)
{
