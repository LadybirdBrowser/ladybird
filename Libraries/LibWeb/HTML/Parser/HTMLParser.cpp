...
void HTMLParser::handle_element_popped(DOM::Element& element)
{
    ...
    if (is<HTML::HTMLOptionElement>(element)) {
        // Non-standard: Flush buffered text so the option's content is up-to-date before cloning.
        flush_character_insertions();

        MUST(as<HTML::HTMLOptionElement>(element).maybe_clone_into_selectedcontent());
    }
}
