...
void HTMLTokenizer::will_emit(HTMLToken& token)
{
    if (token.is_start_tag())
        m_last_emitted_start_tag_name = token.tag_name().bytes_as_string_view();
...
bool HTMLTokenizer::current_end_tag_token_is_appropriate() const
{
    ...
    if (!m_last_emitted_start_tag_name.has_value())
        return false;
    return m_current_token.tag_name().bytes_as_string_view() == m_last_emitted_start_tag_name.value();
}
