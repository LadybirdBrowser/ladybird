...
WebIDL::ExceptionOr<GC::Ref<IDBIndex>> IDBObjectStore::create_index(Utf16String const& name, KeyPath key_path, IndexParameters const& options)
{
    ...
    if (store.uses_inline_keys()) {
        ...
        if (!maybe_kpk.is_error()) {
            ...
            if (key_value->is_invalid())
                return WebIDL::DataError::create(key_value->value_as_string());
        }
