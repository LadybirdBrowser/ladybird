...
WebIDL::ExceptionOr<void> IDBObjectStore::set_name(String const& value)
{
    ...
    if (store->is_deleted())
        return WebIDL::InvalidStateError::create("Object store has been deleted"_utf16);
    ...
    if (!transaction->is_upgrade_transaction())
        return WebIDL::InvalidStateError::create("Attempted to set name outside of version change"_utf16);

    // 6. If transaction’s state is not active, throw a "TransactionInactiveError" DOMException.
    if (!transaction->is_active())
        return WebIDL::TransactionInactiveError::create("Transaction is not active while updating object store name"_utf16);
    ...
    if (store->database()->object_store_with_name(name))
        return WebIDL::ConstraintError::create("Object store with the given name already exists"_utf16);
...
Optional<KeyPath> const& IDBObjectStore::key_path() const
{
    return m_store->key_path();
}

JS::ThrowCompletionOr<JS::Value> IDBObjectStore::key_path_value(JS::Realm& realm) const
{
    auto const& key_path = this->key_path();
    if (!key_path.has_value())
        return JS::js_null();

    return convert_a_key_path_to_a_value(realm, key_path.value());
}
...
WebIDL::ExceptionOr<GC::Ref<IDBIndex>> IDBObjectStore::create_index(String const& name, KeyPath key_path, IndexParameters const& options)
{
    ...
    if (!transaction->is_active())
        return WebIDL::TransactionInactiveError::create("Transaction is not active while creating index"_utf16);
    ...
    if (!is_valid_key_path(key_path))
        return WebIDL::SyntaxError::create("Key path is not valid"_utf16);
    ...
    if (key_path.has<Vector<String>>() && multi_entry)
        return WebIDL::InvalidAccessError::create("Key path is a sequence and multiEntry is true"_utf16);
...
WebIDL::ExceptionOr<GC::Ref<IDBIndex>> IDBObjectStore::index(String const& name)
{
    ...
    if (!index.has_value())
        return WebIDL::NotFoundError::create("Index not found in object store"_utf16);
...
WebIDL::ExceptionOr<void> IDBObjectStore::delete_index(String const& name)
{
    ...
    // 5. If transaction’s state is not active, then throw a "TransactionInactiveError" DOMException.
    if (!transaction->is_active())
        return WebIDL::TransactionInactiveError::create("Transaction is not active while deleting index"_utf16);

    // 6. Let index be the index named name in store if one exists, or throw a "NotFoundError" DOMException otherwise.
    auto index = m_indexes.get(name);
    if (!index.has_value())
        return WebIDL::NotFoundError::create("Index not found while trying to delete it"_utf16);
...
WebIDL::ExceptionOr<GC::Ref<IDBRequest>> IDBObjectStore::add_or_put(GC::Ref<IDBObjectStore> handle, JS::Value value, Optional<JS::Value> const& key, bool no_overwrite)
{
    ...
    if (!transaction->is_active())
        return WebIDL::TransactionInactiveError::create("Transaction is not active while running add/put"_utf16);

    // 5. If transaction is a read-only transaction, throw a "ReadOnlyError" DOMException.
    if (transaction->is_readonly())
        return WebIDL::ReadOnlyError::create("Transaction is read-only"_utf16);
    ...
    if (store.uses_inline_keys() && key_was_given)
        return WebIDL::DataError::create("Store uses in-line keys and key was given"_utf16);
    ...
    if (store.uses_out_of_line_keys() && !store.uses_a_key_generator() && !key_was_given)
        return WebIDL::DataError::create("Store uses out-of-line keys and has no key generator and key was not given"_utf16);
    ...
    if (key_was_given) {
        ...
        if (r->is_invalid())
            return WebIDL::DataError::create("Key is invalid"_utf16);
    ...
    if (store.uses_inline_keys()) {
        ...
        if (!maybe_kpk.is_error()) {
            ...
            if (key_value->is_invalid())
                return WebIDL::DataError::create(Utf16String::from_utf8(key_value->value_as_string()));
        }
        ...
        // 4. Otherwise (kpk is failure):
            ...
            if (!store.uses_a_key_generator())
                return WebIDL::DataError::create("Store does not have a key generator"_utf16);
            ...
            if (!check_that_a_key_could_be_injected_into_a_value(realm, clone, store.key_path().value()))
                return WebIDL::DataError::create("Key could not be injected into value"_utf16);
        }
    }
...
WebIDL::ExceptionOr<GC::Ref<IDBRequest>> IDBObjectStore::count(Optional<JS::Value> query)
{
    ...
    // 4. If transaction’s state is not active, then throw a "TransactionInactiveError" DOMException.
    if (!transaction->is_active())
        return WebIDL::TransactionInactiveError::create("Transaction is not active while doing count"_utf16);
...
WebIDL::ExceptionOr<GC::Ref<IDBRequest>> IDBObjectStore::open_cursor(Optional<JS::Value> query, CursorDirection direction)
{
    auto& realm = HTML::relevant_realm(relevant_global_object());
...
WebIDL::ExceptionOr<GC::Ref<IDBRequest>> IDBObjectStore::delete_(JS::Value query)
{
    ...
    // 5. If transaction is a read-only transaction, throw a "ReadOnlyError" DOMException.
    if (transaction->is_readonly())
        return WebIDL::ReadOnlyError::create("Transaction is read-only while deleting object store"_utf16);
...
WebIDL::ExceptionOr<GC::Ref<IDBRequest>> IDBObjectStore::clear()
{
    ...
    // 4. If transaction’s state is not active, then throw a "TransactionInactiveError" DOMException.
    if (!transaction->is_active())
        return WebIDL::TransactionInactiveError::create("Transaction is not active while clearing object store"_utf16);

    // 5. If transaction is a read-only transaction, throw a "ReadOnlyError" DOMException.
    if (transaction->is_readonly())
        return WebIDL::ReadOnlyError::create("Transaction is read-only while clearing object store"_utf16);
...
WebIDL::ExceptionOr<GC::Ref<IDBRequest>> IDBObjectStore::open_key_cursor(Optional<JS::Value> query, CursorDirection direction)
{
    auto& realm = HTML::relevant_realm(relevant_global_object());
...
WebIDL::ExceptionOr<GC::Ref<IDBRequest>> IDBObjectStore::get_all_records(RetrieveMultipleItemsOptions const& options)
{
    // 1. Return the result of creating a request to retrieve multiple items with the current Realm record, this,
