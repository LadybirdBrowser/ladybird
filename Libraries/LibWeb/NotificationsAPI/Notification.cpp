    ...
    HighResolutionTime::EpochTimeStamp fallbackTimestamp)
{
    ...
    // FIXME: 15. If options["vibrate"] exists, then validate and normalize it and set notification’s vibration pattern to the return value.
    ...
    GC::Ptr<HTML::EnvironmentSettingsObject> settings)
{
    ...
    HighResolutionTime::EpochTimeStamp fallbacktimestamp = round_to<HighResolutionTime::EpochTimeStamp>(settings->current_wall_time());

    // 4. Return the result of creating a notification given title, options, origin, baseURL, and fallbackTimestamp.
    return create_a_notification(realm, move(title), options, origin, base_URL, fallbacktimestamp);
}
    ...
    Optional<NotificationOptions> options)
{
    ...
    auto exception_or_notification = create_a_notification_with_a_settings_object(realm, move(title), options, relevant_settings_object);

    if (exception_or_notification.is_exception())
        return exception_or_notification.release_error();
    ConceptNotification notification = exception_or_notification.release_value();
    ...
    // FIXME: 1. If the result of getting the notifications permission state is not "granted", then queue a task to fire an event named error on this, and abort these steps.
