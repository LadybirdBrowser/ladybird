...
[[nodiscard]] static MatchResult port_part_matches(Optional<StringView> input, URL::URL const& url)
{
    ...
    if (input.has_value()) {
        ...
        auto maybe_port = AK::StringUtils::convert_to_uint<u16>(input.value(), TrimWhitespace::No);
...
[[nodiscard]] static MatchResult does_integrity_metadata_match_source_list(String const& integrity_metadata, Vector<String> const& source_list)
{
    // 1. Assert: source list is not null.
    ...
    for (auto const& source : integrity_sources) {
        ...
        auto maybe_match = integrity_expressions.find_if([&source](auto const& integrity_expression) {
            ...
            return Infra::is_ascii_case_insensitive_match(integrity_expression.hash_algorithm.value(), source.algorithm)
...
Directive::Result script_directives_pre_request_check(GC::Ref<Fetch::Infrastructure::Request const> request, GC::Ref<Directive const> directive, GC::Ref<Policy const> policy)
{
    ...
    if (request->destination_is_script_like()) {
        ...
        auto maybe_strict_dynamic = directive->value().find_if([](auto const& directive_value) {
            return Infra::is_ascii_case_insensitive_match(directive_value, KeywordSources::StrictDynamic);
        });
...
enum class AllowsResult {
    DoesNotAllow,
    Allows,
};

[[nodiscard]] static AllowsResult does_a_source_list_allow_all_inline_behavior_for_type(Vector<String> const& source_list, Directive::InlineType type)
{
    ...
    for (auto const& expression : source_list) {
        ...
        if (type == Directive::InlineType::Script || type == Directive::InlineType::ScriptAttribute || type == Directive::InlineType::Navigation) {
            if (Infra::is_ascii_case_insensitive_match(expression, KeywordSources::StrictDynamic))
                return AllowsResult::DoesNotAllow;
        }

        // 3. If expression is an ASCII case-insensitive match for the keyword-source "'unsafe-inline'", set allow all
        //    inline to true.
        if (Infra::is_ascii_case_insensitive_match(expression, KeywordSources::UnsafeInline))
...
MatchResult does_element_match_source_list_for_type_and_source(GC::Ptr<DOM::Element const> element, Vector<String> const& source_list, Directive::InlineType type, String const& source)
{
        ...
        for (auto const& expression : source_list) {
            ...
            if (nonce_source_parse_result.has_value()) {
                ...
                if (is<HTML::HTMLElement>(element.ptr())) {
                    ...
                    auto const& svg_element = verify_cast<SVG::SVGElement>(*element);
    ...
    for (auto const& expression : source_list) {
        ...
        if (Infra::is_ascii_case_insensitive_match(expression, KeywordSources::UnsafeHashes)) {
    ...
    if (type == Directive::InlineType::Script || type == Directive::InlineType::Style || unsafe_hashes_flag) {
        ...
        for (auto const& expression : source_list) {
            ...
            if (hash_source_parse_result.has_value()) {
                ...
                if (Infra::is_ascii_case_insensitive_match(hash_algorithm_from_expression, "sha256"sv))
                ...
                if (Infra::is_ascii_case_insensitive_match(hash_algorithm_from_expression, "sha384"sv))
                ...
                if (Infra::is_ascii_case_insensitive_match(hash_algorithm_from_expression, "sha512"sv))
