...
Directive::Result DefaultSourceDirective::pre_request_check(JS::Realm& realm, GC::Ref<Fetch::Infrastructure::Request const> request, GC::Ref<Policy const> policy) const
{
    ...
    auto directive = create_directive(realm, name->to_string(), value());
    return directive->pre_request_check(realm, request, policy);
}

// https://w3c.github.io/webappsec-csp/#default-src-post-request
Directive::Result DefaultSourceDirective::post_request_check(JS::Realm& realm, GC::Ref<Fetch::Infrastructure::Request const> request, GC::Ref<Fetch::Infrastructure::Response const> response, GC::Ref<Policy const> policy) const
{
    // 1. Let name be the result of executing § 6.8.1 Get the effective directive for request on request.
    auto name = get_the_effective_directive_for_request(request);
    ...
    auto directive = create_directive(realm, name->to_string(), value());
    return directive->post_request_check(realm, request, response, policy);
}

// https://w3c.github.io/webappsec-csp/#default-src-inline
Directive::Result DefaultSourceDirective::inline_check(JS::Realm& realm, GC::Ptr<DOM::Element const> element, InlineType type, GC::Ref<Policy const> policy, String const& source) const
{
    ...
    auto directive = create_directive(realm, name.to_string(), value());
    return directive->inline_check(realm, element, type, policy, source);
}
