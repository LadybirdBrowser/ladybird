...
GC::Ref<SecurityPolicyViolationEvent> SecurityPolicyViolationEvent::create(JS::Realm& realm, Utf16FlyString const& event_name, Bindings::SecurityPolicyViolationEventInit const& event_init)
{
    return realm.create<SecurityPolicyViolationEvent>(realm, event_name, event_init);
}

WebIDL::ExceptionOr<GC::Ref<SecurityPolicyViolationEvent>> SecurityPolicyViolationEvent::construct_impl(JS::Realm& realm, Utf16FlyString const& event_name, Bindings::SecurityPolicyViolationEventInit const& event_init)
{
    return realm.create<SecurityPolicyViolationEvent>(realm, event_name, event_init);
}

SecurityPolicyViolationEvent::SecurityPolicyViolationEvent(JS::Realm& realm, Utf16FlyString const& event_name, Bindings::SecurityPolicyViolationEventInit const& event_init)
