    ...
    for (auto& wrappable : m_preserved_wrappables) {
        if (wrappable) {
            for (auto const& entry : wrappable->m_preserved_wrappers) {
                if (entry.world.ptr() != this)
                    continue;
                VERIFY(entry.wrapper);
                VERIFY(m_wrappers.get(*wrappable) == entry.wrapper.ptr());
            }
...
void WrapperWorld::register_preserved_wrappable(Wrappable& wrappable)
{
    VERIFY(!is_main_world());
    VERIFY(!is_detached());
    if (!m_preserved_wrappables.contains_slow(GC::Weak<Wrappable> { wrappable }))
        m_preserved_wrappables.append(wrappable);
}
