...
JsonObject timeouts_object(TimeoutsConfiguration const& timeouts)
{
    ...
    serialized.set("script"_sv, timeouts.script_timeout.has_value() ? *timeouts.script_timeout : JsonValue {});

    // 3. Set serialized["pageLoad"] to timeouts' page load timeout.
    serialized.set("pageLoad"_sv, timeouts.page_load_timeout.has_value() ? *timeouts.page_load_timeout : JsonValue {});

    // 4. Set serialized["implicit"] to timeouts' implicit wait timeout.
    serialized.set("implicit"_sv, timeouts.implicit_wait_timeout.has_value() ? *timeouts.implicit_wait_timeout : JsonValue {});
...
ErrorOr<void, Error> json_deserialize_as_a_timeouts_configuration_into(JsonValue const& timeouts, TimeoutsConfiguration& configuration)
{
    ...
    if (!timeouts.is_object())
        return Error::from_code(ErrorCode::InvalidArgument, "Payload is not a JSON object"_sv);
    ...
    TRY(timeouts.as_object().try_for_each_member([&](auto const& key, JsonValue const& value) -> ErrorOr<void, Error> {
        ...
        if (!key.is_one_of("script"_sv, "pageLoad"_sv, "implicit"_sv))
            return {};
        ...
        if (!value.is_null()) {
            ...
            if (!duration.has_value() || *duration > JS::MAX_ARRAY_LIKE_INDEX)
                return Error::from_code(ErrorCode::InvalidArgument, "Invalid timeout value"_sv);
