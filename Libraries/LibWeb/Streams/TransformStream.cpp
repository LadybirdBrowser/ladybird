...
WebIDL::ExceptionOr<GC::Ref<TransformStream>> TransformStream::create_for_constructor(JS::Object& relevant_global_object, GC::Ptr<JS::Object> transformer_object, QueuingStrategy const& writable_strategy, QueuingStrategy const& readable_strategy)
{
    ...
    if (transformer_dict.readable_type.has_value())
        return WebIDL::SimpleException { WebIDL::SimpleExceptionType::RangeError, "Invalid use of reserved key 'readableType'"_utf16 };
    if (transformer_dict.writable_type.has_value())
        return WebIDL::SimpleException { WebIDL::SimpleExceptionType::RangeError, "Invalid use of reserved key 'writableType'"_utf16 };
...
WebIDL::ExceptionOr<GC::Ref<TransformStream>> TransformStream::create(JS::Realm& realm, GC::Ptr<JS::Object> transformer_object, Transformer const& transformer_dict, QueuingStrategy const& writable_strategy, QueuingStrategy const& readable_strategy)
{
    ...
    if (transformer_dict.start) {
        ...
        WebIDL::resolve_promise(start_promise, result);
    }
    // 13. Otherwise, resolve startPromise with undefined.
    else {
        WebIDL::resolve_promise(start_promise);
    }
