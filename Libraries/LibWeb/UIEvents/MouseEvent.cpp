    ...
    , m_buttons(options.buttons)
{
    ...
    set_related_target(options.related_target);
}

MouseEvent::MouseEvent(Utf16FlyString const& event_name, MouseEventOptions const& options, double page_x, double page_y, double offset_x, double offset_y, HighResolutionTime::DOMHighResTimeStamp time_stamp)
    : UIEvent(event_name, time_stamp)
    , m_screen_x(options.screen_x)
    , m_screen_y(options.screen_y)
    , m_page_x(page_x)
...
void MouseEvent::init_mouse_event(Utf16FlyString const& type, bool bubbles, bool cancelable, GC::Ptr<HTML::WindowProxy> view, WebIDL::Long detail, WebIDL::Long screen_x, WebIDL::Long screen_y, WebIDL::Long client_x, WebIDL::Long client_y, bool ctrl_key, bool alt_key, bool shift_key, bool meta_key, WebIDL::Short button, GC::Ptr<DOM::EventTarget> related_target)
{
...
GC::Ref<MouseEvent> MouseEvent::create(Utf16FlyString const& event_name, MouseEventOptions const& options, double page_x, double page_y, double offset_x, double offset_y, HighResolutionTime::DOMHighResTimeStamp time_stamp)
{
    return GC::Heap::the().allocate<MouseEvent>(event_name, options, page_x, page_y, offset_x, offset_y, time_stamp);
}
...
WebIDL::ExceptionOr<GC::Ref<MouseEvent>> MouseEvent::create_for_constructor(Utf16String const& event_name, Bindings::MouseEventInit const& event_init, HighResolutionTime::DOMHighResTimeStamp time_stamp)
{
    return create(Utf16FlyString::from_utf16(event_name.utf16_view()), mouse_event_options_from_bindings(event_init), event_init.client_x, event_init.client_y, event_init.client_x, event_init.client_y, time_stamp);
}
...
GC::Ref<MouseEvent> MouseEvent::create_from_mouse_event(MouseEvent const& mouse_event, HighResolutionTime::DOMHighResTimeStamp time_stamp)
{
    ...
    options.related_target = mouse_event.related_target();
