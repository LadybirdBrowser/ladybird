...
struct Operation {
    ...
    // FIXME: Add a less than operator to properly sort arguments out
...
ErrorOr<String> Compiler::emit_spirv_text()
{
    ...
    for (auto const& decl : functions_decls) {
        ...
        // FIXME: AST could be adjusted to allow for using OpComposite* instructions instead when working with structs
    ...
    for (size_t i = 0; i < num_entry_points; ++i) {
        ...
        case SPVC_BACKEND_HLSL: {
            if (backend == SPVC_BACKEND_HLSL) {
                result = spvc_compiler_options_set_uint(options, SPVC_COMPILER_OPTION_HLSL_SHADER_MODEL, 50);
                if (result != SPVC_SUCCESS)
                    return Error::from_string_literal("Unable to set HLSL shader model");

                result = spvc_compiler_options_set_bool(options, SPVC_COMPILER_OPTION_HLSL_FORCE_STORAGE_BUFFER_AS_UAV, false);
                if (result != SPVC_SUCCESS)
                    return Error::from_string_literal("Unable to set HLSL UAV option");

                result = spvc_compiler_options_set_bool(options, SPVC_COMPILER_OPTION_HLSL_POINT_SIZE_COMPAT, true);
                if (result != SPVC_SUCCESS)
                    return Error::from_string_literal("Unable to set HLSL point size compatibility");

                result = spvc_compiler_options_set_bool(options, SPVC_COMPILER_OPTION_HLSL_SUPPORT_NONZERO_BASE_VERTEX_BASE_INSTANCE, true);
                if (result != SPVC_SUCCESS)
                    return Error::from_string_literal("Unable to set HLSL vertex base support");
            }
            break;
        }
        default:
...
ErrorOr<Vector<Compiler::Shader>> Compiler::emit_hlsl()
{
    Vector<uint32_t> const spirv = TRY(emit_spirv_binary(TRY(emit_spirv_text())));
    return emit_backend(SPVC_BACKEND_HLSL, spirv);
}
