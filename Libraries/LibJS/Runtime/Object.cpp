...
ThrowCompletionOr<GC::RootVector<Value>> Object::enumerable_own_property_names(PropertyKind kind) const
{
    ...
    TRY(for_each_own_property_with_enumerability([&](PropertyKey const& property_key, Value key, bool enumerable) -> ThrowCompletionOr<void> {
...
ThrowCompletionOr<void> Object::for_each_own_property_with_enumerability(Function<ThrowCompletionOr<void>(PropertyKey const&, Value, bool)>&& callback) const
{
    auto& vm = this->vm();
    if (eligible_for_own_property_enumeration_fast_path()) {
        ...
        for (auto& entry : m_indexed_properties) {
            auto key = PrimitiveString::create(vm, String::number(entry.index()));
            keys.unchecked_append({ PropertyKey(entry.index()), key, entry.enumerable() });
        }

        for (auto const& [property_key, metadata] : shape().property_table()) {
            if (!property_key.is_string())
                continue;
            keys.unchecked_append({ property_key, property_key.to_value(vm), metadata.attributes.is_enumerable() });
        }

        for (auto& key : keys)
            TRY(callback(key.property_key, key.key, key.enumerable));
    } else {
        auto keys = TRY(internal_own_property_keys());
        for (auto& key : keys) {
            ...
            TRY(callback(property_key, key, enumerable));
        }
    }
    return {};
}
