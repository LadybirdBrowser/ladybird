...
static ThrowCompletionOr<Value> regexp_builtin_exec(VM& vm, RegExpObject& regexp_object, GC::Ref<PrimitiveString> string)
{
    ...
    if (has_indices) {
        ...
        if (has_groups) {
            ...
            for (auto const& group_name : regex.parser_result.capture_groups) {
                auto existing_value = indices_groups_object.get_without_side_effects(Utf16FlyString::from_utf8(group_name));
                auto value_to_add = existing_value.is_undefined() ? js_undefined() : existing_value;
                MUST(ordered_indices_groups_object->create_data_property_or_throw(Utf16FlyString::from_utf8(group_name), value_to_add));
            }
