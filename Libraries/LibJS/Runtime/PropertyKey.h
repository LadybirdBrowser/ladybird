...
public:
    ...
    PropertyKey(char const (&chars)[N])
        : PropertyKey(MUST(FlyString::from_utf8(StringView { chars, N - 1 })))
    {
    }
...
private:
    ...
    static Variant<FlyString, u32> try_coerce_into_number(FlyString string, StringMayBeNumber string_may_be_number)
    {
        ...
        if (string.is_empty())
            return string;
        if (string.bytes_as_string_view().starts_with("0"sv) && string.bytes_as_string_view().length() != 1)
            return string;
