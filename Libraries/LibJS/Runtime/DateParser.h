...
private:
    ...
    ALWAYS_INLINE bool one_number(u64 const number)
    { // The whole input string is just one stand-alone number.}
        switch (number) {
        case 0:
            m_year = 2000;
            return true;
    ...
    // --- Permissive date parser for non-standard date strings. -- //

    ALWAYS_INLINE bool loop()
    { // Greedy shallow parse for date components.
        switch (peek()) {
        case '0' ... '9':
    ...
    ALWAYS_INLINE bool maybe_ampm()
    { // Side effect: consumes space even if there is no AM/PM.
    ...
    ALWAYS_INLINE bool maybe_time(std::pair<u32, size_t> const& hours)
    ...
    ALWAYS_INLINE bool maybe_sign()
    {
        ...
        case 0:
            return (sign == -1); // Not a sign after all; '+' is forbidden if it is not a sign. '-' is ignored as punctuation.
        case 1 ... 5:
            break;
        case 6:                      // signed year
            if (m_hours.has_value()) // Candidate for timezone offset.
                break;
            if (m_year.has_value())
                return false;
        ...
        if (!m_hours.has_value()) {
            m_numbers.append(number.first); // No "bare" timezone offset without time.
            return true;
        }
    ...
    ALWAYS_INLINE bool separator()
    { // Ignore space and Firefox punctuation.
        if (is_eof())
            return true;
        switch (peek()) {
        case ' ':
    ...
    ALWAYS_INLINE bool gmt(StringView const& str)
    { // Z or GMT or UTC can be used interchangeably.
    ...
    {                              // Same as Chrome and Firefox, we support abbreviations for all US mainland timezones.
    ...
    ALWAYS_INLINE bool word()
    {
        std::ignore = consume_while(isalpha);
        return (m_numbers.is_empty() && !m_hours.has_value() && !m_year.has_value()); // Ignore junk (bare words) at the beginning, just like Firefox and Chrome.
    }
    ALWAYS_INLINE bool maybe_word()
    ...
    // --- Capture simplified ISO8601 date format. --- //
    ...
    ALWAYS_INLINE ErrorOr<bool> maybe_iso_year4()
    { // 2025
    ...
    ALWAYS_INLINE ErrorOr<bool> maybe_iso_signed_year6()
    { // +002025
        i64 sign = +1;
        if (consume() == '-')
            sign = -1;
        ...
        case 0:
            ...
            return false;
        case 1 ... 5:
            m_numbers.append(number.first); // This is not an ISO 8601 date string.
            return false;
        case 6:
            break;
        default:
    ...
    ALWAYS_INLINE ErrorOr<bool> maybe_iso_month_day()
    { // [-MM[-DD]]
        if (!consume_specific('-'))
            return true;

        auto month = read_number<u16, 3>();
        if (consume_specific(':')) { // 2000-12:34; Firefox and Chrome parse it correctly. We do the same.
            if (maybe_time(month))
                return false;
    ...
    ALWAYS_INLINE ErrorOr<bool> maybe_iso_time()
    { // THH:MM[:SS[.M[SS...]]]
    ...
    ALWAYS_INLINE ErrorOr<void> maybe_iso_tz()
    {
        switch (consume()) {
        case 'Z':
            m_timezone_utc = true;
        ...
        if (!tz_offset(read_number<u32, 7>(), true))
            return Error::from_string_literal("Invalid timezone offset format.");

        return {};
    }

    ALWAYS_INLINE double build_date(bool date_iso8601 = false)
    {
        VERIFY(is_eof());
    ...
    ALWAYS_INLINE ErrorOr<double> _parse()
    {
        if (TRY(maybe_iso_8601()))
            return build_date(true);

        // Convert the input string to uppercase after parsing ISO8601 failed.
        // This saves some time on parsing ISO8601.
        // The index stays exactly where it was.
