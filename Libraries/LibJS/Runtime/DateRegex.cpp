...
DateRegexGenerator::DatePatterns DateRegexGenerator::create_date_patterns()
{
    ...
    DateRegex const FORBIDDEN { "[T+:?~{}!@#$&*()=`>;<]"_string }; // Fail if these are not captured by other productions.
    ...
    DateRegex const month_name = one_of(jan, feb, mar, apr, may, jun, jul, aug, sep, oct, nov, dec);
    DateRegex const month_name3 = one_of(jan3, feb3, mar3, apr3, may3, jun3, jul3, aug3, sep3, oct3, nov3, dec3); // 3-letter month name. Use in date_tostring and date_toutcstring
    ...
    DateRegex const tzh = T24.group(TZHH); // permissive hour
    ...
    DateRegex const invalid_minus_zero_year_iso8601 = DASH + DateRegex { "000000"_string }.group(FAIL);

    // Fragments that support specific terminals.
    DateRegex const year_iso8601 = one_of(year4, invalid_minus_zero_year_iso8601, signed_year); // https://tc39.es/ecma262/#sec-expanded-years
    ...
    DateRegex const tz_iso8601 = one_of(
        Z.group(TZUTC),
        one_of(
            tzsign + tzhh + COLON + tzmin + maybe(COLON + tzsec + maybe((DOT + tzns))), // extended format
    ...
    DateRegex const time_with_colon = // H[H]:MM[:SS[.MSs...]]
        T24.group(HH) + COLON + MAYBE_SPACES + maybe(min + maybe(COLON + MAYBE_SPACES + sec + maybe(one_of(DOT + ms_relaxed, DOT.group(FAIL)))))
        + MAYBE_SPACES + maybe(ampm);
    DateRegex const tz_with_colon = // +H[H]:MM[:SS[.Ns]]
        maybe(gmt) + MAYBE_SPACES + PLUS_OR_MINUS.group(TZSIGN) + tzh + COLON + tzmin + maybe(COLON + tzsec + maybe(DOT + tzns));
    ...
    DateRegex const ecma_script_datetime = full(year4.group(ISO8601) + maybe(DASH + month + maybe(DASH + day)) + maybe(T + hour + COLON + min + maybe(COLON + sec + maybe(DOT + ms)) + maybe(one_of(Z.group(TZUTC), (tzsign + tzhh + COLON + tzmin)))));

    // ecma_script_date_time plus iso8601 extensions.
    // https://tc39.es/ecma262/#sec-date-time-string-format
    DateRegex const iso8601_simplified_date_time = full(year_iso8601.group(ISO8601) + maybe(DASH + month + maybe(DASH + day)) + maybe(T + hour + COLON + min + maybe(COLON + sec + maybe(DOT + ms_relaxed)) + maybe(tz_iso8601)));

    DateRegex const date_tostring = full( // Wed Dec 08 1999 00:00:00 GMT-0800 (Pacific Standard Time) https://tc39.es/ecma262/#sec-todatestring
    ...
    DateRegex const date_toutcstring = full( // Wed, 08 Dec 1999 08:00:00 GMT  https://tc39.es/ecma262/#sec-date.prototype.toutcstring
    ...
    DateRegex const permissive = one_of(
        ...
        FORBIDDEN.group(FAIL),
        SOME_PUNCT,            // Accept punctuation anywhere.
        SOME_JUNK.group(JUNK), // Junk. Ignore at the beginning, fail after that.
        SOME_SPACE             // Do nothing on space.
    );

    return DateRegexGenerator::DatePatterns {
        .ecma_script_datetime = ecma_script_datetime,
        .iso8601_simplified_datetime = iso8601_simplified_date_time,
        .date_tostring = date_tostring,
        .date_toutcstring = date_toutcstring,
