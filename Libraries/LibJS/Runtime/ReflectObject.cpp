...
GC_DEFINE_ALLOCATOR(ReflectObject);
...
JS_DEFINE_NATIVE_FUNCTION(ReflectObject::construct)
{
    ...
    if (!target.is_constructor())
        return vm.throw_completion<TypeError>(ErrorType::NotAConstructor, target);
    ...
    // 3. Else if IsConstructor(newTarget) is false, throw a TypeError exception.
        ...
        return vm.throw_completion<TypeError>(ErrorType::NotAConstructor, new_target);
