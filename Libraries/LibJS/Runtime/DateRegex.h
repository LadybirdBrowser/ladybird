...
// - Compatible with Mozilla Firefox 134.0.1 and Chromium 131.0.6778.264. Within reason.
//   Differences indicated in comments.
//   - Where both agree on a parse, support it.
//   - Where they disagree, support the one that seems more sane.
//   - In very limited cases, pick our own way. Example: "<number> Month":
//     - Firefox fails on all "<number> Month" productions
...
public:
    ...
    DateRegex(String const& str, Precedence type = Precedence::Character)
        : RegexPattern(str, type)
    {
    }

    ALWAYS_INLINE double parse(StringView const& str, regex::PosixFlags flags = regex::PosixFlags::Insensitive) const
    {
        RegexResult result = match(str, flags);
        ...
        for (size_t i = 0; i < result.capture_group_matches.size(); ++i) {
            ...
            for (size_t j = 0; j < groups.size(); ++j) {
                ...
                case YEAR:
                    ...
                    break;
                case SIGN:
                    sign = does_group_match_char(group, '-') ? -1 : 1;
                    break;
                case MONTH:
                ...
                case ONE_NUMBER:
                    ...
                    break;
                case TZUTC:
                    timezone_utc = true; // Simply indicates that Z/GMT/UTC was caught. Can be overwritten later.
                ...
                case TZSIGN:
                    if (timezone_sign.has_value()) // Firefox fails if there is more than one explicit time zone. We do the same.
                        return NAN;
                    timezone_utc = false;
                    timezone_sign = does_group_match_char(group, '-') ? -1 : +1;
                    break;
                case TZHH:
                ...
                default:
                    warnln("RegexPattern uncaptured group: '{}'", name.value());
                    break;
                }
            }
        }

        // "24" for hours needs 0 min 0 sec
        if ((hours == 24) && ((minutes.has_value() && minutes.value() > 0) || (seconds.has_value() && seconds.value() > 0)))
            return NAN;
    ...
    ALWAYS_INLINE static DateRegex ecma_script_datetime()
    {
    ...
    ALWAYS_INLINE static DateRegex iso8601_simplified_datetime()
    {
        return DateRegex { "^(?<L>(?<B>[0-9]{4})|-(?<Y>000000)|(?<B>[+\\-][0-9]{6}))(?:-(?<C>0[0-9]|1[0-2])(?:-(?<D>[0-2][0-9]|3[01]))?)?(?:T(?<E>[01][0-9]|2[0-4]):(?<F>[0-5][0-9])(?::(?<G>[0-5][0-9])(?:[.](?<I>[0-9]{1,3})[0-9]*)?)?(?:(?<S>Z)|(?<M>[+\\-])(?<N>[01][0-9]|2[0-4]):(?<O>[0-5][0-9])(?::(?<P>[0-5][0-9])(?:[.](?<Q>[0-9]{1,9}))?)?|(?<M>[+\\-])(?<R>[0-9]{4})|(?<Y>[ ]*[+\\-][0-9]{1,3}))?)?$"_string };
    }
    ALWAYS_INLINE static DateRegex date_tostring()
    {
        return DateRegex { "^(?:Sun|Mon|Tue|Wed|Thu|Fri|Sat)[ ](?:(?<a>Jan)|(?<b>Feb)|(?<c>Mar)|(?<d>Apr)|(?<e>May)|(?<f>Jun)|(?<g>Jul)|(?<h>Aug)|(?<i>Sep)|(?<j>Oct)|(?<k>Nov)|(?<l>Dec))[ ](?<D>[0-2][0-9]|3[01])[ ](?<B>[0-9]{4})[ ](?<E>[01][0-9]|2[0-4]):(?<F>[0-5][0-9]):(?<G>[0-5][0-9])[ ](?<S>GMT)(?:(?<M>[+\\-])(?<R>[0-9]{4})(?:[ ][(][a-z ]+[)])?)?$"_string };
    }
    ALWAYS_INLINE static DateRegex date_toutcstring()
    {
        return DateRegex { "^(?:Sun|Mon|Tue|Wed|Thu|Fri|Sat),[ ](?<D>[0-2][0-9]|3[01])[ ](?:(?<a>Jan)|(?<b>Feb)|(?<c>Mar)|(?<d>Apr)|(?<e>May)|(?<f>Jun)|(?<g>Jul)|(?<h>Aug)|(?<i>Sep)|(?<j>Oct)|(?<k>Nov)|(?<l>Dec))[ ](?<B>[0-9]{4})[ ](?<E>[01][0-9]|2[0-4]):(?<F>[0-5][0-9]):(?<G>[0-5][0-9])[ ](?<S>GMT)$"_string };
    }

    ALWAYS_INLINE static DateRegex es_date_parse()
    {
        return DateRegex { "^(?<L>(?<B>[0-9]{4})|-(?<Y>000000)|(?<B>[+\\-][0-9]{6}))(?:-(?<C>0[0-9]|1[0-2])(?:-(?<D>[0-2][0-9]|3[01]))?)?(?:T(?<E>[01][0-9]|2[0-4]):(?<F>[0-5][0-9])(?::(?<G>[0-5][0-9])(?:[.](?<I>[0-9]{1,3})[0-9]*)?)?(?:(?<S>Z)|(?<M>[+\\-])(?<N>[01][0-9]|2[0-4]):(?<O>[0-5][0-9])(?::(?<P>[0-5][0-9])(?:[.](?<Q>[0-9]{1,9}))?)?|(?<M>[+\\-])(?<R>[0-9]{4})|(?<Y>[ ]*[+\\-][0-9]{1,3}))?)?$|(?<Y>[0-9]{3}:)|(?<E>[01][0-9]|2[0-4]|[0-9]):[ ]*(?:(?<F>[0-5][0-9])(?::[ ]*(?<G>[0-5][0-9])(?:[.](?<I>[0-9]{1,3})[0-9]*|(?<Y>[.]))?)?)?[ ]*(?:(?<J>am)|(?<K>pm))?|(?<S>GMT|UTC|Z)?[ ]*(?<M>[+\\-])(?<N>[01][0-9]|2[0-4]|[0-9]):(?<O>[0-5][0-9])(?::(?<P>[0-5][0-9])(?:[.](?<Q>[0-9]{1,9}))?)?|(?<S>GMT|UTC|Z)[ ]*(?<M>[+\\-])(?<R>[0-9]{1,4})(?<Y>[0-9]{1,3})?|^[-./, ]*(?<V>[0-9]{1,6})[-./, ]*$|(?<B>[+\\-][0-9]{6})|(?<A>[+\\-])(?<U>[0-9]{1,4})|(?<T>[0-9]{1,6})|(?<a>jan[a-z]{0,8})|(?<b>feb[a-z]{0,8})|(?<c>mar[a-z]{0,8})|(?<d>apr[a-z]{0,8})|(?<e>may[a-z]{0,8})|(?<f>jun[a-z]{0,8})|(?<g>jul[a-z]{0,8})|(?<h>aug[a-z]{0,8})|(?<i>sep[a-z]{0,8})|(?<j>oct[a-z]{0,8})|(?<k>nov[a-z]{0,8})|(?<l>dec[a-z]{0,8})|(?<n>est)|(?<m>edt)|(?<p>cst)|(?<o>cdt)|(?<r>mst)|(?<q>mdt)|(?<t>pst)|(?<s>pdt)|(?<S>GMT|UTC|Z)|[(][^)]+[)]?$|(?<Y>[T+:?~{}!@#$&*()=`>;<])|[-./,]+|(?<X>[A-Za-z][A-Za-z./-,]*)|[ ]+"_string };
    }
};
