...
ThrowCompletionOr<Value> ECMAScriptFunctionObject::ordinary_call_evaluate_body(VM& vm, ExecutionContext& context)
{
    ...
    if (kind() == FunctionKind::AsyncGenerator) {
        auto async_generator_object = TRY(AsyncGenerator::create(*context.realm, result, GC::Ref { *this }, context.copy()));
        return async_generator_object;
    }

    auto generator_object = TRY(GeneratorObject::create(*context.realm, result, GC::Ref { *this }, context.copy()));
