...
// flips between PST/PDT (ok) and the latter overrides PDT seasonally (agruably incorrect).
...
test("ambiguous date: 1 number", () => {
    ...
    // Puunctuation is generally ignored.
...
test("ambiguous date: 2 numbers", () => {
    ...
    expect("-002002-02-01T08:12:28.000Z").toBe(new Date("-002002 2").toISOString());
    setTimeZone(originalTimeZone);
});
...
test("ambiguous date: month + number", () => {
    // Firefox fails every time; Chrome is [arguably] inconsistent. We are correct every time.
    const originalTimeZone = setTimeZone("America/Vancouver");
...
test("timezone offset", () => {
    // Mostly with Chrome and Firefox
    const originalTimeZone = setTimeZone("America/Vancouver");
    ...
    expect("2019-07-18T11:22:00.000Z").toBe(new Date("07/18/2019 11:22GMT").toISOString());
    ...
    expect("1989-03-07T22:37:00.000Z").toBe(new Date("3/8/89 +01:23").toISOString());
    ...
    expect("2025-02-11T00:02:00.000Z").toBe(new Date("2/11/25 1:02+1").toISOString());
    expect("2025-02-11T00:02:00.000Z").toBe(new Date("2/11/25 1:02+01").toISOString());
    expect("2025-02-11T00:50:00.000Z").toBe(new Date("2/11/25 1:02+012").toISOString()); // Same as +00:00
    ...
    expect(Date.parse("2/11/25 1:02+012345")).toBeNaN();
    expect(Date.parse("2/11/25 1:02+01234")).toBeNaN(); // Firefox supports 5 or 6 digit timezone offset. Chrome and us do not support it.
    expect(Date.parse("2/11 GMT+002025")).toBeNaN(); // What looks like 6-digit year after GMT fails
...
test("garbage", () => {
    ...
    expect(Date.parse("2204 Jan 29 10:30:28.")).toBeNaN(); // But we do not accept a dot after time.
