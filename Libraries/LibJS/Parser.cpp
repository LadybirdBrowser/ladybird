...
NonnullRefPtr<RegExpLiteral const> Parser::parse_regexp_literal()
{
    ...
    return create_ast_node<RegExpLiteral>(move(range), move(parsed_regex), move(parsed_pattern), move(parsed_flags), MUST(String::from_utf8(pattern)), MUST(String::from_utf8(flags.view())));
}
...
NonnullRefPtr<Expression const> Parser::parse_property_key()
{
    ...
    if (match(TokenType::StringLiteral)) {
        ...
        return create_ast_node<StringLiteral>({ m_source_code, rule_start.position(), position() }, consume().string_value());
    }
}
    ...
    while (!done() && !match(TokenType::CurlyClose)) {
        ...
        if (match(TokenType::Asterisk)) {
            ...
            if (identifier.original_value() == "get"sv && match_property_key()) {
                ...
                property_key = create_ast_node<StringLiteral>({ m_source_code, rule_start.position(), position() }, identifier.string_value());
...
static AK::Array<FlyString, 36> s_reserved_words = {
    "break"_fly_string,
    "case"_fly_string,
    "catch"_fly_string,
    "class"_fly_string,
    "const"_fly_string,
    "continue"_fly_string,
    "debugger"_fly_string,
    "default"_fly_string,
    "delete"_fly_string,
    "do"_fly_string,
    "else"_fly_string,
    ...
    "finally"_fly_string,
    "for"_fly_string,
    "function"_fly_string,
    "if"_fly_string,
    "import"_fly_string,
    "in"_fly_string,
    "instanceof"_fly_string,
    "new"_fly_string,
    "null"_fly_string,
    "return"_fly_string,
    "super"_fly_string,
        ...
        if (is_object) {
            ...
            if (allow_member_expressions == AllowMemberExpressions::Yes && is_rest) {
                ...
                if (match(TokenType::StringLiteral)) {
                    ...
                    name = create_identifier_and_register_in_current_scope({ m_source_code, rule_start.position(), position() }, MUST(FlyString::from_utf8(string_value.bytes_as_string_view().substring_view(0, string_value.bytes_as_string_view().length() - 1))));
                } else {
...
FlyString Parser::consume_string_value()
{
    ...
    if (value.bytes_as_string_view().length() < 3)
        return value;

    Utf8View view { value.bytes_as_string_view().substring_view(value.bytes_as_string_view().length() - 3) };
