...
/// Pinned constant registers on aarch64.
/// These callee-saved registers hold frequently-compared tag constants,
/// avoiding repeated materialization of values that exceed the 12-bit
/// cmp immediate range.
struct PinnedConstants {
    /// Maps constant values to their pinned register names.
...
impl PinnedConstants {
    fn new(program: &Program) -> Self {
        ...
        if let Some(&val) = program.constants.get("INT32_TAG") {
            map.insert(val, "x22");
        }
        if let Some(&val) = program.constants.get("BOOLEAN_TAG") {
            map.insert(val, "x23");
        }
        if let Some(&val) = program.constants.get("NAN_BASE_TAG") {
            map.insert(val, "x24");
        }
        PinnedConstants { map }
    }

    /// If `val` is a pinned constant, return the register holding it.
    fn get(&self, val: i64) -> Option<&'static str> {
            ...
            if insn.operands.len() >= 3 {
                ...
                for (i, val) in values.iter().enumerate() {
                    if i == 0 {
                        ...
                        if let Some(v) = val {
                            emit_ccmp_imm(out, &reg, *v, pinned);
                        } else {
...
/// Emit a cmp against an immediate, using a pinned register if available.
fn emit_cmp_imm(out: &mut String, reg: &str, val: i64, pinned: &PinnedConstants) {
    let uval = val as u64;
    if uval <= 4095 {
        w!(out, "    cmp {reg}, #{val}");
    } else if let Some(preg) = pinned.get(val) {
        w!(out, "    cmp {reg}, {preg}");
...
/// Emit a ccmp against an immediate, using a pinned register if available.
fn emit_ccmp_imm(out: &mut String, reg: &str, val: i64, pinned: &PinnedConstants) {
    let uval = val as u64;
    if uval <= 31 {
        w!(out, "    ccmp {reg}, #{val}, #4, ne");
    } else if let Some(preg) = pinned.get(val) {
        w!(out, "    ccmp {reg}, {preg}, #4, ne");
    } else {
        emit_mov_imm(out, "x9", val);
