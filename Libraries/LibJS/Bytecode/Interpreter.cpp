...
static Value instantiate_ordinary_function_expression(Interpreter& interpreter, FunctionNode const& function_node, Utf16FlyString given_name, bool should_be_constructible)
{
...
inline Value new_function(Interpreter& interpreter, FunctionNode const& function_node, Optional<IdentifierTableIndex> const& lhs_name, Optional<Operand> const& home_object, bool is_method)
{
    ...
    if (!function_node.has_name()) {
        ...
        value = instantiate_ordinary_function_expression(interpreter, function_node, name, should_be_constructible);
    } else {
