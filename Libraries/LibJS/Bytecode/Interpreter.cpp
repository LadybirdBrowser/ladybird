...
void handle(Interpreter& interpreter, u8 const* bytecode, size_t& program_counter);
template<typename OP>
void handle_generic(Interpreter& interpreter, u8 const* bytecode, size_t& program_counter);
template<typename OP>
void handle_jump(Interpreter& interpreter, u8 const* bytecode, size_t& program_counter);

template<>
FLATTEN void handle<Op::Mov>(Interpreter& interpreter, u8 const* bytecode, size_t& program_counter)
{
...
template<>
FLATTEN void handle<Op::End>(Interpreter& interpreter, u8 const* bytecode, size_t& program_counter)
{
...
template<>
FLATTEN void handle_jump<Op::Jump>(Interpreter& interpreter, u8 const* bytecode, size_t& program_counter)
{
...
template<>
FLATTEN void handle_jump<Op::JumpIf>(Interpreter& interpreter, u8 const* bytecode, size_t& program_counter)
{
...
template<>
FLATTEN void handle_jump<Op::JumpTrue>(Interpreter& interpreter, u8 const* bytecode, size_t& program_counter)
{
...
template<>
FLATTEN void handle_jump<Op::JumpFalse>(Interpreter& interpreter, u8 const* bytecode, size_t& program_counter)
{
...
template<>
FLATTEN void handle_jump<Op::JumpNullish>(Interpreter& interpreter, u8 const* bytecode, size_t& program_counter)
{
...
#define HANDLE_COMPARISON_OP(op_TitleCase, op_snake_case, numeric_operator)                                                    \
    template<>                                                                                                                 \
    FLATTEN void handle_jump<Op::Jump##op_TitleCase>(Interpreter & interpreter, u8 const* bytecode, size_t& program_counter)   \
    {                                                                                                                          \
...
template<>
FLATTEN void handle_jump<Op::JumpUndefined>(Interpreter& interpreter, u8 const* bytecode, size_t& program_counter)
{
...
template<>
FLATTEN void handle<Op::EnterUnwindContext>(Interpreter& interpreter, u8 const* bytecode, size_t& program_counter)
{
...
template<>
FLATTEN void handle<Op::ContinuePendingUnwind>(Interpreter& interpreter, u8 const* bytecode, size_t& program_counter)
{
    ...
    if (!interpreter.saved_return_value().is_special_empty_value()) {
        ...
        if (auto handlers = interpreter.current_executable().exception_handlers_for_offset(program_counter); handlers.has_value()) {
            if (auto finalizer = handlers.value().finalizer_offset; finalizer.has_value()) {
                ...
                VERIFY(unwind_context.executable == interpreter.m_current_executable);
    ...
    if (interpreter.m_scheduled_jump.has_value()) {
        program_counter = interpreter.m_scheduled_jump.value();
        interpreter.m_scheduled_jump = {};
    } else {
        program_counter = instruction.resume_target().address();
        // set the scheduled jump to the old value if we continue
        // where we left it
        interpreter.m_scheduled_jump = old_scheduled_jump;
    }
    DISPATCH_NEXT(interpreter);
}

template<>
FLATTEN void handle<Op::ScheduleJump>(Interpreter& interpreter, u8 const* bytecode, size_t& program_counter)
{
    auto& instruction = *reinterpret_cast<Op::ScheduleJump const*>(&bytecode[program_counter]);
    interpreter.m_scheduled_jump = instruction.target().address();
...
template<typename OP>
FLATTEN void handle_generic(Interpreter& interpreter, u8 const* bytecode, size_t& program_counter)
{
...
#define HANDLE_INSTRUCTION(name) \
    template FLATTEN void handle_generic<Op::name>(Interpreter & interpreter, u8 const* bytecode, size_t& program_counter);
...
template<>
FLATTEN void handle<Op::Await>(Interpreter& interpreter, u8 const* bytecode, size_t& program_counter)
{
...
template<>
FLATTEN void handle<Op::Return>(Interpreter& interpreter, u8 const* bytecode, size_t& program_counter)
{
...
template<>
FLATTEN void handle<Op::Yield>(Interpreter& interpreter, u8 const* bytecode, size_t& program_counter)
{
