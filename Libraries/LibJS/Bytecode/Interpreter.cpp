...
struct InterpreterPrivate {
    ...
    static GC::Ptr<Executable> const& current_executable(Interpreter const& interpreter) { return interpreter.m_current_executable; }
};
...
static size_t handle(Interpreter& interpreter, u8 const* bytecode, size_t program_counter);
template<typename OP>
static size_t handle_generic(Interpreter& interpreter, u8 const* bytecode, size_t program_counter);
template<typename OP>
    ...
    if (!interpreter.saved_return_value().is_special_empty_value()) {
        ...
        if (auto handlers = InterpreterPrivate::current_executable(interpreter)->exception_handlers_for_offset(program_counter); handlers.has_value()) {
            if (auto finalizer = handlers.value().finalizer_offset; finalizer.has_value()) {
                ...
                VERIFY(unwind_context.executable == InterpreterPrivate::current_executable(interpreter));
