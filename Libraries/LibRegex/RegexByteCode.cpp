...
ALWAYS_INLINE ExecutionResult OpCode_SaveRightNamedCaptureGroup::execute(MatchInput const& input, MatchState& state) const
{
    ...
    if (input.regex_options & AllFlags::StringCopyMatches) {
        match = Match { view.to_byte_string(), MUST(String::from_utf8(name())), input.line, start_position, input.global_offset + start_position }; // create a copy of the original string
    } else {
        match = Match { view, MUST(String::from_utf8(name())), input.line, start_position, input.global_offset + start_position }; // take view to original string
    }
