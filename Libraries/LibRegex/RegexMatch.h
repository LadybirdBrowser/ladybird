...
public:
    ...
    Match(RegexStringView const view_, FlyString capture_group_name_, size_t const line_, size_t const column_, size_t const global_offset_)
        : view(view_)
        , capture_group_name(move(capture_group_name_))
        , line(line_)
        , column(column_)
        , global_offset(global_offset_)
        , left_column(column_)
