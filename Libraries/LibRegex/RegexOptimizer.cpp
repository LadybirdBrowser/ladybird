...
void Regex<Parser>::attempt_rewrite_adjacent_compares_as_string_compare(BasicBlockList const& basic_blocks)
{
    ...
    for (auto const& block : basic_blocks) {
        ...
        for (state.instruction_position = block.start; state.instruction_position <= block.end;) {
            ...
            if (opcode.opcode_id() == OpCodeId::Compare) {
                ...
                if (in_sequence) {
                    static_capture_groups_in_sequence.append({ capture.id(), capture.length(), (ssize_t)current_chars.size() - (ssize_t)capture.offset() });
                }
                state.instruction_position += opcode.size();
                continue;
            }
        ...
        if (in_sequence && current_chars.size() >= 2) {
            sequences.append({ .start_ip = sequence_start, .end_ip = state.instruction_position, .characters = move(current_chars), .static_capture_groups = move(static_capture_groups_in_sequence) });
        }
    }
    ...
    for (auto const& seq : sequences) {
        ...
        for (auto const& capture : seq.static_capture_groups) {
            auto offset = capture.start_in_string + string_builder.length();
...
void Regex<Parser>::attempt_rewrite_static_length_captures()
{
    ...
    // FIXME/Note: We need a flat list here, as we want to slice out the entire capture group ops at once
    bytecode.flatten();
    struct Range {
        size_t start_ip;
        size_t end_ip;
