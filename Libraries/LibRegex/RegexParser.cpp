...
bool ECMA262Parser::parse_capture_group(ByteCode& stack, size_t& match_length_minimum, ParseFlags flags)
{
    ...
    if (match(TokenType::Questionmark)) {
        ...
        if (match(TokenType::Char) || match(TokenType::HyphenMinus)) {
            auto is_modifier_flag = [](char ch) {
                return ch == 'i' || ch == 'm' || ch == 's';
            };

            auto is_potential_modifier_start = [&] {
                if (match(TokenType::HyphenMinus))
                    return true;
                auto value = m_parser_state.current_token.value();
                return value.length() == 1 && is_modifier_flag(value[0]);
            };

            if (!is_potential_modifier_start())
                goto not_a_modifier;
            ...
            auto parse_regular_expression_flags = [&](bool& has_i, bool& has_m, bool& has_s) -> ModifierParseResult {
                has_i = false;
                has_m = false;
                has_s = false;
            ...
            bool add_m = false;
            bool add_s = false;
            auto add_result = parse_regular_expression_flags(add_i, add_m, add_s);
            ...
            if (match(TokenType::HyphenMinus)) {
                consume();
                auto remove_result = parse_regular_expression_flags(remove_i, remove_m, remove_s);
