...
COMPOSITING_API SnapDestination select_snap_destination(SnapContainerGeometry const&, SnapAxisCandidates const&, CSSPixelPoint destination, SnapSelectionStrategy const&, SnapAxes evaluated_axes);
COMPOSITING_API SnapDestination select_snap_destination(SnapContainerGeometry const&, ReadonlySpan<SnapAreaGeometry>, CSSPixelPoint destination, SnapSelectionStrategy const& = {});

COMPOSITING_API Optional<SnapAxisChoice> choose_snap_offset_for_axis(Vector<SnapPositionCandidate> const&, SnapAxisSelection const&, CSSPixels snapport_size, SnapStrictness, Optional<CSSPixels> cross_axis_offset, Optional<SnapAreaIdentity> only_area = {});
COMPOSITING_API bool candidate_has_snap_position_at(SnapPositionCandidate const&, CSSPixels offset);
COMPOSITING_API bool snap_area_is_visible_at_cross_axis_offset(SnapPositionCandidate const&, CSSPixels cross_axis_offset);
COMPOSITING_API bool chosen_offsets_are_mutually_visible(SnapAxisCandidates const&, CSSPixels x_offset, CSSPixels y_offset);
COMPOSITING_API SnapDestination snap_destination_for(CSSPixelPoint unsnapped_destination, Optional<CSSPixels> x_offset, Optional<CSSPixels> y_offset, SnapAxes evaluated_axes);
COMPOSITING_API Vector<SnapAreaIdentity> snap_areas_at_offset(Vector<SnapPositionCandidate> const&, CSSPixels offset, CSSPixels cross_axis_offset);
