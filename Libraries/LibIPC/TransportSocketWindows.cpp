...
{
    if (m_peer_process_handle)
        CloseHandle(m_peer_process_handle);
}

void TransportSocketWindows::set_peer_pid(int pid)
{
    VERIFY(!m_peer_process_handle);
    m_peer_process_handle = OpenProcess(PROCESS_DUP_HANDLE, FALSE, pid);
    VERIFY(m_peer_process_handle);
}
...
ErrorOr<void> TransportSocketWindows::transfer(Bytes bytes_to_write, Vector<size_t> const& handle_offsets)
{
    for (auto offset : handle_offsets) {
        ...
        VERIFY(m_peer_process_handle);
        HANDLE& handle = *(HANDLE*)(bytes_to_write.data() + offset);
        auto rc = DuplicateHandle(GetCurrentProcess(), handle, m_peer_process_handle, &handle, 0, FALSE, DUPLICATE_SAME_ACCESS);
        VERIFY(rc);
    }
