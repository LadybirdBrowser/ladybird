...
ErrorOr<NonnullRefPtr<SingleSinkSessionClient>>
SingleSinkSessionClient::try_create(RefPtr<SessionClientOfAudioServer> session_client)
{
    if (!session_client)
    ...
    auto session = TRY(
        adopt_nonnull_ref_or_enomem(new (nothrow) SingleSinkSessionClient(session_client.release_nonnull())));

    session->m_output_sink_id = session->m_session_client->add_output_sink(
        [weak_session = session->make_weak_ptr()](OutputSink const& output_sink) {
...
ErrorOr<void> SingleSinkSessionClient::request_output_sink(OutputSinkReadyHandler on_ready,
    ...
    u32 target_latency_ms)
{
    ...
    if (m_create_request_in_flight || m_active_session_id.has_value() || m_pending_session_id.has_value()) {
        if (m_device_handle.has_value() && m_device_handle.value() != device_handle)
            return Error::from_string_literal(
                "SingleSinkSessionClient: output session already active or pending for different device");
        return {};
    }
    ...
    auto request_or_error = m_session_client->create_output_session(
        target_latency_ms,
        ...
        move(wrapped_on_error), device_handle, m_output_sink_id);
...
ErrorOr<u64>
SingleSinkSessionClient::destroy_output_sink(Function<void()> on_success,
...
ErrorOr<u64>
SingleSinkSessionClient::set_output_sink_volume(double volume, Function<void()> on_success,
    SessionClientOfAudioServer::RequestErrorHandler on_error)
{
    verify_thread_affinity();
    if (!m_active_session_id.has_value())
    ...
    return m_session_client->set_output_sink_volume(m_active_session_id.value(), volume, move(on_success),
        move(on_error));
}
