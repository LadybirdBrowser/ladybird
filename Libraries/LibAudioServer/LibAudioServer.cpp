...
template<>
ErrorOr<void> encode(Encoder& encoder, Audio::DeviceInfo const& info)
{
...
template<>
ErrorOr<Audio::DeviceInfo> decode(Decoder& decoder)
{
    Audio::DeviceInfo info;

    u8 raw_type = TRY(decoder.decode<u8>());
    if (raw_type < static_cast<u8>(Audio::DeviceInfo::Type::Output) || raw_type > static_cast<u8>(Audio::DeviceInfo::Type::Input))
        return Error::from_string_literal("Invalid DeviceInfo type");

    info.type = static_cast<Audio::DeviceInfo::Type>(raw_type);
    info.device_handle = TRY(decoder.decode<Audio::DeviceHandle>());
...
{
    Audio::InputStreamDescriptor descriptor;
...
{
    Audio::CreateClientRequest info;
...
{
    TRY(encoder.encode(info.handle));
...
{
    Audio::CreateClientResponse info;
    info.handle = TRY(decoder.decode<IPC::TransportHandle>());
    info.grant_id = TRY(decoder.decode<ByteString>());
    return info;
}

template<>
ErrorOr<void> encode(Encoder& encoder, Audio::OutputSinkTransport const& pack)
{
...
template<>
ErrorOr<Audio::OutputSinkTransport> decode(Decoder& decoder)
{
    Audio::OutputSinkTransport pack;
    ...
    auto sample_ring_or_error = Audio::SharedCircularBuffer::attach(move(sample_ring_anonymous_buffer));
