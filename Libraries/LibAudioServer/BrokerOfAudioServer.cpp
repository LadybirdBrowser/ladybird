...
BrokerOfAudioServer::BrokerOfAudioServer(NonnullOwnPtr<IPC::Transport> transport)
    : IPC::ConnectionToServer<ToBrokerFromAudioServerEndpoint, ToAudioServerFromBrokerEndpoint>(
          *this, move(transport))
{
}
...
ErrorOr<CreateClientResponse>
BrokerOfAudioServer::connect_new_client(StringView origin, StringView top_level_origin, bool can_use_mic)
{
    Vector<CreateClientRequest> requests;
    TRY(requests.try_append(
        { .origin = origin, .top_level_origin = top_level_origin, .can_use_mic = can_use_mic }));
...
BrokerOfAudioServer::connect_new_clients(Vector<CreateClientRequest> requests)
{
    verify_thread_affinity();
...
ErrorOr<void> BrokerOfAudioServer::connect_new_clients_async(
    Vector<CreateClientRequest>&& requests, Function<void(ErrorOr<Vector<CreateClientResponse>>)> callback)
{
    verify_thread_affinity();
