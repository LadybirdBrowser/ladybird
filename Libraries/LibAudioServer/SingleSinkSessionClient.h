...
class SingleSinkSessionClient final : public RefCounted<SingleSinkSessionClient>
    , public Weakable<SingleSinkSessionClient> {
public:
    static constexpr u32 default_target_latency_ms = 50;

    using OutputSinkReadyHandler = Function<void(OutputSink const&)>;
    using OutputSinkFailedHandler = Function<void(u64 session_id, ByteString const& error)>;

    static ErrorOr<NonnullRefPtr<SingleSinkSessionClient>>
    try_create(RefPtr<SessionClientOfAudioServer> session_client = {});
    ~SingleSinkSessionClient();

    // Request one output session together with a sink for the selected device.
    ErrorOr<void> request_output_sink(OutputSinkReadyHandler on_ready, OutputSinkFailedHandler on_failed = {},
        DeviceHandle device_handle = 0,
        u32 target_latency_ms = default_target_latency_ms);

    ErrorOr<u64> destroy_output_sink(Function<void()> on_success = {},
        SessionClientOfAudioServer::RequestErrorHandler on_error = {});
    ErrorOr<void> release_output_sink_if_any();

    ErrorOr<u64> set_output_sink_volume(double volume, Function<void()> on_success = {},
        SessionClientOfAudioServer::RequestErrorHandler on_error = {});
...
private:
    ...
    Optional<DeviceHandle> m_device_handle;
