...
public:
    ...
    ErrorOr<CreateClientResponse> connect_new_client(StringView origin = "*"sv,
        StringView top_level_origin = "*"sv,
        bool can_use_mic = false);
    ErrorOr<Vector<CreateClientResponse>> connect_new_clients(Vector<CreateClientRequest> requests);
    ErrorOr<void> connect_new_clients_async(Vector<CreateClientRequest>&& requests,
        Function<void(ErrorOr<Vector<CreateClientResponse>>)> callback);
