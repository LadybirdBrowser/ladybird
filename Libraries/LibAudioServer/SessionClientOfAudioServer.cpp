...
SessionClientOfAudioServer::SessionClientOfAudioServer(NonnullOwnPtr<IPC::Transport> transport)
    : IPC::ConnectionToServer<ToSessionClientFromAudioServerEndpoint, ToAudioServerFromSessionClientEndpoint>(
          *this, move(transport))
{
}
...
ErrorOr<u64> SessionClientOfAudioServer::set_output_sink_volume(u64 session_id, double volume,
    ...
    RequestErrorHandler on_error)
{
    ...
    store_pending_request(
        request_id,
        [on_success = move(on_success)]() mutable {
            if (on_success)
                on_success();
        },
        move(on_error));
    ...
    if (post_message(SetOutputSinkVolume(request_id, session_id, volume)).is_error()) {
        complete_pending_request_error(request_id,
            ByteString("AudioServerClient: set output session gain IPC failed"));
...
ErrorOr<u64> SessionClientOfAudioServer::create_output_session(u32 target_latency_ms,
    Function<void(u64)> on_success,
    RequestErrorHandler on_error,
    DeviceHandle device_handle,
    Optional<u64> sink_id)
{
    verify_thread_affinity();
    u64 request_id = next_request_id();
    store_pending_request(
        request_id,
        [this, request_id, on_success = move(on_success)](u64 created_session_id) mutable {
            if (created_session_id == 0) {
                complete_pending_request_error(
                    request_id, ByteString("AudioServerClient: server returned invalid audio session id"));
                return;
            }
            if (on_success)
                on_success(created_session_id);
        },
        move(on_error));

    if (post_message(CreateSession(request_id, m_grant_id, target_latency_ms, device_handle)).is_error()) {
        complete_pending_request_error(request_id,
            ByteString("AudioServerClient: create output session IPC failed"));
...
void SessionClientOfAudioServer::deliver_output_sink_ready(Optional<u64> sink_id,
...
void SessionClientOfAudioServer::deliver_output_sink_failed(u64 sink_id, u64 session_id,
...
ErrorOr<u64> SessionClientOfAudioServer::destroy_session(u64 session_id, Function<void()> on_success,
    RequestErrorHandler on_error)
{
    verify_thread_affinity();
    u64 request_id = next_request_id();
    ...
    if (post_message(DestroySession(request_id, session_id)).is_error()) {
        complete_pending_request_error(request_id,
            ByteString("AudioServerClient: destroy output session IPC failed"));
...
ErrorOr<u64> SessionClientOfAudioServer::get_devices(Function<void(Vector<DeviceInfo>)> on_success,
    RequestErrorHandler on_error)
{
    verify_thread_affinity();
    u64 request_id = next_request_id();
    store_pending_request(
        request_id,
        [on_success = move(on_success)](Vector<DeviceInfo> devices) mutable {
            if (on_success)
                on_success(move(devices));
        },
        move(on_error));
...
ErrorOr<u64> SessionClientOfAudioServer::create_input_stream(DeviceHandle device_handle, u64 capacity_frames,
    ...
    RequestErrorHandler on_error)
{
    ...
    store_pending_request(
        request_id,
        [this, request_id, on_success = move(on_success)](InputStreamDescriptor created_descriptor) mutable {
            if (created_descriptor.stream_id == 0 || !created_descriptor.shared_memory.is_valid()) {
                complete_pending_request_error(
                    request_id,
                    ByteString("AudioServerClient: server returned invalid audio input stream descriptor"));
                return;
            }
            if (on_success)
                on_success(move(created_descriptor));
        },
        move(on_error));

    if (post_message(CreateInputStream(request_id, m_grant_id, device_handle, capacity_frames)).is_error()) {
        complete_pending_request_error(request_id,
            ByteString("AudioServerClient: create input stream IPC failed"));
...
ErrorOr<u64> SessionClientOfAudioServer::start_input_stream(u64 stream_id, Function<void()> on_success,
    RequestErrorHandler on_error)
{
    verify_thread_affinity();
    u64 request_id = next_request_id();
    ...
    if (post_message(StartInputStream(request_id, stream_id)).is_error()) {
        complete_pending_request_error(request_id,
            ByteString("AudioServerClient: start input stream IPC failed"));
...
ErrorOr<u64> SessionClientOfAudioServer::stop_input_stream(u64 stream_id, Function<void()> on_success,
    RequestErrorHandler on_error)
{
    verify_thread_affinity();
    u64 request_id = next_request_id();
...
ErrorOr<u64> SessionClientOfAudioServer::destroy_input_stream(u64 stream_id, Function<void()> on_success,
    RequestErrorHandler on_error)
{
    verify_thread_affinity();
    if (stream_id == 0)
    ...
    if (post_message(DestroyInputStream(request_id, stream_id)).is_error()) {
        complete_pending_request_error(request_id,
            ByteString("AudioServerClient: destroy input stream IPC failed"));
