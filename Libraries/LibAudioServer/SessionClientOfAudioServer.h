...
class SessionClientOfAudioServer final
    : public IPC::ConnectionToServer<ToSessionClientFromAudioServerEndpoint,
...
public:
    ...
    ErrorOr<u64> get_devices(Function<void(Vector<DeviceInfo>)> on_success, RequestErrorHandler on_error = {});

    ErrorOr<u64> create_output_session(u32 target_latency_ms, Function<void(u64)> on_success,
        RequestErrorHandler on_error = {}, DeviceHandle device_handle = 0,
        Optional<u64> output_sink_id = {});

    ErrorOr<u64> destroy_session(u64 session_id, Function<void()> on_success = {},
        RequestErrorHandler on_error = {});

    u64 add_output_sink(OutputSinkReady on_ready, OutputSinkFailed on_failed = {});
    ...
    ErrorOr<u64> create_input_stream(DeviceHandle device_handle, u64 capacity_frames,
        Function<void(InputStreamDescriptor)> on_success,
        RequestErrorHandler on_error = {});

    ErrorOr<u64> start_input_stream(u64 stream_id, Function<void()> on_success = {},
        RequestErrorHandler on_error = {});

    ErrorOr<u64> stop_input_stream(u64 stream_id, Function<void()> on_success = {},
        RequestErrorHandler on_error = {});
    ErrorOr<u64> destroy_input_stream(u64 stream_id, Function<void()> on_success = {},
        RequestErrorHandler on_error = {});

    ErrorOr<u64> set_output_sink_volume(u64 session_id, double volume, Function<void()> on_success = {},
        RequestErrorHandler on_error = {});
...
private:
    ...
    using PendingRequestResult = Variant<PendingDeviceInfosResult, PendingInputStreamDescriptorResult,
        PendingVoidResult, PendingU64Result>;
