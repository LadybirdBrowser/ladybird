...
ErrorOr<NonnullOwnPtr<ReadStream>> ReadStream::try_create(int const reader_fd)
{
    auto local_socket = TRY(Core::LocalSocket::adopt_fd(reader_fd));
    return adopt_own(*new ReadStream(move(local_socket), local_socket->notifier()));
}
#else
ErrorOr<NonnullOwnPtr<ReadStream>> ReadStream::try_create(int const reader_fd)
{
...
void Request::set_request_fd(Badge<Requests::RequestClient>, int fd)
{
    ...
    auto read_stream = MUST(ReadStream::try_create(fd));
...
void Request::set_up_internal_stream_data(DataReceived on_data_available)
{
    ...
    if (fd() != -1)
        m_internal_stream_data->read_stream = MUST(ReadStream::try_create(fd()));
