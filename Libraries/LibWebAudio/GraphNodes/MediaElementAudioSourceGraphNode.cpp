...
ErrorOr<MediaElementAudioSourceGraphNode>
MediaElementAudioSourceGraphNode::decode_wire_payload(WireDecoder& decoder)
{
    ASSERT_CONTROL_THREAD();
...
OwnPtr<RenderNode> MediaElementAudioSourceGraphNode::make_render_node(NodeID node_id, size_t quantum_size,
    GraphResources const& resources) const
{
    ASSERT_CONTROL_THREAD();
    auto it = resources.media_element_audio_sources.find(provider_id);
    if (it == resources.media_element_audio_sources.end())
        return make<OhNoesRenderNode>(node_id, quantum_size);
    return make<MediaElementAudioSourceRenderNode>(node_id, it->value, quantum_size);
}

GraphUpdateKind
MediaElementAudioSourceGraphNode::classify_update(MediaElementAudioSourceGraphNode const& new_desc) const
{
    ASSERT_CONTROL_THREAD();
