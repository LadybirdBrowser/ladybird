...
struct BiquadFilterGraphNode {
    ...
    void initialize_param_state(f32 sample_rate, SetState&& set_state) const
    {
        ...
        set_state(BiquadFilterParamIndex::detune, detune_cents, -AK::NumericLimits<f32>::max(),
            AK::NumericLimits<f32>::max());
        set_state(BiquadFilterParamIndex::q, q, -AK::NumericLimits<f32>::max(), AK::NumericLimits<f32>::max());
        set_state(BiquadFilterParamIndex::gain, gain_db, -AK::NumericLimits<f32>::max(),
            AK::NumericLimits<f32>::max());
    }
