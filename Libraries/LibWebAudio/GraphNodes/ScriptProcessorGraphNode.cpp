...
OwnPtr<RenderNode> ScriptProcessorGraphNode::make_render_node(NodeID node_id, size_t quantum_size,
...
{
    ASSERT_CONTROL_THREAD();
    if (buffer_size == new_desc.buffer_size && input_channel_count == new_desc.input_channel_count && output_channel_count == new_desc.output_channel_count)
