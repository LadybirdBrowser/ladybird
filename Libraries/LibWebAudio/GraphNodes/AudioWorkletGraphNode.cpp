...
#include <LibWebAudio/RenderNodes/AudioWorkletRenderNode.h>
...
OwnPtr<RenderNode> AudioWorkletGraphNode::make_render_node(NodeID node_id, size_t quantum_size,
...
{
    ASSERT_CONTROL_THREAD();
    if (processor_name == new_desc.processor_name && number_of_inputs == new_desc.number_of_inputs && number_of_outputs == new_desc.number_of_outputs && output_channel_count == new_desc.output_channel_count && channel_count == new_desc.channel_count && channel_count_mode == new_desc.channel_count_mode && channel_interpretation == new_desc.channel_interpretation && parameter_names == new_desc.parameter_names)
