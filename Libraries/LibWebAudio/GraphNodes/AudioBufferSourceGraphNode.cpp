...
ErrorOr<void> AudioBufferSourceGraphNode::encode_wire_payload(WireEncoder& encoder) const
{
    ...
    return encoder.append_u64(static_cast<u64>(buffer_id));
}
...
ErrorOr<AudioBufferSourceGraphNode> AudioBufferSourceGraphNode::decode_wire_payload(WireDecoder& decoder)
{
    ...
    node.buffer_id = ResourceID { TRY(decoder.read_u64()) };
    return node;
}

OwnPtr<RenderNode> AudioBufferSourceGraphNode::make_render_node(NodeID node_id, size_t quantum_size,
    GraphResources const& resources) const
{
    ASSERT_CONTROL_THREAD();
    RefPtr<SharedAudioBuffer> buffer;
    if (buffer_id != ResourceID {}) {
        if (auto it = resources.audio_buffers.find(buffer_id); it != resources.audio_buffers.end())
            buffer = it->value;
    }
...
GraphUpdateKind
AudioBufferSourceGraphNode::classify_update(AudioBufferSourceGraphNode const& new_desc) const
{
    ASSERT_CONTROL_THREAD();
