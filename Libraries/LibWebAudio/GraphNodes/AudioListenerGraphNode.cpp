...
OwnPtr<RenderNode> AudioListenerGraphNode::make_render_node(NodeID node_id, size_t quantum_size,
    GraphResources const&) const
{
    ASSERT_CONTROL_THREAD();
...
{
    ASSERT_CONTROL_THREAD();
    auto differs = [](f32 a, f32 b) { return bit_cast<u32>(a) != bit_cast<u32>(b); };

    if (differs(position_x, new_desc.position_x) || differs(position_y, new_desc.position_y) || differs(position_z, new_desc.position_z) || differs(forward_x, new_desc.forward_x) || differs(forward_y, new_desc.forward_y) || differs(forward_z, new_desc.forward_z) || differs(up_x, new_desc.up_x) || differs(up_y, new_desc.up_y) || differs(up_z, new_desc.up_z)) {
