...
SharedAudioBuffer::SharedAudioBuffer(f32 sample_rate, size_t channel_count, size_t length_in_sample_frames,
...
NonnullRefPtr<SharedAudioBuffer> SharedAudioBuffer::create(f32 sample_rate, size_t channel_count,
    size_t length_in_sample_frames,
    Vector<Vector<f32>>&& channels)
{
    ...
    f32* samples = reinterpret_cast<f32*>(header + 1);
    size_t sample_count = MUST(checked_sample_count(channel_count, length_in_sample_frames));
    for (size_t i = 0; i < sample_count; ++i)
        samples[i] = 0.0f;

    for (size_t ch = 0; ch < min(channel_count, channels.size()); ++ch) {
    ...
    return adopt_ref(
        *new SharedAudioBuffer(sample_rate, channel_count, length_in_sample_frames, move(storage)));
}

ErrorOr<NonnullRefPtr<SharedAudioBuffer>>
SharedAudioBuffer::create_from_buffer(Core::AnonymousBuffer shared_memory)
{
    if (!shared_memory.is_valid() || shared_memory.size() < sizeof(SharedAudioBufferHeader))
    ...
    return adopt_ref(*new SharedAudioBuffer(header->sample_rate_hz, channel_count, length_in_sample_frames,
        move(shared_memory)));
}

f32 SharedAudioBuffer::sample_rate() const { return m_sample_rate; }

size_t SharedAudioBuffer::channel_count() const { return m_channel_count; }

size_t SharedAudioBuffer::length_in_sample_frames() const { return m_length_in_sample_frames; }

ReadonlySpan<f32> SharedAudioBuffer::channel(size_t index) const
{
    if (index >= m_channel_count || !m_data.is_valid() || m_data.size() < sizeof(SharedAudioBufferHeader))
        return {};
    ...
    f32 const* base = reinterpret_cast<f32 const*>(header + 1);

    return ReadonlySpan<f32> { base + (index * m_length_in_sample_frames), m_length_in_sample_frames };
}

Core::AnonymousBuffer SharedAudioBuffer::to_buffer() const { return m_data; }
...
{
    TRY(encoder.encode(static_cast<u64>(binding.buffer_id)));
...
ErrorOr<Web::WebAudio::Render::SharedAudioBufferBinding> decode(Decoder& decoder)
{
    ...
    binding.buffer_id = Web::WebAudio::Render::ResourceID { TRY(decoder.decode<u64>()) };
