...
using StreamID = u64;
AK_TYPEDEF_DISTINCT_NUMERIC_GENERAL(u64, ResourceID, CastToUnderlying, Comparison);
...
struct AudioInputStreamMetadata {
    Audio::DeviceHandle device_handle { 0 };
    u32 sample_rate_hz { 0 };
    u32 channel_count { 0 };
    u64 capacity_frames { 0 };
};
...
struct AudioParamDescriptor {
    ...
    float min_value { -AK::NumericLimits<float>::max() };
    float max_value { AK::NumericLimits<float>::max() };
    bool is_k_rate { false };
};
...
struct WorkletNodeDefinition {
    ...
    u8 channel_count_mode { 0 };
    u8 channel_interpretation { 0 };
...
inline size_t analyser_feedback_page_size(u32 fft_size)
{
    ...
    return sizeof(AnalyserFeedbackHeader) + (static_cast<size_t>(fft_size) * sizeof(f32)) + (static_cast<size_t>(fft_size) / 2 * sizeof(f32));
}
...
inline bool ring_stream_consumer_fix_overrun(RingHeader& header, u64& in_out_read_frame, u64 write_frame)
{
    ...
    header.overrun_frames_total += new_read - in_out_read_frame;
...
template<>
ErrorOr<void> encode(Encoder&, Web::WebAudio::Render::AudioParamDescriptor const&);

template<>
ErrorOr<Web::WebAudio::Render::AudioParamDescriptor> decode(Decoder&);
