...
template<>
ErrorOr<void> encode(Encoder& encoder, Web::WebAudio::Render::AudioParamDescriptor const& descriptor)
{
    TRY(encoder.encode(descriptor.name));
    ...
    TRY(encoder.encode(descriptor.is_k_rate));
    return {};
}

template<>
ErrorOr<Web::WebAudio::Render::AudioParamDescriptor> decode(Decoder& decoder)
{
    Web::WebAudio::Render::AudioParamDescriptor descriptor;
    ...
    descriptor.is_k_rate = TRY(decoder.decode<bool>());
    return descriptor;
}
...
ErrorOr<Web::WebAudio::Render::AudioInputStreamMetadata> decode(Decoder& decoder)
{
    ...
    metadata.device_handle = TRY(decoder.decode<Audio::DeviceHandle>());
...
template<>
ErrorOr<void> encode(Encoder& encoder,
    Web::WebAudio::Render::MediaElementAudioSourceStreamDescriptor const& descriptor)
{
...
template<>
ErrorOr<void> encode(Encoder& encoder,
    Web::WebAudio::Render::MediaStreamAudioSourceStreamDescriptor const& descriptor)
{
    TRY(encoder.encode(descriptor.provider_id));
...
template<>
ErrorOr<void> encode(Encoder& encoder,
    Web::WebAudio::Render::SharedBufferStreamDescriptor const& descriptor)
{
...
template<>
ErrorOr<void> encode(Encoder& encoder,
    Web::WebAudio::Render::ScriptProcessorStreamDescriptor const& descriptor)
{
...
{
    TRY(encoder.encode(descriptor.node_id));
    TRY(encoder.encode(descriptor.processor_port_handle));
    return {};
}
...
ErrorOr<Web::WebAudio::Render::WorkletNodePortDescriptor> decode(Decoder& decoder)
{
    ...
    descriptor.node_id = TRY(decoder.decode<u64>());
    descriptor.processor_port_handle = TRY(decoder.decode<IPC::TransportHandle>());
    return descriptor;
}
...
ErrorOr<Web::WebAudio::Render::WorkletNodeDefinition> decode(Decoder& decoder)
{
    ...
    definition.node_id = TRY(decoder.decode<u64>());
    ...
    definition.channel_count_mode = TRY(decoder.decode<u8>());
    definition.channel_interpretation = TRY(decoder.decode<u8>());
    ...
    definition.serialized_processor_options = TRY(decoder.decode<Optional<IPC::MessageDataType>>());
    return definition;
}
...
void write_timing_page(TimingFeedbackPage& page, u32 sample_rate_hz, u32 channel_count,
    u64 rendered_frames_total, u64 underrun_frames_total, u64 graph_generation,
    bool is_suspended, u64 suspend_generation)
{
