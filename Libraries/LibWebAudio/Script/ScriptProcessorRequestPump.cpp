...
#include <LibWebAudio/Script/ScriptProcessorRequestPump.h>
...
void ScriptProcessorRequestPump::process_requests(NodeID node_id, ScriptProcessorNodeState& state)
{
    ...
    while (true) {
        ...
        if (header->magic != script_processor_request_magic || header->version != script_processor_stream_version) {
            if (WebAudio::should_log_webaudio(LOG_SCRIPT_PROCESSOR))
                dbgln("WebAudio: ScriptProcessor bad header node={} magic=0x{:x} version={}", node_id.value(),
        ...
        if (header->node_id != node_id.value() || header->buffer_size != state.buffer_size || header->input_channel_count != state.input_channel_count || header->output_channel_count != state.output_channel_count) {
            if (WebAudio::should_log_webaudio(LOG_SCRIPT_PROCESSOR)) {
                dbgln("WebAudio: ScriptProcessor mismatch node={} got node={} bs={} in={} out={} expected bs={} "
                      "in={} out={}",
                    node_id.value(), header->node_id, header->buffer_size, header->input_channel_count,
                    header->output_channel_count, state.buffer_size, state.input_channel_count,
        ...
        for (u32 ch = 0; ch < state.output_channel_count; ++ch)
            output_channels[ch] = Span<f32> {
                state.output_scratch.data() + (static_cast<size_t>(ch) * state.buffer_size), state.buffer_size
            };

        (void)m_host->process_script_processor(
            node_id, render_process_context, header->playback_time_seconds, state.buffer_size,
            state.input_channel_count, state.output_channel_count, input_channels.span(), output_channels.span());
        ...
        if (response_block.size() < response_bytes) {
            if (WebAudio::should_log_webaudio(LOG_SCRIPT_PROCESSOR))
                dbgln("WebAudio: ScriptProcessor response block too small node={} have={} need={}", node_id.value(),
                    response_block.size(), response_bytes);
        ...
        *response_header = ScriptProcessorResponseHeader {
        ...
        (void)state.response_stream.try_submit_ready_block(*response_block_index,
