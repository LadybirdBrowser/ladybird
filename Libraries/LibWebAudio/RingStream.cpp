...
ErrorOr<RingStreamView> validate_ring_stream_descriptor(RingStreamDescriptor& info)
{
    if (info.stream_id == 0 || !info.shared_memory.is_valid() || info.shared_memory.size() < sizeof(RingHeader))
        return Error::from_string_literal("RingStream: invalid descriptor");

    RingHeader* header = info.shared_memory.data<RingHeader>();
    if (!header)
        return Error::from_string_literal("RingStream: shared memory header mapping failed");

    if (header->capacity_frames == 0 || header->channel_capacity == 0 || header->channel_count == 0 || header->channel_count > header->channel_capacity)
        return Error::from_string_literal("RingStream: invalid capacity/count");

    if (info.format.sample_rate_hz != 0 && info.format.sample_rate_hz != header->sample_rate_hz)
        return Error::from_string_literal("RingStream: descriptor sample rate does not match shared header");
    if (info.format.channel_count != 0 && info.format.channel_count != header->channel_count)
        return Error::from_string_literal("RingStream: descriptor channel count does not match shared header");
    if (info.format.channel_capacity != 0 && info.format.channel_capacity != header->channel_capacity)
        return Error::from_string_literal("RingStream: descriptor channel capacity does not match shared header");
    if (info.format.capacity_frames != 0 && info.format.capacity_frames != header->capacity_frames)
    ...
    if (info.shared_memory.size() < required_bytes)
        return Error::from_string_literal("RingStream: shared memory too small for ring data");

    Span<f32> data { reinterpret_cast<f32*>(header + 1), data_bytes / sizeof(f32) };

    return RingStreamView { .header = header, .interleaved_frames = data };
}

size_t ring_stream_pop_planar(RingStreamView view, u64 read_frame, size_t frames_to_read,
    Span<Span<f32>> out_channels, u32 expected_channel_count)
{
    if (!view.header || frames_to_read == 0 || expected_channel_count == 0 || out_channels.size() < expected_channel_count)
        return 0;
...
RingHeader& RingStreamConsumer::header() const { return *m_view.header; }
...
RingStreamPeekResult RingStreamConsumer::peek_with_timing() const
{
    ...
    (void)ring_stream_consumer_fix_overrun(header(), read_frame, write_frame);
    ...
    if (timeline_sample_rate != 0 && result.available_frames > 0) {
        ...
        u64 const clamped = media_frame_at_read < static_cast<u64>(AK::NumericLimits<i64>::max())
            ? media_frame_at_read
            : static_cast<u64>(AK::NumericLimits<i64>::max());
...
RingStreamPopResult RingStreamConsumer::try_pop_planar_with_timing(Span<Span<f32>> out_channels,
    size_t requested_frames,
    u32 expected_channel_count) const
{
    RingStreamPopResult result;

    if (requested_frames == 0 || expected_channel_count == 0 || out_channels.size() < expected_channel_count)
        return result;
    ...
    result.frames_read = ring_stream_pop_planar(m_view, read_frame, frames_to_read, out_channels, expected_channel_count);
    return result;
}

size_t RingStreamConsumer::try_pop_planar(Span<Span<f32>> out_channels, size_t requested_frames,
    u32 expected_channel_count) const
{
    if (requested_frames == 0 || expected_channel_count == 0 || out_channels.size() < expected_channel_count)
        return 0;
    ...
    return ring_stream_pop_planar(m_view, read_frame, frames_to_read, out_channels, expected_channel_count);
}
...
RingHeader& RingStreamProducer::header() const { return *m_view.header; }

void RingStreamProducer::initialize_format(u32 sample_rate_hz, u32 channel_count, u32 channel_capacity,
    u64 capacity_frames) const
{
    ...
    AK::atomic_store(&header().timeline_media_start_frame, static_cast<u64>(0),
        AK::MemoryOrder::memory_order_relaxed);
    AK::atomic_store(&header().timeline_media_start_at_ring_frame, static_cast<u64>(0),
        AK::MemoryOrder::memory_order_relaxed);
}

size_t RingStreamProducer::try_push_interleaved(ReadonlySpan<f32> interleaved_samples,
    u32 input_channel_count) const
{
    ...
    auto push_chunk = [&](size_t chunk_frame_index, size_t chunk_frames, size_t input_chunk_offset) {
        ...
        if (can_memcpy_whole_frames) {
            __builtin_memcpy(dst_base, src_base,
                chunk_frames * static_cast<size_t>(channel_capacity) * sizeof(f32));
            return;
        }
...
void RingStreamProducer::set_timeline_for_start(u32 timeline_sample_rate, u64 media_start_frame,
    u64 ring_start_frame) const
{
    AK::atomic_store(&header().timeline_media_start_frame, media_start_frame,
        AK::MemoryOrder::memory_order_relaxed);
    AK::atomic_store(&header().timeline_media_start_at_ring_frame, ring_start_frame,
        AK::MemoryOrder::memory_order_relaxed);
    AK::atomic_store(&header().timeline_sample_rate, timeline_sample_rate,
        AK::MemoryOrder::memory_order_relaxed);
    (void)AK::atomic_fetch_add(&header().timeline_generation, static_cast<u64>(1),
        AK::MemoryOrder::memory_order_release);
}
