...
ChannelSplitterRenderNode::ChannelSplitterRenderNode(NodeID node_id, ChannelSplitterGraphNode const& desc,
    size_t quantum_size)
    : RenderNode(node_id)
...
void ChannelSplitterRenderNode::process(RenderContext&, Vector<Vector<AudioBus const*>> const& inputs,
    Vector<Vector<AudioBus const*>> const&)
{
    ...
    if (should_log_this_quantum) {
        ...
        WA_NODE_DBGLN("[WebAudio][ChannelSplitter:{}] outputs={} connections={} mixed0_ch={}", node_id(),
            m_number_of_outputs, connection_count, mixed_input_channels);
    }
