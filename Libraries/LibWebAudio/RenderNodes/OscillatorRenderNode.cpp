...
#include <LibWebAudio/RenderNodes/OscillatorRenderNode.h>
...
static f32 oscillator_sample_from_custom(PeriodicWaveCoefficients const& coeffs, f32 phase,
...
static void render_oscillator_mono_in_range(OscillatorType type, Span<f32> out, size_t render_start,
    size_t render_end, f32 sample_rate, f32 base_frequency_hz,
    f32 base_detune_cents, bool has_frequency_param_input,
    ReadonlySpan<f32> frequency_input_hz, bool has_detune_param_input,
    ReadonlySpan<f32> detune_input_cents, f32 amplitude,
    f64& in_out_phase)
{
...
static void render_custom_oscillator_mono_in_range(
    PeriodicWaveCoefficients const& coeffs, f32 normalization_gain, Span<f32> out, size_t render_start,
    size_t render_end, f32 sample_rate, f32 base_frequency_hz, f32 base_detune_cents,
    bool has_frequency_param_input, ReadonlySpan<f32> frequency_input_hz, bool has_detune_param_input,
    ReadonlySpan<f32> detune_input_cents, f32 amplitude, f64& in_out_phase)
{
    size_t const frames = out.size();
    if (frames == 0)
        return;
    ...
    for (size_t i = render_start; i < render_end; ++i) {
        ...
        out[i] = oscillator_sample_from_custom(coeffs, static_cast<f32>(in_out_phase), normalization_gain) * amplitude;
...
OscillatorRenderNode::OscillatorRenderNode(NodeID node_id, OscillatorGraphNode const& desc,
    size_t quantum_size)
    : RenderNode(node_id)
    , m_type(desc.type)
    ...
    , m_custom_normalization_gain(
          desc.periodic_wave.has_value() ? normalization_gain_for(desc.periodic_wave.value()) : 1.0f)
...
void OscillatorRenderNode::process(RenderContext& context, Vector<Vector<AudioBus const*>> const&,
    Vector<Vector<AudioBus const*>> const& param_inputs)
{
    ...
    if (!m_start_frame.has_value()) {
        ...
        if (should_log_this_quantum)
            WA_NODE_DBGLN("[WebAudio][OscillatorNode:{}] frame={} sr={} start=none stop={} type={} freq={} "
                          "output_peak=0 (not started)",
                node_id(), context.current_frame, context.sample_rate,
                m_stop_frame.has_value() ? static_cast<u64>(m_stop_frame.value()) : 0,
                static_cast<u32>(m_type), m_frequency);
        return;
    }
    ...
    if (quantum_start + frames <= m_start_frame.value()) {
        m_output.set_channel_count(0);
        if (should_log_this_quantum)
            WA_NODE_DBGLN("[WebAudio][OscillatorNode:{}] frame={} sr={} start={} stop={} type={} freq={} "
                          "output_peak=0 (before start window)",
                node_id(), context.current_frame, context.sample_rate,
                static_cast<u64>(m_start_frame.value()),
                m_stop_frame.has_value() ? static_cast<u64>(m_stop_frame.value()) : 0,
                static_cast<u32>(m_type), m_frequency);
        return;
    }
    ...
    if (m_type == OscillatorType::Custom) {
        if (!m_periodic_wave.has_value()) {
            if (should_log_this_quantum)
                WA_NODE_DBGLN("[WebAudio][OscillatorNode:{}] frame={} sr={} start={} stop={} type={} freq={} "
                              "output_peak=0 (missing periodic wave)",
                    node_id(), context.current_frame, context.sample_rate,
                    static_cast<u64>(m_start_frame.value()),
                    m_stop_frame.has_value() ? static_cast<u64>(m_stop_frame.value()) : 0,
                    static_cast<u32>(m_type), m_frequency);
        ...
        render_custom_oscillator_mono_in_range(m_periodic_wave.value(), m_custom_normalization_gain, out,
            render_start, render_end, context.sample_rate, m_frequency,
            m_detune_cents, has_frequency_param_input, frequency_in,
            has_detune_param_input, detune_in, 1.0f, m_phase);
    } else {
        render_oscillator_mono_in_range(m_type, out, render_start, render_end, context.sample_rate, m_frequency,
            m_detune_cents, has_frequency_param_input, frequency_in,
            has_detune_param_input, detune_in, 1.0f, m_phase);
    }
    ...
    if (should_log_this_quantum) {
        ...
        WA_NODE_DBGLN(
            "[WebAudio][OscillatorNode:{}] frame={} sr={} start={} stop={} type={} freq={} output_peak={:.6f}",
            node_id(), context.current_frame, context.sample_rate, static_cast<u64>(m_start_frame.value()),
            m_stop_frame.has_value() ? static_cast<u64>(m_stop_frame.value()) : 0, static_cast<u32>(m_type),
            m_frequency, peak);
    }
}
