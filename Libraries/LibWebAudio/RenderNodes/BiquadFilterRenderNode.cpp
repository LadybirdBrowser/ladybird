...
BiquadFilterRenderNode::BiquadFilterRenderNode(NodeID node_id, BiquadFilterGraphNode const& desc,
    size_t quantum_size)
    : RenderNode(node_id)
    , m_type(desc.type)
...
void BiquadFilterRenderNode::process(RenderContext& context, Vector<Vector<AudioBus const*>> const& inputs,
    Vector<Vector<AudioBus const*>> const& param_inputs)
{
    ...
    if (!any_audio_rate_param) {
        BiquadCoefficients const c = compute_biquad_normalized_coefficients(
            m_type, context.sample_rate, effective_frequency(0), m_q, m_gain_db);
        ...
        for (size_t ch = 0; ch < m_output.channel_count(); ++ch) {
            ...
            for (size_t i = 0; i < frames; ++i) {
                ...
                f64 const y = (c.b0 * x) + (c.b1 * x1) + (c.b2 * x2) - (c.a1 * y1) - (c.a2 * y2);
                x2 = x1;
                x1 = x;
                y2 = y1;
                y1 = y;
    ...
    for (size_t i = 0; i < frames; ++i) {
        ...
        BiquadCoefficients const c = compute_biquad_normalized_coefficients(m_type, context.sample_rate,
            effective_frequency(i), q, gain_db);

        for (size_t ch = 0; ch < m_output.channel_count(); ++ch) {
            f64 const x = static_cast<f64>(mixed_input ? mixed_input->channel(ch)[i] : 0.0f);
            f64 const y = (c.b0 * x) + (c.b1 * m_x1[ch]) + (c.b2 * m_x2[ch]) - (c.a1 * m_y1[ch]) - (c.a2 * m_y2[ch]);
            m_x2[ch] = m_x1[ch];
            m_x1[ch] = x;
            m_y2[ch] = m_y1[ch];
            m_y1[ch] = y;
...
// https://webaudio.github.io/web-audio-api/#filters-characteristics
BiquadCoefficients compute_biquad_normalized_coefficients(BiquadFilterType type, f64 sample_rate,
    f32 computed_frequency_hz, f32 q, f32 gain_db)
{
    auto const passthrough = [] { return BiquadCoefficients {}; };
    ...
    auto const wire = [] { return BiquadCoefficients {}; };
