...
ConvolverRenderNode::ConvolverRenderNode(NodeID node_id, ConvolverGraphNode const& desc,
...
void ConvolverRenderNode::renormalize_impulse()
{
    ...
    f32 gain = m_normalize
        ? compute_normalization_gain(m_impulse, max<size_t>(1, m_impulse_buffer_channel_count))
        : 1.0f;
    if (gain == 1.0f)
        return;
...
void ConvolverRenderNode::process(RenderContext& context, Vector<Vector<AudioBus const*>> const& inputs,
