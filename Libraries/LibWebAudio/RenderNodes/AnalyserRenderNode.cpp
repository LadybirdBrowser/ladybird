...
void AnalyserRenderNode::process(RenderContext&, Vector<Vector<AudioBus const*>> const& inputs,
    Vector<Vector<AudioBus const*>> const&)
{
    ...
    out.slice(0, samples_until_wrap)
        .copy_to(m_ring_buffer.span().slice(m_ring_write_index, samples_until_wrap));

    size_t const samples_after_wrap = analysis_frames - samples_until_wrap;
    if (samples_after_wrap != 0)
        out.slice(samples_until_wrap, samples_after_wrap)
            .copy_to(m_ring_buffer.span().slice(0, samples_after_wrap));
...
void AnalyserRenderNode::update_time_domain_snapshot_buffer()
{
    ...
    compute_frequency_data_db_in_place(time_domain_out.span().slice(0, fft_size), fft_size,
        smoothing_time_constant, m_previous_block_render, frequency_out,
...
void AnalyserRenderNode::initialize_storage()
{
    ASSERT_CONTROL_THREAD();
    m_fft_size.store(
        min(max(m_fft_size.load(AK::MemoryOrder::memory_order_relaxed), MIN_FFT_SIZE), MAX_FFT_SIZE),
