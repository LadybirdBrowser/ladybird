...
static void apply_distance_model(DistanceModelType model, f32& distance, f64 ref_distance, f64 max_distance,
    f64 rolloff_factor)
{
...
static void apply_cone_gain(f32& gain, f32 source_to_listener_x, f32 source_to_listener_y,
    f32 source_to_listener_z, f32 orientation_x, f32 orientation_y, f32 orientation_z,
    f64 inner_angle, f64 outer_angle, f64 outer_gain)
{
...
void PannerRenderNode::process(RenderContext& context, Vector<Vector<AudioBus const*>> const& inputs,
    Vector<Vector<AudioBus const*>> const& param_inputs)
{
    ...
    for (size_t i = 0; i < frames; ++i) {
        ...
        apply_cone_gain(gain, sl_x, sl_y, sl_z, ori_x, ori_y, ori_z, m_cone_inner_angle, m_cone_outer_angle,
            m_cone_outer_gain);
