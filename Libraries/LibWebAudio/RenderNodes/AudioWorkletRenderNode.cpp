...
AudioWorkletRenderNode::AudioWorkletRenderNode(NodeID node_id, AudioWorkletGraphNode const& desc,
    size_t quantum_size)
    : RenderNode(node_id)
...
void AudioWorkletRenderNode::process(RenderContext& context, Vector<Vector<AudioBus const*>> const& inputs,
    Vector<Vector<AudioBus const*>> const& param_inputs)
{
    ...
    m_keep_processing = context.worklet_processor_host->process_audio_worklet(
        node_id(), context, m_processor_name, m_number_of_inputs, m_number_of_outputs,
        m_output_channel_count.has_value() ? m_output_channel_count->span() : ReadonlySpan<size_t> {},
        m_inputs_for_invocation, output_buses.span(), m_parameters_for_invocation.span());
}
