...
void MediaElementAudioSourceProvider::declare_discontinuity()
{
    ...
    (void)AK::atomic_fetch_add(&local.header.timeline_generation, static_cast<u64>(1),
    ...
    if (should_log_webaudio(LOG_MEDIA)) {
        ...
        if ((now_ms - last_ms) >= 250 && m_last_discontinuity_log_ms.compare_exchange_strong(last_ms, now_ms, AK::MemoryOrder::memory_order_relaxed)) {
            ...
            WA_MEDIA_DBGLN("[WebAudio] media-tap discontinuity: cid={} session={} provider={} read={} write={} "
                           "gen={} eos={} timeline_sr={}",
                debug_client_id(), debug_session_id(), m_provider_id, read_frame, write_frame,
                generation, end_of_stream, timeline_sample_rate);
        }
    }
    ...
    if (should_log_webaudio(LOG_MEDIA)) {
        ...
        if (!was_eos) {
            ...
            if ((now_ms - last_ms) >= 250 && m_last_eos_log_ms.compare_exchange_strong(last_ms, now_ms, AK::MemoryOrder::memory_order_relaxed)) {
                ...
                u64 const generation = AK::atomic_load(&local.header.timeline_generation, AK::MemoryOrder::memory_order_relaxed);
                u32 const timeline_sample_rate = AK::atomic_load(&local.header.timeline_sample_rate, AK::MemoryOrder::memory_order_relaxed);
                WA_MEDIA_DBGLN("[WebAudio] media-tap declare-eos: cid={} session={} provider={} read={} write={} "
                               "gen={} eos_before={} timeline_sr={}",
                    debug_client_id(), debug_session_id(), m_provider_id, read_frame, write_frame,
                    generation, eos_before, timeline_sample_rate);
            }
        }
    }
        ...
        if (should_log_webaudio(LOG_MEDIA)) {
            ...
            if (!was_eos) {
                ...
                if ((now_ms - last_ms) >= 250 && m_last_eos_log_ms.compare_exchange_strong(last_ms, now_ms, AK::MemoryOrder::memory_order_relaxed)) {
                    ...
                    WA_MEDIA_DBGLN("[WebAudio] media-tap declare-eos (transport): cid={} session={} provider={} "
                                   "read={} write={} gen={} eos_before={} timeline_sr={}",
                        debug_client_id(), debug_session_id(), m_provider_id, read_frame, write_frame,
                        generation, eos_before, timeline_sample_rate);
                }
            }
...
NonnullRefPtr<MediaElementAudioSourceProvider>
MediaElementAudioSourceProvider::create(size_t channel_capacity, size_t capacity_frames)
{
    ASSERT_CONTROL_THREAD();
...
NonnullRefPtr<MediaElementAudioSourceProvider> MediaElementAudioSourceProvider::create_for_remote_consumer(
    u64 provider_id, Render::RingStreamView view, Core::AnonymousBuffer shared_memory, int notify_read_fd)
{
    ASSERT_CONTROL_THREAD();
    TransportConsumerState state {
        .view = view, .shared_memory = move(shared_memory), .notify_read_fd = notify_read_fd
    };
    ...
    : m_consumer(move(state))
{
    ...
    AK::atomic_store(&local.header.timeline_generation, static_cast<u64>(1),
        AK::MemoryOrder::memory_order_release);
    AK::atomic_store(&local.header.timeline_sample_rate, 0u, AK::MemoryOrder::memory_order_relaxed);
    AK::atomic_store(&local.header.timeline_media_start_frame, static_cast<u64>(0),
        AK::MemoryOrder::memory_order_relaxed);
    AK::atomic_store(&local.header.timeline_media_start_at_ring_frame, static_cast<u64>(0),
        AK::MemoryOrder::memory_order_relaxed);
}

MediaElementAudioSourceProvider::MediaElementAudioSourceProvider(u64 provider_id,
    TransportConsumerState state)
...
void MediaElementAudioSourceProvider::set_stream_transport_producer(Render::RingStreamView view,
    int notify_write_fd)
{
    ASSERT_CONTROL_THREAD();
    if (!view.header)
    ...
    AK::atomic_store(&transport_header.end_of_stream,
        AK::atomic_load(&local_header.end_of_stream, AK::MemoryOrder::memory_order_relaxed),
        AK::MemoryOrder::memory_order_relaxed);

    AK::atomic_store(&transport_header.timeline_generation,
        AK::atomic_load(&local_header.timeline_generation, AK::MemoryOrder::memory_order_relaxed),
        AK::MemoryOrder::memory_order_relaxed);
    AK::atomic_store(&transport_header.timeline_sample_rate,
        AK::atomic_load(&local_header.timeline_sample_rate, AK::MemoryOrder::memory_order_relaxed),
        AK::MemoryOrder::memory_order_relaxed);
    AK::atomic_store(
        &transport_header.timeline_media_start_frame,
        AK::atomic_load(&local_header.timeline_media_start_frame, AK::MemoryOrder::memory_order_relaxed),
        AK::MemoryOrder::memory_order_relaxed);
    AK::atomic_store(&transport_header.timeline_media_start_at_ring_frame,
        AK::atomic_load(&local_header.timeline_media_start_at_ring_frame,
            AK::MemoryOrder::memory_order_relaxed),
        AK::MemoryOrder::memory_order_relaxed);
...
MediaElementAudioSourceProvider::PeekResult MediaElementAudioSourceProvider::peek_with_timing()
{
    ...
    if (m_consumer.has<TransportConsumerState>()) {
        ...
        (void)Render::ring_stream_consumer_fix_overrun(header, read_frame, write_frame);
    ...
    Render::RingStreamView view { .header = &local.header,
        .interleaved_frames = { local.ring.data(), local.ring.size() } };
...
void MediaElementAudioSourceProvider::push_interleaved(ReadonlySpan<float> interleaved_samples,
    u32 sample_rate, u32 channel_count)
{
    ...
    Render::RingStreamView local_view { .header = &local.header,
        .interleaved_frames = { local.ring.data(), local.ring.size() } };
    Render::RingStreamProducer local_producer(local_view);
    size_t const frames_written = local_producer.try_push_interleaved(
        { interleaved_samples.data(), interleaved_samples.size() }, channel_count);
    ...
    if (m_stream_transport_producer_view.has_value()) {
        ...
        size_t const transport_frames_written = transport_producer.try_push_interleaved(
            { interleaved_samples.data(), interleaved_samples.size() }, channel_count);
    ...
    if (should_log_webaudio(LOG_MEDIA)) {
        ...
        if (should_log && s_last_log_ms.compare_exchange_strong(last_ms, now_ms, AK::MemoryOrder::memory_order_relaxed)) {
            u64 const read_frame = Render::ring_stream_load_read_frame(local.header);
            u64 const write_frame = Render::ring_stream_load_write_frame(local.header);
            WA_MEDIA_DBGLN("[WebAudio] media-tap push: cid={} session={} provider={} frames={} sr={} ch_in={} "
                           "ch_store={} read={} write={} total_pushed={}",
                debug_client_id(), debug_session_id(), m_provider_id, input_frame_count, sample_rate,
                channel_count, clamped_channels, read_frame, write_frame,
    ...
    AK::Duration media_time)
{
    ...
    auto reset_timeline = [&] {
        ...
        AK::atomic_store(&local.header.timeline_media_start_frame, start_media_frame,
            AK::MemoryOrder::memory_order_relaxed);
        AK::atomic_store(&local.header.timeline_media_start_at_ring_frame, write_frame,
            AK::MemoryOrder::memory_order_relaxed);
    ...
    auto update_timeline_mapping = [&](u64 start_media_frame, u64 ring_frame) {
        AK::atomic_store(&local.header.timeline_media_start_frame, start_media_frame,
            AK::MemoryOrder::memory_order_relaxed);
        AK::atomic_store(&local.header.timeline_media_start_at_ring_frame, ring_frame,
            AK::MemoryOrder::memory_order_relaxed);
        AK::atomic_store(&local.header.timeline_sample_rate, sample_rate, AK::MemoryOrder::memory_order_relaxed);
    };

    auto reset_stream_transport_timeline = [&](RingHeader& header, u64 start_media_frame,
                                               u64 stream_write_frame) {
        // Clear any buffered data so the consumer observes a clean discontinuity.
        Render::ring_stream_store_read_frame(header, stream_write_frame);
        Render::ring_stream_store_write_frame(header, stream_write_frame);

        AK::atomic_store(&header.timeline_media_start_frame, start_media_frame,
            AK::MemoryOrder::memory_order_relaxed);
        AK::atomic_store(&header.timeline_media_start_at_ring_frame, stream_write_frame,
            AK::MemoryOrder::memory_order_relaxed);
    ...
    auto update_stream_transport_timeline_mapping = [&](RingHeader& header, u64 start_media_frame,
                                                        u64 stream_write_frame) {
        AK::atomic_store(&header.timeline_media_start_frame, start_media_frame,
            AK::MemoryOrder::memory_order_relaxed);
        AK::atomic_store(&header.timeline_media_start_at_ring_frame, stream_write_frame,
    ...
    if (should_log_webaudio(LOG_MEDIA) && ring_was_empty && frames_written > 0) {
        i64 now_ms = AK::MonotonicTime::now().milliseconds();
        ...
        if ((now_ms - last_ms) >= 250 && m_last_refill_log_ms.compare_exchange_strong(last_ms, now_ms, AK::MemoryOrder::memory_order_relaxed)) {
            ...
            WA_MEDIA_DBGLN("[WebAudio] media-tap refill: cid={} session={} provider={} frames={} sr={} ch_in={} "
                           "ch_store={} media_time_ms={} read={} write_before={} write_after={} gen={}",
                debug_client_id(), debug_session_id(), m_provider_id, frames_written, sample_rate,
                channel_count, clamped_channels, media_time.to_milliseconds(), read_frame,
                local_anchor_ring_frame, new_write_frame, generation);
        }
    }
    ...
    if (should_log_webaudio(LOG_MEDIA)) {
        ...
        if (should_log && s_last_log_ms.compare_exchange_strong(last_ms, now_ms, AK::MemoryOrder::memory_order_relaxed)) {
            WA_MEDIA_DBGLN(
                "[WebAudio] media-tap push: cid={} session={} provider={} frames={} sr={} ch_in={} ch_store={} "
                "media_time_ms={} read={} write={} gen={} total_pushed={}",
                debug_client_id(), debug_session_id(), m_provider_id, input_frame_count, sample_rate, channel_count,
                clamped_channels, media_time.to_milliseconds(), read_frame,
...
MediaElementAudioSourceProvider::PopResult
MediaElementAudioSourceProvider::pop_planar_with_timing(Span<Span<f32>> out_channels, size_t requested_frames,
    u32 expected_channel_count)
{
    ...
    if (m_consumer.has<TransportConsumerState>()) {
        ...
        if (frames_to_read == 0) {
            if (should_log_webaudio(LOG_MEDIA)) {
                ...
                if ((now_ms - last_ms) >= 250 && m_last_empty_log_ms.compare_exchange_strong(last_ms, now_ms, AK::MemoryOrder::memory_order_relaxed)) {
                    u32 const timeline_sample_rate = AK::atomic_load(&header.timeline_sample_rate, AK::MemoryOrder::memory_order_relaxed);
                    WA_MEDIA_DBGLN("[WebAudio] media-tap empty (remote): cid={} session={} provider={} want={} "
                                   "avail={} read={} write={} gen={} eos={} header_sr={} header_ch={} timeline_sr={}",
                        debug_client_id(), debug_session_id(), m_provider_id, requested_frames, available,
                        read_frame, write_frame, result.timeline_generation, result.end_of_stream,
                        header.sample_rate_hz, header.channel_count, timeline_sample_rate);
                }
            }
            return result;
        }

        if (should_log_webaudio(LOG_MEDIA) && frames_to_read < requested_frames) {
            i64 now_ms = AK::MonotonicTime::now().milliseconds();
            ...
            if ((now_ms - last_ms) >= 250 && m_last_short_read_log_ms.compare_exchange_strong(last_ms, now_ms, AK::MemoryOrder::memory_order_relaxed)) {
                u32 const timeline_sample_rate = AK::atomic_load(&header.timeline_sample_rate, AK::MemoryOrder::memory_order_relaxed);
                WA_MEDIA_DBGLN(
                    "[WebAudio] media-tap short-read (remote): cid={} session={} provider={} need={} got={} avail={} "
                    "read={} write={} gen={} eos={} header_sr={} header_ch={} timeline_sr={}",
                    debug_client_id(), debug_session_id(), m_provider_id, requested_frames, frames_to_read, available,
                    read_frame, write_frame, result.timeline_generation, result.end_of_stream, header.sample_rate_hz,
                    header.channel_count, timeline_sample_rate);
            }
        }
        ...
        (void)Render::ring_stream_pop_planar(view, read_frame, frames_to_read, out_channels,
    ...
    if (frames_to_read == 0) {
        if (should_log_webaudio(LOG_MEDIA)) {
            ...
            if ((now_ms - last_ms) >= 250 && m_last_empty_log_ms.compare_exchange_strong(last_ms, now_ms, AK::MemoryOrder::memory_order_relaxed)) {
                u32 const timeline_sample_rate = AK::atomic_load(&header.timeline_sample_rate, AK::MemoryOrder::memory_order_relaxed);
                WA_MEDIA_DBGLN("[WebAudio] media-tap empty (local): cid={} session={} provider={} want={} avail={} "
                               "read={} write={} gen={} eos={} timeline_sr={}",
                    debug_client_id(), debug_session_id(), m_provider_id, requested_frames, available,
                    read_frame, write_frame, result.timeline_generation, result.end_of_stream,
    ...
    if (should_log_webaudio(LOG_MEDIA) && frames_to_read < requested_frames) {
        ...
        if ((now_ms - last_ms) >= 250 && m_last_short_read_log_ms.compare_exchange_strong(last_ms, now_ms, AK::MemoryOrder::memory_order_relaxed)) {
            u32 const timeline_sample_rate = AK::atomic_load(&header.timeline_sample_rate, AK::MemoryOrder::memory_order_relaxed);
            WA_MEDIA_DBGLN("[WebAudio] media-tap short-read (local): cid={} session={} provider={} need={} got={} "
                           "avail={} read={} write={} gen={} eos={} timeline_sr={}",
                debug_client_id(), debug_session_id(), m_provider_id, requested_frames, frames_to_read,
                available, read_frame, write_frame, result.timeline_generation, result.end_of_stream,
                timeline_sample_rate);
        }
    }
...
size_t MediaElementAudioSourceProvider::pop_planar(Span<Span<f32>> out_channels, size_t requested_frames,
    u32 expected_channel_count)
{
    ASSERT_RENDER_THREAD();
