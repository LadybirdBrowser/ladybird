...
#include <LibWebAudio/RenderNodes/ScriptProcessorRenderNode.h>
#include <LibWebAudio/Script/ScriptProcessorHost.h>

namespace Web::WebAudio::Render {

ScriptProcessorRenderNode::ScriptProcessorRenderNode(NodeID node_id, ScriptProcessorGraphNode const& desc,
    size_t quantum_size)
    : RenderNode(node_id)
...
void ScriptProcessorRenderNode::process(RenderContext& context,
...
void ScriptProcessorRenderNode::process_completed_input_block(RenderContext& context)
{
    ...
    if (context.script_processor_host) {
        ...
        bool ok = context.script_processor_host->process_script_processor(
            node_id(), context, playback_time_seconds, m_buffer_size, m_input_channel_count,
            m_output_channel_count, input_channels.span(), output_channels.span());

        if (!ok) {
            bool const input_all_zeros = is_all_zeros(m_input_block);
            WA_SP_DBGLN("[WebAudio][SP] process failed: node={} t={}s ctx_frame={} buffer={} in_ch={} out_ch={} "
                        "input_all_zeros={} pending_outputs={} initial_silent_remaining={}",
                node_id(), playback_time_seconds, context.current_frame, m_buffer_size,
                m_input_channel_count, m_output_channel_count, input_all_zeros,
                m_pending_output_blocks.size(), m_initial_silent_blocks_remaining);
        }
