...
#include <LibWebAudio/RenderNodes/GainRenderNode.h>
...
void GainRenderNode::process(RenderContext&, Vector<Vector<AudioBus const*>> const& inputs,
    Vector<Vector<AudioBus const*>> const& param_inputs)
{
    ...
    if (should_log_this_quantum) {
        ...
        WA_NODE_DBGLN("[WebAudio][GainNode:{}] out_ch={} connections={} mixed0_ch={} gain={}", node_id(),
            m_output.channel_count(), connection_count, mixed_input_channels, m_gain);
    }
