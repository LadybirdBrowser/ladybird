...
#include <LibWebAudio/RenderNodes/MediaElementAudioSourceRenderNode.h>
...
static constexpr i64 FUTURE_MEDIA_RESYNC_THRESHOLD_MS = 200;

MediaElementAudioSourceRenderNode::MediaElementAudioSourceRenderNode(
...
void MediaElementAudioSourceRenderNode::process(RenderContext& context,
    ...
    Vector<Vector<AudioBus const*>> const&)
{
        ...
        if (peek_result.available_frames == 0) {
            ...
            if (should_log_webaudio(LOG_MEDIA)) {
                ...
                if ((now_ms - last_ms) >= 500 && s_last_empty_log_ms.compare_exchange_strong(last_ms, now_ms, AK::MemoryOrder::memory_order_relaxed)) {
                    WA_MEDIA_DBGLN("[WebAudio] media-source node: cid={} session={} provider={} EMPTY want_frames={} "
                                   "out_ch={} provider_sr={} provider_ch={} pushed={} popped={}",
                        m_provider->debug_client_id(), m_provider->debug_session_id(),
                        m_provider->provider_id(), quantum_frames, output_channel_count,
                        m_provider->sample_rate(), m_provider->channel_count(),
                        m_provider->debug_total_frames_pushed(), m_provider->debug_total_frames_popped());

                    WA_MEDIA_DBGLN("[WebAudio] media-source node: cid={} session={} provider={} EMPTY ctx_frame={} "
                                   "ctx_time_ms={} gen={} avail={} eos={}",
                        m_provider->debug_client_id(), m_provider->debug_session_id(),
                        m_provider->provider_id(), context.current_frame, context_time.to_milliseconds(),
                        peek_result.timeline_generation, peek_result.available_frames,
    ...
    if (provider_sample_rate == 0) {
        if (should_log_webaudio(LOG_MEDIA)) {
            ...
            if ((now_ms - last_ms) >= 500 && s_last_sr0_log_ms.compare_exchange_strong(last_ms, now_ms, AK::MemoryOrder::memory_order_relaxed)) {
                WA_MEDIA_DBGLN("[WebAudio] media-source node: cid={} session={} provider={} provider sample_rate=0 "
                               "(avail={} eos={} gen={} pushed={} popped={})",
                    m_provider->debug_client_id(), m_provider->debug_session_id(),
                    m_provider->provider_id(), peek_result.available_frames, peek_result.end_of_stream,
                    peek_result.timeline_generation, m_provider->debug_total_frames_pushed(),
                    m_provider->debug_total_frames_popped());

                WA_MEDIA_DBGLN("[WebAudio] media-source node: cid={} session={} provider={} provider sample_rate=0 "
                               "ctx_frame={} ctx_time_ms={}",
                    m_provider->debug_client_id(), m_provider->debug_session_id(),
                    m_provider->provider_id(), context.current_frame, context_time.to_milliseconds());
            }
        }
    ...
    if (m_media_to_context_offset.has_value() && peek_result.start_time.has_value()) {
        ...
        // If control-plane churn lets the media element keep advancing while we keep draining
        // older buffered audio, do not re-anchor the old audio to "now". Drop the stale frames
        // and keep the media source aligned to the context timeline.
        if (delta_ms <= -STALE_MEDIA_RESYNC_THRESHOLD_MS) {
            i64 const stale_ms = -delta_ms;
            size_t const stale_frames = static_cast<size_t>(
                AK::ceil((static_cast<double>(stale_ms) * static_cast<double>(provider_sample_rate)) / 1000.0));
            ...
            if (skipped_frames > 0) {
                ...
                if (should_log_webaudio(LOG_MEDIA)) {
                    WA_MEDIA_DBGLN("[WebAudio] media-source node: cid={} session={} provider={} skipped stale "
            ...
            if (peek_result.start_time.has_value())
                m_media_to_context_offset = context_time - peek_result.start_time.value();
        } else if (delta_ms >= FUTURE_MEDIA_RESYNC_THRESHOLD_MS) {
            // Future-dated media after a large timing jump cannot be corrected by skipping.
            // Re-anchor the offset so we do not accumulate extra latency.
    ...
    if (m_resample_ratio_hold_quanta > 0) {
        ...
        // Keep correction gentle so we smooth jitter without audible pitch changes.
    ...
    while (output_frames_produced < output_frames_to_fill) {
        if (m_resample_input_pending_frames == 0) {
            ...
            if (frames_read == 0) {
                if (m_provider->wait_for_frames(1, 1))
                    frames_read = m_provider->pop_planar(pop_spans.span(), max_input_frames, output_channel_count);
            }
        ...
        for (size_t ch = 0; ch < output_channel_count; ++ch)
            input_spans[ch] = m_resample_input_channels[ch].span().slice(m_resample_input_start_frame,
                m_resample_input_pending_frames);
