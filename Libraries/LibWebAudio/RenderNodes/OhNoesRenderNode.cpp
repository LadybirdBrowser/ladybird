...
void OhNoesRenderNode::process(RenderContext& context, Vector<Vector<AudioBus const*>> const& inputs,
    Vector<Vector<AudioBus const*>> const&)
{
    ...
    copy_planar_to_interleaved(m_planar_channels.span(),
        m_interleaved_samples.span().slice(0, interleaved_sample_count), frames);
    (void)m_wav_writer->write_interleaved_samples(
        m_interleaved_samples.span().slice(0, interleaved_sample_count));
#else
    (void)context;
#endif
}
