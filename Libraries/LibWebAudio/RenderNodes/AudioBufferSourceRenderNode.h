...
public:
    AudioBufferSourceRenderNode(NodeID node_id, AudioBufferSourceGraphNode const& desc,
        RefPtr<SharedAudioBuffer> buffer, size_t quantum_size);

    void process(RenderContext& context, Vector<Vector<AudioBus const*>> const&,
    ...
    void schedule_stop(Optional<size_t> stop_frame) override;
...
private:
    ...
    f64 increment_for_sample(size_t sample_index, f64 buffer_to_context_ratio, bool has_playback_rate_input,
        bool has_detune_input) const;
    void render_samples(RenderContext const&, size_t graph_start, size_t quantum_render_start,
        size_t quantum_render_end, size_t offset, size_t channels_to_render,
        size_t buffer_length, bool loop_enabled, size_t loop_start, size_t loop_end,
        size_t loop_length, Optional<size_t> effective_duration, f64 buffer_to_context_ratio,
