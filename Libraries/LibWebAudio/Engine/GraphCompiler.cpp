...
OwnPtr<Render::RenderNode> make_render_node(GraphDescription const&, NodeID node_id,
    GraphNodeDescription const& node, size_t quantum_size,
    GraphResources const& resources)
{
    ASSERT_CONTROL_THREAD();
    return node.visit(
        [&](auto const& payload) { return payload.make_render_node(node_id, quantum_size, resources); });
}
...
GraphUpdateKind classify_graph_update(GraphDescription const& old_desc, GraphDescription const& new_desc)
{
    ...
    if (old_desc.param_automations.size() != new_desc.param_automations.size()) {
        ...
        for (size_t i = 0; i < old_desc.param_automations.size(); ++i) {
            ...
            if (a.destination != b.destination || a.destination_param_index != b.destination_param_index || a.initial_value != b.initial_value || a.default_value != b.default_value || a.min_value != b.min_value || a.max_value != b.max_value || a.automation_rate != b.automation_rate || a.segments.size() != b.segments.size()) {
                any_param_automation_changed = true;
                break;
            }
            ...
            for (size_t j = 0; j < a.segments.size(); ++j) {
                ...
                if (sa.type != sb.type || sa.start_frame != sb.start_frame || sa.end_frame != sb.end_frame || sa.start_value != sb.start_value || sa.end_value != sb.end_value || sa.time_constant != sb.time_constant || sa.target != sb.target || sa.curve.size() != sb.curve.size()) {
                    any_param_automation_changed = true;
                    break;
                }
...
GraphUpdateKind GraphCompiler::classify_node_update(GraphNodeDescription const& old_desc,
    GraphNodeDescription const& new_desc)
{
    ASSERT_CONTROL_THREAD();
...
GraphUpdateKind GraphCompiler::classify_update(GraphDescription const& old_description,
    GraphDescription const& new_description)
{
    ASSERT_CONTROL_THREAD();
    return classify_graph_update(old_description, new_description);
}

RenderNodeSet GraphCompiler::compile_nodes(GraphDescription const& description, size_t quantum_size,
    GraphResources const& resources)
{
    ASSERT_CONTROL_THREAD();
    RenderNodeSet compiled_nodes;
    compiled_nodes.node_ids.ensure_capacity(description.nodes.size());
    compiled_nodes.node_types_by_index.ensure_capacity(description.nodes.size());
    compiled_nodes.nodes.ensure_capacity(description.nodes.size());

    Vector<NodeID> sorted_node_ids;
    sorted_node_ids.ensure_capacity(description.nodes.size());
    for (auto const& it : description.nodes)
        sorted_node_ids.append(it.key);
    quick_sort(sorted_node_ids, [](NodeID const& a, NodeID const& b) { return a.value() < b.value(); });

    for (auto const& node_id : sorted_node_ids) {
        auto maybe_node = description.nodes.get(node_id);
        ...
        size_t const node_index = compiled_nodes.nodes.size();
        compiled_nodes.node_ids.append(node_id);
        compiled_nodes.node_types_by_index.append(node_type);
        compiled_nodes.node_index_by_id.set(node_id, node_index);

        compiled_nodes.nodes.append(make_render_node(description, node_id, node, quantum_size, resources));

        if (node_type == GraphNodeType::Analyser)
            compiled_nodes.analyser_node_indices.append(node_index);

        VERIFY(compiled_nodes.nodes.size() == node_index + 1);
    }

    return compiled_nodes;
}

static void rebuild_processing_order(RenderNodeSet const& compiled_nodes, GraphTopology& topology,
    GraphDescription const& description, size_t quantum_size)
{
    ASSERT_CONTROL_THREAD();
    ...
    size_t const real_node_count = compiled_nodes.nodes.size();
    ...
    // 4.2.1: Let ordered node list be an empty list of AudioNodes and AudioListener. It will contain an ordered
    // list of AudioNodes and the AudioListener when this ordering algorithm terminates.
    ...
    for (GraphConnection const& c : topology.connections) {
        auto source_index_it = compiled_nodes.node_index_by_id.find(c.source);
        if (source_index_it == compiled_nodes.node_index_by_id.end())
            continue;
        auto destination_index_it = compiled_nodes.node_index_by_id.find(c.destination);
        if (destination_index_it == compiled_nodes.node_index_by_id.end())
            continue;
    ...
    // 4.2.4: Let cycle breakers be an empty set of DelayNodes. It will contain all the DelayNodes that are part
    // of a cycle.
    ...
    // 4.2.5: For each AudioNode node in nodes: If node is a DelayNode that is part of a cycle, add it to cycle
    // breakers and remove it from nodes.
    for (size_t const node_index : nodes) {
        if (compiled_nodes.node_types_by_index[node_index] != GraphNodeType::Delay)
            continue;
    ...
    // 4.2.6: For each DelayNode delay in cycle breakers: Let delayWriter and delayReader respectively be a
    // DelayWriter and a DelayReader, for delay. Add delayWriter and delayReader to nodes. Disconnect delay from
    // all its input and outputs.
    for (size_t node_index = 0; node_index < real_node_count; ++node_index) {
        GraphNodeType const node_type = compiled_nodes.node_types_by_index[node_index];
    ...
    for (GraphConnection const& c : topology.connections) {
        ...
        Optional<size_t> source_virtual = delay_in_cycle[source_real] ? delay_reader_index_for_real[source_real]
                                                                      : virtual_index_for_real[source_real];
        Optional<size_t> destination_virtual = delay_in_cycle[destination_real]
            ? delay_writer_index_for_real[destination_real]
            : virtual_index_for_real[destination_real];
    ...
    for (GraphParamConnection const& c : topology.param_connections) {
        ...
        Optional<size_t> source_virtual = delay_in_cycle[source_real] ? delay_reader_index_for_real[source_real]
                                                                      : virtual_index_for_real[source_real];
        Optional<size_t> destination_virtual = delay_in_cycle[destination_real]
            ? delay_reader_index_for_real[destination_real]
            : virtual_index_for_real[destination_real];
        if (!source_virtual.has_value() || !destination_virtual.has_value())
            continue;
    ...
    // 4.2.7: If nodes contains cycles, mute all the AudioNodes that are part of this cycle, and remove them
    // from nodes.
    ...
    for (size_t node_index = 0; node_index < virtual_nodes.size(); ++node_index) {
        ...
        GraphTopology::ProcessingNodeKind kind = GraphTopology::ProcessingNodeKind::Real;
        if (virtual_node.kind == VirtualNode::Kind::DelayWriter)
            kind = GraphTopology::ProcessingNodeKind::DelayWriter;
        else if (virtual_node.kind == VirtualNode::Kind::DelayReader)
            kind = GraphTopology::ProcessingNodeKind::DelayReader;

        topology.nodes.append(GraphTopology::ProcessingNode {
            .kind = kind,
            ...
            .render_node = compiled_nodes.nodes[virtual_node.real_node_index].ptr(),
        });
    }
    ...
    auto param_count_for_node = [&](GraphTopology::ProcessingNode const& node) -> size_t {
        if (node.kind == GraphTopology::ProcessingNodeKind::DelayWriter)
            return 0;
        if (node.kind == GraphTopology::ProcessingNodeKind::DelayReader)
        ...
        if (node.node_type == GraphNodeType::AudioWorklet) {
            NodeID const node_id = compiled_nodes.node_ids[node.real_node_index];
    ...
    for (size_t node_index = 0; node_index < node_count; ++node_index) {
        GraphTopology::ProcessingNode const& node = topology.nodes[node_index];
        ...
        GraphTopology::ChannelMixingSettings settings {};
    ...
    for (VirtualAudioConnection const& connection : virtual_audio_connections) {
        ...
        per_input_connections[connection.destination_input].append(IndexedConnection {
    ...
    for (VirtualParamConnection const& connection : virtual_param_connections) {
        ...
        if (param_index >= per_param_connections.size())
            continue;
        per_param_connections[param_index].append(IndexedConnection {
            .source_node_index = source_index,
            .source_output = connection.source_output,
        });
    ...
    for (size_t node_index = 0; node_index < node_count; ++node_index) {
        if (topology.nodes[node_index].node_type != GraphNodeType::AudioWorklet)
            continue;
        NodeID const node_id = compiled_nodes.node_ids[topology.nodes[node_index].real_node_index];
    ...
    for (size_t node_index = 0; node_index < node_count; ++node_index) {
        ...
        for (size_t input_index = 0; input_index < per_input_connections.size(); ++input_index)
            per_input_mix_buses.append(make<AudioBus>(1, quantum_size, max_mixing_channel_count));
    ...
    size_t destination_node_index = 0;
    auto destination_index_it = compiled_nodes.node_index_by_id.find(description.destination_node_id);
    if (destination_index_it != compiled_nodes.node_index_by_id.end()) {
        size_t const destination_real = destination_index_it->value;
        Optional<size_t> destination_virtual = delay_in_cycle[destination_real]
            ? delay_reader_index_for_real[destination_real]
    ...
    auto visit_node = [&](auto& self, size_t node_index) -> void {
        ...
        // 4.2.8.4: For each AudioParam param of node: For each AudioNode param input node connected to param:
    ...
    // 4.2.8: Consider all elements in nodes to be unmarked. While there are unmarked elements in nodes: Choose
    // an element node in nodes. Visit node.
    for (size_t node_index = 0; node_index < node_count; ++node_index) {
        if (!marked[node_index])
...
GraphTopology* GraphCompiler::build_topology(RenderNodeSet const& compiled_nodes,
    GraphDescription const& description, size_t quantum_size)
{
    ASSERT_CONTROL_THREAD();
    auto* topology = new GraphTopology;
    topology->connections = description.connections;
    topology->param_connections = description.param_connections;
    rebuild_processing_order(compiled_nodes, *topology, description, quantum_size);
    return topology;
}
