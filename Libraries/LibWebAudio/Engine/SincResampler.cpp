...
void prepare_sinc_resampler_kernel(SincResamplerKernel& kernel, f64 input_frames_per_output_frame)
{
    ...
    for (size_t phase = 0; phase < SincResamplerKernel::phase_count; ++phase) {
        ...
        if (sum != 0.0 && !__builtin_isnan(sum) && __builtin_isfinite(sum)) {
            ...
            for (size_t tap = 0; tap < SincResamplerKernel::tap_count; ++tap)
                kernel.coefficients[(phase * SincResamplerKernel::tap_count) + tap] = static_cast<f32>(
                    static_cast<f64>(kernel.coefficients[(phase * SincResamplerKernel::tap_count) + tap]) * inv_sum);
...
void sample_rate_converter_init(SampleRateConverter& state, size_t channel_count,
...
ResampleResult sample_rate_converter_process(SampleRateConverter& state,
    ...
    Span<Span<f32>> output_channels, bool flush)
{
    ...
    while (produced < output_frames) {
        ...
        bool const window_fully_available = start_index >= 0 && end_index < static_cast<i64>(state.total_frames_written) && (state.total_frames_written <= state.ring_size || static_cast<u64>(start_index) >= (state.total_frames_written - state.ring_size));
...
ALWAYS_INLINE void SampleRateConverter::write_input_frame(Span<ReadonlySpan<f32>> input_channels,
    size_t input_index)
{
