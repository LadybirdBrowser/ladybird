...
#include <LibWebAudio/Engine/GraphCompiler.h>
...
#include <LibWebAudio/RenderNodes/AnalyserRenderNode.h>
#include <LibWebAudio/RenderNodes/AudioListenerRenderNode.h>
#include <LibWebAudio/RenderNodes/DelayRenderNode.h>
#include <LibWebAudio/RenderNodes/DynamicsCompressorRenderNode.h>
#include <LibWebAudio/RenderNodes/RenderNode.h>

namespace Web::WebAudio::Render {

struct GraphExecutor::ParameterUpdateBatch {
    Vector<GraphNodeDescription> nodes_by_index;
    Vector<GraphParamAutomation> param_automations;
};

GraphExecutor::GraphExecutor(GraphDescription const& description, f32 sample_rate, size_t quantum_size,
    ...
    , m_resources(resources)
{
    ...
    m_nodes = GraphCompiler::compile_nodes(m_description, quantum_size, m_resources);

    for (size_t i = 0; i < m_nodes.nodes.size(); ++i) {
        if (m_nodes.node_types_by_index[i] == GraphNodeType::AudioListener) {
            m_context.listener = static_cast<AudioListenerRenderNode*>(m_nodes.nodes[i].ptr());
            break;
        }
    }

    auto param_count_for_node = [&](size_t node_index) -> size_t {
        auto const node_type = m_nodes.node_types_by_index[node_index];
        size_t param_count = RenderParamLayout::param_count(node_type);
        if (node_type == GraphNodeType::AudioWorklet) {
            auto const node_id = m_nodes.node_ids[node_index];
    ...
    m_param_automation_state.resize(m_nodes.nodes.size());
    m_param_automation_buses.resize(m_nodes.nodes.size());
    for (size_t node_index = 0; node_index < m_nodes.nodes.size(); ++node_index) {
    ...
    for (auto const& automation : m_description.param_automations) {
        auto node_index_it = m_nodes.node_index_by_id.find(automation.destination);
        if (node_index_it == m_nodes.node_index_by_id.end())
            continue;
    ...
    m_topology = GraphCompiler::build_topology(m_nodes, m_description, m_context.quantum_size);
    ...
    if (m_topology) {
        ...
        if (destination_index < m_topology->inputs_by_input.size()) {
            auto const destination_node_id = m_nodes.node_ids[destination_index];
            ...
            for (size_t input_index = 0; input_index < inputs.size(); ++input_index) {
                ...
                for (auto const& connection : connections) {
                    auto source_node_id = m_nodes.node_ids[connection.source_node_index];
                    builder.appendff(" {}", source_node_id.value());
                }
            }
            WA_DBGLN("[GraphExecutor] destination {} audio inputs:{}", destination_node_id.value(),
                builder.string_view());
        }
    }
}
...
{
    ASSERT_RENDER_THREAD();
    // Let the output driver apply graph/param changes without rendering
    try_commit_pending_topology();
    try_commit_pending_parameter_updates();
    m_context.current_frame = current_frame;
}
...
{
    ASSERT_RENDER_THREAD();
    auto it = m_nodes.node_index_by_id.find(node_id);
    if (it == m_nodes.node_index_by_id.end())
        return;
    auto node_index = it->value;
    if (node_index >= m_nodes.nodes.size())
        return;
    m_nodes.nodes[node_index]->schedule_start(start_frame);
}
...
void GraphExecutor::schedule_source_stop(NodeID node_id, Optional<size_t> stop_frame)
{
    ...
    if (node_index >= m_nodes.nodes.size())
        return;
    m_nodes.nodes[node_index]->schedule_stop(stop_frame);
}
...
bool GraphExecutor::enqueue_topology_update(GraphDescription const& description)
{
    ...
    for (auto const node_id : m_nodes.node_ids) {
        auto old_node = m_description.nodes.get(node_id);
        auto new_node = description.nodes.get(node_id);
        if (!old_node.has_value() || !new_node.has_value() || graph_node_type(*old_node) != graph_node_type(*new_node) || GraphCompiler::classify_node_update(*old_node, *new_node) != GraphUpdateKind::None) {

            return false;
        }
    }

    auto* new_topology = GraphCompiler::build_topology(m_nodes, description, m_context.quantum_size);
...
bool GraphExecutor::enqueue_parameter_update(GraphDescription const& description)
{
    ...
    if (description.nodes.size() != m_nodes.node_ids.size())
        return false;

    auto* batch = new ParameterUpdateBatch;
    batch->nodes_by_index.resize_with_default_value(m_nodes.node_ids.size(),
        GraphNodeDescription { OhNoesGraphNode {} });
    batch->param_automations = description.param_automations;

    for (size_t node_index = 0; node_index < m_nodes.node_ids.size(); ++node_index) {
        auto const node_id = m_nodes.node_ids[node_index];
        ...
        if (graph_node_type(*maybe_desc) != m_nodes.node_types_by_index[node_index]) {
            delete batch;
            return false;
        }
...
{
    ASSERT_RENDER_THREAD();
    return m_nodes.analyser_node_indices.size();
}

NodeID GraphExecutor::analyser_node_id(size_t analyser_index) const
{
    ASSERT_RENDER_THREAD();
    if (analyser_index >= m_nodes.analyser_node_indices.size())
        return 0;
    return m_nodes.node_ids[m_nodes.analyser_node_indices[analyser_index]];
}
...
bool GraphExecutor::copy_analyser_time_domain_data(size_t analyser_index, Span<f32> output) const
{
    ...
    auto node_index = m_nodes.analyser_node_indices[analyser_index];
    VERIFY(node_index < m_nodes.nodes.size());
    VERIFY(m_nodes.node_types_by_index[node_index] == GraphNodeType::Analyser);

    return static_cast<AnalyserRenderNode&>(*m_nodes.nodes[node_index]).copy_analyser_time_domain_data(output);
}
...
bool GraphExecutor::copy_analyser_frequency_data_db(size_t analyser_index, Span<f32> output) const
{
    ...
    return static_cast<AnalyserRenderNode&>(*m_nodes.nodes[node_index])
        .copy_analyser_frequency_data_db(output);
}

bool GraphExecutor::try_copy_dynamics_compressor_reduction(NodeID compressor_node_id,
    f32& out_reduction_db) const
{
    ASSERT_RENDER_THREAD();
    auto node_index_it = m_nodes.node_index_by_id.find(compressor_node_id);
    if (node_index_it == m_nodes.node_index_by_id.end())
        return false;

    size_t const node_index = node_index_it->value;
    if (node_index >= m_nodes.node_types_by_index.size() || m_nodes.node_types_by_index[node_index] != GraphNodeType::DynamicsCompressor)
        return false;

    if (auto* node = as_if<DynamicsCompressorRenderNode>(m_nodes.nodes[node_index].ptr())) {
        out_reduction_db = node->reduction_db();
        return true;
    }
    return false;
}
...
void GraphExecutor::apply_update_offline(GraphDescription const& description)
{
    ...
    if (update_kind == GraphUpdateKind::RebuildRequired) {
        ...
        m_nodes = GraphCompiler::compile_nodes(m_description, m_context.quantum_size, m_resources);
        for (size_t i = 0; i < m_nodes.nodes.size(); ++i) {
            if (m_nodes.node_types_by_index[i] == GraphNodeType::AudioListener) {
                m_context.listener = static_cast<AudioListenerRenderNode*>(m_nodes.nodes[i].ptr());
                break;
        ...
        for (auto const& it : description.nodes) {
            auto node_index_it = m_nodes.node_index_by_id.find(it.key);
            if (node_index_it == m_nodes.node_index_by_id.end())
                continue;

            m_nodes.nodes[node_index_it->value]->apply_description_offline(it.value);
        }
    }
    ...
    for (size_t node_index = 0; node_index < m_nodes.nodes.size(); ++node_index) {
        ...
        case GraphNodeType::Oscillator:
            if (node_desc.has<OscillatorGraphNode>()) {
                ...
                set_state(OscillatorParamIndex::detune, osc.detune_cents, -AK::NumericLimits<f32>::max(),
                    AK::NumericLimits<f32>::max());
            }
            break;
        case GraphNodeType::AudioBufferSource:
            if (node_desc.has<AudioBufferSourceGraphNode>()) {
                auto const& buf = node_desc.get<AudioBufferSourceGraphNode>();
                set_state(AudioBufferSourceParamIndex::playback_rate, buf.playback_rate, 0.0f,
                    AK::NumericLimits<f32>::max());
                set_state(AudioBufferSourceParamIndex::detune, buf.detune_cents, -AK::NumericLimits<f32>::max(),
                    AK::NumericLimits<f32>::max());
            }
            break;
        case GraphNodeType::ConstantSource:
            if (node_desc.has<ConstantSourceGraphNode>())
                set_state(ConstantSourceParamIndex::offset, node_desc.get<ConstantSourceGraphNode>().offset,
                    -AK::NumericLimits<f32>::max(), AK::NumericLimits<f32>::max());
            break;
        case GraphNodeType::Delay:
            if (node_desc.has<DelayGraphNode>()) {
                auto const& delay = node_desc.get<DelayGraphNode>();
                set_state(DelayParamIndex::delay_time, delay.delay_time_seconds, 0.0f,
                    max(delay.max_delay_time_seconds, 0.0f));
            }
            break;
        case GraphNodeType::AudioListener:
            if (node_desc.has<AudioListenerGraphNode>())
...
void GraphExecutor::process_graph_if_needed()
{
    ...
    static constexpr size_t MAX_MIX_CHANNELS = 32;

    auto compute_computed_number_of_channels = [](GraphTopology::ChannelMixingSettings const& settings,
                                                   size_t max_input_channels) -> size_t {
        ...
        for (size_t input_index = 0; input_index < per_input_connections.size(); ++input_index) {
            ...
            if (!connections.is_empty()) {
                ...
                if (any_input_with_channels) {
                    size_t desired_channels = compute_computed_number_of_channels(
                        topology.channel_mixing_by_node[node_index], max_input_channels);
                    desired_channels = clamp(desired_channels, 1ul, MAX_MIX_CHANNELS);
                    ...
                    if (processing_node.node_type == GraphNodeType::AudioWorklet) {
                        NodeID const node_id = m_nodes.node_ids[processing_node.real_node_index];
                        auto node_it = m_description.nodes.find(node_id);
                        if (node_it != m_description.nodes.end() && node_it->value.has<AudioWorkletGraphNode>()) {
        ...
        for (size_t param_index = 0; param_index < per_param_connections.size(); ++param_index) {
            ...
            auto evaluate_segment_at_frame = [&](GraphAutomationSegment const& segment, size_t frame) -> f32 {
                ...
                case GraphAutomationSegmentType::LinearRamp: {
                    f32 const position = static_cast<f32>(pos);
                    f32 const delta = segment.end_value - segment.start_value;
                    return segment.start_value + (delta * position);
                }
                ...
                case GraphAutomationSegmentType::Target: {
                    ...
                    return static_cast<f32>(
                        static_cast<f64>(segment.target) + ((static_cast<f64>(segment.start_value) - static_cast<f64>(segment.target)) * k));
                }
                case GraphAutomationSegmentType::ValueCurve: {
                    ...
                    f64 const curve_duration = segment.curve_duration > 0
                        ? segment.curve_duration
                        : max(0.0, segment.end_time - segment.start_time);
                    f64 const curve_pos = curve_duration > 0 ? clamp((sample_time - segment.curve_start_time) / curve_duration, 0.0, 1.0)
                                                             : pos;
                    ...
                    return static_cast<f32>(
                        static_cast<f64>(segment.curve[idx]) + ((static_cast<f64>(segment.curve[next]) - static_cast<f64>(segment.curve[idx])) * frac));
                }
                }
                return segment.end_value;
            };
            ...
            auto add_intrinsic = [&](size_t start_frame) {
                ...
                while (state.current_segment_index + 1 < state.segments.size() && state.segments[state.current_segment_index].end_frame <= start_frame) {
                ...
                for (size_t i = 0; i < m_context.quantum_size; ++i) {
                    ...
                    while (state.current_segment_index + 1 < state.segments.size() && state.segments[state.current_segment_index].end_frame <= frame) {
                        ++state.current_segment_index;
                    }
                    auto const& seg = state.segments[min(state.current_segment_index, state.segments.size() - 1)];
...
void GraphExecutor::try_commit_pending_parameter_updates()
{
    ...
    VERIFY(pending->nodes_by_index.size() == m_nodes.nodes.size());
    for (size_t node_index = 0; node_index < m_nodes.nodes.size(); ++node_index) {
        m_nodes.nodes[node_index]->apply_description(pending->nodes_by_index[node_index]);
    }
    // Keep intrinsic (value-setter) parameter values in sync with the node descriptions.
    // This matters for audio-rate param connections (modulation): computedValue = intrinsic + sum(param
    // inputs). Automation segments are applied separately via m_param_automation_state[node][param].segments.
    ...
    for (size_t node_index = 0; node_index < pending->nodes_by_index.size(); ++node_index) {
        ...
        case GraphNodeType::BiquadFilter:
            if (node_desc.has<BiquadFilterGraphNode>()) {
                node_desc.get<BiquadFilterGraphNode>().update_intrinsic_values(
                    [&](size_t param_index, f32 intrinsic_value) {
        ...
        case GraphNodeType::DynamicsCompressor:
            if (node_desc.has<DynamicsCompressorGraphNode>()) {
                node_desc.get<DynamicsCompressorGraphNode>().update_intrinsic_values(
                    [&](size_t param_index, f32 intrinsic_value) {
                        update_intrinsic(node_index, param_index, intrinsic_value);
                    });
            }
        ...
        case GraphNodeType::Oscillator:
            if (node_desc.has<OscillatorGraphNode>()) {
                node_desc.get<OscillatorGraphNode>().update_intrinsic_values(
                    [&](size_t param_index, f32 intrinsic_value) {
                        update_intrinsic(node_index, param_index, intrinsic_value);
                    });
            }
        ...
        case GraphNodeType::AudioBufferSource:
            if (node_desc.has<AudioBufferSourceGraphNode>()) {
                node_desc.get<AudioBufferSourceGraphNode>().update_intrinsic_values(
                    [&](size_t param_index, f32 intrinsic_value) {
                        update_intrinsic(node_index, param_index, intrinsic_value);
                    });
            }
        ...
        case GraphNodeType::ConstantSource:
            if (node_desc.has<ConstantSourceGraphNode>()) {
                node_desc.get<ConstantSourceGraphNode>().update_intrinsic_values(
                    [&](size_t param_index, f32 intrinsic_value) {
                        update_intrinsic(node_index, param_index, intrinsic_value);
                    });
            }
        ...
        case GraphNodeType::AudioListener:
            if (node_desc.has<AudioListenerGraphNode>()) {
                node_desc.get<AudioListenerGraphNode>().update_intrinsic_values(
                    [&](size_t param_index, f32 intrinsic_value) {
                        update_intrinsic(node_index, param_index, intrinsic_value);
                    });
            }
        ...
        case GraphNodeType::Panner:
            if (node_desc.has<PannerGraphNode>()) {
                node_desc.get<PannerGraphNode>().update_intrinsic_values(
                    [&](size_t param_index, f32 intrinsic_value) {
                        update_intrinsic(node_index, param_index, intrinsic_value);
                    });
            }
        ...
        case GraphNodeType::StereoPanner:
            if (node_desc.has<StereoPannerGraphNode>()) {
                node_desc.get<StereoPannerGraphNode>().update_intrinsic_values(
                    [&](size_t param_index, f32 intrinsic_value) {
                        update_intrinsic(node_index, param_index, intrinsic_value);
                    });
            }
