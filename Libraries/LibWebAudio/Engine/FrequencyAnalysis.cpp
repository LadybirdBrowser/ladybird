...
#include <LibWebAudio/Engine/FrequencyAnalysis.h>
...
static Span<f32> ensure_blackman_window(FrequencyAnalysisScratch& scratch, size_t fft_size)
{
    ...
    for (size_t i = 0; i < fft_size; ++i) {
        ...
        window[i] = a0 - (a1 * AK::cos(2.0f * AK::Pi<f32> * n / N)) + (a2 * AK::cos(4.0f * AK::Pi<f32> * n / N));
    }
...
static void convert_fft_to_smoothed_db_in_place(ReadonlySpan<f32> real, ReadonlySpan<f32> imaginary,
    f32 smoothing_time_constant, Span<f32> previous_block,
    Span<f32> output_db)
{
    VERIFY(real.size() == imaginary.size());
...
void compute_frequency_data_db_in_place(ReadonlySpan<f32> time_domain_data, size_t fft_size,
    f32 smoothing_time_constant, Vector<f32>& previous_block,
    Vector<f32>& output_db, FrequencyAnalysisScratch& scratch)
{
    VERIFY(fft_size > 0);
    ...
    // 3. Apply a Fourier transform to the windowed time domain input data to get real and imaginary frequency
    // data.
    fft_in_place(real, imaginary);

    // 4. Smooth over time the frequency domain data.
    // 5. Convert to dB.
    convert_fft_to_smoothed_db_in_place(real, imaginary, smoothing_time_constant, previous_block.span(),
        output_db.span());
}
