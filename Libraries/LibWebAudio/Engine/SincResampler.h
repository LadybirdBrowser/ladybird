...
ALWAYS_INLINE ReadonlySpan<f32> sinc_resampler_phase_taps(SincResamplerKernel const& kernel,
    size_t phase_index)
{
    if (phase_index >= SincResamplerKernel::phase_count)
        phase_index = SincResamplerKernel::phase_count - 1;
    return kernel.coefficients.span().slice(phase_index * SincResamplerKernel::tap_count,
        SincResamplerKernel::tap_count);
}
...
ALWAYS_INLINE f32 sinc_resampler_interpolate_at(SincResamplerKernel const& kernel,
    f64 position_in_input_frames, size_t channel,
    SampleAt&& sample_at)
{
...
void sample_rate_converter_init(SampleRateConverter&, size_t channel_count,
    f64 input_frames_per_output_frame, size_t ring_size = 4096);

// Resets the resampler state. Allocation-free.
void sample_rate_converter_reset(SampleRateConverter&);
...
void sample_rate_converter_set_ratio(SampleRateConverter&, f64 input_frames_per_output_frame);
...
ResampleResult sample_rate_converter_process(SampleRateConverter&, Span<ReadonlySpan<f32>> input_channels,
    Span<Span<f32>> output_channels, bool flush = false);
...
private:
    friend void sample_rate_converter_init(SampleRateConverter&, size_t channel_count,
        f64 input_frames_per_output_frame, size_t ring_size);
    ...
    friend ResampleResult sample_rate_converter_process(SampleRateConverter&,
        Span<ReadonlySpan<f32>> input_channels,
        Span<Span<f32>> output_channels, bool flush);
