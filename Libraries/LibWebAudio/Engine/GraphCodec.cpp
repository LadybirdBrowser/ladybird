...
static ErrorOr<void> begin_section(Encoder& encoder, WireSectionTag tag, size_t& out_size_field_offset,
    size_t& out_payload_start)
{
...
static ErrorOr<void> append_node_payload(Encoder& encoder, GraphNodeDescription const& node, u32& flags)
{
    return node.visit([&](auto const& payload) -> ErrorOr<void> {
        ...
        if constexpr (AK::Detail::IsSame<T, ConvolverGraphNode>) {
            if (payload.buffer_id != ResourceID {})
                flags |= WireFlags::contains_external_resources;
        }
...
static ErrorOr<void>
append_buffer_table_section(Encoder& encoder, GraphDescription const& graph,
    Vector<u64> const& sorted_node_ids, GraphResources const& resources, u32& flags,
    Vector<SharedAudioBufferBinding>& shared_audio_buffer_bindings)
{
    Vector<ResourceID> buffer_ids;
    ...
    for (auto node_id_value : sorted_node_ids) {
        ...
        if (node.has<AudioBufferSourceGraphNode>()) {
            ResourceID buffer_id = node.get<AudioBufferSourceGraphNode>().buffer_id;
        ...
        if (node.has<ConvolverGraphNode>()) {
            ResourceID buffer_id = node.get<ConvolverGraphNode>().buffer_id;
            if (buffer_id != ResourceID {})
                buffer_ids.unchecked_append(buffer_id);
            continue;
        }
    ...
    Vector<ResourceID> present_buffer_ids;
    present_buffer_ids.ensure_capacity(buffer_ids.size());
    for (auto buffer_id : buffer_ids) {
        if (resources.audio_buffers.contains(buffer_id))
    ...
    for (auto buffer_id : present_buffer_ids) {
        auto it = resources.audio_buffers.find(buffer_id);
        if (it == resources.audio_buffers.end())
            continue;
        auto buffer = it->value;

        TRY(encoder.append_u64(static_cast<u64>(buffer_id)));

        TRY(shared_audio_buffer_bindings.try_append(SharedAudioBufferBinding {
            .buffer_id = buffer_id,
            .shared_memory = buffer->to_buffer(),
        }));
    }
...
ErrorOr<EncodedRenderGraph> encode_render_graph_for_media_server(GraphDescription const& graph,
    ...
    GraphResources const& resources)
{
    ...
    return EncodedRenderGraph {
        .encoded_graph = encoder.take(),
        .shared_audio_buffers = move(shared_audio_buffer_bindings),
    };
}
static ErrorOr<GraphNodeDescription> decode_node_payload(GraphNodeType type, ReadonlyBytes payload)
{
    ...
    case GraphNodeType::MediaStreamAudioDestination:
        return GraphNodeDescription { TRY(MediaStreamAudioDestinationGraphNode::decode_wire_payload(payload_decoder)) };
    ...
    Span<SharedAudioBufferBinding const> shared_audio_buffers)
{
    ...
    HashMap<ResourceID, Core::AnonymousBuffer> shared_audio_buffer_descriptor_map;
    TRY(shared_audio_buffer_descriptor_map.try_ensure_capacity(shared_audio_buffers.size()));
    for (auto const& shared_audio_buffer : shared_audio_buffers)
        shared_audio_buffer_descriptor_map.set(shared_audio_buffer.buffer_id, shared_audio_buffer.shared_memory);
    ...
    while (!decoder.at_end()) {
        ...
        case WireSectionTag::BufferTable: {
            ...
            for (u32 i = 0; i < buffer_count; ++i) {
                ResourceID buffer_id { TRY(section.read_u64()) };

                auto descriptor = shared_audio_buffer_descriptor_map.get(buffer_id);
                if (!descriptor.has_value())
                    return Error::from_string_literal("Wire: missing shared audio buffer descriptor");

                auto shared = TRY(SharedAudioBuffer::create_from_buffer(descriptor.release_value()));
                result.resources->audio_buffers.set(buffer_id, move(shared));
            }
            break;
        }
        default:
