...
{
#if !defined(AK_OS_WINDOWS)
    if (auto desktop_directory = get_environment_if_not_empty("XDG_DESKTOP_DIR"_sv); desktop_directory.has_value())
        return LexicalPath::canonicalized_path(*desktop_directory);
#endif

    StringBuilder builder;
    builder.append(home_directory());
    builder.append("/Desktop"_sv);
...
{
#if !defined(AK_OS_WINDOWS)
    if (auto documents_directory = get_environment_if_not_empty("XDG_DOCUMENTS_DIR"_sv); documents_directory.has_value())
        return LexicalPath::canonicalized_path(*documents_directory);
#endif

    StringBuilder builder;
    builder.append(home_directory());
    builder.append("/Documents"_sv);
    return LexicalPath::canonicalized_path(builder.to_byte_string());
}
...
{
#if !defined(AK_OS_WINDOWS)
    if (auto downloads_directory = get_environment_if_not_empty("XDG_DOWNLOAD_DIR"_sv); downloads_directory.has_value())
        return LexicalPath::canonicalized_path(*downloads_directory);
#endif

    StringBuilder builder;
    builder.append(home_directory());
    builder.append("/Downloads"_sv);
    return LexicalPath::canonicalized_path(builder.to_byte_string());
}
...
{
#if !defined(AK_OS_WINDOWS)
    if (auto music_directory = get_environment_if_not_empty("XDG_MUSIC_DIR"_sv); music_directory.has_value())
        return LexicalPath::canonicalized_path(*music_directory);
#endif

    StringBuilder builder;
    builder.append(home_directory());
    builder.append("/Music"_sv);
    return LexicalPath::canonicalized_path(builder.to_byte_string());
}
...
{
#if !defined(AK_OS_WINDOWS)
    if (auto pictures_directory = get_environment_if_not_empty("XDG_PICTURES_DIR"_sv); pictures_directory.has_value())
        return LexicalPath::canonicalized_path(*pictures_directory);
#endif

    StringBuilder builder;
    builder.append(home_directory());
    builder.append("/Pictures"_sv);
    return LexicalPath::canonicalized_path(builder.to_byte_string());
}
...
{
#if !defined(AK_OS_WINDOWS)
    if (auto videos_directory = get_environment_if_not_empty("XDG_VIDEOS_DIR"_sv); videos_directory.has_value())
    ...
    builder.append("/Movies"_sv);
#else
    builder.append("/Videos"_sv);
#endif
    return LexicalPath::canonicalized_path(builder.to_byte_string());
}
...
ByteString StandardPaths::config_directory()
{
    ...
    VERIFY_NOT_REACHED();
#endif
    if (auto config_directory = get_environment_if_not_empty("XDG_CONFIG_HOME"_sv); config_directory.has_value())
        return LexicalPath::canonicalized_path(*config_directory);

    StringBuilder builder;
    builder.append(home_directory());
#if defined(AK_OS_MACOS)
    builder.append("/Library/Preferences"_sv);
#elif defined(AK_OS_HAIKU)
    builder.append("/config/settings"_sv);
#else
    builder.append("/.config"_sv);
#endif
    return LexicalPath::canonicalized_path(builder.to_byte_string());
}
...
{
#ifdef AK_OS_WINDOWS
    return ByteString::formatted("{}/Ladybird"_sv, getenv("LOCALAPPDATA"));
#endif
    if (auto data_directory = get_environment_if_not_empty("XDG_DATA_HOME"_sv); data_directory.has_value())
    ...
    builder.append("/Library/Application Support"_sv);
#elif defined(AK_OS_HAIKU)
    builder.append("/config/non-packaged/data"_sv);
#else
    builder.append("/.local/share"_sv);
#endif
...
Vector<ByteString> StandardPaths::system_data_directories()
{
    ...
    VERIFY_NOT_REACHED();
#endif
    auto data_directories = get_environment_if_not_empty("XDG_DATA_DIRS"_sv).value_or("/usr/local/share:/usr/share"_sv);
...
{
#if !defined(AK_OS_WINDOWS)
    if (auto data_directory = get_environment_if_not_empty("XDG_RUNTIME_DIR"_sv); data_directory.has_value())
        return LexicalPath::canonicalized_path(*data_directory);
#endif
    ...
    builder.append("/Library/Application Support"_sv);
#elif defined(AK_OS_HAIKU)
    builder.append("/boot/system/var/shared_memory"_sv);
