...
struct EventLoopNotifier final : CompletionPacket {
    ...
    // These are a space tradeoff for avoiding a double indirection through the notifier*. Is it better to do that?
    Notifier* notifier;
...
struct ThreadData {
    ...
    {
        // Consider a way to have different event loops have a different number of threads
        iocp.handle = CreateIoCompletionPort(INVALID_HANDLE_VALUE, NULL, 0, 1);
        wake_event.handle = CreateEvent(NULL, FALSE, FALSE, NULL);
        NTSTATUS status = ntdll.NtCreateWaitCompletionPacket(&wake_packet.handle, GENERIC_ALL, NULL);
        VERIFY(iocp.handle);
...
size_t EventLoopImplementationWindows::pump(PumpMode)
{
    ...
    // but we don't want to increase the cost of each pump unnecessarily.
    ...
    if (success) {
        for (ULONG i = 0; i < entries_removed; i++) {
            ...
            // If this is too expensive we could remove it
            VERIFY_NOT_REACHED();
        }
    } else {
        ...
        case WAIT_IO_COMPLETION:
            // We don't do any apc calls so trap
            VERIFY_NOT_REACHED();
        default:
...
void EventLoopManagerWindows::register_notifier(Notifier& notifier)
{
    ...
    NTSTATUS status = NtCreateWaitCompletionPacket(&notifier_data->wait_packet.handle, GENERIC_ALL, NULL);
    VERIFY(NT_SUCCESS(status));
