...
RWLock::~RWLock()
{
    int result = pthread_rwlock_destroy(reinterpret_cast<pthread_rwlock_t*>(m_storage));
    if (result != 0) {
...
{
    int result = pthread_rwlock_tryrdlock(reinterpret_cast<pthread_rwlock_t*>(m_storage));
    if (result == 0)
        return true;
    if (result == EBUSY)
        return false;
...
{
    int result = pthread_rwlock_trywrlock(reinterpret_cast<pthread_rwlock_t*>(m_storage));
    if (result == 0)
        return true;
    if (result == EBUSY)
        return false;
...
{
    int result = pthread_rwlock_rdlock(reinterpret_cast<pthread_rwlock_t*>(m_storage));
    if (result != 0) {
...
{
    int result = pthread_rwlock_wrlock(reinterpret_cast<pthread_rwlock_t*>(m_storage));
    if (result != 0) {
...
{
    int result = pthread_rwlock_unlock(reinterpret_cast<pthread_rwlock_t*>(m_storage));
    if (result != 0) {
        warnln("pthread_rwlock_unlock failed with: {}", Error::from_errno(result));
        VERIFY_NOT_REACHED();
    }
