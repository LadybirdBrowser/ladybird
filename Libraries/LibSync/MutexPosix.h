...
{
    int result = pthread_mutex_destroy(reinterpret_cast<pthread_mutex_t*>(m_storage));
    if (result != 0) {
...
template<typename R, typename I>
ALWAYS_INLINE bool MutexBase<R, I>::try_lock()
{
    int result = pthread_mutex_trylock(reinterpret_cast<pthread_mutex_t*>(m_storage));
    if (result == 0)
        return true;
    if (result == EBUSY)
        return false;
...
template<typename R, typename I>
ALWAYS_INLINE void MutexBase<R, I>::lock()
{
    int result = pthread_mutex_lock(reinterpret_cast<pthread_mutex_t*>(m_storage));
    if (result != 0) {
        warnln("pthread_mutex_lock failed with: {}", Error::from_errno(result));
        VERIFY_NOT_REACHED();
    }
...
template<typename R, typename I>
ALWAYS_INLINE void MutexBase<R, I>::unlock()
{
    int result = pthread_mutex_unlock(reinterpret_cast<pthread_mutex_t*>(m_storage));
    if (result != 0) {
...
ALWAYS_INLINE MutexBase<RecursivePolicy, InterprocessPolicy>::MutexBase()
{
