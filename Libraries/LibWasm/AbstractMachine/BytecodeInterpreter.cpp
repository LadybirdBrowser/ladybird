    ...
    {                                                         \
        ...
        break;                                                \
            ...
            case Instructions::block.value(): {
                __builtin_prefetch(&configuration.label_stack(), 1, 1);
                size_t arity = 0;
...
CompiledInstructions try_compile_instructions(Expression const& expression, Span<FunctionType const>)
{
    ...
    for (auto& instruction : expression.instructions()) {
        ...
        case InsnPatternState::GetLocal:
            if (instruction.opcode() == Instructions::local_get) {
                ...
                result.dispatches[result.dispatches.size() - 1] = { &nop, { .sources = { Dispatch::Stack, Dispatch::Stack, Dispatch::Stack }, .destination = Dispatch::Stack } };
        ...
        case InsnPatternState::GetLocalx2:
            if (instruction.opcode() == Instructions::i32_add) {
                ...
                result.dispatches[result.dispatches.size() - 1] = { &nop, { .sources = { Dispatch::Stack, Dispatch::Stack, Dispatch::Stack }, .destination = Dispatch::Stack } };
                result.dispatches[result.dispatches.size() - 2] = { &nop, { .sources = { Dispatch::Stack, Dispatch::Stack, Dispatch::Stack }, .destination = Dispatch::Stack } };
                result.extra_instruction_storage.append(Instruction {
        ...
        result.dispatches.append({ &instruction, { .sources = { Dispatch::Stack, Dispatch::Stack, Dispatch::Stack }, .destination = Dispatch::Stack } });
    }
    ...
    Vector<size_t> forced_stack_values;

    Vector<size_t> parent;
    Vector<size_t> rank;
    Vector<size_t> final_roots;

    auto ensure_id_space = [&](size_t id) {
        if (id >= parent.size()) {
            size_t old_size = parent.size();
            parent.resize(id + 1);
            rank.resize(id + 1);
            final_roots.resize(id + 1);
    ...
    Function<size_t(size_t)> find_root = [&](size_t x) -> size_t {
        if (parent[x] != x)
            parent[x] = find_root(parent[x]);
        return parent[x];
    };

    auto union_alias = [&](size_t a, size_t b) {
    ...
    HashTable<size_t> stack_forced_roots;

    Vector<Vector<size_t>> live_at_instr;
...
#undef M
        ...
        auto output_id = NumericLimits<size_t>::max();
        for (size_t j = 0; j < outputs; ++j) {
            size_t id = next_value_id++;
        ...
        if (outputs > 0) {
            ...
            if (maybe_input_ids.has_value() && !maybe_input_ids->is_empty()) {
                size_t last_input_id = maybe_input_ids->last();
                union_alias(output_id, last_input_id);

                size_t alias_root = find_root(last_input_id);

                for (size_t j = 0; j < maybe_input_ids->size() - 1; ++j) {
                    size_t input_root = find_root((*maybe_input_ids)[j]);
    ...
    for (auto const& it : values) {
        ...
        size_t start = value.definition_index;
        size_t end = max(start, value.last_use);
        intervals.append({ value.id, start, end });
    }

    for (auto id : forced_stack_values)
        stack_forced_roots.set(final_roots[id]);
    for (auto& interval : intervals)
        interval.forced_to_stack = stack_forced_roots.contains(final_roots[interval.value_id]);
    ...
    auto expire_old_intervals = [&](size_t current_start) {
        while (true) {
            auto it = active_by_end.find_smallest_not_below_iterator(current_start);
            if (it.is_end())
                break;
    ...
    HashMap<size_t, Vector<LiveInterval*>> alias_groups;
    for (auto& interval : intervals) {
        size_t root = final_roots[interval.value_id];
    ...
    for (auto& entry : alias_groups) {
        auto& group = entry.value;
        size_t group_start = NumericLimits<size_t>::max();
        size_t group_end = 0;
        ...
        bool group_forced_to_stack = false;
        for (auto* interval : group) {
        ...
        if (!group_forced_to_stack) {
            ...
            for (u8 r = 0; r < Dispatch::CountRegisters; ++r) {
                ...
                for (auto* interval : group) {
                    auto interval_root = final_roots[interval->value_id];

                    for (auto const* other_interval : reg_intervals[r]) {
                        if (interval_root == final_roots[other_interval->value_id])
                            continue;
                ...
                if (can_assign) {
                    ...
                    active_by_end.insert(group_end, { entry.key, group_end, reg });
    ...
    for (size_t i = 0; i < result.dispatches.size(); ++i) {
        ...
        auto output_id = instr_to_output_value.get(i).value_or(NumericLimits<size_t>::max());
        if (output_id != NumericLimits<size_t>::max())
