...
static void* g_single_byte_handlers[256 + Instructions::SyntheticInstructionCount];

template<size_t N>
static auto make_single_byte_handlers(Array<Tuple<size_t, void*>, N> values)
{
    Array<void*, 256 + Instructions::SyntheticInstructionCount> result;
    for (auto entry : values)
        result[entry.template get<0>()] = entry.template get<1>();
    return result;
}
void BytecodeInterpreter::interpret(Configuration& configuration)
{
    ...
    u64 max_ip_value = expression.compiled_instructions.dispatches.is_empty() ? instructions.size() : expression.compiled_instructions.dispatches.size();
...
#if 0
    static Array<u64, 256 + Instructions::SyntheticInstructionCount> instruction_counts { 0 };
    static Array<u64, 256 + Instructions::SyntheticInstructionCount> instruction_timings { 0 };
    static size_t add2locals_count = 0;
    static size_t addconstlocal_count = 0;
    static ScopeGuard instruction_counts_guard = [&] {
        size_t totals = 0;
        for (size_t i = 0; i < instruction_counts.size(); ++i) {
            totals += instruction_counts[i];
            if (instruction_counts[i] > 0)
                dbgln("Instruction {} executed {} times, for a total of {}ms", instruction_name(OpCode { i > 255 ? Instructions::SyntheticInstructionBase.value() + i - 256 : i }), instruction_counts[i], instruction_timings[i] / 1000000);
        }
        dbgln("Add2locals executed {} times ({} of {} insns)", add2locals_count, add2locals_count * 3, totals);
        dbgln("Addconstlocal executed {} times ({} of {} insns)", addconstlocal_count, addconstlocal_count * 3, totals);
    };

#    if 0
    fprintf(stderr, "INSN: BREAK\n");
#    endif

    OpCode bits[3] { Instructions::nop, Instructions::nop, Instructions::nop };
    u64 bitidx { 0 };
    u64 time_start = 0;
#endif
    auto note = [&](OpCode opcode) {
        (void)opcode;
#if 0
#    if 0
        fprintf(stderr, "INSN: %s %zu\n", instruction_name(opcode).characters(), configuration.value_stack().size());
#    endif

        if (opcode.value() <= 255)
            instruction_counts[opcode.value()]++;
        else
            instruction_counts[opcode.value() - Instructions::SyntheticInstructionBase.value() + 256]++;
        bits[(bitidx++) % 3] = opcode;

        auto at = [&](auto index) { return bits[(bitidx + index - 3) % 3]; };

        if (at(0) == Instructions::local_get && at(1) == Instructions::local_get && at(2) == Instructions::i32_add) {
            add2locals_count++;
        } else if (at(0) == Instructions::local_get && at(1) == Instructions::i32_const && at(2) == Instructions::i32_add) {
            addconstlocal_count++;
        } else if (at(0) == Instructions::i32_const && at(1) == Instructions::local_get && at(2) == Instructions::i32_add) {
            addconstlocal_count++;
        }

        time_start = UnixDateTime::now().nanoseconds_since_epoch();
#endif
    };

    auto done = [&](OpCode opcode) {
        (void)opcode;
#if 0
        auto end = UnixDateTime::now().nanoseconds_since_epoch();
        if (opcode.value() <= 255)
            instruction_timings[opcode.value()] += end - time_start;
        else
            instruction_timings[opcode.value() - Instructions::SyntheticInstructionBase.value() + 256] += end - time_start;
#endif
    };

    static auto single_byte_handlers = make_single_byte_handlers(Array {
#define M(opcode, value, ...) Tuple<size_t, void*> { value, &&handle_##opcode },
        ENUMERATE_SINGLE_BYTE_WASM_OPCODES(M)
#undef M
#define M(opcode, _value, ...) Tuple<size_t, void*> { _value - Instructions::SyntheticInstructionBase.value() + 256, &&handle_##opcode },
            ENUMERATE_SYNTHETIC_INSTRUCTION_OPCODES(M)
#undef M
    });

    [[maybe_unused]] static bool single_byte_handlers_initialized = [] {
        for (size_t i = 0; i < 256 + Instructions::SyntheticInstructionCount; ++i)
            g_single_byte_handlers[i] = single_byte_handlers[i];
        return true;
    }();
...
#undef M
    ...
    auto const has_compiled_list = !expression.compiled_instructions.dispatches.is_empty();

    while (current_ip_value < max_ip_value) {
        if (should_limit_instruction_count) [[unlikely]] {
        ...
        {
            Dispatch const* c = nullptr;
#define DISPATCH(no_ip_change)                                         \
    done(instruction->opcode());                                       \
    if (has_compiled_list) {                                           \
        if constexpr (no_ip_change)                                    \
            ++c;                                                       \
        else                                                           \
            c = &cc[current_ip_value];                                 \
        instruction = c->instruction;                                  \
        memcpy(configuration.sources, c->sources, sizeof(c->sources)); \
        configuration.destination = c->destination;                    \
        note(instruction->opcode());                                   \
        goto * c->label;                                               \
    } else {                                                           \
        instruction = &instructions.data()[old_ip];                    \
        note(instruction->opcode());                                   \
...
#define RUN_NEXT_INSTRUCTION(could_have_changed_ip)                                                                                                                                \
    do {                                                                                                                                                                           \
        ...
        if (current_ip_value >= max_ip_value || (should_limit_instruction_count && ++executed_instructions >= Constants::max_allowed_executed_instructions_per_call)) [[unlikely]] \
            goto continue_interpretation;                                                                                                                                          \
        old_ip = current_ip_value;                                                                                                                                                 \
        DISPATCH(could_have_changed_ip == CouldHaveChangedIP::No);                                                                                                                 \
    } while (0)
        ...
        start:
            ...
            switch (opcode) {
            handle_synthetic_i32_add2local:
            case Instructions::synthetic_i32_add2local.value():
                configuration.push_to_destination(Value(Operators::Add {}(configuration.frame().locals().data()[instruction->local_index().value()].to<i32>(), configuration.frame().locals().data()[instruction->arguments().get<LocalIndex>().value()].to<i32>())));
                RUN_NEXT_INSTRUCTION(CouldHaveChangedIP::No);
            handle_synthetic_i32_addconstlocal:
            case Instructions::synthetic_i32_addconstlocal.value():
                configuration.push_to_destination(Value(Operators::Add {}(configuration.frame().locals().data()[instruction->local_index().value()].to<i32>(), instruction->arguments().get<i32>())));
                RUN_NEXT_INSTRUCTION(CouldHaveChangedIP::No);
            handle_synthetic_i32_andconstlocal:
            case Instructions::synthetic_i32_andconstlocal.value():
                configuration.push_to_destination(Value(Operators::BitAnd {}(configuration.frame().locals().data()[instruction->local_index().value()].to<i32>(), instruction->arguments().get<i32>())));
                RUN_NEXT_INSTRUCTION(CouldHaveChangedIP::No);
            handle_synthetic_i32_storelocal:
            case Instructions::synthetic_i32_storelocal.value():
                if (store_value(configuration, *instruction, ConvertToRaw<i32> {}(configuration.frame().locals().data()[instruction->local_index().value()].to<i32>()), 0))
                    return;
                RUN_NEXT_INSTRUCTION(CouldHaveChangedIP::No);
            handle_synthetic_i64_storelocal:
            case Instructions::synthetic_i64_storelocal.value():
                if (store_value(configuration, *instruction, ConvertToRaw<i64> {}(configuration.frame().locals().data()[instruction->local_index().value()].to<i64>()), 0))
                    return;
                RUN_NEXT_INSTRUCTION(CouldHaveChangedIP::No);
            handle_synthetic_local_seti32_const:
            case Instructions::synthetic_local_seti32_const.value():
                configuration.frame().locals().data()[instruction->local_index().value()] = Value(instruction->arguments().get<i32>());
                RUN_NEXT_INSTRUCTION(CouldHaveChangedIP::No);
            handle_unreachable:
            ...
            case Instructions::local_get.value():
                ...
                configuration.push_to_destination(Value(configuration.frame().locals().data()[instruction->local_index().value()]));
                RUN_NEXT_INSTRUCTION(CouldHaveChangedIP::No);
            handle_local_set:
            ...
            case Instructions::table_get.value(): {
                // bounds checked by verifier.
                auto index = (size_t)(configuration.take_source(0).to<i32>());
                auto table_index = instruction->arguments().get<TableIndex>();
                auto address = configuration.frame().module().tables()[table_index.value()];
                auto table = configuration.store().get(address);
                TRAP_IN_LOOP_IF_NOT(index < table->elements().size());
                configuration.push_to_destination(Value(table->elements()[index]));
                RUN_NEXT_INSTRUCTION(CouldHaveChangedIP::No);
            }
            handle_table_grow:
...
{
    // bounds checked by verifier.
    return bit_cast<VectorType>(configuration.take_source(0).to<u128>());
}
...
bool BytecodeInterpreter::interpret_instruction_chain(Configuration& configuration, u64& ip, u64& ip_at_start, Span<Instruction const> instructions, u64 max_ip_value, u64& executed_instructions)
{
    (void)configuration;
    (void)ip_at_start;
    (void)instructions;
...
CompiledInstructions try_compile_instructions(Expression const& expression)
{
    [[maybe_unused]] static bool s_did_prepare_labels = [] {
        StackInfo info;
        BytecodeInterpreter interpreter(info);
        Store store;
        Configuration nothing(store);
        ModuleInstance empty_module { {}, {}, {}, {}, {}, {}, {} };
        Expression empty_expression { {} };
        nothing.set_frame(Frame {
            empty_module,
            {},
            empty_expression,
            0,
    ...
    for (auto& instruction : expression.instructions()) {
        ...
        case InsnPatternState::GetLocal:
            if (instruction.opcode() == Instructions::local_get) {
                ...
                result.dispatches[result.dispatches.size() - 1] = { &nop, g_single_byte_handlers[Instructions::nop.value()] };
                ...
                result.dispatches.append({
                    &result.extra_instruction_storage.unsafe_last(),
                    g_single_byte_handlers[256 + Instructions::synthetic_i32_storelocal.value() - Instructions::SyntheticInstructionBase.value()],
                });
                pattern_state = InsnPatternState::Nothing;
                continue;
                ...
                result.dispatches.append({
                    &result.extra_instruction_storage.unsafe_last(),
                    g_single_byte_handlers[256 + Instructions::synthetic_i64_storelocal.value() - Instructions::SyntheticInstructionBase.value()],
                });
                pattern_state = InsnPatternState::Nothing;
                continue;
            } else {
        ...
        case InsnPatternState::GetLocalx2:
            if (instruction.opcode() == Instructions::i32_add) {
                ...
                result.dispatches[result.dispatches.size() - 1] = { &nop, g_single_byte_handlers[Instructions::nop.value()] };
                result.dispatches[result.dispatches.size() - 2] = { &nop, g_single_byte_handlers[Instructions::nop.value()] };
                result.extra_instruction_storage.append(Instruction {
                ...
                result.dispatches.append({
                    &result.extra_instruction_storage.unsafe_last(),
                    g_single_byte_handlers[256 + Instructions::synthetic_i32_add2local.value() - Instructions::SyntheticInstructionBase.value()],
                });
                pattern_state = InsnPatternState::Nothing;
                continue;
            }
        ...
        case InsnPatternState::I32Const:
            if (instruction.opcode() == Instructions::local_get) {
                ...
                result.dispatches.append({
                    &result.extra_instruction_storage.unsafe_last(),
                    g_single_byte_handlers[256 + Instructions::synthetic_local_seti32_const.value() - Instructions::SyntheticInstructionBase.value()],
                });
                pattern_state = InsnPatternState::Nothing;
                continue;
            } else {
        ...
        case InsnPatternState::I32ConstGetLocal:
            if (instruction.opcode() == Instructions::i32_const) {
                ...
                result.dispatches.append({
                    &result.extra_instruction_storage.unsafe_last(),
                    g_single_byte_handlers[256 + Instructions::synthetic_i32_addconstlocal.value() - Instructions::SyntheticInstructionBase.value()],
                });
                pattern_state = InsnPatternState::Nothing;
                continue;
            }
            if (instruction.opcode() == Instructions::i32_and) {
                ...
                result.dispatches.append({
                    &result.extra_instruction_storage.unsafe_last(),
                    g_single_byte_handlers[256 + Instructions::synthetic_i32_andconstlocal.value() - Instructions::SyntheticInstructionBase.value()],
                });
                pattern_state = InsnPatternState::Nothing;
                continue;
            }
        ...
        result.dispatches.append({ &instruction, g_single_byte_handlers[instruction.opcode().value()] });
    }
    ...
    nops_to_remove.reverse();
    auto nops_to_remove_copy = nops_to_remove;
    size_t offset_accumulated = 0;
    for (size_t i = 0; i < result.dispatches.size(); ++i) {
        if (result.dispatches[i].instruction->opcode() == Instructions::nop) {
            offset_accumulated++;
            nops_to_remove.take_last();
            continue;
        }
        ...
        if (auto ptr = args.get_pointer<Instruction::StructuredInstructionArgs>()) {
            auto offset_to = [&](InstructionPointer ip) {
                size_t offset = 0;
                for (auto nop_ip : nops_to_remove.in_reverse()) {
                    if (nop_ip < ip.value())
                        ++offset;
                    else
                        break;
    ...
    for (auto index : nops_to_remove_copy)
        result.dispatches.remove(index);

    struct Value {
        size_t id;
        size_t def_instr_index;
        Vector<size_t> uses;
    };

    HashMap<size_t, Value> values;
    Vector<size_t> value_stack;
    size_t next_value_id = 0;
    HashMap<size_t, size_t> instr_to_output_value;
    HashMap<size_t, Vector<size_t>> instr_to_input_values;
    HashMap<size_t, Vector<size_t>> instr_to_dependent_values;
    ...
    case Instructions::name.value():                    \
        if constexpr (ins == -1 || outs == -1) {        \
            for (auto val : value_stack) {              \
                values.get(val).value().uses.append(i); \
...
#undef M
        ...
        Vector<size_t> input_ids;
        for (size_t j = 0; j < inputs; ++j) {
            size_t input_value = value_stack.take_last();
            input_ids.append(input_value);
            dependent_ids.append(input_value);
            values.get(input_value).value().uses.append(i);
        }
        instr_to_input_values.set(i, move(input_ids));
        instr_to_dependent_values.set(i, move(dependent_ids));

        for (size_t j = 0; j < outputs; ++j) {
            size_t val_id = next_value_id++;
            values.set(val_id, Value { val_id, i, {} });
            value_stack.append(val_id);
            instr_to_output_value.set(i, val_id);
        }
    }
    ...
    HashMap<size_t, size_t> alias_group_map;
    Vector<AliasGroup> alias_groups;

    auto find_or_create_group = [&](size_t v) -> size_t {
        if (alias_group_map.contains(v))
            return alias_group_map.get(v).value();
        size_t id = alias_groups.size();
        alias_groups.empend();
        alias_groups.last().members.append(v);
        alias_group_map.set(v, id);
        return id;
    };

    for (size_t i = 0; i < result.dispatches.size(); ++i) {
        auto input_ids = instr_to_input_values.get(i).value_or({});
        auto dependent_ids = instr_to_dependent_values.get(i).value_or({});
        auto output_val = instr_to_output_value.get(i).value_or(NumericLimits<size_t>::max());

        if (!input_ids.is_empty() && output_val != NumericLimits<size_t>::max()) {
            size_t last_src = input_ids.last();
            auto& uses = values.get(last_src).value().uses;
            if (!uses.is_empty() && uses.last() == i) {
                size_t g1 = find_or_create_group(last_src);
                size_t g2 = find_or_create_group(output_val);
                if (g1 != g2) {
                    for (auto v : alias_groups[g2].members) {
                        alias_group_map.set(v, g1);
                        alias_groups[g1].members.append(v);
                    }
                    alias_groups[g2].members.clear();
                }
            }
        }

        if (dependent_ids.size() > input_ids.size()) {
            for (auto v : dependent_ids)
                alias_groups[find_or_create_group(v)].forced_to_stack = true;
            if (output_val != NumericLimits<size_t>::max())
                alias_groups[find_or_create_group(output_val)].forced_to_stack = true;
        }
    }

    // Pretend there's a return at the end to make sure we're not left with a value in a register.
    for (auto v : value_stack) {
        auto group_id = find_or_create_group(v);
        alias_groups[group_id].forced_to_stack = true;
    }

    HashMap<size_t, Dispatch::RegisterOrStack> value_alloc;
    size_t next_reg = 0;
    for (auto& group : alias_groups) {
        auto loc = Dispatch::RegisterOrStack::Stack;
        if (!group.forced_to_stack && next_reg < 3) {
            loc = static_cast<Dispatch::RegisterOrStack>(next_reg);
            ++next_reg;
        }
        for (auto v : group.members)
            value_alloc.set(v, loc);
    }
    ...
    for (size_t i = 0; i < result.dispatches.size(); ++i) {
        ...
        auto input_ids = instr_to_input_values.get(i).value_or({});
        u8 live_mask = 0;
        Vector<AK::Tuple<u8, u8>> live_order;

        for (auto const& it : value_alloc) {
            auto value_id = it.key;
            auto reg = it.value;
            if (reg == Dispatch::RegisterOrStack::Stack)
                continue;

            auto const& value = values.get(value_id).value();
            bool is_live = value.def_instr_index < i;
            ...
            if (is_live)
                live_mask |= (1 << static_cast<u8>(reg));
        }
        ...
        auto output_val = instr_to_output_value.get(i).value_or(NumericLimits<size_t>::max());
        if (output_val != NumericLimits<size_t>::max()) {
            dispatch.destination = value_alloc.get(output_val).value_or(Dispatch::RegisterOrStack::Stack);
        }

        for (u8 r = 0; r < 3; ++r) {
            if (live_mask & (1 << r))
                live_order.append(AK::Tuple<u8, u8>(r, static_cast<u8>(i)));
        }
        Dispatch::LivenessOrder order = Dispatch::LivenessOrder::_012;
        if (live_order.size() == 2) {
            auto a = live_order[0].get<0>();
            auto b = live_order[1].get<0>();
            if (a == 0 && b == 1)
                order = Dispatch::LivenessOrder::_012;
            else if (a == 0 && b == 2)
                order = Dispatch::LivenessOrder::_021;
            else if (a == 1 && b == 0)
                order = Dispatch::LivenessOrder::_102;
            else if (a == 1 && b == 2)
                order = Dispatch::LivenessOrder::_120;
            else if (a == 2 && b == 0)
                order = Dispatch::LivenessOrder::_201;
            else if (a == 2 && b == 1)
                order = Dispatch::LivenessOrder::_210;
        }
        dispatch.register_liveness_record = {
            .order = order,
            .is_live = live_mask
        };
    }

    //     dbgln("Compiled {} insns to {} dispatches, {} nops", expression.instructions().size(), result.dispatches.size(), offset_accumulated);
    //     for (auto& dispatch : result.dispatches) {
    //         auto label_index = ReadonlySpan<void*>(g_single_byte_handlers).index_of({ &dispatch.label, 1 }).value_or(258);
    //         auto label_text = label_index < 255 ? instruction_name(OpCode(label_index)) : "<synthetic>";
    //         ByteString regs;
    //         auto first = true;
    //         ssize_t in_count = 0;
    //         bool has_out = false;
    // #define M(name, _, ins, outs) case Instructions::name.value(): in_count = ins; has_out = outs != 0; break;
    ...
    //                 if (first)
    //                     regs = ByteString::formatted("{} ({}", regs, to_underlying(dispatch.sources[index]));
    //                 else
    //                     regs = ByteString::formatted("{}, {}", regs, to_underlying(dispatch.sources[index]));
    //                 first = false;
    //             }
    //             if (has_out) {
    //                 if (first)
    //                     regs = ByteString::formatted(" () -> {}", to_underlying(dispatch.destination));
    //                 else
    //                     regs = ByteString::formatted("{}) -> {}", regs, to_underlying(dispatch.destination));
    //             } else {
    //                 if (first)
    ...
    //             }
    //         }
    //         dbgln("  {} -> {}@0x{:x}{}", instruction_name(dispatch.instruction->opcode()), label_text, dispatch.label, regs);
    //         if (auto args = dispatch.instruction->arguments().get_pointer<Instruction::StructuredInstructionArgs>()) {
    //             dbgln("     end: {}", instruction_name(result.dispatches[args->end_ip.value() - args->else_ip.has_value()].instruction->opcode()));
    //             if (args->else_ip.has_value())
    //                 dbgln("   else: {}", instruction_name(result.dispatches[args->else_ip->value() - 1].instruction->opcode()));
    //         }
    //     }
