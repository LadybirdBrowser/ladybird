...
Optional<MemoryCache::Entry const&> MemoryCache::open_entry(URL::URL const& url, StringView method, HeaderList const& request_headers, CacheMode cache_mode)
{
    ...
    auto lifetime_status = cache_lifetime_status(request_headers, cache_entry->response_headers, freshness_lifetime, current_age);
    switch (lifetime_status) {
    ...
    case CacheLifetimeStatus::StaleWhileRevalidate:
        if (lifetime_status != CacheLifetimeStatus::MustRevalidate && cache_mode_permits_stale_responses(cache_mode)) {
