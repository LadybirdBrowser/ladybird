...
ErrorOr<VulkanContext> create_vulkan_context()
{
    ...
    PFN_vkGetMemoryFdKHR pfnVkGetMemoryFdKHR = (PFN_vkGetMemoryFdKHR)vkGetDeviceProcAddr(logical_device, "vkGetMemoryFdKHR");
    if (pfnVkGetMemoryFdKHR == nullptr) {
        return Error::from_string_literal("vkGetMemoryFdKHR unavailable");
    }
    PFN_vkGetImageDrmFormatModifierPropertiesEXT pfnVkGetImageDrmFormatModifierPropertiesEXT = (PFN_vkGetImageDrmFormatModifierPropertiesEXT)vkGetDeviceProcAddr(logical_device, "vkGetImageDrmFormatModifierPropertiesEXT");
    if (pfnVkGetImageDrmFormatModifierPropertiesEXT == nullptr) {
    ...
    return VulkanContext {
        ...
        .ext_procs = {
            .GetMemoryFdKHR = pfnVkGetMemoryFdKHR,
            .GetImageDrmFormatModifierPropertiesEXT = pfnVkGetImageDrmFormatModifierPropertiesEXT,
        },
    };
}
...
void VulkanImage::transition_layout(VkImageLayout oldLayout, VkImageLayout newLayout)
{
...
int VulkanImage::get_dma_buf_fd()
{
    ...
    int fd = -1;
    VkResult result = context.ext_procs.GetMemoryFdKHR(context.logical_device, &get_fd_info, &fd);
    if (result != VK_SUCCESS) {
...
ErrorOr<NonnullRefPtr<VulkanImage>> create_shared_vulkan_image(VulkanContext const& context, uint32_t width, uint32_t height, VkFormat format, uint32_t num_modifiers, uint64_t const* modifiers)
{
    ...
    result = context.ext_procs.GetImageDrmFormatModifierPropertiesEXT(context.logical_device, image->image, &image_format_mod_props);
    if (result != VK_SUCCESS) {
