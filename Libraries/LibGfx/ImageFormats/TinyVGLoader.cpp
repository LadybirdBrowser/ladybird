...
void TinyVGDecodedImageData::draw(Painter& painter) const
{
    for (auto const& command : draw_commands()) {
        ...
        if (command.fill.has_value()) {
            draw_path.close_all_subpaths();
            command.fill->visit(
                [&](Color color) { painter.fill_path(draw_path, color, WindingRule::EvenOdd); },
                [&](NonnullRefPtr<SVGGradientPaintStyle> const& style) {
                    painter.fill_path(draw_path, style, 1.0f, WindingRule::EvenOdd);
                });
        }
