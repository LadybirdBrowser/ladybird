...
static ErrorOr<void> decode_frame(GIFLoadingContext& context, size_t frame_index)
{
    ...
    for (size_t i = start_frame; i <= frame_index; ++i) {
        ...
        if (!image->lzw_encoded_bytes.is_empty()) {
            auto stream = make<FixedMemoryStream>(image->lzw_encoded_bytes.bytes());
            decoded_stream = TRY(Compress::LzwDecompressor<LittleEndianInputBitStream>::decompress_all(move(stream), image->lzw_min_code_size));
        }
