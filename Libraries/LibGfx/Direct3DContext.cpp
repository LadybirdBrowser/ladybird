...
// FIXME: Use the modern ComPtr equivalent from the Windows Runtime C++ Template Library (WRL)
...
ErrorOr<NonnullOwnPtr<Direct3DContext>> Direct3DContext::try_create()
{
    ...
    ComPtr<ID3D12Device> d12_device;
    if (HRESULT const hr = D3D12CreateDevice(adapter.Get(), D3D_FEATURE_LEVEL_12_0, IID_PPV_ARGS(&d12_device)); FAILED(hr)) {
        return Error::from_windows_error(hr);
    }
