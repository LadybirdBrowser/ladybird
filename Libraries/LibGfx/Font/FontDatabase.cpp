...
RefPtr<Gfx::Font> FontDatabase::get(FlyString const& family, float point_size, unsigned weight, unsigned width, unsigned slope, Optional<FontVariationSettings> const& font_variation_settings)
{
    return m_system_font_provider->get_font(family, point_size, weight, width, slope, font_variation_settings);
}
...
RefPtr<Gfx::Font> FontDatabase::get_font_for_code_point(u32 code_point, float point_size, u16 weight, u16 width, u8 slope)
{
    ...
    // FIXME: Does it matter that we don't pass a FontVariationSettings here?
    if (entry.typeface)
