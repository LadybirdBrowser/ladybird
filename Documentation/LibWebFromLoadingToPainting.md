...
- The final values for uninitialized `reversed` counters cannot be calculated in one pass, since they depend on the number of `counter-increment` occurrences.
  An originating value for a `reversed` counter is updated on each occurrence of the counter by following the [algorithm for instantiating counters](https://drafts.csswg.org/css-lists-3/#instantiating-counters).
  The final value of each occurrence of the `reversed` counter is fixed-up by adding the originating counter value.
