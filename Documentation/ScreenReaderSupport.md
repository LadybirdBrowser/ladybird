...
Once the UI has requested the tree, WebContent pushes a rebuilt tree via `did_get_accessibility_tree` whenever the DOM changes, and after every accessibility action (press, focus) — since an action can change what the tree reports without mutating the DOM. `PageClient::schedule_accessibility_tree_update()` debounces the rebuilds with a 200ms timer.

Until that first request, WebContent reports nothing: `Page::accessibility_interested()` gates the tree pushes and the focus-change reports alike, and only a tree request sets it. And the UIs request the tree only once an assistive technology is present — so users without one pay for neither, just as in Gecko, WebKit, and Blink, none of which report a focus change before an assistive technology turns accessibility on.
...
- `on_loading_state_change` (load finished) → `request_accessibility_tree()`, but only while an assistive technology is present: VoiceOver is on (`NSWorkspace.isVoiceOverEnabled`), or an assistive technology has already queried web content in this process. Without one, the view never requests the tree — so WebContent never sends it tree updates or focus changes (see _Live DOM-mutation updates_ above).
- The first query an assistive technology makes into a view’s web content (through `accessibilityChildren`, `accessibilityHitTest:`, `accessibilityFocusedUIElement`, or `accessibilityAttributeValue:forParameter:`) → `request_accessibility_tree()`, if the view hasn’t requested the tree yet. That’s how an assistive technology that turns up after a page has loaded gets its tree — as WebKit and Chrome turn their accessibility support on at the first query, too. That tree doesn’t take focus or announce a load; if the view is the key window’s first responder, it has AppKit look the focus up again instead (`NSAccessibilityHandleFocusChanged()`).
- `on_accessibility_tree_received` → update manager, clear cache, post `NSAccessibilityLayoutChangedNotification` on the view; then, for the first tree after each load only: transfer window first-responder to the view (so AppKit resolves `accessibilityFocusedUIElement` through it); locate the `AXWebArea` element and post `NSAccessibilityFocusedUIElementChangedNotification` and `@"AXLoadComplete"` on it (matching what Chromium and Firefox do); call `NSAccessibilityHandleFocusChanged()` — so AppKit refreshes its cached focused-UI-element.
...
- Builds the accessibility tree and then serializes it. Notifies the page client when the focused element changes — once the UI has requested the tree.
- Has a mechanism for driving Orca focus-ring display movement without moving DOM focus.

### `Libraries/LibWeb/Page/Page.h`
- Has a mechanism for notifying the UI process when focus changes. Also has `accessibility_interested()`, which the first tree request sets, and which gates the focus-change reports and the tree pushes.
...
- Builds and sends the full tree (and on the first request, sets `accessibility_interested()`), and has `perform_accessibility_action` — which looks up DOM nodes by `UniqueNodeID` and dispatches to the `focus` and `scroll_into_view` action handlers. Also implements the `AccessibilityTree` page-info dump for `test-web`.

`PageClient.cpp` / `PageClient.h`
- Sends focus-change IPC and pushes tree updates when the page changes — once the UI has requested the tree.
...
- Integration point on macOS. Owns the `AccessibilityTreeManager` and element cache. Wires up IPC callbacks, and requests the tree once an assistive technology is present. Ensures VoiceOver moves its cursor into the web content on page load. Also posts layout-changed notifications and live-region-announcement notifications.
...
- Owns the `AccessibilityTreeManager` and element cache. Sets up IPC callbacks, and requests the tree once an assistive technology is present: at load finish if `QAccessible::isActive()`, or through `accessibilityActiveChanged()` for one that turns up later. Posts the document-root accessibility focus event. Rewires `QAccessibleTextInterface::scrollToSubstring` to the `scroll_into_view` accessibility action.
