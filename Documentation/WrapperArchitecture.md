...
> about a realm in exactly two situations: (1) you create a wrappable that
> scripts from several realms might see first and it is not a `Node` — call
> `set_wrapper_realm_hint()` once in your `create()` path, using a realm-free
> helper when one is available; (2) you mint a `Promise` for later resolution —
> use the promise vending helper for the relevant object or settings object.
> Everything else — exceptions, return values, events, callbacks — is
> realm-free in ordinary implementation code.
...
structured serialization, module/script plumbing, callback invocation, or the
wrapper hint machinery itself. New realm mentions outside `Bindings/`,
...
- Preservation can over-retain wrappers after state returns to default, such as
  ...
  detector.
- Wrapper realm hints are weak. If the hinted realm dies, main-world wrapper
  allocation falls back to the caller's preferred realm. This degrades placement,
  not identity: the `WrapperWorld` still owns identity.
