...
ErrorOr<String> UnixDateTime::to_string(StringView format) const
{
    struct tm tm;

    auto gmt_timestamp = m_offset.to_timespec().tv_sec;
    (void)gmtime_r(&gmt_timestamp, &tm);
...
ByteString UnixDateTime::to_byte_string(StringView format) const
{
