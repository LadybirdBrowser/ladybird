...
static bool is_ignored_role(StringView role, String const& name)
{
    ...
    // text-leaf children bubble up and Orca sees them as Role.LABEL — which isn't a text_block_element — and sentence
...
// An ARIA grid or treegrid is a table to the AT, as a table element is: Blink's IsTableLike() and WebKit's
// isTableRole() take grid and treegrid along with table, and Gecko's role map types both eTable. Qt has no tree-table
// role (QAccessible::Role), so a treegrid is a Table here too, where WebKit and Gecko give AT-SPI a tree table.
static bool is_table_container_role(StringView role)
{
    return role == "table"sv || role == "grid"sv || role == "treegrid"sv;
}

// A text control holds its own text (its value) rather than text-leaf children: an input or a textarea (role textbox
// or searchbox), a password field, or a combobox the user can type into.
static bool is_text_control(WebView::AccessibilityNodeData const& data)
{
    auto role = data.role.bytes_as_string_view();
    return role == "textbox"sv || role == "searchbox"sv || data.is_password
        || (role == "combobox"sv && data.is_editable);
}
...
AccessibilityInterface::AccessibilityInterface(i64 node_id, WebView::AccessibilityTreeManager const* manager, WebContentView* view)
    ...
    , m_view(view)
{
    // Qt's bridge requires a non-null QObject* from object(). This interface owns it outright (no parent), so
    // ~AccessibilityInterface deletes it and it never leaks as a child of the view.
...
// The ordered unignored children of node_id, with ignored generic containers flattened recursively so their unignored
// descendants are promoted to this level. This single traversal defines the hypertext offset space: build_hypertext(),
// characterRect(), and append_offset_ys() all walk it, so their offsets stay aligned.
static void gather_unignored_children(WebView::AccessibilityTreeManager const* manager, i64 node_id, QList<i64>& out)
{
    ...
    for (auto child_id : data->child_ids) {
        ...
        if (is_ignored_role(child_data->role.bytes_as_string_view(), child_data->name))
            gather_unignored_children(manager, child_id, out);
        else
            out.append(child_id);
    }
}
...
{
    gather_unignored_children(m_manager, node_id, out);
}
...
void* AccessibilityInterface::interface_cast(QAccessible::InterfaceType type)
{
    ...
    case QAccessible::ActionInterface: {
        // Every node advertises setFocus (see actionNames), so every node must expose the Action interface — not just
        // the press-able roles — or Qt can't reach the advertised action.
        if (!node_data())
            break;
    ...
    case QAccessible::TextInterface: {
        ...
        // A text control always has Text, whatever it holds. An unlabeled field that's still empty has no name and no
        // hypertext, so the checks below would leave it without the interface until the user typed into it, and Orca
        // would have no text object for caret tracking or typing echo on the first keystrokes — while Gecko (every
        // HyperTextAccessible gets MAI_INTERFACE_TEXT, an entry included) and Blink (AXPlatformNodeAuraLinux::
        // GetGTypeInterfaceMask() gives a text field AtkText from its role) expose it unconditionally.
        if (is_text_control(*data))
        ...
        if (!data->name.is_empty() && childCount() == 0)
            return static_cast<QAccessibleTextInterface*>(this);
        // An unnamed container such as an article or blockquote with direct text matches none of the conditions above,
        // yet collect_exposed_children strips its text-leaf children (they stay reachable through this node's own
        // hypertext). Without a Text interface, that text would be exposed nowhere. Grant Text when build_hypertext()
        // carries real textual content — but never when it holds only U+FFFC embedded-object markers (a list of
        // listitems, a row of links). That's for the same reason the list role is excluded above: Orca skips a
        // marker-only text object it can't expand.
        if (hypertext_has_real_text())
            return static_cast<QAccessibleTextInterface*>(this);
        break;
    }
...
// Qt's UIA and Cocoa bridges ask an interface for its focused descendant; the AT-SPI adaptor never does (it follows
// the Focus events WebContentView posts). The tree records which node has focus, so walk up from that node to this one
// rather than down thru every child — child() rebuilds the exposed-children list and registers an interface for each
// node it visits, which on a big page is thousands of interfaces for one question.
QAccessibleInterface* AccessibilityInterface::focusChild() const
{
    auto const* data = node_data();
    if (data && data->is_focused)
        return const_cast<AccessibilityInterface*>(this);

    auto focused_id = m_manager->focused_node_id();
    if (!focused_id.has_value())
        return nullptr;

    for (auto const* node = m_manager->node(*focused_id); node && node->parent_id != -1; node = m_manager->node(node->parent_id)) {
        if (node->parent_id == m_node_id)
            return m_view->accessibility_interface_for_node(*focused_id);
    }
    return nullptr;
}
...
int AccessibilityInterface::indexOfChild(QAccessibleInterface const* iface) const
{
    ...
    // Qt may pass any interface here and expects -1 for one that's not our child — so a downcast would be undefined for
    // anything that isn't an AccessibilityInterface. Instead compare identity against what our own children resolve to;
    // child() hands out these same pointers.
    QList<i64> children;
    collect_exposed_children(m_node_id, children);
    for (int index = 0; index < children.size(); ++index) {
        if (m_view->accessibility_interface_for_node(children[index]) == iface)
            return index;
    }
    return -1;
}
...
// The node data's geometry is in CSS pixels, and the view shows the page at its zoom level: WebContent folds zoom
// into its device scale, the way Gecko does (nsPresContext::SetFullZoom() refreshes the app-units-per-device-pixel
// ratio that LocalAccessible::Bounds() converts with), so a CSS-pixel rect is zoom_level() times as large on the
// widget. Blink and WebKit zoom in layout instead, so their AX bounds come out zoomed already.
static QRect css_rect_to_global(WebContentView const& view, QRect css_rect)
{
    auto zoom = view.zoom_level();
    QRect local(qRound(css_rect.x() * zoom), qRound(css_rect.y() * zoom),
        qRound(css_rect.width() * zoom), qRound(css_rect.height() * zoom));
    auto global_pos = view.mapToGlobal(QPoint(local.x(), local.y()));
...
QRect AccessibilityInterface::rect() const
{
    ...
    return css_rect_to_global(*m_view, QRect(data->bounds.x(), data->bounds.y(), data->bounds.width(), data->bounds.height()));
}
...
QAccessible::Role AccessibilityInterface::map_role() const
{
    ...
    // A password field has no ARIA role (HTML-AAM gives it none), but it's a text field to the platform: EditableText
    // with the passwordEdit state, which Qt's AT-SPI adaptor reports as ATSPI_ROLE_PASSWORD_TEXT (AtSpiAdaptor::
    // getRole) — the role Gecko (HTMLTextFieldAccessible::NativeRole), WebKit (AccessibilityObjectAtspi::effectiveRole)
    // and Blink (AXPlatformNodeAuraLinux, a protected kTextField) give it.
    if (data->is_password)
    ...
    if (is_table_container_role(role))
    ...
    if (role == "log"sv)
        return QAccessible::Section;
    // Qt has no internal-frame role (the ATK_ROLE_INTERNAL_FRAME of Gecko's OuterDocAccessible and Blink's
    // Role::kIframe), so a frame is a section: ATSPI_ROLE_SECTION, per QSpiAccessibleBridge::namesForRole().
    if (role == "iframe"sv)
        return QAccessible::Section;
    if (role == "text leaf"sv)
    ...
    if (role == "option"sv)
        return QAccessible::ListItem;
    if (role == "menuitemcheckbox"sv)
        return QAccessible::CheckBox;
    if (role == "menuitemradio"sv)
        return QAccessible::RadioButton;
...
QAccessible::State AccessibilityInterface::state() const
{
    ...
    // Only report focusable for roles that really are keyboard-focusable. Reporting it on non-interactive elements
    // (headings, paragraphs, lists, list items, generic containers) breaks Orca heuristics; is_text_block_element
    // returns False for any focusable object — which causes sentence-contents extension to run past paragraph
    // boundaries and pull in the *entire* document.
    ...
    if (data->is_password)
        accessible_state.passwordEdit = true;
    // The text-editing bits Qt's AT-SPI adaptor turns into EDITABLE, READ_ONLY, MULTI_LINE and MULTISELECTABLE
    // (spiStatesFromQState()) — the states Gecko (nsStateMap.h) and Blink (AXPlatformNodeAuraLinux::GetAtkStateSet())
    // put on their entries and listboxes, and the ones Orca reads to treat a text object as an editable field: focus
    // mode, typing echo, and a read-only field announced as one. Gecko leaves read-only to the absence of EDITABLE,
    // which holds here too, since is_read_only clears is_editable.
    if (data->is_editable)
        accessible_state.editable = true;
    if (data->is_read_only)
        accessible_state.readOnly = true;
    if (data->is_multi_line)
        accessible_state.multiLine = true;
    if (data->is_multi_selectable)
        accessible_state.multiSelectable = true;
    ...
    // The node data carries these, and without them AT-SPI2 clients can't tell a ticked checkbox from an unticked one,
    // or an open disclosure from a closed one. NotApplicable means the element has no such state — so leave the
    // corresponding bits alone, rather than reporting a false negative.
    if (data->is_selected) {
        accessible_state.selected = true;
        accessible_state.selectable = true;
    }

    switch (data->checked_state) {
    case WebView::AccessibilityNodeData::CheckedState::Unchecked:
        accessible_state.checkable = true;
        break;
    case WebView::AccessibilityNodeData::CheckedState::Checked:
        accessible_state.checkable = true;
        accessible_state.checked = true;
        break;
    case WebView::AccessibilityNodeData::CheckedState::Mixed:
        accessible_state.checkable = true;
        accessible_state.checkStateMixed = true;
        break;
    case WebView::AccessibilityNodeData::CheckedState::NotApplicable:
        break;
    }

    switch (data->expanded_state) {
    case WebView::AccessibilityNodeData::ExpandedState::Collapsed:
        accessible_state.expandable = true;
        accessible_state.collapsed = true;
        break;
    case WebView::AccessibilityNodeData::ExpandedState::Expanded:
        accessible_state.expandable = true;
        accessible_state.expanded = true;
        break;
    case WebView::AccessibilityNodeData::ExpandedState::NotApplicable:
        break;
    }

    // Qt has no tri-state for pressed, so Mixed reports pressed as the closer of the two.
    switch (data->pressed_state) {
    case WebView::AccessibilityNodeData::PressedState::Pressed:
    case WebView::AccessibilityNodeData::PressedState::Mixed:
        accessible_state.pressed = true;
        break;
    case WebView::AccessibilityNodeData::PressedState::NotPressed:
    case WebView::AccessibilityNodeData::PressedState::NotApplicable:
        break;
    }
...
QStringList AccessibilityInterface::actionNames() const
{
    ...
    if (role == "button"sv || role == "link"sv || role == "checkbox"sv
        ...
        || role == "switch"sv || role == "menuitemcheckbox"sv || role == "menuitemradio"sv)
...
// Qt indexes text in UTF-16 code units, while character_offsets holds one entry per code point. The two agree across
// the whole BMP, and diverge at the first surrogate pair. So a caller's offset has to be translated before it indexes
// that array. Without this, every character after a non-BMP one reports its neighbor's rectangle, or none at all.
static int code_point_offset_for_utf16_offset(String const& text, int utf16_offset)
{
    int code_points = 0;
    int utf16_units = 0;
...
// is populated by LibWeb from the text's rendered visual lines and correctly places each character on its own line —
// even when the text wraps. Coordinates are widget-local.
static QRect leaf_character_rect_local(WebView::AccessibilityNodeData const* leaf_data, int offset)
{
    ...
    offset = code_point_offset_for_utf16_offset(leaf_data->name, offset);
...
static void append_offset_ys(WebView::AccessibilityTreeManager const* manager, i64 node_id, Vector<i32>& out);
static int representative_line_height(WebView::AccessibilityTreeManager const* manager, i64 node_id);
...
QRect AccessibilityInterface::characterRect(int offset) const
{
    ...
    auto to_global = [this](QRect css_rect) {
        return css_rect_to_global(*m_view, css_rect);
    };
    ...
    if (offset > 0) {
        // Compare the y of offset-1 and offset; if y jumps, treat offset as a line start and return empty.
        Vector<i32> ys;
        append_offset_ys(m_manager, m_node_id, ys);
        // Same tolerance compute_visual_line_starts() applies: only a jump of more than half a line means a new
        // visual line. A bare inequality would report no extents for a character merely shifted off the baseline
        // (a superscript, or text beside an inline image), dropping it from the union rect GetRangeExtents builds.
        int const line_tolerance = representative_line_height(m_manager, m_node_id) / 2;
        if (static_cast<size_t>(offset) < ys.size() && qAbs(ys[offset] - ys[offset - 1]) > line_tolerance)
            return {};
    }
...
QString AccessibilityInterface::build_hypertext() const
{
    ...
    // A text control's text is what it holds (its value), not its name: an input or textarea has no text-leaf children
    // in the tree, and its name is its label. Qt's own QAccessibleLineEdit hands out the field's display text the same
    // way, and it's this text Orca reads back for typing echo and caret navigation — against the same offsets the
    // caret, selection and editable-text plumbing use.
    if (is_text_control(*data))
        return QString::fromUtf8(reinterpret_cast<char const*>(data->value.bytes().data()), data->value.bytes().size());
...
// Real text means at least one character that is neither an embedded-object marker (U+FFFC) nor whitespace. A container
// whose hypertext is only markers must not advertise Text: Orca treats it as an unexpandable text object and skips it
// during Say All, and the Qt bridge exposes no Hypertext interface to expand the markers. Incidental whitespace between
// markers doesn't make the content readable, so it doesn't count.
bool AccessibilityInterface::hypertext_has_real_text() const
{
    auto const hypertext = build_hypertext();
    for (QChar const ch : hypertext) {
        if (ch != QChar(0xFFFC) && !ch.isSpace())
            return true;
    }
    return false;
}
...
{
    // Orca calls scrollToSubstring when its browse-mode cursor moves to an element. To get the accessibility focus ring
    // painted, we use the "scroll_into_view" action to scroll the element into view and mark it as the document's
...
// The vertical extent of one UTF-16 code unit of build_hypertext()'s text: the line it renders on, in the same space
// (CSS px) as every bounds here.
struct OffsetLineExtent {
    i32 y { 0 };
    i32 height { 0 };
    // An embedded object with no text of its own (an <img>, e.g.): its y starts as its top edge and is settled onto a
    // neighbor's line by settle_textless_objects() once every entry is known.
    bool is_textless_object { false };
};

// The y of the first text character inside a node, and that character's line height.
static Optional<OffsetLineExtent> first_text_leaf_extent(WebView::AccessibilityTreeManager const* manager, i64 node_id)
{
    auto const* data = manager->node(node_id);
    if (!data)
        return {};
    if (data->role.bytes_as_string_view() == "text leaf"sv) {
        ...
        if (!data->character_offsets.is_empty())
            y += data->character_offsets.first().y();
        i32 height = data->line_heights.is_empty() ? data->bounds.height() : data->line_heights.first();
        return OffsetLineExtent { y, height, false };
    }
    for (auto child_id : data->child_ids) {
        if (auto extent = first_text_leaf_extent(manager, child_id); extent.has_value())
            return extent;
    }
    return {};
}

// Gathers the extent of every code unit of a node's hypertext, in one pass over the same flattened child sequence
// build_hypertext() walks: a text-leaf child contributes an entry per code unit (two for a surrogate pair, both at the
// code point's line — character_offsets has one entry per code point), every other child one entry for the U+FFFC it
// occupies. Resolving offsets one at a time would walk the children, decode every earlier leaf and rescan the leaf's
// code points for each offset — O(n²) in the container's text, enough to stall the UI thread on a long <pre> while Orca
// asks for line after line during Say All or flat review.
static void gather_offset_line_extents(WebView::AccessibilityTreeManager const* manager, i64 node_id, Vector<OffsetLineExtent>& out)
{
    auto const* data = manager->node(node_id);
    if (!data)
        return;
    ...
    if (data->role.bytes_as_string_view() == "text leaf"sv) {
        ...
        size_t line_index = 0;
        for (auto code_point : data->name.code_points()) {
            // line_break_character_offsets is ascending, so the line a code point is on only ever advances.
            while (line_index + 1 < data->line_break_character_offsets.size()
                && static_cast<size_t>(data->line_break_character_offsets[line_index + 1]) <= code_point_index)
                ++line_index;
            i32 y = data->bounds.y();
            if (!data->character_offsets.is_empty())
                y += data->character_offsets[min(code_point_index, data->character_offsets.size() - 1)].y();
            i32 height = line_index < data->line_heights.size()
                ? data->line_heights[line_index]
                : data->bounds.height();
            out.append({ y, height, false });
    ...
    QList<i64> children;
    gather_unignored_children(manager, node_id, children);
    for (auto child_id : children) {
        auto const* child = manager->node(child_id);
        if (!child)
            continue;
        if (child->role.bytes_as_string_view() == "text leaf"sv) {
            gather_offset_line_extents(manager, child_id, out);
        } else if (auto extent = first_text_leaf_extent(manager, child_id); extent.has_value()) {
            // An object with text sits on the line its first character is on — so a link that wraps onto a second line
            // stays anchored to the line it starts on.
            out.append(*extent);
        } else {
            out.append({ child->bounds.y(), child->bounds.height(), true });
        }
    }
}

// Puts each textless embedded object on the line of the text beside it: the nearest text character before it whose
// line the object's box overlaps vertically, else the nearest one after it. A text character's y is the top of the
// text's box on that line, while an inline image's is its own top edge — for a tall image, well above the text's on
// the same line, and far enough for compute_visual_line_starts() to read a line break before the image and another
// after it. Overlap is what marks the shared line, whatever the image's vertical-align, since every inline box is
// inside its line box. An object with no text beside it on its line (one alone on a line, e.g.) keeps its top edge.
static void settle_textless_objects(Vector<OffsetLineExtent>& extents)
{
    auto overlaps = [](OffsetLineExtent const& object, OffsetLineExtent const& text) {
        return !text.is_textless_object && object.y < text.y + text.height && text.y < object.y + object.height;
    };
    for (size_t i = 0; i < extents.size(); ++i) {
        if (!extents[i].is_textless_object)
            continue;
        Optional<i32> line_y;
        for (size_t j = i; j-- > 0;) {
            if (extents[j].is_textless_object)
                continue;
            if (overlaps(extents[i], extents[j]))
                line_y = extents[j].y;
            break;
        }
        for (size_t j = i + 1; !line_y.has_value() && j < extents.size(); ++j) {
            if (extents[j].is_textless_object)
                continue;
            if (overlaps(extents[i], extents[j]))
                line_y = extents[j].y;
...
// The y (in the same space as every bounds here) of the line each UTF-16 code unit of build_hypertext()'s text sits on.
static void append_offset_ys(WebView::AccessibilityTreeManager const* manager, i64 node_id, Vector<i32>& out)
{
    Vector<OffsetLineExtent> extents;
    gather_offset_line_extents(manager, node_id, extents);
    settle_textless_objects(extents);
    out.ensure_capacity(out.size() + extents.size());
    for (auto const& extent : extents)
        out.unchecked_append(extent.y);
}

// A representative line height (CSS px) for the node's text, used as the same-line y tolerance below. line_heights is
// populated on text leaves — so descend to the first one.
static int representative_line_height(WebView::AccessibilityTreeManager const* manager, i64 node_id)
{
    auto const* data = manager->node(node_id);
    if (!data)
        return 0;
    if (data->role.bytes_as_string_view() == "text leaf"sv) {
        if (!data->line_heights.is_empty())
            return data->line_heights.first();
        return data->bounds.height();
    }
    for (auto child_id : data->child_ids) {
        if (int height = representative_line_height(manager, child_id); height > 0)
            return height;
    }
    return 0;
}

// Compute the character offsets where each visual line starts — from the per-character y that append_offset_ys()
// gathers. The last entry is char_count as an end sentinel. Robust against small y-jitter by only treating a change in
// y as a new line when the jump is larger than half the current line height.
static QList<int> compute_visual_line_starts(AccessibilityInterface const& iface, int char_count)
{
    ...
    // Only treat a y change as a new visual line when the jump exceeds half the line height, so intra-line
    // baseline shifts (superscripts, inline images) don't split one visual line into several.
    int const tolerance = representative_line_height(manager, node_id) / 2;
    Vector<i32> ys;
    append_offset_ys(manager, node_id, ys);
    i32 prev_y = ys.is_empty() ? 0 : ys[0];
    int const known = min(char_count, static_cast<int>(ys.size()));
    for (int i = 1; i < known; ++i) {
        if (qAbs(ys[i] - prev_y) > tolerance) {
            starts.append(i);
            prev_y = ys[i];
        }
    }
...
static QString text_at_visual_line_containing(QList<int> const& line_starts, QString const& full_text, int offset,
    ...
    int* startOffset, int* endOffset) const
{
    ...
    auto full_text = build_hypertext();
    auto line_starts = compute_visual_line_starts(*this, full_text.size());
    return text_at_visual_line_containing(line_starts, full_text, offset, startOffset, endOffset);
}
    ...
    int* startOffset, int* endOffset) const
{
    ...
    text_at_visual_line_containing(line_starts, full_text, offset, &this_start, &this_end);
    if (this_start <= 0) {
        *startOffset = *endOffset = -1;
        return {};
    }
    return text_at_visual_line_containing(line_starts, full_text, this_start - 1, startOffset, endOffset);
}
    ...
    int* startOffset, int* endOffset) const
{
    ...
    return text_at_visual_line_containing(line_starts, full_text, this_end, startOffset, endOffset);
}
...
QVariant AccessibilityInterface::attributeValue(QAccessible::Attribute key) const
{
    ...
    // "tag" attribute: Orca's web script uses this to identify web elements (is_web_element checks for it), so
    // synthesize one from the role.
    if (role == "document"sv)
    ...
    // "level": a heading's level, or a treeitem's, row's or listitem's depth — the object attribute that Gecko
    // (GroupPosition) and Blink (AXPlatformNodeAuraLinux, from kHierarchicalLevel) export for both, and the one
    // Orca reads for tree items.
    if (data->heading_level > 0)
        attrs[QStringLiteral("level")] = QString::number(data->heading_level);
    else if (data->hierarchical_level > 0)
        attrs[QStringLiteral("level")] = QString::number(data->hierarchical_level);
...
int AccessibilityInterface::columnIndex() const
{
    ...
    // Prefer the colspan-aware index LibWeb already computed; the flat sibling loop below mis-reports every
    // cell after a colspan>1 cell.
    if (data->cell_column_index >= 0)
        return data->cell_column_index;
...
int AccessibilityInterface::rowIndex() const
{
    ...
    while (ancestor_id != -1) {
        ...
        if (!ancestor)
            break;
        if (is_table_container_role(ancestor->role.bytes_as_string_view())) {
