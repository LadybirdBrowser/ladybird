...
// characterRect(), and offset_to_y_in_container() all walk it, so their offsets stay aligned.
...
// that array. Without this, every character after a non-BMP one reports its neighbour's rectangle, or none at all.
...
QRect AccessibilityInterface::characterRect(int offset) const
{
    ...
    if (offset > 0) {
                ...
                if (crole == "text leaf"sv) {
                    ...
                    if (off < cum + len) {
                        ...
                        if (cd->character_offsets.is_empty() || leaf_off < 0)
                            return cd->bounds.y();
                        // leaf_off counts UTF-16 code units, like every offset Qt hands us; character_offsets has one
                        // entry per code point.
                        leaf_off = code_point_offset_for_utf16_offset(cd->name, leaf_off);
        ...
        if (y_prev.has_value() && y_here.has_value() && qAbs(y_here.value() - y_prev.value()) > line_tolerance)
            return {};
    }
...
static Optional<i32> offset_to_y_in_container(WebView::AccessibilityTreeManager const* manager, i64 node_id, int offset)
{
    ...
    if (role == "text leaf"sv) {
        ...
        // The offset counts UTF-16 code units, like every offset Qt hands us; character_offsets has one entry per
        // code point.
        int idx = offset < 0 ? 0 : code_point_offset_for_utf16_offset(data->name, offset);
        if (idx >= static_cast<int>(data->character_offsets.size()))
            idx = static_cast<int>(data->character_offsets.size()) - 1;
        return data->bounds.y() + data->character_offsets[idx].y();
    }

    // Walk the same flattened child sequence build_hypertext() uses, so a character offset resolves through the
    // identical recursive promotion of ignored generics. A text-leaf child contributes its own characters; every other
    // child is one embedded-object position (U+FFFC).
    QList<i64> children;
    ...
    for (auto child_id : children) {
        ...
        if (!cd)
            continue;
        if (cd->role.bytes_as_string_view() == "text leaf"sv) {
            auto qs = QString::fromUtf8(reinterpret_cast<char const*>(cd->name.bytes().data()), cd->name.bytes().size());
            int len = qs.size();
            if (offset < cum + len)
...
static QList<int> compute_visual_line_starts(AccessibilityInterface const& iface, int char_count)
{
    ...
    for (int i = 1; i < char_count; ++i) {
        ...
        if (!y.has_value())
            continue;
        if (qAbs(y.value() - prev_y) > tolerance) {
            starts.append(i);
            prev_y = y.value();
        }
    }
...
int AccessibilityInterface::columnIndex() const
{
    ...
    // Prefer the colspan-aware index LibWeb already computed; the flat sibling loop below mis-reports every cell after
    // a colspan>1 cell.
