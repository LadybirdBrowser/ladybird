...
def _ladybird_is_active():
    """True when the active Orca script is the Ladybird one. The monkey-patches below reassign methods on
    Orca-global classes, so each applies its custom behavior only while Ladybird is the focused application and
    otherwise delegates to the original method. That keeps them from changing how Orca behaves in other apps."""
    try:
        from orca import script_manager

        script = script_manager.get_manager().get_active_script()
        return script is not None and script.get_toolkit_name() == "Ladybird"
    except Exception:
        return False
...
def _install_scroll_patch():
    ...
    the accessibility focus ring is painted — without moving DOM focus, so no focus event fires back to Orca."""
    ...
    try:
        ...
        def _patched_scroll_to_center(obj, start_offset=None, end_offset=None):
            if not _ladybird_is_active():
                return _orig_scroll_to_center(obj, start_offset, end_offset)
...
def _install_collection_fallback_patch():
    ...
    try:
        ...
        def _patched_find_all_with_role(root, role_list, pred=None):
            if not _ladybird_is_active():
                return _original_find_all_with_role(root, role_list, pred)
            try:
        ...
        def _patched_find_all_with_role_and_all_states(root, role_list, state_list, pred=None):
            if not _ladybird_is_active():
                return _original_find_all_with_role_and_all_states(root, role_list, state_list, pred)
            try:
    ...
    *also* "here" *again*, from the link zone — duplicated. So, this patch filters the descendant zone when its full
    ...
    try:
        ...
        def _patched_get_on_screen_objects(root, cancellation_event, bounding_box=None):
            if not _ladybird_is_active():
                return _orig(root, cancellation_event, bounding_box)
    ...
    try:
        ...
        def _wrap(method_name):
            ...
            def wrapper(self, *args, **kwargs):
                ...
                if advanced and _ladybird_is_active():
...
def _install_hypertext_fallback_patch():
    ...
    try:
        ...
        def _patched_find_child(obj, offset):
            if not _ladybird_is_active():
                return _orig_find_child(obj, offset)
...
class Utilities(web.Utilities):
    ...
    def active_document(self):
        ...
        # Try the standard EMBEDS-based approach first — but trust it only for the *active* tab. EMBEDS can resolve to
        # an inactive tab's document (both report showing=True; only the parent panel differs), and the cache
        # revalidation below can't catch that (the doc's own is_showing is always True), so validate the showing parent.
        doc = super().active_document()
        if doc is not None and not AXObject.is_dead(doc):
            parent = AXObject.get_parent(doc)
            if parent and AXUtilities.is_showing(parent):
                self._cached_active_document = doc
                return doc

        # Revalidate the cache the same way. After a tab switch the cached document belongs to a background tab, and
        # its own state can't tell that: it still reports showing=True. Only its parent panel goes hidden.
        if self._cached_active_document is not None:
            cached = self._cached_active_document
            cached_parent = None if AXObject.is_dead(cached) else AXObject.get_parent(cached)
            if cached_parent is not None and AXUtilities.is_showing(cached) and AXUtilities.is_showing(cached_parent):
                return cached
            self._cached_active_document = None
    ...
    def get_caret_context(self, document=None, get_replicant=False, search_if_needed=True):
        ...
        if doc is not None:
            ...
            first_obj, first_offset = self.first_context(doc, 0)
            # If first_context returned the document itself (meaning it couldn't descend), then manually find the
            # first child with content.
