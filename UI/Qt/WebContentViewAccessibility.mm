...
- (id)accessibilityFocusedUIElement
{
    ...
    // The focused DOM element, if there is one (an input the user clicked into, e.g.) — the same answer the AppKit
    // port's LadybirdWebView gives.
...
void update_accessibility_tree(WebContentView* view, bool take_initial_focus)
{
    ...
    // Only a page's first tree takes the window's first responder — the gate WebContentView puts on its setFocus(),
    // for the same reason: WebContent rebuilds the tree on DOM mutations too, and taking the first responder on each
    // of those would yank keyboard focus out of the address bar mid-keystroke.
    if (take_initial_focus && ns_view && ns_view.window)
