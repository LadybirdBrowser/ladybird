    ...
    , m_test_promise(TestPromise::construct())
{
    ...
    on_reposition_window = [this](auto position) {
        m_cached_dimensions.set_location(position.template to_type<Web::DevicePixels>());
    ...
    on_maximize_window = [this]() {
        ...
        m_cached_dimensions = screen_rect;
    ...
    on_fullscreen_window = [this]() {
        m_cached_dimensions.set_size(m_viewport_size);
        m_viewport_size = screen_rect.size();
    ...
    on_exit_fullscreen_window = [this]() {
        m_viewport_size = m_cached_dimensions.size();
        client().async_set_window_position(m_client_state.page_index, m_cached_dimensions.location());
        client().async_set_window_size(m_client_state.page_index, m_cached_dimensions.size());
        client().async_set_viewport_size(m_client_state.page_index, m_cached_dimensions.size());
