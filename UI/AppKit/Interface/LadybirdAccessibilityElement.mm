...
// Text marker: encodes a node ID and character offset for cursor-level text navigation.
...
static AXTextMarkerRef createTextMarker(i64 node_id, i32 offset = 0)
{
    ...
    return AXTextMarkerCreate(kCFAllocatorDefault, reinterpret_cast<UInt8 const*>(&data), sizeof(data));
}
...
// Text-marker offsets are Unicode code-point offsets (VoiceOver navigates character by character), while a leaf's
// name is UTF-8, so its byte length is not its character count for non-ASCII text.
static i32 codePointCount(StringView text)
{
    return static_cast<i32>(Utf8View { text }.length());
}

// An ARIA grid or treegrid is a table to VoiceOver, as a table element is: WebKit and Blink both give the two
// NSAccessibilityTableRole (AXCoreObjectCocoa.mm, ax_platform_node_cocoa.mm), and count their rows the same way.
static bool is_table_container_role(StringView role)
{
    return role == "table"sv || role == "grid"sv || role == "treegrid"sv;
}

// Whitespace-only text is nothing for VoiceOver to stop on, so it's ignored whether it renders or not — as WebKit
// ignores every whitespace-only RenderText (AccessibilityRenderObject::computeIsIgnored()). The text markers still walk
// it, so the space between two inline siblings stays in the text they read out.
static bool is_ignored_role(StringView role, String const& name)
{
    return (role == "generic"sv && name.is_empty())
        ...
        || (role == "text leaf"sv && name.bytes_as_string_view().is_whitespace());
}
...
static NSAccessibilityRole aria_role_to_ns_role(StringView role)
{
    ...
    // A header cell is a cell to VoiceOver, as WebKit (AXCoreObjectCocoa.mm maps ColumnHeader and RowHeader to
    // NSAccessibilityCellRole), Blink (AXPlatformNodeCocoa, kColumnHeader and kRowHeader) and Gecko (RoleMap.inc,
    // COLUMNHEADER and ROWHEADER) expose it; the table's AXHeader and the cells' index ranges tell the headers apart.
    if (role == "cell"sv || role == "gridcell"sv || role == "columnheader"sv || role == "rowheader"sv)
    ...
    // A frame is a group, as Blink exposes it (AXPlatformNodeCocoa maps Role::kIframe to NSAccessibilityGroupRole).
    // Gecko (NSAccessibilityScrollAreaRole for roles::INTERNAL_FRAME) and WebKit (an AccessibilityScrollView under the
    // frame's object) expose the child document's scroll view instead — which this tree doesn't carry yet.
    if (role == "iframe"sv)
        return NSAccessibilityGroupRole;
    if (role == "document"sv)
        return @"AXWebArea";
...
static NSString* nsStringFromAK(String const& string)
{
    if (string.is_empty())
        return nil;
...
@implementation LadybirdAccessibilityElement
{
    ...
    __weak id<LadybirdAccessibilityView> _view;
}
...
- (BOOL)accessibilityIsIgnored
{
    ...
    // Generic elements (div, span, body) are transparent to assistive technology unless they have a name. Their
    // children get promoted to the parent.
    if (role == "generic"sv && data->name.is_empty())
        return YES;

    // Paragraph containers are transparent — VoiceOver navigates to the text content inside them directly.
    if (role == "paragraph"sv && data->name.is_empty())
        return YES;

    // Whitespace-only text: see is_ignored_role().
    if (role == "text leaf"sv && data->name.bytes_as_string_view().is_whitespace())
        return YES;
...
- (NSArray*)accessibilityAttributeNames
{
    ...
    // Text markers and loading attributes belong only on the document root element — not on structural containers.
    ...
    NSArray* attributes = baseAttributes;
    if (role_sv == "document"sv)
        attributes = documentAttributes;
    else if (is_table_container_role(role_sv))
        attributes = tableAttributes;
    else if (role_sv == "row"sv)
        attributes = rowAttributes;
    else if (role_sv == "cell"sv || role_sv == "gridcell"sv || role_sv == "columnheader"sv || role_sv == "rowheader"sv)
        attributes = cellAttributes;
    // A row or tree item whose depth is known advertises it, as WebKit's outline rows and Blink's rows and tree items
    // do — and only then, so a depth nobody authored never reads as 0.
    if ((role_sv == "row"sv || role_sv == "treeitem"sv) && data->hierarchical_level > 0)
        return [attributes arrayByAddingObject:NSAccessibilityDisclosureLevelAttribute];
    return attributes;
}
...
- (id)accessibilityAttributeValue:(NSString*)attribute
{
    ...
    if ([attribute isEqualToString:NSAccessibilityRoleAttribute]) {
        if (!data)
            return NSAccessibilityGroupRole;
        // A password field has no ARIA role (HTML-AAM gives it none), but it's a text field to the platform, with the
        // secure subrole below — as WebKit (AccessibilityObject::subrolePlatformString() for isSecureField()), Blink
        // (AXPlatformNodeCocoa, a protected kTextField) and Gecko (mozTextAccessible, roles::PASSWORD_TEXT) expose it.
        if (data->is_password)
            return NSAccessibilityTextFieldRole;
    ...
    if ([attribute isEqualToString:NSAccessibilityRoleDescriptionAttribute]) {
        ...
        auto role_sv = data->role.bytes_as_string_view();
        // Landmarks need custom role descriptions because NSAccessibilityRoleDescription doesn't recognize the
        // AXLandmark* subrole strings.
        if (role_sv == "document"sv)
            return @"HTML content";
    ...
    if ([attribute isEqualToString:NSAccessibilityValueAttribute]) {
        ...
        // A tab is an AXRadioButton whose value is whether it's the selected one, as Blink's AXPlatformNodeCocoa
        // AXValue returns AXSelected for a tab.
        if (data->role.bytes_as_string_view() == "tab"sv)
            return @(data->is_selected);
        // A checkable control's value is its checked state as 0, 1 or 2: WebKit's AXCoreObject::value() hands out
        // checkboxOrRadioValue() (AccessibilityButtonState Off/On/Mixed) for every role with a checked state,
        // Gecko's mozCheckboxAccessible moxValue and Blink's AXValue return the same numbers, and VoiceOver reads
        // the value back as the state.
    ...
    if ([attribute isEqualToString:NSAccessibilityFocusedAttribute]) {
        if (!data)
            return @NO;
        // The document AXWebArea reports focused when its containing view is first responder. VoiceOver uses this to
        // know which web area is the active reading target on the page. Without it, AXLoadComplete fires — but
        // VoiceOver doesn't move its cursor into the web content.
        if (data->role.bytes_as_string_view() == "document"sv)
            return [_view accessibilityViewIsFirstResponder] ? @YES : @NO;
    ...
    if ([attribute isEqualToString:NSAccessibilityURLAttribute]) {
        if (data && data->role.bytes_as_string_view() == "document"sv)
            return [_view accessibilityPageURL];
        return nil;
    }
    ...
    if ([attribute isEqualToString:@"AXEndTextMarker"]) {
        ...
        if (!leaves.is_empty()) {
            ...
            i32 len = last ? codePointCount(last->name.bytes_as_string_view()) : 0;
    ...
    if ([attribute isEqualToString:@"AXElementBusy"])
        return @NO;
    // An option's, tab's, row's or cell's selection, as WebKit (handleSelectedAttribute), Blink (accessibilitySelected)
    // and Gecko (mozSelectableChildAccessible moxSelected) report it.
    if ([attribute isEqualToString:@"AXSelected"])
        return @(data && data->is_selected);
    ...
    if ([attribute isEqualToString:@"AXColumnCount"] || [attribute isEqualToString:@"AXRowCount"]
        ...
        if (!data || !is_table_container_role(data->role.bytes_as_string_view()))
            return nil;
        return [self tableAttributeValue:attribute];
    }

    if ([attribute isEqualToString:NSAccessibilityDisclosureLevelAttribute]) {
        // 0-based, the way WebKit (handleDisclosureLevelAttribute) and Blink (accessibilityDisclosureLevel) both
        // convert the 1-based aria-level.
        if (!data || data->hierarchical_level <= 0)
            return nil;
        return @(data->hierarchical_level - 1);
    }
...
- (id)tableAttributeValue:(NSString*)attribute
{
    ...
    for (auto child_id : data->child_ids) {
        ...
        if (child_role == "row"sv) {
            ...
            max_columns = MAX(max_columns, static_cast<int>(child->child_ids.size()));
            ...
            for (auto gc_id : child->child_ids) {
                ...
                if (gc && gc->role.bytes_as_string_view() == "row"sv) {
                    ...
                    max_columns = MAX(max_columns, static_cast<int>(gc->child_ids.size()));
                }
            }
        }
    }
    ...
    if ([attribute isEqualToString:@"AXColumnCount"]) {
        // WebContent counts the columns with every cell's colspan (AccessibilityTreeNode's table_column_count), as
        // Gecko (HTMLTableAccessible::ColCount() reads the table frame's count), WebKit (AccessibilityNodeObject::
        // columnCount() counts its table's cell slots) and Blink (AXTableInfo takes the widest col_index + col_span)
        // do. The widest row's cell count stands in only for a table without one.
        return @(data->table_column_count >= 0 ? data->table_column_count : max_columns);
    }
...
- (NSNumber*)rowIndex
{
    ...
    // Find this row among all rows in the parent-table walk-up to find the table.
    i64 table_id = -1;
...
- (id)cellAttributeValue:(NSString*)attribute
{
    ...
    // Prefer the colspan-aware column index WebContent serialized for the cell. Counting the cell's preceding siblings
    // one column each mis-reports every cell that follows a cell spanning more than one column, so that stays only as
    // the fallback for a cell that arrived without an index.
    int col_index = data->cell_column_index;
    if (col_index < 0) {
        col_index = 0;
...
- (NSArray*)accessibilityParameterizedAttributeNames
{
    ...
    // Text-marker parameterized attributes belong only on the document root element.
...
- (id)accessibilityAttributeValue:(NSString*)attribute forParameter:(id)parameter
{
    ...
    if ([attribute isEqualToString:@"AXUIElementsForSearchPredicate"] ||
        ...
        // The count variant is "AXUIElementCountForSearchPredicate". Its name ends with "Predicate", not "Count" — so a
        // hasSuffix:@"Count" check (the obvious-looking shortcut) silently misses it, and the count branch never fires.
        // So, use isEqualToString explicitly.
        bool wantsCount = [attribute isEqualToString:@"AXUIElementCountForSearchPredicate"];
        ...
        // Build flat depth-first list of all non-ignored elements within this element's subtree.
        ...
        for (int i = static_cast<int>(selfData->child_ids.size()) - 1; i >= 0; --i)
        ...
        while (!stack.is_empty()) {
            ...
            // Only descend into children of container elements. Leaf-like roles (links, buttons, headings) have their
        ...
        if (startEl && [startEl isKindOfClass:[LadybirdAccessibilityElement class]]) {
            ...
            for (NSInteger i = 0; i < static_cast<NSInteger>([allElements count]); i++) {
                id el = allElements[i];
        ...
        // When navigating forward past a container element, skip its descendants to avoid re-traversing them. In
        // pre-order DFS, descendants form a contiguous block right after their ancestor.
        NSInteger effectiveStart = startIdx;
        if (forward && startIdx >= 0
            && startEl && [startEl isKindOfClass:[LadybirdAccessibilityElement class]]) {
            i64 startID = ((LadybirdAccessibilityElement*)startEl).nodeID;
            for (NSInteger i = startIdx + 1; i < static_cast<NSInteger>([allElements count]); i++) {
                id el = allElements[i];
                if (![el isKindOfClass:[LadybirdAccessibilityElement class]])
                    break;
        ...
        if (forward) {
            for (NSInteger i = effectiveStart + 1;
                i < static_cast<NSInteger>([allElements count]) && (limit < 0 || static_cast<int>([results count]) < limit);
                i++)
                [results addObject:allElements[i]];
        } else {
            // With no start element (or one the tree no longer holds), startIdx is -1 and the search runs from the end,
            // as Blink (OneShotAccessibilityTreeSearch::SearchByIteratingOverChildren starts a BACKWARDS search at the
            // last child) and WebKit (AXSearchManager takes every child when no start object is given) do.
            NSInteger backwardStart = startIdx >= 0 ? startIdx - 1 : static_cast<NSInteger>([allElements count]) - 1;
            for (NSInteger i = backwardStart;
                i >= 0 && (limit < 0 || static_cast<int>([results count]) < limit);
                i--)
                [results addObject:allElements[i]];
        }
    ...
    if ([attribute isEqualToString:@"AXNextTextMarkerForTextMarker"]) {
        ...
        i32 text_len = codePointCount(node->name.bytes_as_string_view());
    ...
    if ([attribute isEqualToString:@"AXPreviousTextMarkerForTextMarker"]) {
        ...
        for (size_t i = 0; i < leaves.size(); i++) {
            if (leaves[i] == md.node_id && i > 0) {
                ...
                if (prev) {
                    i32 len = codePointCount(prev->name.bytes_as_string_view());
    ...
    if ([attribute isEqualToString:@"AXTextMarkerRangeForUIElement"]) {
        ...
        if (last_leaf != -1) {
            ...
            if (last)
                end_offset = codePointCount(last->name.bytes_as_string_view());
        }
    ...
    if ([attribute isEqualToString:@"AXStringForTextMarkerRange"]
        ...
        // The length answer counts code points, the unit every marker offset is in. [result length] would count UTF-16
        // code units instead, one more per character outside the BMP than the markers step through.
        i32 code_point_count = 0;
        auto leaves = _manager->text_leaves_in_order();
        bool collecting = false;

        // Append the [start_char, end_char) code-point sub-range of a UTF-8 leaf, converting to a byte range first.
        // Slicing on raw code-point offsets would split a multi-byte character (e.g. in "caf\u00e9" or an emoji) and
        // produce invalid UTF-8, making initWithBytes return nil and appendString: throw. The AT-supplied offsets may
        // be reversed or out of range, so order and clamp them; an omitted end means "to the end of the leaf".
        auto appendSlice = [result, &code_point_count](StringView text, i32 start_char, Optional<i32> end_char_opt = {}) {
            auto view = Utf8View { text };
            auto char_count = static_cast<i32>(view.length());
            auto start = clamp(start_char, 0, char_count);
            auto end = clamp(end_char_opt.value_or(char_count), 0, char_count);
            if (end < start)
                swap(start, end);
            auto byte_start = view.byte_offset_of(start);
            auto byte_end = view.byte_offset_of(end);
            auto slice = text.substring_view(byte_start, byte_end - byte_start);
            NSString* ns = [[NSString alloc] initWithBytes:slice.characters_without_null_termination()
                                                    length:slice.length()
                                                  encoding:NSUTF8StringEncoding];
            if (ns) {
        ...
        for (auto leaf_id : leaves) {
            ...
            if (leaf_id == startData.node_id && leaf_id == endData.node_id) {
                appendSlice(text, startData.offset, endData.offset);
                break;
            }

            if (leaf_id == startData.node_id) {
                collecting = true;
                appendSlice(text, startData.offset);
                continue;
            }

            if (leaf_id == endData.node_id) {
                appendSlice(text, 0, endData.offset);
                break;
            }
...
// The accessibility system may query via the "modern" property API rather than the "informal" protocol. These delegate
// to accessibilityAttributeValue.
...
{
    return [[self accessibilityAttributeValue:@"AXSelected"] boolValue];
}
...
{
    return [self accessibilityAttributeValue:NSAccessibilityURLAttribute];
}
