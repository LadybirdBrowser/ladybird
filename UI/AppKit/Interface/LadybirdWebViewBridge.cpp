...
{
    Gfx::Bitmap const* bitmap = backing_store().visible_bitmap();
    if (!bitmap)
        return {};

    return Paintable { *bitmap, backing_store().visible_bitmap_size(), backing_store().visible_bitmap_iosurface_ref() };
}
