...
unsafe fn window_from_ptr(
    ...
    unsafe { from_glib_borrow(window.cast::<glib::subclass::basic::InstanceStruct<imp::LadybirdBrowserWindow>>()) }
}

/// # Safety
///
/// `app` must be a valid pointer to an `AdwApplication`.
#[unsafe(no_mangle)]
...
/// # Safety
///
/// `window` must be a valid pointer to a `LadybirdBrowserWindow`.
#[unsafe(no_mangle)]
