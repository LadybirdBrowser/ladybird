...
# Note: VCPKG_TARGET_ANDROID is not an official vcpkg variable. it is introduced for the need of this script
...
if (VCPKG_TARGET_ANDROID)
    ...
    #
    # 1. Check the presence of environment variable VCPKG_ROOT
    #
    ...
    #
    # 1. Set VCPKG_TARGET_TRIPLET according to ANDROID_ABI
    #
    # There are four different Android ABI, each of which maps to a vcpkg triplet. The following table outlines the
    # mapping from vcpkg architectures to android architectures
    #
    # |VCPKG_TARGET_TRIPLET       | ANDROID_ABI          |
    # |---------------------------|----------------------|
    # |arm64-android              | arm64-v8a            | |arm-android                | armeabi-v7a          |
    # |x64-android                | x86_64               | |x86-android                | x86                  |
    #
    ...
    #
    # When using vcpkg, the vcpkg toolchain shall be specified first. However, vcpkg provides a way to preload and
    # additional toolchain, with the VCPKG_CHAINLOAD_TOOLCHAIN_FILE option.
    ...
    # vcpkg depends on the environment variables ANDROID_NDK_HOME and VCPKG_ROOT being set. However, we cannot set those
    # through the Android Gradle plugin (we can only set CMake variables). Therefore, we forward our CMake variables to
    # environment variables. FIXME: would be nice if vcpkg's android toolchain did not require this...
