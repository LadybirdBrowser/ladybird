...
ErrorOr<int> ladybird_main(Main::Arguments)
{
    ...
    auto client = TRY(create_client_from_fd(fds[0]));

    // Create one audio output session. AudioServer returns a shared memory ring
    // buffer that we write interleaved f32 frames into.
    auto session = TRY(client->create_audio_output_session(target_latency_ms));
