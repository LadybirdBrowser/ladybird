...
ErrorOr<int> ladybird_main(Main::Arguments)
{
    ...
    auto broker_socket = TRY(Core::LocalSocket::adopt_fd(fds[0]));
    auto broker_transport = make<IPC::Transport>(move(broker_socket));
    auto broker_client = adopt_ref(*new AudioServer::BrokerOfAudioServer(move(broker_transport)));

    auto response = TRY(broker_client->connect_new_client("*"sv, "*"sv, true));
    auto session_socket = TRY(Core::LocalSocket::adopt_fd(response.socket.take_fd()));
    auto session_transport = make<IPC::Transport>(move(session_socket));
    auto session_client = adopt_ref(*new AudioServer::SessionClientOfAudioServer(move(session_transport)));
    auto client = TRY(AudioServer::SingleSinkSessionClient::try_create(session_client));
    ...
    TRY(client->request_output_sink(
        [&](AudioServer::OutputSink const& session) {
