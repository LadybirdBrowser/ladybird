...
ErrorOr<int> ladybird_main(Main::Arguments)
{
    ...
    Vector<NonnullRefPtr<AudioServer::SessionClientOfAudioServer>> clients;
    auto first_response = TRY(broker_client->connect_new_client("*"sv, "*"sv, true));
    clients.append(TRY(create_client_from_fd(first_response.socket.take_fd())));
    ...
    auto responses = TRY(broker_client->connect_new_clients(move(requests)));
    ...
    for (auto& response : responses) {
        int fd = response.socket.take_fd();
    ...
    for (size_t i = 0; i < clients.size(); ++i) {
        ...
        clients[i]->on_output_sink_ready = [&](AudioServer::OutputSink session) {
        ...
        TRY(clients[i]->create_session(target_latency_ms, [&](u64 session_id) {
                if (!requested_session_id.has_value())
