...
ErrorOr<int> ladybird_main(Main::Arguments)
{
    ...
    ByteString server_name = ByteString::formatted("multiple-audioserver-example.{}", Core::System::getpid());
    ...
    Vector<NonnullRefPtr<Audio::SessionClientOfAudioServer>> clients;
    auto first_response = TRY(audio_server.broker->connect_new_client("*"sv, "*"sv, true));
    clients.append(TRY(create_session_client_for_example(first_response.handle)));

    Vector<Audio::CreateClientRequest> requests;
    requests.resize(15);
    for (auto& request : requests)
        request = { .origin = "*"sv, .top_level_origin = "*"sv, .can_use_mic = true };

    auto responses = TRY(audio_server.broker->connect_new_clients(move(requests)));
    if (responses.size() != 15)
        return Error::from_string_literal("ExampleAudioServerClient: connect_new_clients returned unexpected count");

    for (auto& response : responses) {
        clients.append(TRY(create_session_client_for_example(response.handle)));
    }

    struct Session {
        u32 sample_rate { 0 };
        u32 channel_count { 0 };
        Audio::SharedCircularBuffer ring;
        Instrument instrument { Instrument::Tone };
        double note_hz { 0.0 };
        double bpm { 120.0 };
        u16 rhythm_mask { 0 };
    ...
    for (size_t i = 0; i < clients.size(); ++i) {
        ...
        clients[i]->on_output_sink_ready = [&](Audio::OutputSink session) {
        ...
        clients[i]->on_output_sink_failed = [&](u64 session_id, ByteString error) {
        ...
        TRY(clients[i]->create_output_session(target_latency_ms, [&](u64 session_id) {
                if (!requested_session_id.has_value())
        ...
        if (maybe_session_error.has_value())
            return Error::from_string_literal("ExampleAudioServerClient: async audio output session creation failed");
