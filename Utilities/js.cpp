 ...
 */
 ...
 // Global state toggles for various features.
 ...
 // Helper to print JS::Value with optional ANSI stripping.
 ...
 // Overload for PrintTarget-based printing.
 ...
 // Tracks consecutive Ctrl+C presses.
 static size_t s_ctrl_c_hit_count = 0;
 
 /*
  * Helper that appends indentation for the REPL prompt
  * based on the nesting level (e.g., open brackets/braces).
  */
 static void append_repl_indentation(StringBuilder& builder, int level)
 {
 ...
 /*
  * Builds the REPL prompt string based on the current indentation
  * level, and the number of Ctrl+C presses so far.
  */
 static ErrorOr<String> prompt_for_level(int level)
 {
     ...
     append_repl_indentation(prompt_builder, level);
     return prompt_builder.to_string();
 }
 
 /*
  * Reads a chunk of input from the user in the REPL,
  * continuing until the bracket/brace/parenthesis levels
  * have balanced out or any label/object key is completed.
  */
 static ErrorOr<String> read_next_piece()
 {
     ...
     do {
         auto prompt = TRY(prompt_for_level(s_repl_line_level));
         auto line_result = s_editor->get_line(prompt.to_byte_string());
         ...
         // State machine to detect if the last token was a label or object literal key.
         enum {
             NotInLabelOrObjectKey,
             InLabelOrObjectKeyIdentifier,
             InLabelOrObjectKey
         } token_label_state { NotInLabelOrObjectKey };
         ...
         for (JS::Token token = lexer.next(); token.type() != JS::TokenType::Eof; token = lexer.next()) {
             ...
             case JS::TokenType::StringLiteral:
                 if (token_label_state == NotInLabelOrObjectKey)
                     token_label_state = InLabelOrObjectKeyIdentifier;
                 else
                     token_label_state = NotInLabelOrObjectKey;
                 break;
             case JS::TokenType::Colon:
                 if (token_label_state == InLabelOrObjectKeyIdentifier)
                     token_label_state = InLabelOrObjectKey;
                 else
                     token_label_state = NotInLabelOrObjectKey;
                 break;
             default:
         ...
         // If we ended with a label or object literal key, we keep reading more lines.
         if (token_label_state == InLabelOrObjectKey) {
 ...
 // Writes the REPL statements to a file, used by SAVE(...) in the REPL.
 ...
 /*
  * Parses and runs the provided source code in the given Realm.
  * Returns false if there was a runtime error (so we can exit with error).
  */
 static ErrorOr<bool> parse_and_run(JS::Realm& realm, StringView source, StringView source_name)
 {
     ...
     // This lambda runs either script or module, dumping AST if requested.
     ...
     // Parse as script or module depending on s_as_module.
     if (!s_as_module) {
         auto script_or_error_result = JS::Script::parse(source, realm, source_name);
         if (script_or_error_result.is_error()) {
             auto parse_error = script_or_error_result.error()[0];
             auto hint = parse_error.source_location_hint(source);
             if (!hint.is_empty())
                 outln("{}", hint);
 
             auto error_string = parse_error.to_string();
             outln("{}", error_string);
             result = vm.throw_completion<JS::SyntaxError>(move(error_string));
         } else {
             run_script_or_module(script_or_error_result.value());
         }
     } else {
         auto module_or_error_result = JS::SourceTextModule::parse(source, realm, source_name);
         if (module_or_error_result.is_error()) {
             auto parse_error = module_or_error_result.error()[0];
             auto hint = parse_error.source_location_hint(source);
             if (!hint.is_empty())
                 outln("{}", hint);
             ...
             run_script_or_module(module_or_error_result.value());
         }
     }
 
     // Handle exceptions from execution if any.
     ...
     // Optional printing of the last result for interactive REPL usage.
 ...
 /*
  * load_ini_impl and load_json_impl keep the same error strings,
  * only restructured the local variable naming for clarity.
  */
 static JS::ThrowCompletionOr<JS::Value> load_ini_impl(JS::VM& vm)
 {
     auto& realm = *vm.current_realm();
     auto filename = TRY(vm.argument(0).to_byte_string(vm));
     auto file_open_or_error = Core::File::open(filename, Core::File::OpenMode::Read);
     if (file_open_or_error.is_error())
         return vm.throw_completion<JS::Error>(TRY_OR_THROW_OOM(vm, String::formatted("Failed to open '{}': {}", filename, file_open_or_error.error())));
 
     auto config_file = MUST(Core::ConfigFile::open(filename, file_open_or_error.release_value()));
 ...
 static JS::ThrowCompletionOr<JS::Value> load_json_impl(JS::VM& vm)
 {
     ...
     auto file_contents_or_error = file_open_or_error.value()->read_until_eof();
 ...
 // REPL object initialization, binding all relevant globals and functions.
 void ReplObject::initialize(JS::Realm& realm)
 {
     ...
     define_native_accessor(
         ...
         [](JS::VM& vm) -> JS::ThrowCompletionOr<JS::Value> {
             ...
             // We must delete it first; otherwise, this setter recurses infinitely.
 ...
 {
     auto print_result = ::print(vm.argument(0));
     if (print_result.is_error()) {
         return g_vm->throw_completion<JS::InternalError>(
             TRY_OR_THROW_OOM(*g_vm, String::formatted("Failed to print value: {}", print_result.error())));
     }
 ...
 /*
  * Primary REPL loop, reads input using read_next_piece() until the user
  * decides to exit (e.g., calling exit() or pressing Ctrl+C twice).
  */
 ...
 /*
  * Console client for the REPL, implements printing, grouping, etc.
  */
 ...
 public:
     ...
     // 2.3. Printer(logLevel, args[, options]) from console spec
 ...
 /*
  * Entry point for the entire "js" program: sets up VM, REPL or script,
  * handles arguments, etc.
  */
 ErrorOr<int> serenity_main(Main::Arguments arguments)
 {
     ...
     // If no scripts to run and no -c option, start the REPL.
     if (evaluate_script.is_empty() && script_paths.is_empty()) {
         ...
         // Syntax highlighting for the REPL line input.
         s_editor->on_display_refresh = [syntax_highlight](Line::Editor& editor) {
             ...
             for (auto token = lexer.next(); token.type() != JS::TokenType::Eof; token = lexer.next()) {
                 ...
                 if (indenters_starting_line) {
                     if (token.type() != JS::TokenType::ParenClose
                         && token.type() != JS::TokenType::BracketClose
                         && token.type() != JS::TokenType::CurlyClose) {
         ...
         auto complete = [&realm, &global_environment](Line::Editor const& editor) -> Vector<Line::CompletionSuggestion> {
             ...
             for (; js_token.type() != JS::TokenType::Eof; js_token = lexer.next()) {
                 ...
                 case CompleteProperty:
                     // Once we've started property completion, if we see any more tokens, break out.
                     break;
                 case Initial:
             ...
             // Helper to list properties from an object's shape and prototype chain.
             Function<void(JS::Shape const&, StringView)> list_all_properties = [&results, &list_all_properties](JS::Shape const& shape, auto property_pattern) {
                 for (auto const& descriptor : shape.property_table()) {
                     ...
                     if (key.view().starts_with(property_pattern)) {
                         Line::CompletionSuggestion suggestion { key, Line::CompletionSuggestion::ForSearch };
                         if (!results.contains_slow(suggestion)) {
             ...
             case CompleteProperty: {
                 ...
                 auto variable_value = value_or_error.value();
                 if (!variable_value.is_object())
                     break;
 
                 auto* object = MUST(variable_value.to_object(*g_vm));
                 list_all_properties(object->shape(), property_name);
                 break;
             }
             case CompleteVariable: {
                 auto& root_object = realm.global_object();
                 list_all_properties(root_object.shape(), variable_name);
 
                 for (auto const& name : global_environment.declarative_record().bindings()) {
                     if (name.starts_with(variable_name)) {
                         Line::CompletionSuggestion suggestion { name, Line::CompletionSuggestion::ForSearch };
                         suggestion.invariant_offset = variable_name.length();
                         results.append(suggestion);
                     }
                 }
                 break;
         ...
         if (evaluate_script.is_empty()) {
             ...
             // Evaluate passed-in script.
             builder.append(evaluate_script);
             source_name = "eval"sv;
         }
 
         // Parse and run; error exit if parse_and_run fails.
