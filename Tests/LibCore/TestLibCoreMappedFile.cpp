...
TEST_CASE(mapped_file_open)
{
    ...
    {
        auto maybe_file = Core::File::open(TRY_OR_FAIL(temp_file_path("file-open-test.txt"sv)), Core::File::OpenMode::Write);
    ...
    auto maybe_file = Core::MappedFile::map(TRY_OR_FAIL(temp_file_path("file-open-test.txt"sv)), Core::MappedFile::Mode::ReadWrite);
    if (maybe_file.is_error()) {
        warnln("Failed to open the file: {}", strerror(maybe_file.error().code()));
        VERIFY_NOT_REACHED();
    }
