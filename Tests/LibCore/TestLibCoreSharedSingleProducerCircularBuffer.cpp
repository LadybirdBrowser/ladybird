...
TEST_CASE(shared_single_producer_circular_buffer_two_threads_ordered_u32)
{
    ...
    IGNORE_USE_IN_ESCAPING_LAMBDA auto buffer = buffer_or_error.release_value();

    IGNORE_USE_IN_ESCAPING_LAMBDA Atomic<bool> done { false };
    IGNORE_USE_IN_ESCAPING_LAMBDA Atomic<bool> failed { false };

    constexpr u32 iterations = 50'000;

    auto consumer = MUST(Threading::Thread::try_create("SPCB consumer"sv, [&]() -> intptr_t {
        u32 expected = 0;
    ...
    auto producer = MUST(Threading::Thread::try_create("SPCB producer"sv, [&]() -> intptr_t {
