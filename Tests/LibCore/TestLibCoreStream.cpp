...
{
    auto file = TRY_OR_FAIL(Core::File::open(TRY_OR_FAIL(temp_file_path("file-write-bytes-test.txt"sv)), Core::File::OpenMode::Write));
...
{
    auto file = TRY_OR_FAIL(Core::OutputBufferedFile::create(TRY_OR_FAIL(Core::File::open(TRY_OR_FAIL(temp_file_path("file-buffered-write-test.txt"sv)), Core::File::OpenMode::Truncate | Core::File::OpenMode::ReadWrite))));
...
{
    auto file = TRY_OR_FAIL(Core::File::open(TRY_OR_FAIL(temp_file_path("file-truncate-test.txt"sv)), Core::File::OpenMode::Write));
...
TEST_CASE(local_socket_read)
{
    ...
    auto socket_path = TRY_OR_FAIL(temp_file_path("test-socket"sv)).to_byte_string();
...
TEST_CASE(local_socket_write)
{
    ...
    local_server->on_accept = [&](NonnullOwnPtr<Core::LocalSocket> server_socket) {
        ...
        [[maybe_unused]] auto read_bytes = TRY_OR_FAIL(server_socket->read_some(receive_buffer));
...
{
    auto const filename = TRY_OR_FAIL(temp_file_path("file-without-newlines"sv));
