...
TEST_CASE(test_bigint_shift_left)
{
    ...
    std::pair<size_t, Vector<u32>> results[] = {
    ...
    for (size_t i = 0; i < tests; ++i)
        EXPECT_EQ(num.shift_left(results[i].first).words(), results[i].second);
}
...
TEST_CASE(test_bigint_shift_right)
{
    ...
    std::pair<size_t, Vector<u32>> results1[] = {
    ...
    std::pair<size_t, Vector<u32>> results2[] = {
        { 1, { 0xa2222222, 0x7fffffff } },
        { 2, { 0xd1111111, 0x3fffffff } },
    };

    for (size_t i = 0; i < tests1; ++i)
        EXPECT_EQ(num1.shift_right(results1[i].first).words(), results1[i].second);

    for (size_t i = 0; i < tests2; ++i)
        EXPECT_EQ(num2.shift_right(results2[i].first).words(), results2[i].second);
}
