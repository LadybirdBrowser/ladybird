...
TEST_CASE(test_hmac_md5_name)
{
    ...
    EXPECT_EQ(hmac.class_name(), "HMAC-MD5"_sv);
}
...
TEST_CASE(test_hmac_md5_process)
{
    ...
    auto mac = hmac.process("Some bogus data"_sv);
...
TEST_CASE(test_hmac_md5_process_reuse)
{
    ...
    auto mac_0 = hmac.process("Some bogus data"_sv);
    auto mac_1 = hmac.process("Some bogus data"_sv);
...
TEST_CASE(test_hmac_sha1_name)
{
    ...
    EXPECT_EQ(hmac.class_name(), "HMAC-SHA1"_sv);
}
...
TEST_CASE(test_hmac_sha256_name)
{
    ...
    EXPECT_EQ(hmac.class_name(), "HMAC-SHA256"_sv);
}
...
TEST_CASE(test_hmac_sha256_data_is_same_size_as_block)
{
    ...
    auto mac = hmac.process("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"_sv);
    EXPECT(memcmp(result, mac.data(), hmac.digest_size()) == 0);
}
...
TEST_CASE(test_hmac_sha256_data_is_bigger_size_as_block)
{
    ...
    auto mac = hmac.process("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"_sv);
    EXPECT(memcmp(result, mac.data(), hmac.digest_size()) == 0);
}
