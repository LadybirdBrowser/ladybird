...
TEST_CASE(bitap)
{
    ...
    auto const* result_0 = AK::memmem(haystack.data(), haystack.size(), needle_0.data(), needle_0.size());
    auto const* result_1 = AK::memmem(haystack.data(), haystack.size(), needle_1.data(), needle_1.size());
    auto const* result_2 = AK::memmem(haystack.data(), haystack.size(), needle_2.data(), needle_2.size());
    auto const* result_3 = AK::memmem(haystack.data(), haystack.size(), needle_3.data(), needle_3.size());
    ...
    auto const* result = AK::Detail::bitap_bitwise(haystack_string.characters_without_null_termination(), haystack_string.length(), needle_string.characters_without_null_termination(), needle_string.length());
...
{
    Array<u8, 4> haystack_first_half { 1, 0, 1, 2 };
    Array<u8, 4> haystack_second_half { 3, 4, 5, 0 };
