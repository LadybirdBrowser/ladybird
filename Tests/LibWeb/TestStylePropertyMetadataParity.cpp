...
TEST_CASE(inherited_flags_match)
{
    for (auto i = to_underlying(first_longhand_property_id); i <= to_underlying(last_longhand_property_id); ++i)
        EXPECT_EQ(ComputedValuesFFI::rust_property_metadata_is_inherited(i), is_inherited_property(static_cast<PropertyID>(i)));
}

TEST_CASE(computation_order_matches)
{
    size_t length = 0;
    auto const* order = ComputedValuesFFI::rust_property_metadata_computation_order(&length);
        ...
        for (u8 writing_mode = 0; writing_mode < 5; ++writing_mode) {
            for (u8 direction = 0; direction < 2; ++direction) {
                auto expected = property_is_logical_alias(property_id)
                    ...
                    : property_id;
                EXPECT_EQ(ComputedValuesFFI::rust_map_logical_alias_to_physical(i, writing_mode, direction), to_underlying(expected));
            }
        }
    }
}
    ...
    for (auto i = to_underlying(first_longhand_property_id); i <= to_underlying(last_longhand_property_id); ++i) {
        ...
        for (u8 writing_mode = 0; writing_mode < 5; ++writing_mode) {
            for (u8 direction = 0; direction < 2; ++direction) {
                ...
                EXPECT_EQ(ComputedValuesFFI::rust_map_physical_to_logical_alias(i, writing_mode, direction), to_underlying(expected));
            }
        }
    }
}
...
TEST_CASE(shorthand_expansions_match)
{
    for (auto i = to_underlying(first_property_id); i <= to_underlying(last_property_id); ++i) {
        auto property_id = static_cast<PropertyID>(i);
        EXPECT_EQ(ComputedValuesFFI::rust_property_metadata_is_shorthand(i), property_is_shorthand(property_id));
        if (!property_is_shorthand(property_id))
            continue;
        size_t length = 0;
        auto const* longhands = ComputedValuesFFI::rust_property_metadata_longhands_for_shorthand(i, &length);
...
TEST_CASE(initial_value_table_matches)
{
    ...
    for (auto i = to_underlying(first_longhand_property_id); i <= to_underlying(last_longhand_property_id); ++i) {
        ...
        EXPECT_EQ(data, initial_value->rust_style_value_data());
    }
}
...
TEST_CASE(accepted_numeric_ranges_match)
{
    for (auto i = to_underlying(first_longhand_property_id); i <= to_underlying(last_longhand_property_id); ++i) {
        ...
        size_t length = 0;
        auto const* ranges = ComputedValuesFFI::rust_property_metadata_numeric_ranges(i, &length);
