    ...
    def test_every_role_exposes_tag(self):
        ...
        def visit(obj, _depth):
            try:
                ...
                # Do not swallow accessor failures: a genuinely broken node must surface, not be dropped.
        ...
        walk(self.doc, visit)
        self.assertEqual(errors, [], f"errors while inspecting nodes: {errors[:5]}")
...
class XmlRolesAttributeTests(AccessibilityBridgeTestCase):
    ...
    def _all_xml_roles(self):
        ...
        def visit(obj, _depth):
            # Do not swallow accessor failures: a node whose xml-roles can't be read must surface, not be dropped.
            attrs = get_attributes_dict(obj)
...
class TreeRowLevelTests(AccessibilityBridgeTestCase):
    """aria-level on a row is the row's depth. It reaches Orca as the "level" object attribute, like a heading's level
    (grid.html's treegrid rows carry aria-level 1 and 2), and never as the row's value."""

    FIXTURE = "grid.html"

    def test_treegrid_rows_expose_their_level(self):
        # Wait for the deepest row, so the collection below can't run against a tree that's still filling in.
        row = wait_for_descendant_by_role(
            self.doc, Atspi.Role.TABLE_ROW, pred=lambda obj: get_attributes_dict(obj).get("level") == "2"
        )
        self.assertIsNotNone(row, "grid.html must expose the level-2 treegrid row")
        rows = find_all_by_role(self.doc, Atspi.Role.TABLE_ROW)
        levels = sorted(level for level in (get_attributes_dict(row).get("level") for row in rows) if level)
        self.assertEqual(levels, ["1", "2"], f"the treegrid rows' levels are off; rows: {len(rows)}")

    def test_cells_have_no_level(self):
        cell = wait_for_descendant_by_role(self.doc, Atspi.Role.TABLE_CELL, pred=lambda obj: obj.get_name() == "file")
        self.assertIsNotNone(cell, "grid.html must expose the treegrid's file cell, the one with the stray aria-level")
        cells = find_all_by_role(self.doc, Atspi.Role.TABLE_CELL)
        self.assertTrue(cells, "grid.html must expose its cells")
        for cell in cells:
            self.assertNotIn("level", get_attributes_dict(cell))
