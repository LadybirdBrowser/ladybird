...
class TreeShapeTests(AccessibilityBridgeTestCase):
    ...
    def test_unnamed_div_is_hidden_but_child_is_reachable(self):
        ...
        for p in paragraphs:
            try:
                ...
                # Only an AT-SPI lookup failure means "try the next paragraph". Letting AssertionError through keeps a
                # real parent-role regression reporting the role it actually saw.
                continue
    ...
    def test_role_presentation_promotes_children(self):
        ...
        # Existing somewhere in the tree is not the claim: role="none" must remove the div itself, so the paragraph's
        # parent is the document. Asserting only that the text appears would pass with the div still exposed and still
        # wrapping it, which is the exact regression this test is named for.
...
class WhitespaceTextTests(AccessibilityBridgeTestCase):
    """Rendered whitespace between inline siblings stays in the text.

    inline_whitespace.html puts a space between two links and between two styled words. The paragraph's text keeps each
    space — it's what tells Orca the words apart, as it does in Gecko and Blink — while the tree carries no whitespace
    that renders nothing, such as a line's leading or trailing whitespace (Node::build_accessibility_tree)."""

    FIXTURE = "inline_whitespace.html"

    def test_paragraph_text_keeps_the_rendered_spaces(self):
        paragraph = wait_for_descendant_by_role(self.doc, "paragraph")
        self.assertIsNotNone(paragraph, "inline_whitespace.html must expose its paragraph")
        text = Atspi.Text.get_text(paragraph, 0, -1)
        marker = chr(0xFFFC)
        self.assertTrue(
            text.startswith(marker + " " + marker + " and "),
            f"the space between the links or after them is gone from the paragraph's text: {text!r}",
        )
        self.assertIn("bold italic", text, f"the space between the styled words is gone: {text!r}")
