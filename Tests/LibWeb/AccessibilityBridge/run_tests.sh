...
# Initialize the markers we own so an inherited value can't make the EXIT trap act on something we didn't start.
XVFB_PID=""

if [[ ! -S "${XVFB_SOCKET}" ]]; then
    XVFB_LOG="$(mktemp "${TMPDIR:-/tmp}/ladybird-xvfb.XXXXXX")"
