...
class SayAllSentenceContentsTests(LadybirdOrcaTestCase):
    ...
    def test_paragraph_with_one_link_contents_include_pre_and_post_prose(self):
        ...
        self.assertIn("one", joined, f"link text 'one' missing from sentence contents: {strings!r}")
    ...
    def test_paragraph_with_two_links_reads_in_dom_order(self):
        ...
        # The segments must appear in DOM order: pre-prose, first link, between-prose, second link, post-prose.
        order = ["Paragraph with", "first", "and", "second", "links."]
        positions = [joined.find(seg) for seg in order]
        self.assertNotIn(-1, positions, f"missing segment {list(zip(order, positions))} in {strings!r}")
        self.assertEqual(positions, sorted(positions), f"segments out of DOM order: {strings!r}")

        # Confirm Orca sees exactly the two named link objects, in order, so they're announced with the 'link' role.
        link_texts = [t for (obj, _s, _e, t) in contents if obj.get_role_name() == "link"]
        self.assertEqual(
            [t.strip() for t in link_texts],
            ["first", "second"],
            f"expected the two named links in order; got {link_texts!r}",
        )
