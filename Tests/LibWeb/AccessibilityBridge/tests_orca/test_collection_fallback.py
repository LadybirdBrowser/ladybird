...
class CollectionFallbackTests(LadybirdOrcaTestCase):
    ...
    def test_find_all_with_role_returns_links(self):
        ...
        self.assertEqual(len(results), 2, "expected exactly the two links from roles.html, not a partial result")
    ...
    def test_find_all_list_items_via_collection(self):
        ...
        self.assertEqual(len(results), 1, "expected exactly the one list item from roles.html, not a partial result")
