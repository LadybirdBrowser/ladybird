...
code paths. Here we can catch bugs where the bridge output is technically correct but the end-to-end speech is still
wrong for some reason. Only a test harness that drives Orca's _generate_list_item → speech.speak chain can see that.
...
def _initialize_orca_once() -> None:
    ...
    # Orca's internal modules have circular imports that only resolve cleanly when script_manager is imported first. So,
    # import that one before anything else.
    try:
...
def _load_ladybird_script_class():
    ...
    # First load the script_utilities submodule, then "script" — which imports it via a relative "from .script_utilities
    # import Utilities". Register both under a package name so the relative import resolves.
    package_name = "ladybird_orca_script_under_test"
    ladybird_dir = LADYBIRD_ORCA_SCRIPTS_SRC / "Ladybird"

    # Pre-create the package by registering a synthetic module whose __path__ is the directory — then the relative
...
def load_ladybird_script_utilities_module():
    """Import the Ladybird script's script_utilities module, for unit tests of its logic that need neither a running
    Ladybird nor an Orca session. Raises OrcaNotInstalled when Orca isn't importable, like the session setup does."""
    _load_ladybird_script_class()
    return sys.modules["ladybird_orca_script_under_test.script_utilities"]


class OrcaSession:
    """Wraps the setup/teardown of an Orca-in-process test session.

    Each test method should get a fresh OrcaSession - so captured speech doesn't leak between tests. Speech is monkey-
    patched on start(), and restored on stop()."""
    ...
    def __enter__(self) -> "OrcaSession":
        try:
            ...
            # start() registers a process-global active script partway through; if a later step raises, __exit__ won't
            # run (entry failed). So, clean up here — or the next test inherits stale Orca state.
            self.stop()
            raise
        return self
    ...
    def start(self) -> None:
        ...
        # → speech_presenter.speak_contents — which early-returns when script_manager has no active script
        # registered. Real Orca registers the active script via focus events — but our isolated test never fires
        # one. So, we register it directly.
