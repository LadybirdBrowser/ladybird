...
# Prefer the enum form for any role whose localized name has changed across libatspi releases. For example,
# Atspi.Role.PUSH_BUTTON is rendered as "push button" by Ubuntu noble's libatspi but as "button" by Fedora
# 43's newer libatspi. Comparing against the enum is version-stable; comparing against the string isn't.
...
def wait_for_all_by_role(
    ...
    *,
    ...
    """find_all_by_role once at least one match is on the bus, for the same reason wait_for_descendant_by_role polls: a
    one-shot find_all_by_role over a tree that is still populating comes back empty, and a loop over it checks nothing.
    Returns [] when nothing matched within the timeout."""
    if wait_for_descendant_by_role(root, role, pred=pred, timeout=timeout, interval=interval) is None:
        return []
    return find_all_by_role(root, role, pred=pred)
