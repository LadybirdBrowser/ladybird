...
class LadybirdContext:
    ...
    def __init__(self, url: str, binary: Optional[pathlib.Path] = None, *, accessibility_at_launch: bool = True):
        self._url = url
        self._accessibility_at_launch = accessibility_at_launch
    ...
    def start(self) -> None:
        # Guard both entry paths — the 'with' block and setUpClass's direct start() call: if resource acquisition raises
        # after the temp file or process is created, neither __exit__ nor tearDownClass runs — so, clean up here.
        try:
            self._start_impl()
    ...
    def _start_impl(self) -> None:
        ...
        # environment variable tells Qt to skip the IsEnabled check, and bring the bridge up unconditionally. A test of
        # what happens when an assistive technology turns up later launches without it, and sets IsEnabled itself.
        if self._accessibility_at_launch:
        ...
        # --force-cpu-painting: skip the GPU/Vulkan path. Headless CI runners have no Vulkan ICD, and the CPU backend is
        # also what other test harnesses use, per Services/WebContent/main.cpp ("the GPU backend is not deterministic").
        # The Vulkan probe failure itself is non-fatal — we just don't want it cluttering diagnostics.
        # --temporary-profile: a throwaway profile of its own. Without one, the launch hands its URL to any browser
        # already running on the default profile and exits, and the tests would read (and write) whoever's real profile.
        self._process = subprocess.Popen(
            [str(self._binary), "--force-cpu-painting", "--temporary-profile", self._url],
            env=env,
        ...
        # Without accessibility, Ladybird doesn't register on the bus at all; see wait_for_app().
        if not self._accessibility_at_launch:
            return
    ...
    def wait_for_app(self, timeout: float = 20.0) -> Optional[Atspi.Accessible]:
        """For a context started without accessibility: Waits for Ladybird to register on the AT-SPI2 desktop once
        something has turned accessibility on, and returns its app (None on timeout)."""
        app = wait_for(self._find_app_on_desktop, timeout=timeout, description="Ladybird AT-SPI2 app to appear")
        if app:
            self._app = app
            return app
        return None

    def find_app_on_desktop(self) -> Optional[Atspi.Accessible]:
        """Ladybird's app on the AT-SPI2 desktop right now, or None while it isn't registered there."""
        return self._find_app_on_desktop()
