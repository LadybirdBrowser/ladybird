...
class LadybirdContext:
    ...
    def __init__(self, url: str, binary: Optional[pathlib.Path] = None, *, wait_for_web_area: bool = True):
        self._url = url
        self._wait_for_web_area = wait_for_web_area
    ...
    def start(self) -> None:
        # Guard both entry paths — the 'with' block, and setUpClass's direct start() call: If resource acquisition
        # raises after the temp file or process is created, neither __exit__ nor tearDownClass runs — so, clean up here.
        try:
            self._start_impl()
    ...
    def _start_impl(self) -> None:
        ...
        # A test that decides for itself when an assistive technology first reads the page waits only for a window —
        # which AppKit answers without asking the web view anything.
        if not self._wait_for_web_area:
            if not wait_for(self.window, timeout=startup_timeout, description="a Ladybird window"):
                raise RuntimeError(
                    f"Ladybird showed no window on NSAccessibility within {startup_timeout:g}s.\n"
                    f"URL: {self._url}\nBinary: {self._binary}\nAX surface at timeout:\n{self._describe_ax_state()}"
                )
            return
    ...
    def window(self):
        """The application's focused window, or else its main window. AppKit answers this without the web view."""
        return _ax_attr(self._app, kAXFocusedWindowAttribute) or _ax_attr(self._app, kAXMainWindowAttribute)
    ...
    @property
    def pid(self) -> int:
        """The Ladybird process id. Raises if start() hasn't completed."""
        if self._process is None:
            raise RuntimeError("Ladybird context not started. Call start() or use as context manager.")
        return self._process.pid

    def find_web_area(self):
        """A fresh lookup of the AXWebArea from the application's window, bypassing the handle start() cached.

        The cached handle is what every test normally wants. This is for tests that check whether an element handle
        survives a tree update: they need a second, independent lookup to compare it against."""
        return self._find_web_area()
