#!/usr/bin/env python3
"""Diagnostic probe for the macOS AX tests (mainly intended for troubleshooting CI environment problems).

Launches Ladybird on the roles.html fixture exactly the way the test harness does, then records (from outside the app)
everything needed to tell *why* an AXWebArea did or did not appear:
...
The probe is diagnostic only: every phase tolerates failure, and the process always exits 0. And it runs on healthy
environments too — so you can diff, line-by-line, a failing environment's output against a healthy environment's.
...
class Probe:
    ...
    def headless_engine_check(self):
        ...
        started = time.monotonic()
        # The run gets a session of its own, so a timeout kills exactly that process group: the headless UI process and
        # the helpers it spawned. A pkill by bundle path would also take down whichever Ladybird a developer has open,
        # and another job's on a shared runner.
        proc = subprocess.Popen(argv, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, start_new_session=True)
        try:
            out, err = proc.communicate(timeout=self.headless_timeout)
            rc = proc.returncode
        except subprocess.TimeoutExpired:
            rc = None
            try:
