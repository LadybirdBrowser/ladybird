...
class AccessibilityFocusedUIElementStableTests(AccessibilityBridgeMacTestCase):
    ...
    def test_focused_ui_element_is_stable_across_reads(self):
        ...
        # No focused element at all likely means the page has no focusable elements; that's fine, not a regression.
        if first is None:
            return
        for _ in range(4):
...
class ColspanTableColumnCountTests(AccessibilityBridgeMacTestCase):
    """Symptom (latent, found by review): a table whose every row holds a spanning cell reported fewer columns thru
    AXColumnCount than it has — two for a three-column table whose rows each hold two cells.

    Root cause: the wrapper answered AXColumnCount with the widest row's cell count, one column per cell, ignoring
    colspan.

    Fix: answer with the colspan-aware column count WebContent serializes for every table, and keep the cell count only
    as a fallback for a table that comes without one."""

    FIXTURE = "table_colspan_every_row.html"

    def test_column_count_counts_spanned_columns(self):
        tables = find_all_by_role(self.web, "AXTable")
        self.assertEqual(len(tables), 1, "the fixture holds one table")
        self.assertEqual(_ax_attr(tables[0], "AXColumnCount"), 3)
        # Control: the row count has nothing to do with colspan.
        self.assertEqual(_ax_attr(tables[0], "AXRowCount"), 2)
...
def _rect(obj):
    """(x, y, width, height) of obj in screen points, read from its AXPosition and AXSize (as test_display_contents.py
    reads them)."""
...
class TextLeafFrameIsItsOwnBoxTests(AccessibilityBridgeMacTestCase):
    """Symptom: VoiceOver's cursor outline for a short static text spanned its whole paragraph; every AXStaticText
    reported its parent element's AXPosition and AXSize.

    Root cause: A text node has no layout box of its own, so the tree handed a text leaf the bounding client rect of
    its parent element.

    Fix: A text leaf's bounds are the union of its rendered lines, in the viewport space every element's box is in —
    the box Gecko, WebKit and Blink give a text leaf."""

    FIXTURE = "paragraphs.html"

    def test_short_static_text_is_narrower_than_its_paragraph(self):
        leaves = find_all_by_role(self.web, "AXStaticText")
        plain = next(
            (leaf for leaf in leaves if str(_ax_attr(leaf, kAXValueAttribute)).startswith("Plain paragraph")), None
        )
        self.assertIsNotNone(plain, "expected the 'Plain paragraph' AXStaticText")
        parent = _ax_attr(plain, kAXParentAttribute)
        self.assertIsNotNone(parent)
        leaf_rect = _rect(plain)
        parent_rect = _rect(parent)
        self.assertIsNotNone(leaf_rect, "AXStaticText must have an AXPosition and an AXSize")
        self.assertIsNotNone(parent_rect, "its parent must have an AXPosition and an AXSize")
        self.assertLess(leaf_rect[2], parent_rect[2], f"leaf {leaf_rect} is as wide as its parent {parent_rect}")
        self.assertGreaterEqual(
            leaf_rect[0], parent_rect[0], f"leaf {leaf_rect} starts left of its parent {parent_rect}"
        )
        self.assertLessEqual(
            leaf_rect[1] + leaf_rect[3],
            parent_rect[1] + parent_rect[3],
            f"leaf {leaf_rect} ends below its parent {parent_rect}",
        )
...
class TreeUpdatesUnderContinuousMutationTests(AccessibilityBridgeMacTestCase):
    ...
    def _status_text(self):
        def is_status(obj):
            value = _ax_attr(obj, kAXValueAttribute)
            return isinstance(value, str) and value.startswith("Tick ")

        status = wait_for_descendant_by_role(self.web, "AXStaticText", pred=is_status)
        self.assertIsNotNone(status, "ticker.html must expose its status text")
        return _ax_attr(status, kAXValueAttribute)
    ...
    def test_status_text_keeps_changing(self):
        ...
        # so focus the page's link first, as VoiceOver does when it lands on a page.
        link = wait_for_descendant_by_role(self.web, "AXLink")
        self.assertIsNotNone(link, "ticker.html must expose its link")
        err = AXUIElementSetAttributeValue(link, kAXFocusedAttribute, True)
        self.assertEqual(err, 0, "setting AXFocused on the link failed")
        self.assertTrue(
            wait_for(lambda: self._status_text() != initial, timeout=5.0),
            f"the tree never updated after the AT's action: the status text still reads {initial!r}",
        )
        ...
        self.assertNotEqual(first, second, f"the tree stopped updating: the status text still reads {first!r}")
