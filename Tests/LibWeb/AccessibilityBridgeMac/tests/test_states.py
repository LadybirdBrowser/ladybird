...
class CheckedValueTests(AccessibilityBridgeMacTestCase):
    """A checkable control's AXValue is its checked state as 0, 1 or 2 — what WebKit, Blink and Gecko expose, and what
    VoiceOver reads as the state."""

    FIXTURE = "states.html"

    def _checkbox(self, title):
        cb = wait_for_descendant_by_role(self.web, "AXCheckBox", title=title)
        self.assertIsNotNone(cb, f"expected the {title!r} checkbox in the tree")
        return cb

    def test_unchecked_checkbox_value_is_0(self):
        self.assertEqual(_ax_attr(self._checkbox("Checkbox"), kAXValueAttribute), 0)

    def test_checked_checkbox_value_is_1(self):
        self.assertEqual(_ax_attr(self._checkbox("Checked checkbox"), kAXValueAttribute), 1)

    def test_mixed_checkbox_value_is_2(self):
        self.assertEqual(_ax_attr(self._checkbox("Mixed checkbox"), kAXValueAttribute), 2)

    def test_unchecked_radio_value_is_0(self):
        radio = wait_for_descendant_by_role(self.web, "AXRadioButton", title="Radio")
        self.assertIsNotNone(radio, "expected the 'Radio' input in the tree")
        self.assertEqual(_ax_attr(radio, kAXValueAttribute), 0)

    def test_text_input_value_is_not_a_checked_state(self):
        entry = wait_for_descendant_by_role(self.web, "AXTextField", title="Text field")
        self.assertIsNotNone(entry, "expected the 'Text field' input in the tree")
        self.assertIn(_ax_attr(entry, kAXValueAttribute), (None, ""))


class SelectedStateTests(AccessibilityBridgeMacTestCase):
    """AXSelected follows the node's selection, as WebKit, Blink and Gecko report it; a tab, an AXRadioButton, carries
    the selection as its AXValue too, as Blink's AXValue does for a tab."""

    FIXTURE = "states.html"

    def _tab(self, title):
        tab = wait_for_descendant_by_role(self.web, "AXRadioButton", title=title)
        self.assertIsNotNone(tab, f"expected the {title!r} tab in the tree")
        return tab

    def test_selected_tab_is_selected(self):
        tab = self._tab("Selected tab")
        self.assertEqual(_ax_attr(tab, "AXSelected"), True)
        self.assertEqual(_ax_attr(tab, kAXValueAttribute), 1)

    def test_other_tab_is_not_selected(self):
        tab = self._tab("Other tab")
        self.assertEqual(_ax_attr(tab, "AXSelected"), False)
        self.assertEqual(_ax_attr(tab, kAXValueAttribute), 0)
