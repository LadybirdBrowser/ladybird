...
class LandmarkSubroleTests(AccessibilityBridgeMacTestCase):
    ...
    def test_unnamed_section_does_not_create_landmark(self):
        ...
        landmark. Only a named <section> becomes an AXLandmarkRegion, so every region must carry a title; a
        title-less region would mean an unnamed section leaked into the tree."""
        regions = _find_by_subrole(self.web, "AXLandmarkRegion")
        for region in regions:
            title = _ax_attr(region, kAXTitleAttribute)
            self.assertTrue(
                title,
                f"every AXLandmarkRegion must come from a named <section>; found one without a title: {title!r}",
            )
...
class PasswordFieldTests(AccessibilityBridgeMacTestCase):
    """<input type=password> is an AXTextField with the AXSecureTextField subrole, and its value masked.

    HTML-AAM gives a password input no ARIA role, so the tree's role is empty; the AppKit bridge exposes it as a text
    field with the secure subrole, as WebKit, Blink and Gecko do. Its value is the mask the field renders (one U+25CF
    per character), never the password."""

    FIXTURE = "password.html"

    def test_password_field_is_a_secure_text_field(self):
        password = wait_for_descendant_by_role(self.web, "AXTextField", title="Password")
        self.assertIsNotNone(password, "the password input isn't exposed as an AXTextField")
        self.assertEqual(_ax_attr(password, kAXSubroleAttribute), "AXSecureTextField")
        self.assertEqual(_ax_attr(password, kAXRoleDescriptionAttribute), "secure text field")
        # Control: the text input beside it is a plain text field.
        username = wait_for_descendant_by_role(self.web, "AXTextField", title="Username")
        self.assertIsNotNone(username, "the text input isn't exposed as an AXTextField")
        self.assertIsNone(_ax_attr(username, kAXSubroleAttribute))

    def test_password_field_value_is_masked(self):
        password = wait_for_descendant_by_role(self.web, "AXTextField", title="Password")
        self.assertIsNotNone(password, "the password input isn't exposed as an AXTextField")
        self.assertEqual(_ax_attr(password, kAXValueAttribute), chr(0x25CF) * 7)
        # Control: the text input beside it reads its value in the clear.
        username = wait_for_descendant_by_role(self.web, "AXTextField", title="Username")
        self.assertIsNotNone(username, "the text input isn't exposed as an AXTextField")
        self.assertEqual(_ax_attr(username, kAXValueAttribute), "ada")
    ...
    def test_unnamed_password_field_is_exposed(self):
        ...
        the empty role HTML-AAM gives it would otherwise leave it out — as a secure text field with its value masked."""
        unnamed = wait_for_descendant_by_role(
            self.web, "AXTextField", pred=lambda obj: not _ax_attr(obj, kAXTitleAttribute)
        )
        self.assertIsNotNone(unnamed, "an unnamed password input isn't exposed as an AXTextField")
        self.assertEqual(_ax_attr(unnamed, kAXSubroleAttribute), "AXSecureTextField")
        self.assertEqual(_ax_attr(unnamed, kAXValueAttribute), chr(0x25CF) * 6)
        # Control: the named password input and the text input are their own nodes beside it.
        self.assertEqual(len(find_all_by_role(self.web, "AXTextField")), 3)
