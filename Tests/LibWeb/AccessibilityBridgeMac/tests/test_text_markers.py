...
from ApplicationServices import AXValueCreate  # noqa: E402
...
from ApplicationServices import kAXValueCGPointType  # noqa: E402
...
class DocumentMarkerEndpointsTests(AccessibilityBridgeMacTestCase):
    ...
    def test_start_and_end_markers_are_distinct(self):
        ...
        # Each read hands back its own AXTextMarkerRef, and repr() of a CF object carries the object's address — so a
        # comparison of the markers themselves can't fail. Compare what they resolve to instead: paragraphs.html has
        # several paragraphs, so the start and end markers land in different text leaves.
        start_leaf = get_parameterized_attribute_value(self.web, "AXUIElementForTextMarker", start)
        end_leaf = get_parameterized_attribute_value(self.web, "AXUIElementForTextMarker", end)
        self.assertIsNotNone(start_leaf, "AXUIElementForTextMarker returned nil for AXStartTextMarker")
        self.assertIsNotNone(end_leaf, "AXUIElementForTextMarker returned nil for AXEndTextMarker")
        self.assertNotEqual(start_leaf, end_leaf, "AXStartTextMarker and AXEndTextMarker resolve to the same leaf")
...
class TextMarkerRangeForElementTests(AccessibilityBridgeMacTestCase):
    ...
    def test_length_matches_string_length(self):
        """AXLengthForTextMarkerRange equals the length of AXStringForTextMarkerRange. The wrapper counts the code
        points it appends while building that string — so this is guarding against the two queries diverging if
        either's plumbing changes."""
        leaves = find_all_by_role(self.web, "AXStaticText")
...
class TextMarkerForPositionTests(AccessibilityBridgeMacTestCase):
    ...
    def test_marker_for_static_text_position_resolves_to_text(self):
        ...
        # neighboring element. The point goes over as an AXValue: the AX API rejects an NSValue parameter with
        # kAXErrorIllegalArgument before it reaches the app, while an AXValue point arrives at the app's wrapper as an
        # NSValue in Cocoa screen coordinates, which is what its hit test converts.
        nudged = AXValueCreate(kAXValueCGPointType, (x + 5, y + 5))
        marker = get_parameterized_attribute_value(self.web, "AXTextMarkerForPosition", nudged)
        self.assertIsNotNone(marker, "AXTextMarkerForPosition returned nil for a point inside a text leaf")
