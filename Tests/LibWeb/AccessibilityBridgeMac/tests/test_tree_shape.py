...
class TreeShapeTests(AccessibilityBridgeMacTestCase):
    ...
    def test_no_ignored_role_node_is_an_accessibility_element(self):
        ...
        def visit(obj, _depth):
            ...
            subrole = _ax_attr(obj, kAXSubroleAttribute)
            ...
            if role == "AXGroup" and not title and not subrole:
        ...
        walk(self.web, visit)
        # Landmark groups carry an AXSubrole (with empty AXTitle), so they are filtered out above; anything left
        # is an unnamed generic that leaked through isAccessibilityIgnored.
        self.assertEqual(offenders, [], f"ignored AXGroups leaked into the tree: {offenders}")


class WhitespaceTextTests(AccessibilityBridgeMacTestCase):
    """Whitespace-only text is never an AXStaticText of its own — VoiceOver has nothing to stop on there.

    inline_whitespace.html puts a rendered space between two links and between two styled words. The space keeps the
    words apart in the text VoiceOver reads (the text markers walk every text leaf), but it isn't an element of the
    tree, as WebKit ignores every whitespace-only RenderText (LadybirdAccessibilityElement.mm: is_ignored_role)."""

    FIXTURE = "inline_whitespace.html"

    def test_no_whitespace_only_static_text(self):
        values = []

        def visit(obj, _depth):
            if _ax_attr(obj, kAXRoleAttribute) == "AXStaticText":
                values.append(_ax_attr(obj, kAXValueAttribute))

        walk(self.web, visit)
        self.assertIn("bold", values, f"the paragraph's text isn't exposed; static texts: {values!r}")
        self.assertEqual(
            [value for value in values if isinstance(value, str) and value.strip() == ""],
            [],
            f"whitespace-only static text in the tree: {values!r}",
        )
