...
TEST_CASE(main_world_reuses_hinted_wrapper_across_realms)
{
    auto vm = JS::VM::create();
    auto main_world = vm->heap().allocate<Web::Bindings::WrapperWorld>(Web::Bindings::WrapperWorld::Type::Main);
    TestRealm allocation_realm { *vm, main_world };
    TestRealm first_caller_realm { *vm, main_world };
    TestRealm second_caller_realm { *vm, main_world };
    auto wrappable = allocation_realm.realm().create<TestWrappable>(allocation_realm.realm());
    wrappable->set_wrapper_realm_hint(first_caller_realm.realm());
...
TEST_CASE(main_world_wrapper_reuses_hinted_realm_after_cache_clear)
{
    auto vm = JS::VM::create();
    auto main_world = vm->heap().allocate<Web::Bindings::WrapperWorld>(Web::Bindings::WrapperWorld::Type::Main);
    TestRealm allocation_realm { *vm, main_world };
    ...
    auto first_wrapper = wrap_test_wrappable(first_caller_realm.realm(), wrappable);
    Web::Bindings::host_defined_wrapper_world(first_caller_realm.realm()).clear_wrapper(*wrappable, *first_wrapper);

    auto second_wrapper = wrap_test_wrappable(second_caller_realm.realm(), wrappable);

    EXPECT(&second_wrapper->realm() == &first_caller_realm.realm());
    EXPECT(second_wrapper.ptr() == cached_wrapper_for(second_caller_realm.realm(), *wrappable).ptr());
}
...
TEST_CASE(hinted_main_world_wrapper_ignores_preferred_realm)
{
