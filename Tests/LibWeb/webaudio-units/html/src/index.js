...
import { withDryWetMix } from "./processor-mix.js";
import {
    clampNumber,
    createDefaultLogger,
...
const STORAGE_KEY = "webaudio-graph";
...
function createAddSeparator(options) {
    ...
    const select = h("select", "node-picker");
        ...
        serialize() {
            return {
                source: {
                    ...
                    state: inspector.source.saveState ? inspector.source.saveState() : null,
                },
                processors: inspector.processors.map(p => ({
                    kind: p.kind,
                    state: p.saveState ? p.saveState() : null,
                })),
            };
        },
...
function createProcessorInstance(kind, log) {
    ...
    if (kind === "gain") processor = createGainProcessor(log, nodeUi);
    else if (kind === "delay") processor = createDelayProcessor(log, nodeUi);
    else if (kind === "stereoPanner") processor = createStereoPannerProcessor(log, nodeUi);
    else if (kind === "panner") processor = createPannerProcessor(log, nodeUi);
    else if (kind === "biquadFilter") processor = createBiquadFilterProcessor(log, nodeUi);
    else if (kind === "waveShaper") processor = createWaveShaperProcessor(log, nodeUi);
    else if (kind === "iirFilter") processor = createIIRFilterProcessor(log, nodeUi);
    else if (kind === "convolver") processor = createConvolverProcessor(log, nodeUi);
    else if (kind === "dynamicsCompressor") processor = createDynamicsCompressorProcessor(log, nodeUi);
    else if (kind === "analyser") processor = createAnalyserProcessor(log, nodeUi);
    else if (kind === "audioWorklet") processor = createAudioWorkletProcessor(log, nodeUi);
    else if (kind === "ohnoes") processor = createOhNoesProcessor(log, nodeUi);
    else processor = createGainProcessor(log, nodeUi);
    ...
    if (
        ...
        return withDryWetMix(processor, log, nodeUi);
    }
