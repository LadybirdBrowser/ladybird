...
constexpr ResourceID resource_id(u64 value)
{
...
GraphDescription make_graph_with_buffer_source_node()
{
    ...
    payload.buffer_id = resource_id(1);
...
Web::WebAudio::Render::GraphResources make_resources_for_test_graph_with_buffer_source_node()
{
    Web::WebAudio::Render::GraphResources resources;
    Vector<Vector<f32>> channels;
    channels.resize(1);
    channels[0].resize(16);
    channels[0].fill(0.0f);
    resources.audio_buffers.set(resource_id(1), Web::WebAudio::Render::SharedAudioBuffer::create(48000.0f, 1, 16, move(channels)));
    return resources;
}
...
TEST_CASE(render_graph_wire_serializer_minimal_graph_layout)
{
    ...
    auto encoded_graph = buffer_or_error.release_value();

    LocalWireDecoder decoder(encoded_graph.encoded_graph);
...
TEST_CASE(render_graph_wire_serializer_sets_external_resources_flag_for_buffer_source)
{
    ...
    EXPECT_EQ(encoded_graph.shared_audio_buffers.size(), 1u);
    EXPECT_EQ(encoded_graph.shared_audio_buffers[0].buffer_id, resource_id(1));
    EXPECT(encoded_graph.shared_audio_buffers[0].shared_memory.is_valid());
}
...
TEST_CASE(render_graph_wire_serializer_is_deterministic_for_node_insertion_order)
{
    ...
    EXPECT_EQ(a.encoded_graph.size(), b.encoded_graph.size());
    EXPECT_EQ(a.encoded_graph.bytes(), b.encoded_graph.bytes());
    EXPECT_EQ(a.shared_audio_buffers.size(), b.shared_audio_buffers.size());
}
