...
GraphDescription make_graph_with_inline_buffer_source_node()
{
    ...
    payload.buffer_id = resource_id(2);
...
Web::WebAudio::Render::GraphResources make_resources_for_buffer_source_test_graph()
{
...
Web::WebAudio::Render::GraphResources make_resources_for_inline_buffer_table_test_graph()
{
    Web::WebAudio::Render::GraphResources resources;
    Vector<Vector<f32>> channels;
    channels.resize(1);
    channels[0] = { 0.25f, -0.5f, 0.75f, -1.0f };
    resources.audio_buffers.set(resource_id(2), Web::WebAudio::Render::SharedAudioBuffer::create(48000.0f, 1, 4, move(channels)));
    return resources;
}
...
TEST_CASE(render_graph_wire_round_trip_destination_only)
{
    ...
    auto encoded_or_error = encode_render_graph_for_media_server(graph, 48000.0f, resources);
    EXPECT(!encoded_or_error.is_error());

    auto encoded_graph = encoded_or_error.release_value();
    auto decoded_or_error = Web::WebAudio::Render::decode_render_graph_wire_format(encoded_graph.encoded_graph.bytes(), encoded_graph.shared_audio_buffers.span());
...
TEST_CASE(render_graph_wire_round_trip_audio_worklet_parameters)
{
    ...
    auto encoded_or_error = encode_render_graph_for_media_server(graph, 44100.0f, resources);
    EXPECT(!encoded_or_error.is_error());

    auto encoded_graph = encoded_or_error.release_value();
    auto decoded_or_error = decode_render_graph_wire_format(encoded_graph.encoded_graph.bytes(), encoded_graph.shared_audio_buffers.span());
    EXPECT(!decoded_or_error.is_error());
...
TEST_CASE(render_graph_wire_round_trip_sets_external_resources_flag)
{
    ...
    EXPECT_EQ(buffer_source.buffer_id, resource_id(1));

    RefPtr<Web::WebAudio::Render::SharedAudioBuffer> buffer;
    if (auto it = build.resources->audio_buffers.find(resource_id(1)); it != build.resources->audio_buffers.end())
        buffer = it->value;
...
TEST_CASE(render_graph_wire_round_trip_inline_buffer_table)
{
    ...
    auto const& node = it->value;
    EXPECT(node.has<AudioBufferSourceGraphNode>());
    EXPECT(node.get<AudioBufferSourceGraphNode>().buffer_id != ResourceID {});

    RefPtr<Web::WebAudio::Render::SharedAudioBuffer> buffer;
    if (auto it = build.resources->audio_buffers.find(node.get<AudioBufferSourceGraphNode>().buffer_id); it != build.resources->audio_buffers.end())
        buffer = it->value;
    EXPECT(buffer != nullptr);
...
TEST_CASE(render_graph_wire_decode_fails_on_unknown_node_type)
{
    ...
    auto encoded_graph = encoded_or_error.release_value();
    ByteBuffer mutated = MUST(ByteBuffer::copy(encoded_graph.encoded_graph.bytes()));
    ...
    auto decoded_or_error = decode_render_graph_wire_format(mutated.bytes(), encoded_graph.shared_audio_buffers.span());
    EXPECT(decoded_or_error.is_error());
}
...
TEST_CASE(render_graph_wire_round_trip_skips_unknown_sections)
{
    ...
    auto encoded_graph = encoded_or_error.release_value();
    auto original_bytes = encoded_graph.encoded_graph.bytes();
