...
static void expect_first_token_is_ident(StringView input, StringView encoding, StringView expected_ident, StringView expected_source, TokenizerInput tokenizer_input = TokenizerInput::EncodedBytes)
{
...
TEST_CASE(tokenizer_decodes_valid_utf8_bytes_with_requested_single_byte_encoding)
{
    ...
    expect_first_token_is_ident(StringView(bytes(input)), "windows-1252"sv, "Ã©"sv, "Ã©"sv);
}

TEST_CASE(tokenizer_decodes_valid_utf8_bytes_with_requested_utf16_encoding)
{
    auto input = Vector<u8> { 'a', 0x00, ' ', 0x00, '{', 0x00, '}', 0x00 };
    expect_first_token_is_ident(StringView(bytes(input)), "utf-16le"sv, "a"sv, "a"sv);
}

TEST_CASE(tokenizer_keeps_utf8_fast_path_for_utf8_encoding)
{
    expect_first_token_is_ident("é {}"sv, "utf-8"sv, "é"sv, "é"sv);
}

TEST_CASE(tokenizer_strips_bom_in_utf8_fast_path)
{
    auto input = Vector<u8> { 0xef, 0xbb, 0xbf, 'b', 'o', 'd', 'y', ' ', '{', '}' };
    expect_first_token_is_ident(StringView(bytes(input)), "utf-8"sv, "body"sv, "body"sv);
}

TEST_CASE(tokenizer_strips_bom_for_utf8_encoding_alias)
{
    auto input = Vector<u8> { 0xef, 0xbb, 0xbf, 'b', 'o', 'd', 'y', ' ', '{', '}' };
    expect_first_token_is_ident(StringView(bytes(input)), "utf8"sv, "body"sv, "body"sv);
}

TEST_CASE(tokenizer_does_not_strip_bom_bytes_for_non_utf8_encoding)
{
    auto input = Vector<u8> { 0xef, 0xbb, 0xbf, 'b', 'o', 'd', 'y', ' ', '{', '}' };
    expect_first_token_is_ident(StringView(bytes(input)), "windows-1252"sv, "ï»¿body"sv, "ï»¿body"sv);
}
...
TEST_CASE(tokenizer_decodes_truncated_utf8_tail_as_single_replacement)
{
    ...
    expect_first_token_is_ident(StringView(bytes(input)), "utf-8"sv, "�"sv, "�"sv);
}

TEST_CASE(tokenizer_decodes_surrogate_shaped_bytes_with_requested_single_byte_encoding)
{
    auto input = Vector<u8> { 0xed, 0xa0, 0x80, 'b', 'o', 'd', 'y', ' ', '{', '}' };
    expect_first_token_is_ident(StringView(bytes(input)), "windows-1252"sv, "í €body"sv, "í €body"sv);
}

TEST_CASE(tokenizer_filters_decoded_surrogate_code_points_as_single_replacements)
{
    expect_first_token_is_ident("foo\xed\xa0\x80"sv, "utf-8"sv, "foo�"sv, "foo�"sv, TokenizerInput::DecodedText);
}
