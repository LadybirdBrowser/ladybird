...
def output_file_for(file: Path) -> Path:
    return CSS_TOKENIZER_TEST_DIR / "output" / file.with_suffix(".rust.txt")
...
def run_tokenizer(file: Path, encoding: str) -> tuple[str, Path]:
    args = [
    ...
    process = subprocess.run(args, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, check=False)
...
def test(file: Path, rebaseline: bool) -> bool:
    ...
    stdout, output_file = run_tokenizer(file, encoding)
