...
{
    auto map_path = LexicalPath::join(Application::the().test_root_path, "WptResourceMap.json"sv).string();
    auto file_or_error = Core::File::open(map_path, Core::File::OpenMode::Read);
    if (file_or_error.is_error()) {
        if (file_or_error.error().code() == ENOENT)
            return;
        warnln("test-web: Unable to open WPT resource map '{}': {}", map_path, file_or_error.error());
        return;
    }

    auto content_or_error = file_or_error.value()->read_until_eof();
    if (content_or_error.is_error()) {
        warnln("test-web: Unable to read WPT resource map '{}': {}", map_path, content_or_error.error());
        return;
    }

    auto json_or_error = JsonValue::from_string(content_or_error.value());
    if (json_or_error.is_error()) {
        warnln("test-web: Unable to parse WPT resource map '{}': {}", map_path, json_or_error.error());
        return;
    }
