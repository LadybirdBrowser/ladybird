...
HandshakeResult run_handshake(bool duplicate_accept)
{
    ...
    auto accept_header = ByteString::formatted("Sec-WebSocket-Accept: {}", implementation->expected_accept());
    Vector<ByteString> response {
    ...
    if (duplicate_accept)
        response.append(accept_header);
    response.append({});
    implementation->receive(move(response));
    return result;
}

}

TEST_CASE(duplicate_accept_proof_fails_handshake)
{
