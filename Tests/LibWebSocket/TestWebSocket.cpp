...
HandshakeResult run_handshake(bool change_accept_case)
{
    ...
    auto accept = implementation->expected_accept();
    if (change_accept_case) {
        bool changed = false;
        accept = ByteString::create_and_overwrite(accept.length(), [&](Bytes bytes) {
            for (size_t i = 0; i < accept.length(); ++i) {
                auto ch = accept.characters()[i];
                if (!changed && ch >= 'A' && ch <= 'Z') {
                    ch += 'a' - 'A';
                    changed = true;
                } else if (!changed && ch >= 'a' && ch <= 'z') {
                    ch -= 'a' - 'A';
                    changed = true;
                }
                bytes[i] = ch;
    ...
    implementation->receive({
        ...
        "Upgrade: websocket",
        "Connection: Upgrade",
        ByteString::formatted("Sec-WebSocket-Accept: {}", accept),
        "",
    });
    return result;
}
...
TEST_CASE(accept_proof_is_case_sensitive)
{
