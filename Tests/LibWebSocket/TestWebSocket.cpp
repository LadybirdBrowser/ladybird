...
struct HandshakeResult {
    ...
    ByteString subprotocol;
};

HandshakeResult run_handshake(bool change_protocol_case)
{
    ...
    connection_info.set_protocols({ "chat.v1" });
    auto websocket = WebSocket::WebSocket::create(move(connection_info), implementation);
    ...
    implementation->receive({
        ...
        "Upgrade: websocket",
        "Connection: Upgrade",
        ByteString::formatted("Sec-WebSocket-Accept: {}", implementation->expected_accept()),
        ByteString::formatted("Sec-WebSocket-Protocol: {}", change_protocol_case ? "CHAT.V1"sv : "chat.v1"sv),
        "",
    });
    result.subprotocol = websocket->subprotocol_in_use();
    return result;
}

}

TEST_CASE(subprotocol_is_case_sensitive)
{
