...
TEST_CASE(export_bitmap_to_byte_buffer)
{
    ...
    struct TestData {
        ...
        Gfx::BitmapFormat export_format;
            ...
            for (auto maybe_flip_y_flag : Vector<int> { 0, Gfx::ExportFlags::FlipY }) {
                for (auto source_alpha_case : subtest.source_alpha_cases_to_test) {
                    for (auto target_alpha_case : subtest.target_alpha_cases_to_test) {
                        ...
                        dbgln("Running subtest {}: {} -> {}, {} -> {}, {}", count++, bitmap_format_name(source_format), bitmap_format_name(subtest.export_format), alpha_case_name(source_alpha_case), alpha_case_name(target_alpha_case), flip_y_name(export_flags));

                        auto source_alpha_type = source_alpha_case == Premultiplied::Yes ? Gfx::BitmapAlpha::Premultiplied : Gfx::BitmapAlpha::Unpremultiplied;
