...
OpFunctionEnd)"sv;
    ...
    String const spirv_text = TRY_OR_FAIL(compiler.emit_spirv_text());
    ...
    Vector<uint32_t> const spirv_binary = TRY_OR_FAIL(compiler.emit_spirv_binary(spirv_text));
...
fragment fragment_main_out fragment_main(fragment_main_in in [[stage_in]])
{
    ...
    WGSL::Compiler compiler(triangle_shader);
    auto const msl_shader_sources = TRY_OR_FAIL(compiler.emit_msl());
