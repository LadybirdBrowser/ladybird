...
TEST_CASE(compile_simple_triangle_shader_hlsl)
{
    constexpr auto expected_triangle_shader_hlsl_vertex_source = R"(static float4 gl_Position;
static float4 _73;
static float4 _75;
static float4 _77;
...
{
    float4 _75 : TEXCOORD0;
    float4 _73 : TEXCOORD1;
};

struct SPIRV_Cross_Output
{
    float4 _77 : TEXCOORD0;
...
SPIRV_Cross_Output main(SPIRV_Cross_Input stage_input)
{
    ...
    Vector const expected_triangle_shader_hlsl_vertex_lines = expected_triangle_shader_hlsl_vertex_source.split_view('\n');

    constexpr auto expected_triangle_shader_hlsl_fragment_source = R"(static float4 _91;
static float4 _95;

struct SPIRV_Cross_Input
{
    float4 _91 : TEXCOORD0;
};

struct SPIRV_Cross_Output
{
    float4 _95 : SV_Target0;
};
...
SPIRV_Cross_Output main(SPIRV_Cross_Input stage_input)
{
    ...
    frag_main();
    SPIRV_Cross_Output stage_output;
    stage_output._95 = _95;
    return stage_output;
}
)"sv;
    Vector const expected_triangle_shader_hlsl_fragment_lines = expected_triangle_shader_hlsl_fragment_source.split_view('\n');

    WGSL::Compiler compiler(triangle_shader);
    auto const hlsl_shader_sources_result = compiler.emit_hlsl();
    if (hlsl_shader_sources_result.is_error()) {
        FAIL("Unable to compile WGSL triangle shader source code to HLSL source");
        return;
    }
    auto const& hlsl_shader_sources = hlsl_shader_sources_result.value();
    for (auto const& hlsl_shader_source : hlsl_shader_sources) {
        hlsl_shader_source.visit(
            [&expected_triangle_shader_hlsl_vertex_lines](WGSL::Compiler::VertexShader const& vertex_shader) {
                EXPECT(vertex_shader.entry_point == "vertex_main"_string);
                Vector const actual_hlsl_vertex_lines = MUST(vertex_shader.source.split('\n'));
                test_shader_lines_equal(actual_hlsl_vertex_lines, expected_triangle_shader_hlsl_vertex_lines);
            },
            [&expected_triangle_shader_hlsl_fragment_lines](WGSL::Compiler::FragmentShader const& fragment_shader) {
                EXPECT(fragment_shader.entry_point == "fragment_main"_string);
                Vector const actual_hlsl_fragment_lines = MUST(fragment_shader.source.split('\n'));
                test_shader_lines_equal(actual_hlsl_fragment_lines, expected_triangle_shader_hlsl_fragment_lines);
            });
    }
}
